...
TiDB 支持在线 DDL，这意味着在数据库中执行 DDL 语句时，采用特定的方法确保该操作不会阻塞用户应用。你可以在执行 DDL 时提交数据修改，数据库保证数据的一致性和正确性。
...
- **Logical DDL 语句**：逻辑 DDL 语句通常只修改数据库对象的元数据，不处理存储在对象中的数据，例如更改表名或列名。
    ...
    在 TiDB 中，逻辑 DDL 也称为“通用 DDL”。这些语句的执行时间通常较短，常常只需几十毫秒或几秒即可完成。因此，它们占用的系统资源较少，不会影响应用的工作负载。

- **Physical DDL 语句**：物理 DDL 语句不仅修改被改变对象的元数据，还会修改存储在对象中的用户数据。例如，当 TiDB 为一张表创建索引时，不仅会改变表的定义，还会进行全表扫描以构建新添加的索引。
    ...
    > **Note:**
    >
    > 执行物理 DDL 任务通常会对用户应用产生最大影响。因此，为了最小化影响，关键在于在执行过程中优化物理 DDL 语句的设计。这有助于减少对用户应用的影响。

### TiDB DDL 模块

TiDB 的 DDL 模块引入了 DDL Owner（或 Owner）角色，作为在 TiDB 集群中执行所有 DDL 语句的代理。在当前实现中，整个集群中只有一个 TiDB 节点可以被选举为 Owner。一旦某个 TiDB 节点当选为 Owner，该节点启动的工作线程就可以处理集群中的 DDL 任务。

TiDB 使用 etcd 的选举机制，从多个 TiDB 节点中选举出一个节点作为 Owner。默认情况下，每个 TiDB 节点都可能被选举为 Owner（可以通过配置 `run-ddl` 来管理节点参与选举）。当选的 Owner 节点具有一个任期，并通过续期主动维护该任期。当 Owner 节点宕机时，其他节点可以通过 etcd 选举出新的 Owner，继续在集群中执行 DDL 任务。
...
DDL 变更涉及状态的转换，通常从“变更前”状态到“变更后”状态。采用在线 DDL 时，这一转换通过引入多个相互兼容的小版本状态实现。在执行 DDL 语句期间，TiDB 集群中的节点可以拥有不同的小版本变更，只要相邻版本之间互相兼容即可。这是因为相邻的小版本可以互相兼容。
...
对于用户而言，在 `public` 状态之前，新创建的索引是不可用的。

<SimpleTab>
<div label="从 v6.2.0 版本开始的并行 DDL 框架">
...
为了改善 DDL 执行的用户体验，从 v6.2.0 开始，TiDB 允许 Owner 判断 DDL 任务的相关性，逻辑如下：

+ 对同一张表的 DDL 语句相互阻塞。
+ `DROP DATABASE` 和影响数据库中所有对象的 DDL 语句相互阻塞。
+ 在不同表上添加索引和列类型变更可以并发执行。
+ 从 v8.2.0 开始，不同表的 [logical DDL 语句](/ddl-introduction.md#types-of-ddl-statements) 可以并行执行。
+ 在其他情况下，DDL 可以根据并发 DDL 执行的可用性级别进行。
...
+ 增加处理物理 DDL 语句的工作线程数，支持多个物理 DDL 并行执行。
    ...
    由于 TiDB 中所有 DDL 任务都是采用在线变更方式实现的，TiDB 可以通过 Owner 判断新 DDL 任务的相关性，并据此调度 DDL 任务。这一方式使得分布式数据库的 DDL 并发能力达到与传统数据库相同的水平。
...
2. TiDB 服务器收到请求后，在 MySQL 协议层解析优化请求，然后将其发送到 TiDB SQL 层执行。

    一旦 TiDB 的 SQL 层收到 DDL 请求，它会启动 `start job` 模块，将请求封装成特定的 DDL 任务（即 DDL 作业），然后根据语句类型将该作业存入对应的 DDL 作业队列（在 KV 层）。相应的工作线程会被通知处理该作业。

3. 当工作线程收到处理作业的通知时，会判断自己是否为 DDL Owner。如果是，则直接处理该作业；否则不进行任何处理。

    如果 TiDB 服务器不是 Owner 角色，则必须由其他节点担任 Owner。Owner 角色的工作线程会定期检查是否有可执行的作业。一旦发现有，就会处理。

4. 工作线程处理完作业后，会将作业从 KV 层的作业队列中移除，并放入 `job history queue`。封装作业的 `start job` 模块会定期检查 `job history queue` 中作业的 ID，确认是否已处理完毕。如果已完成，整个 DDL 操作也就结束了。
...
这些限制可能导致一些“非预期”的 DDL 阻塞行为。更多详情请参见 [SQL FAQ - DDL 执行](https://docs.pingcap.com/tidb/stable/sql-faq#ddl-execution)。
...
- [`tidb_ddl_reorg_worker_cnt`](/system-variables.md#tidb_ddl_reorg_worker_cnt)：设置 DDL 操作的重组工作线程数，控制回填的并发度。

- [`tidb_ddl_reorg_batch_size`](/system-variables.md#tidb_ddl_reorg_batch_size)：设置 `re-organize` 阶段的批量大小，控制回填的数据量。

    推荐值：

    - 如果没有其他负载，可以增大 `tidb_ddl_reorg_worker_cnt` 和 `tidb_ddl_reorg_batch_size` 的值，加快 `ADD INDEX` 操作。例如，将两个变量的值分别设置为 `20` 和 `2048`。
    - 如果存在其他负载，可以减小这两个变量的值，以最小化对其他应用的影响。例如，将它们的值设置为 `4` 和 `256`。

> **Tip:**
>
> - 以上两个变量可以在 DDL 任务执行过程中动态调整，并在下一批事务中生效。
> - 根据操作类型和应用负载压力，选择合适的时间执行 DDL 操作。例如，建议在应用负载较低时执行 `ADD INDEX`。
...
- `ADMIN CANCEL DDL JOBS job_id [, job_id]`：用于取消已提交但未完成的 DDL 任务。取消完成后，执行 DDL 的 SQL 语句会返回 `ERROR 8214 (HY000): Cancelled DDL job` 错误。

    如果取消已完成的 DDL 任务，可以在 `RESULT` 列看到 `DDL Job:90 not found` 错误，表示该任务已从 DDL 等待队列中移除。

- `ADMIN PAUSE DDL JOBS job_id [, job_id]`：用于暂停正在执行的 DDL 任务。执行后，执行 DDL 的 SQL 语句会显示为正在执行，而后台任务已暂停。详情请参见 [`ADMIN PAUSE DDL JOBS`](/sql-statements/sql-statement-admin-pause-ddl.md)。

    只能暂停进行中或在队列中的 DDL 任务，否则在 `RESULT` 列会显示 `Job 3 can't be paused now` 错误。

- `ADMIN RESUME DDL JOBS job_id [, job_id]`：用于恢复已暂停的 DDL 任务。执行后，执行 DDL 的 SQL 语句会显示为正在执行，后台任务恢复。详情请参见 [`ADMIN RESUME DDL JOBS`](/sql-statements/sql-statement-admin-resume-ddl.md)。

    只能恢复暂停的 DDL 任务，否则在 `RESULT` 列会显示 `Job 3 can't be resumed` 错误。
