...
[Window Functions](/functions-and-operators/window-functions.md) 是一种常见的 SQL 函数类型。当你使用 window function 进行行编号，例如 `ROW_NUMBER()` 或 `RANK()`，在 window function 计算完成后，通常会对结果进行过滤。例如：

```sql
SELECT * FROM (SELECT ROW_NUMBER() OVER (ORDER BY a) AS rownumber FROM t) dt WHERE rownumber <= 3
```

在典型的 SQL 执行过程中，TiDB 首先对表 `t` 中的所有数据进行排序，然后为每一行计算 `ROW_NUMBER()` 结果，最后根据 `rownumber <= 3` 进行过滤。

从 v7.0.0 版本开始，TiDB 支持将从 window functions 派生的 TopN 或 Limit 操作符进行优化。通过此优化规则，TiDB 可以将原始 SQL 重写为等价的形式，例如：
...
重写后，TiDB 可以从 window function 和后续的过滤条件中推导出 TopN 操作符。与原始 SQL 中的 Sort 操作（`ORDER BY`）相比，TopN 操作符具有更高的执行效率。此外，TiKV 和 TiFlash 也支持将 TopN 操作下推，从而进一步提升重写后 SQL 的性能。

从 window functions 派生 TopN 或 Limit 默认是关闭的。你可以通过设置会话变量 [tidb_opt_derive_topn](/system-variables.md#tidb_opt_derive_topn-new-in-v700) 为 `ON` 来启用此功能。
...
* 目前仅支持将 `ROW_NUMBER()` window function 进行 SQL 重写。
* TiDB 仅在过滤条件为 `<` 或 `<=` 时，才能将 SQL 重写为从 window function 派生的操作。
...
> **注意：**
>
> 对于包含 `PARTITION BY` 的 window function，只有当分区列是主键的前缀且主键为聚簇索引时，此优化规则才会生效。
...
在此查询中，优化器从 window function 派生出 Partition TopN 操作符，并将其下推到 TiKV。注意，此 TopN 实际上是分区 TopN，即对每个具有相同 `id1` 值的分组应用。
