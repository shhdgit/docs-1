...
3. 将你的 JAR 包上传为 Lambda：
    ...
    > - 如果你之前已经运行过 `sam deploy --guided`，可以直接运行 `sam deploy`，SAM CLI 会使用 `samconfig.toml` 配置文件简化交互过程。
...
查询 `sf_account` 表，可以看到 Salesforce **Account** 对象中的记录已经写入到该表：
