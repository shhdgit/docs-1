...
> **注意：**
>
> 本教程适用于 TiDB Cloud Serverless、TiDB Cloud Essential、TiDB Cloud Dedicated 以及自建 TiDB。
...
本教程主要以独立 DataGrip IDE 为例。在 JetBrains IDE 中使用 Database Tools and SQL 插件连接 TiDB 的步骤类似。你也可以参考本文档中的步骤，在任意 JetBrains IDE 中连接 TiDB。
...
8. 点击 **Test Connection** 测试与 TiDB Cloud Serverless 集群的连接。
...
6. 将相应的连接字符串复制粘贴到 DataGrip 的 **Data Source and Drivers** 窗口中。DataGrip 字段与 TiDB Cloud Dedicated 连接字符串的映射关系如下：
...
3. 配置以下连接参数：
    ...
    ![为 TiDB 自建集群配置连接参数](/media/develop/datagrip-self-hosted-connect.jpg)

    如果出现 **Download missing driver files** 警告，点击 **Download** 下载驱动文件。

4. 点击 **Test Connection** 测试与 TiDB 自建集群的连接。

    ![测试与 TiDB 自建集群的连接](/media/develop/datagrip-self-hosted-test-connection.jpg)
