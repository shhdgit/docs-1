...
本教程主要以独立 DataGrip IDE 为例。通过 JetBrains IDE 中的 Database Tools and SQL 插件连接 TiDB 的步骤类似。你也可以参考本文档中的步骤，在任意 JetBrains IDE 中连接 TiDB。
...
7. 从 TiDB Cloud 连接对话框复制连接字符串，粘贴到 **URL** 字段，其余参数会自动填充。示例如下：
...
6. 将相应的连接字符串复制粘贴到 DataGrip 的 **Data Source and Drivers** 窗口。DataGrip 字段与 TiDB Cloud Dedicated 连接字符串的对应关系如下：
