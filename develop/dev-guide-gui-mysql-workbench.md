...
> - 建议使用其他官方支持 TiDB 的图形界面工具，例如 [DataGrip](/develop/dev-guide-gui-datagrip.md)、[DBeaver](/develop/dev-guide-gui-dbeaver.md) 和 [VS Code SQLTools](/develop/dev-guide-gui-vscode-sqltools.md)。完整支持 TiDB 的图形界面工具列表，请参见 [Third-party tools supported by TiDB](/develop/dev-guide-third-party-support.md#gui)。
...
8. 如果连接成功，会显示 **Successfully made the MySQL connection** 提示。点击 **OK** 保存连接配置。
...
7. 如果连接成功，会显示 **Successfully made the MySQL connection** 提示。点击 **OK** 保存连接配置。
...
2. 在 **Setup New Connection** 对话框中，配置以下连接参数：
    ...
    - **Password**：点击 **Store in Keychain ...**，输入连接 TiDB 所用的密码，然后点击 **OK** 保存密码。
...
4. 如果连接成功，会显示 **Successfully made the MySQL connection** 提示。点击 **OK** 保存连接配置。
...
2. 在 **SQL Editor** > **MySQL Session** 部分，配置 **DBMS connection read timeout interval (in seconds)** 选项。此设置定义了查询最多可以耗时（以秒为单位），超过后 MySQL Workbench 会断开连接。
...
- 通过 [MySQL Workbench 文档](https://dev.mysql.com/doc/workbench/en/) 了解更多使用方法。
- 学习 TiDB 应用开发的最佳实践，参考 [Developer guide](/develop/dev-guide-overview.md) 中的章节，例如 [Insert data](/develop/dev-guide-insert-data.md)、[Update data](/develop/dev-guide-update-data.md)、[Delete data](/develop/dev-guide-delete-data.md)、[Single table reading](/develop/dev-guide-get-data-from-single-table.md)、[Transactions](/develop/dev-guide-transaction-overview.md) 和 [SQL performance optimization](/develop/dev-guide-optimize-sql-overview.md)。
