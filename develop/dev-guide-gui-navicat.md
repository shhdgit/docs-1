...
7. 点击 **SSL** 标签页，勾选 **Use SSL**、**Use authentication** 和 **Verify server certificate against CA** 复选框，然后在 **CA Certificate** 字段中选择 TiDB Cloud 连接对话框中下载的 `CA` 文件。

    ![Navicat: configure connection SSL panel for {{{ .starter }}}](/media/develop/navicat-premium-connection-config-serverless-ssl.png)

8. 点击 **Test Connection**，验证是否能成功连接到 {{{ .starter }}} 集群。
...
7. 点击 **SSL** 标签页，勾选 **Use SSL**、**Use authentication** 和 **Verify server certificate against CA** 复选框，然后在第 4 步中下载的 CA 文件中选择文件，放入 **CA Certificate** 字段。
...
8. **Test Connection**，验证是否能成功连接到 TiDB Cloud Dedicated 集群。
