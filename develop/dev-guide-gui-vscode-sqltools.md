...
3. 确认连接对话框中的配置与您的操作环境匹配。
    ...
    > **Tip:**
    >
    > 如果你的 VS Code 在远程开发环境中运行，从列表中选择远程操作系统。例如，如果你使用 Windows Subsystem for Linux (WSL)，切换到对应的 Linux 发行版。如果你使用 GitHub Codespaces，则不需要这样做。

4. 点击 **Generate Password** 以生成随机密码。

    > **Tip:**
    >
    > 如果之前创建过密码，可以使用原有密码，或点击 **Reset Password** 生成新密码。
