...
你可以用 `EXPLAIN` 语句检查上述 SQL 的执行计划。如果在任务列中同时出现 `cop[tiflash]` 和 `cop[tikv]`，表示 TiDB 同时调度 TiFlash 和 TiKV 来完成此查询。注意，TiFlash 和 TiKV 存储引擎通常使用不同的 TiDB 节点，因此两者的查询不会相互影响。
