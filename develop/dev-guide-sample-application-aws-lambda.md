...
TiDB 是一个与 MySQL 兼容的数据库，[AWS Lambda Function](https://aws.amazon.com/lambda/) 是一项计算服务，而 [mysql2](https://github.com/sidorares/node-mysql2) 是 Node.js 中流行的开源驱动程序。
...
你可以使用 [SAM CLI](#sam-cli-deployment-recommended) 或 [Web 控制台](#web-console-deployment) 来部署 AWS Lambda 函数。
...
5. 部署 AWS Lambda 函数：
    ...
    #        # 显示即将部署的资源变更，需要输入 'Y' 以确认
    #        Confirm changes before deploy [y/N]:
    #        # SAM 需要权限以创建角色连接到模板中的资源
    #        Allow SAM CLI IAM role creation [Y/n]:
    #        # 在操作失败时保留之前配置的资源状态
    #        Disable rollback [y/N]:
    #        tidbHelloWorldFunction 可能没有定义权限，是否继续？ [y/N]: y
    #        (重复几次确认)
...
3. 按照 [创建 Lambda 函数](https://docs.aws.amazon.com/lambda/latest/dg/lambda-nodejs.html) 的步骤，创建一个 Node.js 类型的 Lambda 函数。
...
- 想了解更多在 AWS Lambda 函数中使用 TiDB 的细节，请参阅我们的 [TiDB-Lambda-integration/aws-lambda-bookstore Demo](https://github.com/pingcap/TiDB-Lambda-integration/blob/main/aws-lambda-bookstore/README.md)。你也可以使用 AWS API Gateway 为你的应用构建 RESTful API。
- 了解更多关于 `mysql2` 的用法，请参考 [mysql2 的文档](https://sidorares.github.io/node-mysql2/docs/documentation)。
- 了解更多关于 AWS Lambda 的用法，请参考 [AWS 开发者指南](https://docs.aws.amazon.com/lambda/latest/dg/welcome.html)。
- 了解 TiDB 应用开发的最佳实践，参考 [开发者指南]( /develop/dev-guide-overview.md)，如 [Insert data](/develop/dev-guide-insert-data.md)、[Update data](/develop/dev-guide-update-data.md)、[Delete data](/develop/dev-guide-delete-data.md)、[Single table reading](/develop/dev-guide-get-data-from-single-table.md)、[Transactions](/develop/dev-guide-transaction-overview.md)、以及 [SQL 性能优化](/develop/dev-guide-optimize-sql-overview.md)。
