...
6. 将对应的连接字符串复制粘贴到 `.env` 文件中。示例结果如下：
    ...
    TiDB Cloud Serverless 需要安全连接，因此 `USE_SSL` 的值需要设置为 `true`。
...
使用该函数时，需要将 `${tidb_host}`、`${tidb_port}`、`${tidb_user}`、`${tidb_password}` 和 `${tidb_db_name}` 替换为你 TiDB 集群的实际值。TiDB Cloud Serverless 需要安全连接，因此 `${use_ssl}` 的值需要设置为 `true`。
