...
summary: 学习如何使用 GORM 连接 TiDB。本教程提供了适用于 TiDB 的 Golang 示例代码片段，演示如何通过 GORM 进行操作。
---

# 使用 GORM 连接 TiDB

TiDB 是一个兼容 MySQL 的数据库，[GORM](https://gorm.io/index.html) 是 Golang 领域流行的开源 ORM 框架。GORM 支持 TiDB 的特性，如 `AUTO_RANDOM`，并且[将 TiDB 作为默认数据库选项进行支持](https://gorm.io/docs/connecting_to_the_database.html#TiDB)。
...
使用该函数时，需要将 `${tidb_host}`、`${tidb_port}`、`${tidb_user}`、`${tidb_password}` 和 `${tidb_db_name}` 替换为你 TiDB 集群的实际值。TiDB Cloud Serverless 需要安全连接，因此你需要将 `${use_ssl}` 设置为 `true`。
