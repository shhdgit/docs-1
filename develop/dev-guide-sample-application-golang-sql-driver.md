...
使用该函数时，你需要将 `${tidb_host}`、`${tidb_port}`、`${tidb_user}`、`${tidb_password}` 和 `${tidb_db_name}` 替换为你 TiDB 集群的实际值。TiDB Cloud Serverless 和 TiDB Cloud Essential 需要安全连接，因此你需要将 `${use_ssl}` 的值设置为 `true`。
