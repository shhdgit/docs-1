...
summary: 学习如何使用 Hibernate 连接 TiDB。本教程提供适用于 TiDB 的 Java 示例代码片段，演示如何通过 Hibernate 进行操作。
---
...
请确保将 `${tidb_jdbc_url}`、`${tidb_user}` 和 `${tidb_password}` 替换为你的 TiDB 集群的实际值。然后定义以下函数：
...
使用此函数时，需要将 `${your_entity_class}` 替换为你自己的数据实体类。若有多个实体类，则需多次调用 `.addAnnotatedClass()`。上述方法只是配置 Hibernate 的一种方式。如果在配置过程中遇到问题或想了解更多信息，可以参考 [Hibernate 官方文档](https://hibernate.org/orm/documentation)。
...
- 了解更多关于 Hibernate 的用法，请参考 [Hibernate 官方文档](https://hibernate.org/orm/documentation)。
- 学习 TiDB 应用开发的最佳实践，参考 [开发者指南](https://github.com/pingcap/tidb-dev-guide)，如 [Insert data](/develop/dev-guide-insert-data.md)、[Update data](/develop/dev-guide-update-data.md)、[Delete data](/develop/dev-guide-delete-data.md)、[单表读取](/develop/dev-guide-get-data-from-single-table.md)、[事务](/develop/dev-guide-transaction-overview.md)、[SQL 性能优化](/develop/dev-guide-optimize-sql-overview.md) 等章节。
