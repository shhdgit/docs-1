...
> **Note:**
>
> 出于安全考虑，建议你在通过互联网连接 TiDB 集群时，使用 `VERIFY_IDENTITY` 建立 TLS 连接。TiDB Cloud Serverless、TiDB Cloud Essential 和 TiDB Cloud Dedicated 使用带有 Subject Alternative Name (SAN) 的证书，这要求 MySQL Connector/J 版本大于等于 [8.0.22](https://dev.mysql.com/doc/relnotes/connector-j/en/news-8-0-22.html)。
...
2. 检查 [Expected-Output.txt](https://github.com/tidb-samples/tidb-java-jdbc-quickstart/blob/main/Expected-Output.txt) 是否与输出结果一致。

## 示例代码片段

你可以参考以下示例代码片段，完成你自己的应用开发。

完整示例代码及运行方法请参见 [tidb-samples/tidb-java-jdbc-quickstart](https://github.com/tidb-samples/tidb-java-jdbc-quickstart) 仓库。
