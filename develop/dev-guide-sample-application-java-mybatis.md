...
在 mapper XML 中添加节点，并在 XML 配置文件的 `mapper.namespace` 属性所配置的接口类中添加同名函数：
...
在 mapper XML 中添加节点，并在 XML 配置文件的 `mapper.namespace` 属性所配置的接口类中添加同名函数。特别地，如果你在 MyBatis 查询函数中使用 `resultMap` 作为返回类型，请确保 `<resultMap/>` 节点配置正确。
...
- 通过 [MyBatis 官方文档](http://www.mybatis.org/mybatis-3/) 学习更多 MyBatis 用法。
