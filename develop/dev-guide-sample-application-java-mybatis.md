...
2. 查看 [Expected-Output.txt](https://github.com/tidb-samples/tidb-java-mybatis-quickstart/blob/main/Expected-Output.txt)，确认输出是否一致。
...
请确保将 `${tidb_jdbc_url}`、`${tidb_user}` 和 `${tidb_password}` 替换为你的 TiDB 集群的实际值。同时，将 `${mapper_location}` 替换为你的 mapper XML 配置文件路径。对于多个 mapper XML 文件，需要为每个添加 `<mapper/>` 标签。然后定义以下函数：
...
- 通过专业的 [TiDB 开发者课程](https://www.pingcap.com/education/) 学习，并在考试通过后获得 [TiDB 认证](/develop/dev-guide-overview.md)。
- 通过 Java 开发者课程： [Working with TiDB from Java](https://eng.edu.pingcap.com/catalog/info/id:212)。
