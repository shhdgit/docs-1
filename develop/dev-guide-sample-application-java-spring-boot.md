...
在本教程中，你可以学习如何将 TiDB 与 [Spring Data JPA](https://spring.io/projects/spring-data-jpa) 及 [Hibernate](https://hibernate.org/orm/)（作为 JPA 提供者）结合使用，完成以下任务：
...
Spring Data JPA 通过 `@Repository` 接口管理数据。要使用 `JpaRepository` 提供的 CRUD 操作，需要扩展 `JpaRepository` 接口：
...
- 针对 Java 开发者的课程：[Working with TiDB from Java](https://eng.edu.pingcap.com/catalog/info/id:212)。
