...
TiDB 是一个兼容 MySQL 的数据库，[node-mysql2](https://github.com/sidorares/node-mysql2) 是一个快速的 [mysqljs/mysql](https://github.com/mysqljs/mysql) 兼容的 Node.js MySQL 驱动。
...
6. 编辑 `.env` 文件，按如下方式设置环境变量，并将对应的占位符 `{}` 替换为连接对话框中的参数：
    ...
    > **Note**
    >
    > 对于 TiDB Cloud Starter，使用公有地址时 **必须** 通过 `TIDB_ENABLE_SSL` 启用 TLS 连接。
...
5. 编辑 `.env` 文件，按如下方式设置环境变量，并将对应的占位符 `{}` 替换为连接对话框中的参数：
    ...
    > **Note**
    >
    > 推荐在使用公有地址连接 TiDB Cloud Dedicated 时启用 TLS 连接。
    >
...
2. 编辑 `.env` 文件，按如下方式设置环境变量，并将对应的占位符 `{}` 替换为连接参数：
...
完整示例代码及运行方式请参考 [tidb-samples/tidb-nodejs-mysql2-quickstart](https://github.com/tidb-samples/tidb-nodejs-mysql2-quickstart) 仓库。

### 使用连接参数连接

以下代码通过环境变量定义的参数连接到 TiDB：
...
> **Note**
>
> 对于 TiDB Cloud Starter 和 TiDB Cloud Essential，使用公有地址时 **必须** 通过 `TIDB_ENABLE_SSL` 启用 TLS 连接。但你**不需要**通过 `TIDB_CA_PATH` 指定 SSL CA 证书，因为 Node.js 默认使用内置的 [Mozilla CA 证书](https://wiki.mozilla.org/CA/Included_Certificates)，该证书已被 TiDB Cloud Starter 信任。
...
- 在处理数据库中的大数（`BIGINT` 和 `DECIMAL` 列）时，推荐开启 `supportBigNumbers: true` 选项。
- 推荐开启 `enableKeepAlive: true` 选项，以避免因网络问题导致的 socket 错误 `read ECONNRESET`。（相关 issue：[sidorares/node-mysql2#683](https://github.com/sidorares/node-mysql2/issues/683)）
