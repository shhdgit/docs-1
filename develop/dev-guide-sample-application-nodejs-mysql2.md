...
6. 编辑 `.env` 文件，按照以下方式设置环境变量，将连接对话框中的参数对应替换到 `{}` 占位符：
...
5. 编辑 `.env` 文件，按照以下方式设置环境变量，将连接对话框中的参数对应替换到 `{}` 占位符：
    ...
    > **Note**
    >
    > 建议在使用公共端点连接 TiDB Cloud Dedicated 时启用 TLS。
    >
...
2. 编辑 `.env` 文件，按照以下方式设置环境变量，将连接对话框中的参数对应替换到 `{}` 占位符：
   ...
   如果你在本地运行 TiDB，默认的主机地址为 `127.0.0.1`，密码为空。
...
完整示例代码及运行方式，请查看 [tidb-samples/tidb-nodejs-mysql2-quickstart](https://github.com/tidb-samples/tidb-nodejs-mysql2-quickstart) 仓库。
...
以下查询创建一个单一的 `Player` 记录，并返回一个 `ResultSetHeader` 对象：
...
以下查询为 ID `1` 的 `Player` 添加 `50` 个硬币和 `50` 件商品：
...
- 建议在处理大数字（如 `BIGINT` 和 `DECIMAL` 列）时启用 `supportBigNumbers: true` 选项。
