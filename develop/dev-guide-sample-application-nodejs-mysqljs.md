...
6. 编辑 `.env` 文件，按照以下方式设置环境变量，将连接对话框中的对应占位符 `{}` 替换为连接参数：
    ...
    > **Note**
    >
    > 对于 {{{ .starter }}}，使用公共端点时 **必须** 通过 `TIDB_ENABLE_SSL` 启用 TLS 连接。
...
5. 编辑 `.env` 文件，按照以下方式设置环境变量，将连接对话框中的对应占位符 `{}` 替换为连接参数：
    ...
    >
    > 要启用 TLS 连接，将 `TIDB_ENABLE_SSL` 改为 `true`，并使用 `TIDB_CA_PATH` 指定从连接对话框下载的 CA 证书文件路径。
...
完整示例代码及运行方式，请查阅 [tidb-samples/tidb-nodejs-mysqljs-quickstart](https://github.com/tidb-samples/tidb-nodejs-mysqljs-quickstart) 仓库。
...
- 在没有大量复杂 SQL 语句的场景中，建议使用 ORM 框架以提升开发效率，例如：[Sequelize](https://sequelize.org/)、[TypeORM](https://typeorm.io/)、以及 [Prisma](/develop/dev-guide-sample-application-nodejs-prisma.md)。
