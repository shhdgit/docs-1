...
6. 编辑 `.env` 文件，按如下方式设置环境变量，并将对应的占位符 {} 替换为连接对话框中的参数：
    ...
    > **注意**
    >
    > 对于 TiDB Cloud Serverless，使用公有端点时 **必须** 通过 `TIDB_ENABLE_SSL` 启用 TLS 连接。
...
> **注意**
>
> 对于 TiDB Cloud Serverless 和 TiDB Cloud Essential，使用公有端点时 **必须** 通过 `TIDB_ENABLE_SSL` 启用 TLS 连接。但你**不需要**通过 `TIDB_CA_PATH` 指定 SSL CA 证书，因为 Node.js 默认使用内置的 [Mozilla CA 证书](https://wiki.mozilla.org/CA/Included_Certificates)，该证书已被 TiDB Cloud Serverless 信任。

### 插入数据

以下语句创建一条 `Player` 记录，并返回新建记录的 ID：
...
- 为避免 SQL 注入攻击，建议在执行 SQL 前使用 [Escaping query values](https://github.com/mysqljs/mysql#escaping-query-values) 对参数进行转义。
