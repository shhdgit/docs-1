...
6. 编辑 `.env` 文件，按如下方式设置环境变量 `DATABASE_URL`，并将对应的占位符 `{}` 替换为连接对话框中的连接字符串：
    ...
    > **Note**
    >
    > 对于 {{{ .starter }}}, 使用 public endpoint 时 **必须** 通过设置 `sslaccept=strict` 启用 TLS 连接。
...
5. 编辑 `.env` 文件，按如下方式设置环境变量 `DATABASE_URL`，并将对应的占位符 `{}` 替换为连接对话框中的连接参数：
    ...
    > **Note**
    >
    > 对于 {{{ .starter }}}, 使用 public endpoint 时 **推荐** 通过设置 `sslaccept=strict` 启用 TLS 连接。当你设置 `sslaccept=strict` 启用 TLS 连接时，**必须** 通过 `sslcert=/path/to/ca.pem` 指定从连接对话框下载的 CA 证书文件路径。
...
2. 编辑 `.env` 文件，按如下方式设置环境变量 `DATABASE_URL`，并将对应的占位符 `{}` 替换为你的 TiDB 集群的连接参数：
...
如需了解如何在 Prisma 中定义数据模型，请参考 [Data model](https://www.prisma.io/docs/concepts/components/prisma-schema/data-model) 文档。
...
- [Prisma relation mode](https://www.prisma.io/docs/concepts/components/prisma-schema/relations/relation-mode) 是在 Prisma Client 端模拟参照完整性。但需要注意，这会带来性能影响，因为它需要额外的数据库查询来维护参照完整性。
