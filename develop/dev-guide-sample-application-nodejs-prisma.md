...
2. 编辑 `.env` 文件，配置环境变量 `DATABASE_URL`，用你的 TiDB 集群的连接参数替换对应的占位符 `{}`：
...
model Profile {
  ...
  // 定义 `Player` 和 `Profile` 之间的一对一关系，带外键。
...
```typescript
// 第一步：导入自动生成的 `@prisma/client` 包。
import {Player, PrismaClient} from '@prisma/client';

async function main(): Promise<void> {
  // 第二步：创建一个新的 `PrismaClient` 实例。
  const prisma = new PrismaClient();
  try {

    // 第三步：使用 Prisma Client 执行一些 CRUD 操作...

  } finally {
    // 第四步：断开 Prisma Client 连接。
...
- [Foreign key](https://docs.pingcap.com/tidb/stable/foreign-key) 是从 TiDB v6.6.0 开始支持的功能，v8.5.0 及以上版本已正式支持。外键允许跨表引用相关数据，而外键约束确保相关数据的一致性。

    > **Warning:**
    >
    > **Foreign keys 适用于小到中等规模数据场景。** 在大数据量场景下使用外键可能导致严重的性能问题，甚至对系统产生不可预料的影响。如果你打算使用外键，务必先进行充分验证，并谨慎使用。
