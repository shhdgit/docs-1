...
6. 编辑 `.env` 文件，按如下方式设置环境变量 `DATABASE_URL`，并将对应的占位符 {} 替换为连接对话框中的连接字符串：
    ...
    > **Note**
    >
    > 对于 TiDB Cloud Starter，使用公有端点时，你 **必须** 通过设置 `sslaccept=strict` 启用 TLS 连接。
...
5. 编辑 `.env` 文件，按如下方式设置环境变量 `DATABASE_URL`，将对应的占位符 {} 替换为连接对话框中的连接参数：
    ...
    > **Note**
    >
    > 对于 TiDB Cloud Starter，**推荐** 在使用公有端点时通过设置 `sslaccept=strict` 启用 TLS 连接。当你设置 `sslaccept=strict` 启用 TLS 连接时，**必须** 通过 `sslcert=/path/to/ca.pem` 指定从连接对话框下载的 CA 证书文件路径。
