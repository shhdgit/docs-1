...
6. 编辑 `.env` 文件，按如下方式设置环境变量 `DATABASE_URL`，并将对应的占位符 {} 替换为连接对话框中的连接字符串：
    ...
    > **Note**
    >
    > 对于 TiDB Cloud Serverless，使用公有端点时，你**必须**通过设置 `sslaccept=strict` 启用 TLS 连接。
...
5. 编辑 `.env` 文件，按如下方式设置环境变量 `DATABASE_URL`，并将对应的占位符 {} 替换为连接对话框中的连接参数：
    ...
    > **Note**
    >
    > 对于 TiDB Cloud Serverless，**推荐**在使用公有端点时通过设置 `sslaccept=strict` 启用 TLS 连接。当你设置 `sslaccept=strict` 启用 TLS 连接时，**必须**通过 `sslcert=/path/to/ca.pem` 指定从连接对话框下载的 CA 证书文件路径。
...
要检查 [参照完整性](https://en.wikipedia.org/wiki/Referential_integrity?useskin=vector)，你可以使用外键约束或 Prisma 关系模式：
...
- [Prisma 关系模式](https://www.prisma.io/docs/concepts/components/prisma-schema/relations/relation-mode) 是在 Prisma Client 端模拟参照完整性。但需要注意，这会带来性能影响，因为它需要额外的数据库查询来维护参照完整性。
