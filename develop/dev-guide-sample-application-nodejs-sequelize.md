...
summary: 学习如何使用 Sequelize 连接 TiDB。本教程提供了适用于 TiDB 的 Node.js 示例代码片段，演示如何通过 Sequelize 进行操作。
---

# 使用 Sequelize 连接 TiDB

TiDB 是一个兼容 MySQL 的数据库，[Sequelize](https://sequelize.org/) 是 Node.js 领域流行的 ORM 框架。
...
4. 点击 **Generate Password** 生成一个随机密码。
    ...
    > **Tip**
    >
    > 如果你之前已经生成过密码，可以继续使用原有密码，或点击 **Reset Password** 重新生成。
...
完整的示例代码及运行方式请参考 [tidb-samples/tidb-nodejs-sequelize-quickstart](https://github.com/tidb-samples/tidb-nodejs-sequelize-quickstart) 仓库。
