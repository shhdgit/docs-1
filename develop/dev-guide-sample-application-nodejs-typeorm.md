...
# TiDBにTypeORMを接続する {#connect-to-tidb-with-typeorm}
...
以下のコマンドを実行して、サンプルアプリケーションに必要なパッケージ（`typeorm`および`mysql2`を含む）をインストールします：
...
- `typeorm`: Node.js用のORMフレームワーク。
- `mysql2`: Node.js用のMySQLドライバ。`mysql`ドライバも使用できます。
...
4. まだパスワードを設定していない場合は、ランダムなパスワードを生成するには **Generate Password** をクリックします。
...
6. `.env` ファイルを編集し、接続ダイアログの接続パラメータに対応するプレースホルダ `{}` を置き換えて、環境変数を次のように設定します：
   ...
   > **注意**
   >
   > TiDB Serverless の場合、パブリックエンドポイントを使用する場合は、`TIDB_ENABLE_SSL` を介して TLS 接続を有効にする必要があります。
...
5. `.env` ファイルを編集し、接続ダイアログの接続パラメータに対応するプレースホルダ `{}` を置き換えて、環境変数を次のように設定します：
   ...
   > **注意**
   >
   > TiDB Dedicated の場合、パブリックエンドポイントを使用する場合は、`TIDB_ENABLE_SSL` を介して TLS 接続を有効にすることが**推奨**されます。`TIDB_ENABLE_SSL=true` を設定すると、接続ダイアログからダウンロードした CA 証明書のパスを `TIDB_CA_PATH=/path/to/ca.pem` で指定する必要があります。
...
2. `.env` ファイルを編集し、TiDBクラスタの接続パラメータに対応するプレースホルダ `{}` を置き換えて、環境変数を次のように設定します：
...
### ステップ4：データベーススキーマの初期化 {#step-4-initialize-the-database-schema}

次のコマンドを実行して、`src/migrations` フォルダに記述された SQL ステートメントを使用してデータベースを初期化するために TypeORM CLI を呼び出します：
...
移行ファイルは、`src/entities`フォルダで定義されたエンティティから生成されます。TypeORMでエンティティを定義する方法については、[TypeORM: Entities](https://typeorm.io/entities)を参照してください。
...
次のクエリは単一の `Player` レコードを作成し、TiDB によって生成された `id` フィールドを含む `Player` オブジェクトを返します。
...
次のクエリは、ID 101の単一の `Player` オブジェクトを返します。レコードが見つからない場合は `null` を返します。
...
次のクエリは、ID `101` の `Player` に `50` 個の商品を追加します。
...
[外部キー制約](https://docs.pingcap.com/tidb/stable/foreign-key) (実験的) を使用すると、データベース側でチェックを追加することで、データの[参照整合性](https://en.wikipedia.org/wiki/Referential_integrity)を確保できます。ただし、これは大容量のデータのシナリオで深刻なパフォーマンスの問題を引き起こす可能性があります。

`createForeignKeyConstraints` オプションを使用して、エンティティ間の関係を構築する際に外部キー制約を作成するかどうかを制御できます（デフォルト値は `true` です）。
...
詳細については、[TypeORM FAQ](https://typeorm.io/relations-faq#avoid-foreign-key-constraint-creation) および [Foreign key constraints](https://docs.pingcap.com/tidbcloud/foreign-key#foreign-key-constraints) を参照してください。

## 次のステップ {#next-steps}

- [TypeORM のドキュメント](https://typeorm.io/) から TypeORM のさらなる使用法を学びます。
...
- プロフェッショナルな [TiDB 開発者コース](https://www.pingcap.com/education/) を通じて学び、試験に合格した後に [TiDB 認定](https://www.pingcap.com/education/certification/) を取得します。
