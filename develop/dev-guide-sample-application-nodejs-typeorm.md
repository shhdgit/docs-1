...
# TypeORMを使用してTiDBに接続 {#connect-to-tidb-with-typeorm}
...
6. `.env` ファイルを編集し、接続ダイアログの接続パラメータに対応するプレースホルダ `{}` を置き換えて、環境変数を次のように設定します：
   ...
   > **注意**
   >
   > TiDB Serverless の場合、パブリックエンドポイントを使用する場合は、`TIDB_ENABLE_SSL` を介してTLS接続を有効にする必要があります。
...
5. `.env` ファイルを編集し、接続ダイアログの接続パラメータに対応するプレースホルダ `{}` を置き換えて、環境変数を次のように設定します：
   ...
   > **注意**
   >
   > TiDB Dedicated の場合、パブリックエンドポイントを使用する場合は、`TIDB_ENABLE_SSL` を介してTLS接続を有効にすることが**推奨**されます。`TIDB_ENABLE_SSL=true` を設定すると、接続ダイアログからダウンロードしたCA証明書のパスを `TIDB_CA_PATH=/path/to/ca.pem` で指定する必要があります。
...
次のSQLステートメントは、`players`テーブルと`profiles`テーブルを作成し、そして2つのテーブルは外部キーを介して関連付けられています。
...
接続に成功した場合、ターミナルにはTiDBクラスタのバージョンが以下のように出力されます：
...
>
> ただし、Node.jsはデフォルトで組み込みの[Mozilla CA証明書](https://wiki.mozilla.org/CA/Included_Certificates)を使用しているため、`TIDB_CA_PATH`を介してSSL CA証明書を指定する必要はありません。この証明書はTiDB Serverlessによって信頼されています。
...
次のクエリは、ID 101の単一の `Player` オブジェクトを返します。見つからない場合は `null` を返します。
...
以下は、[データのクエリ](/develop/dev-guide-get-data-from-single-table.md)に関する詳細情報です。
...
[外部キー制約](https://docs.pingcap.com/tidb/stable/foreign-key) (実験的) を使用すると、データベース側でチェックを追加することで、データの[参照整合性](https://en.wikipedia.org/wiki/Referential_integrity)を確保できます。ただし、これは大容量のデータがあるシナリオでは深刻なパフォーマンスの問題を引き起こす可能性があります。
