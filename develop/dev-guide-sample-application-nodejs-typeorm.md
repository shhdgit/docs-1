...
summary: 学习如何使用 TypeORM 连接 TiDB。本教程提供了适用于 Node.js 的 TypeORM 示例代码片段，可用于操作 TiDB。
---

# 使用 TypeORM 连接 TiDB

TiDB 是一个兼容 MySQL 的数据库，[TypeORM](https://github.com/TypeORM/TypeORM) 是 Node.js 上流行的开源 ORM 框架。
...
2. 编辑 `.env` 文件，按如下方式设置环境变量，并将对应的占位符 `{}` 替换为你的 TiDB 集群连接参数：
...
以下 SQL 语句会创建一个 `players` 表和一个 `profiles` 表，并通过外键将两个表关联起来。
...
Migration 文件是根据 `src/entities` 文件夹下定义的实体生成的。关于如何在 TypeORM 中定义实体，参考 [TypeORM: Entities](https://typeorm.io/entities)。
...
完整示例代码及运行方法请参考 [tidb-samples/tidb-nodejs-typeorm-quickstart](https://github.com/tidb-samples/tidb-nodejs-typeorm-quickstart) 仓库。

### 使用连接选项进行连接

以下代码通过环境变量中定义的选项建立到 TiDB 的连接：
...
更多信息参考 [TypeORM FAQ](https://typeorm.io/relations-faq#avoid-foreign-key-constraint-creation) 和 [外键约束](https://docs.pingcap.com/tidbcloud/foreign-key#foreign-key-constraints)。
