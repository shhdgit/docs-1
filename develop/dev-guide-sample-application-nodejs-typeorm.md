...
运行以下命令，为示例应用安装所需依赖（包括 `typeorm` 和 `mysql2`）：
...
6. 编辑 `.env` 文件，按如下方式设置环境变量，并将对应的占位符 `{}` 替换为连接对话框中的参数：
    ...
    > **Note**
    >
    > 对于 TiDB Cloud Starter 和 TiDB Cloud Essential，当使用公网连接时，**必须** 通过 `TIDB_ENABLE_SSL` 启用 TLS 连接。
...
运行以下命令，调用 TypeORM CLI，根据 `src/migrations` 目录下的 migration 文件中的 SQL 语句初始化数据库：
...
Migration 文件是根据 `src/entities` 目录下定义的实体生成的。关于如何在 TypeORM 中定义实体，参考 [TypeORM: Entities](https://typeorm.io/entities)。
...
> **Note**
>
> 对于 TiDB Cloud Starter 和 TiDB Cloud Essential，当使用公网连接时，**必须** 启用 TLS 连接。在本示例代码中，请在 `.env` 文件中将环境变量 `TIDB_ENABLE_SSL` 设置为 `true`。
>
