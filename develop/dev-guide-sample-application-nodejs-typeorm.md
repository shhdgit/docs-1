...
6. 编辑 `.env` 文件，按如下方式设置环境变量，并将对应的占位符 `{}` 替换为连接对话框中的参数：
    ...
    > **Note**
    >
    > 对于 {{{ .starter }}} 和 {{{ .essential }}}, 使用公网连接时，**必须** 通过 `TIDB_ENABLE_SSL` 启用 TLS 连接。
...
> **Note**
>
> 对于 {{{ .starter }}} 和 {{{ .essential }}}, 使用公网连接时，必须启用 TLS 连接。在本示例代码中，请在 `.env` 文件中将环境变量 `TIDB_ENABLE_SSL` 设置为 `true`。
>
...
使用 [外键约束](https://docs.pingcap.com/tidb/stable/foreign-key) 可以通过在数据库端增加校验，保证数据的 [参照完整性](https://en.wikipedia.org/wiki/Referential_integrity)。但在大数据量场景下，可能会带来严重的性能问题。
