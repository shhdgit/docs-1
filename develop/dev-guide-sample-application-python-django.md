...
summary: 学习如何使用 Django 连接 TiDB。本教程提供了可与 TiDB 配合使用的 Python 示例代码片段。
---
...
6. 将对应的连接字符串复制粘贴到 `.env` 文件中。示例结果如下：
    ...
    TiDB Cloud Serverless 要求安全连接。由于 mysqlclient 的 `ssl_mode` 默认是 `PREFERRED`，你无需手动指定 `CA_PATH`，只需留空即可。但如果你有特殊需求需要手动指定 `CA_PATH`，可以参考 [TLS 连接到 TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/secure-connections-to-serverless-clusters) 获取不同操作系统的证书路径。
...
1. 以开发模式运行应用：
    ...
    应用默认运行在 `8000` 端口。如需使用其他端口，可以在命令后追加端口号。例如：
