...
summary: 学习如何使用 Django 连接 TiDB。本教程提供了可在 Django 中与 TiDB 配合使用的 Python 示例代码片段。
---
...
你需要将 `${tidb_host}`、`${tidb_port}`、`${tidb_user}`、`${tidb_password}`、`${tidb_db_name}` 和 `${ca_path}` 替换为你 TiDB 集群的实际值。
