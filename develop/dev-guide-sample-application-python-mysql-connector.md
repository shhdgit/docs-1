...
このチュートリアルでは、以下のタスクを実行するために、TiDBとMySQL Connector/Pythonを使用する方法を学ぶことができます。

- 環境をセットアップする。
- MySQL Connector/Pythonを使用してTiDBクラスターに接続する。
...
サンプルアプリケーションに必要なパッケージ（mysql-connector-pythonを含む）をインストールするには、次のコマンドを実行してください。
...
3. 接続ダイアログの設定が、実行環境と一致することを確認してください。
   ...
   - **Operating System**は実行環境に合わせて設定されていること
...
2. 対応する接続文字列を`.env`ファイルにコピーして貼り付けます。例の結果は次のとおりです。
   ...
   プレースホルダ`{}`を接続パラメータで置き換え、`CA_PATH`の行を削除してください。TiDBをローカルで実行している場合、デフォルトのホストアドレスは`127.0.0.1`で、パスワードは空です。
...
2. 出力が[Expected-Output.txt](https://github.com/tidb-samples/tidb-python-mysqlconnector-quickstart/blob/main/Expected-Output.txt)と一致するかどうかを確認します。

## サンプルコードスニペット {#sample-code-snippets}

以下のサンプルコードを参考に、独自のアプリケーション開発を完了させることができます。

完全なサンプルコードと実行方法については、[tidb-samples/tidb-python-mysqlconnector-quickstart](https://github.com/tidb-samples/tidb-python-mysqlconnector-quickstart) リポジトリをご確認ください。
...
- データベース接続の手動の確立と解放。
- データベーストランザクションの手動管理。
- データ行（`mysql-connector-python`でタプルまたはdictとして表される）をデータオブジェクトに手動でマップする。
...
- [MySQL Connector/Pythonのドキュメント](https://dev.mysql.com/doc/connector-python/en/)からmysql-connector-pythonのより詳細な使用方法を学びます。
