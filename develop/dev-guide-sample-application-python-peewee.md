...
5. 将对应的连接字符串复制粘贴到 `.env` 文件中。示例内容如下：
    ...
    请确保用连接对话框中获取的参数替换 `{}` 占位符，并将 `CA_PATH` 配置为之前步骤中下载的证书路径。
...
2. 将对应的连接字符串复制粘贴到 `.env` 文件中。示例内容如下：
    ...
    请确保用连接参数替换 `{}` 占位符，并删除 `CA_PATH` 行。如果你在本地运行 TiDB，默认主机地址为 `127.0.0.1`，密码为空。
...
完整示例代码及运行方法，请查看 [tidb-samples/tidb-python-peewee-quickstart](https://github.com/tidb-samples/tidb-python-peewee-quickstart) 仓库。
