...
summary: 学习如何使用 peewee 连接 TiDB。本教程提供了可用于 TiDB 的 Python 示例代码片段，基于 peewee 实现。
---

# 使用 peewee 连接 TiDB

TiDB 是一个兼容 MySQL 的数据库，[peewee](https://docs.peewee-orm.com/) 是 Python 领域流行的对象关系映射（ORM）工具。
...
peewee 是一个支持多种数据库的 ORM 库。它为数据库操作提供了高级抽象，帮助开发者以更面向对象的方式编写 SQL 语句。但 peewee 并不自带数据库驱动。要连接数据库，你需要安装相应的数据库驱动。本示例应用使用 PyMySQL 作为数据库驱动，它是一个纯 Python 实现的 MySQL 客户端库，兼容 TiDB，并可在所有平台上安装。更多信息可参考 [peewee 官方文档](https://docs.peewee-orm.com/en/latest/peewee/database.html?highlight=mysql#using-mysql)。
...
2. 将对应的连接字符串复制粘贴到 `.env` 文件中。示例结果如下：
    ...
    请务必将 `{}` 占位符替换为实际连接参数，并删除 `CA_PATH` 这一行。如果你在本地运行 TiDB，默认主机地址为 `127.0.0.1`，密码为空。
