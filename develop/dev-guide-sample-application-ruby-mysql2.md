...
6. 编辑 `.env` 文件，按如下方式设置环境变量，并将对应的占位符 `{}` 替换为连接对话框中的参数：
   ...
   > **注意**
   >
   > 对于 [TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential)，当使用公有端点时，**必须** 通过 `DATABASE_ENABLE_SSL` 启用 TLS 连接。
...
3. 在连接对话框中，从 **Connection Type** 下拉列表选择 **Public**，然后点击 **CA cert** 下载 CA 证书。
    ...
    除了 **Public** 连接类型，TiDB Cloud Dedicated 还支持 **Private Endpoint** 和 **VPC Peering** 连接类型。更多信息参见 [连接到 TiDB Cloud Dedicated 集群](https://docs.pingcap.com/tidbcloud/connect-to-tidb-cluster)。
...
5. 编辑 `.env` 文件，按如下方式设置环境变量，并将对应的占位符 `{}` 替换为连接对话框中的参数：
   ...
   > **注意**
   >
   > 推荐在使用公有端点连接 TiDB Cloud Dedicated 集群时启用 TLS 连接。
   >
   > 若要启用 TLS 连接，请将 `DATABASE_ENABLE_SSL` 设置为 `true`，并通过 `DATABASE_SSL_CA` 指定从连接对话框下载的 CA 证书文件路径。
...
> **注意**
>
> 对于 [TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential)，当使用公有端点时，**必须** 通过 `DATABASE_ENABLE_SSL` 启用 TLS 连接，但你**不需要**通过 `DATABASE_SSL_CA` 指定 SSL CA 证书，因为 mysql2 gem 会按特定顺序自动查找本地已存在的 CA 证书文件，直到找到为止。

### 插入数据

以下查询会创建一个包含两个字段的玩家，并返回 `last_insert_id`：
...
更多信息参见 [插入数据](/develop/dev-guide-insert-data.md)。

### 查询数据

以下查询会根据 ID 返回指定玩家的记录：
...
更多信息参见 [更新数据](/develop/dev-guide-update-data.md)。
...
- 通过 [开发者指南](/develop/dev-guide-overview.md) 各章节，学习 TiDB 应用开发的最佳实践，例如：[插入数据](/develop/dev-guide-insert-data.md)、[更新数据](/develop/dev-guide-update-data.md)、[删除数据](/develop/dev-guide-delete-data.md)、[查询数据](/develop/dev-guide-get-data-from-single-table.md)、[事务](/develop/dev-guide-transaction-overview.md) 以及 [SQL 性能优化](/develop/dev-guide-optimize-sql-overview.md)。
