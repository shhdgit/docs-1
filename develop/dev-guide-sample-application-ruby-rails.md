...
6. 编辑 `.env` 文件，设置 `DATABASE_URL` 环境变量如下，并将连接对话框中的连接字符串复制为变量值。
...
5. 编辑 `.env` 文件，设置 `DATABASE_URL` 环境变量如下，将连接对话框中的连接字符串复制为变量值，并将 `sslca` 查询参数设置为刚才下载的 CA 证书文件路径：
   ...
   >
   > 启用 TLS 连接时，需要将 `ssl_mode` 查询参数设置为 `verify_identity`，并将 `sslca` 设置为从连接对话框下载的 CA 证书文件路径。
...
2. 编辑 `.env` 文件，设置 `DATABASE_URL` 环境变量如下，并将 `{user}`、`{password}`、`{host}`、`{port}`、`{database}` 替换为你自己的 TiDB 连接信息：
...
`config/database.yml` 文件中的以下代码，通过环境变量定义的参数建立与 TiDB 的连接：
...
> **Note**
>
> 对于 [TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential)，使用公共连接地址时，**必须** 在 `DATABASE_URL` 中通过设置 `ssl_mode` 查询参数为 `verify_identity` 启用 TLS 连接，但**不需要**通过 `DATABASE_URL` 指定 SSL CA 证书，因为 mysql2 gem 会自动按顺序查找本地已存在的 CA 证书文件，直到找到为止。
