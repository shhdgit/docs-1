...
3. 在连接对话框中，从 **Connect With** 下拉列表选择 `Rails`，并保持 **Connection Type** 的默认设置为 `Public`。
...
5. 编辑 `.env` 文件，按如下方式设置 `DATABASE_URL` 环境变量，将连接对话框中的连接字符串复制为变量值，并将 `sslca` 查询参数设置为刚才下载的 CA 证书文件路径：
   ...
   >
   > 启用 TLS 连接时，请将 `ssl_mode` 查询参数设置为 `verify_identity`，并将 `sslca` 设置为从连接对话框下载的 CA 证书文件路径。
...
2. 编辑 `.env` 文件，按如下方式设置 `DATABASE_URL` 环境变量，并将 `{user}`、`{password}`、`{host}`、`{port}`、`{database}` 替换为你自己的 TiDB 连接信息：
