...
summary: 学习如何使用 Rails 框架连接 TiDB。本教程提供了适用于 TiDB 的 Ruby 示例代码片段，基于 Rails 框架和 ActiveRecord ORM。
---
...
6. 编辑 `.env` 文件，按如下方式设置 `DATABASE_URL` 环境变量，并将连接对话框中的连接字符串复制为变量值。
   ...
   > **Note**
   >
   > 对于 [TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential)，使用公共连接地址时，**必须** 通过 `ssl_mode=verify_identity` 查询参数启用 TLS 连接。
...
3. 在连接对话框中，从 **Connection Type** 下拉列表中选择 **Public**，然后点击 **CA cert** 下载 CA 证书。
    ...
    如果你还未配置 IP 访问列表，点击 **Configure IP Access List**，或参考 [配置 IP 访问列表](https://docs.pingcap.com/tidbcloud/configure-ip-access-list) 完成配置后再进行首次连接。
...
5. 编辑 `.env` 文件，按如下方式设置 `DATABASE_URL` 环境变量，将连接对话框中的连接字符串复制为变量值，并将 `sslca` 查询参数设置为刚才下载的 CA 证书文件路径：
   ...
   >
   > 启用 TLS 连接时，请将 `ssl_mode` 查询参数的值设置为 `verify_identity`，并将 `sslca` 的值设置为从连接对话框下载的 CA 证书文件路径。
...
2. 编辑 `.env` 文件，按如下方式设置 `DATABASE_URL` 环境变量，并将 `{user}`、`{password}`、`{host}`、`{port}` 和 `{database}` 替换为你自己的 TiDB 连接信息：
...
以下 `config/database.yml` 文件中的代码，通过环境变量定义的选项建立与 TiDB 的连接：
...
> **Note**
>
> 对于 [TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential)，使用公共连接地址时，**必须** 在 `DATABASE_URL` 中通过设置 `ssl_mode` 查询参数为 `verify_identity` 启用 TLS 连接，但你**不需要**通过 `DATABASE_URL` 指定 SSL CA 证书，因为 mysql2 gem 会按特定顺序自动查找本地已存在的 CA 证书文件，直到找到为止。

### 插入数据

以下查询创建一个包含两个字段的 Player，并返回新建的 `Player` 对象：
...
默认情况下，ActiveRecord ORM 连接 TiDB 时使用的 mysql2 gem 会按如下顺序查找本地已存在的 CA 证书文件，直到找到为止。
...
- 通过 [开发者指南](/develop/dev-guide-overview.md) 中的各章节，学习 TiDB 应用开发的最佳实践，例如：[插入数据](/develop/dev-guide-insert-data.md)、[更新数据](/develop/dev-guide-update-data.md)、[删除数据](/develop/dev-guide-delete-data.md)、[查询数据](/develop/dev-guide-get-data-from-single-table.md)、[事务](/develop/dev-guide-transaction-overview.md) 以及 [SQL 性能优化](/develop/dev-guide-optimize-sql-overview.md)。
