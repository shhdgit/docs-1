...
- 构建并运行你的应用程序。你还可以在 [示例代码片段](#sample-code-snippets) 部分找到基于 ActiveRecord ORM 的基本 CRUD 操作示例。
...
6. 编辑 `.env` 文件，按如下格式设置 `DATABASE_URL` 环境变量，并将连接对话框中的连接字符串复制为变量值。
   ...
   > **Note**
   >
   > 对于 [TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/select-cluster-tier#starter) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential)，使用公有端点时，**必须** 通过 `ssl_mode=verify_identity` 查询参数启用 TLS 连接。
...
5. 编辑 `.env` 文件，按如下格式设置 `DATABASE_URL` 环境变量，将连接对话框中的连接字符串复制为变量值，并将 `sslca` 查询参数设置为刚才下载的 CA 证书文件路径：
   ...
   > **Note**
   >
   > 推荐在使用公有端点连接 TiDB Cloud Dedicated 时启用 TLS 连接。
   >
...
完整示例代码及运行方式请参考 [tidb-samples/tidb-ruby-rails-quickstart](https://github.com/tidb-samples/tidb-ruby-rails-quickstart) 仓库。
