...
6. 编辑 `.env` 文件，设置 `DATABASE_URL` 环境变量，并将连接对话框中的连接字符串作为变量值：
...
5. 编辑 `.env` 文件，设置 `DATABASE_URL` 环境变量，将连接对话框中的连接字符串作为变量值，并将 `sslca` 参数设置为刚才下载的 CA 证书文件路径：
...
2. 编辑 `.env` 文件，设置 `DATABASE_URL` 环境变量，并将 `{user}`、`{password}`、`{host}`、`{port}`、`{database}` 替换为你自己的 TiDB 连接信息：
