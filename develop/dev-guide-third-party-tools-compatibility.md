...
不要使用错误信息进行字符串匹配。建议使用 [Error Codes](/error-codes.md) 进行排查。

</CustomContent>

<CustomContent platform="tidb-cloud">

不要使用错误信息进行字符串匹配。建议使用 [Error Codes](https://docs.pingcap.com/tidb/stable/error-codes) 进行排查。
...
TiDB 不会在协议中设置 `SERVER_QUERY_NO_GOOD_INDEX_USED` 和 `SERVER_QUERY_NO_INDEX_USED` 参数。这会导致以下参数返回的值与实际情况不一致：
...
TiDB 不支持 `UpdatableResultSet`。**切勿**在连接参数中指定 `ResultSet.CONCUR_UPDATABLE`，也不要在 `ResultSet` 内部进行数据更新。
...
在使用 MySQL Connector/J 8.0.32 或更早版本时，如果同时设置 `useLocalTransactionState` 和 `rewriteBatchedStatements` 为 `true`，可能导致事务无法提交。可通过 [此代码](https://github.com/Icemap/tidb-java-gitpod/tree/reproduction-local-transaction-state-txn-error) 重现。

**避免方法**

> **Note:**
>
> `useConfigs=maxPerformance` 包含一组配置。关于 MySQL Connector/J 8.0 和 5.1 的详细配置，请参见 [mysql-connector-j 8.0](https://github.com/mysql/mysql-connector-j/blob/release/8.0/src/main/resources/com/mysql/cj/configurations/maxPerformance.properties) 和 [mysql-connector-j 5.1](https://github.com/mysql/mysql-connector-j/blob/release/5.1/src/com/mysql/jdbc/configs/maxPerformance.properties)。在使用 `maxPerformance` 时，需禁用 `useLocalTransactionState`，即使用 `useConfigs=maxPerformance&useLocalTransactionState=false`。

此问题已在 MySQL Connector/J 8.0.33 中修复。考虑到 8.0.x 系列的更新已停止，强烈建议升级你的 MySQL Connector/J 至 [最新的正式版 (GA)](https://dev.mysql.com/downloads/connector/j/) 以获得更好的稳定性和性能。
...
此问题已在 MySQL Connector/J 8.0.32 中修复。考虑到 8.0.x 系列的更新已停止，强烈建议升级你的 MySQL Connector/J 至 [最新的正式版 (GA)](https://dev.mysql.com/downloads/connector/j/) 以获得更好的稳定性和性能。
...
- 不支持修改整数主键。
- `PROCEDURE` 不支持。
- 不支持 [READ-UNCOMMITTED](https://system-variables.md#transaction_isolation) 和 [SERIALIZABLE](https://system-variables.md#transaction_isolation) 隔离级别。
...
只使用 TiDB 支持的隔离级别：`REPEATABLE-READ` 或 `READ-COMMITTED`。
...
参考 [AUTO_INCREMENT 的限制](https://auto-increment.md#restrictions)。  
若要允许删除 `AUTO_INCREMENT` 属性，可将 `@@tidb_allow_remove_auto_inc` 设置为 `true`。
