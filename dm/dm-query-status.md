...
| Syncフェーズの1つのサブタスクが `Running` の状態ですが、Relay処理ユニットが実行されていません（`Error`/`Paused`/`Stopped` の状態）。 | `エラー - RelayステータスがError/Paused/Stopped` |
| 1つのサブタスクが `Paused` の状態でエラー情報が返されません。                                                       | `Paused`                                |
| すべてのサブタスクが `New` の状態です。                                                                    | `New`                                   |
| すべてのサブタスクが `Finished` の状態です。                                                               | `Finished`                              |
| すべてのサブタスクが `Stopped` の状態です。                                                                | `Stopped`                               |
| その他の状況                                                                                     | `Running`                               |
...
- `sources`: 上流のMySQLインスタンスのリスト。各ソースには次のフィールドが含まれています。
  ...
  - `subTaskStatus`: 上流のMySQLデータベースのすべてのサブタスクの情報。各サブタスクには次のフィールドが含まれる場合があります。
    ...
    - `unit`: DMの処理単位。"Check"、"Dump"、"Load"、"Sync"を含みます。
    - `result`: サブタスクが失敗した場合、エラー情報を表示します。
    ...
    - `sync`: `Sync`処理ユニットのレプリケーション情報。この情報は、現在の処理ユニットと同じコンポーネントに関するものです。
      ...
      - `blockingDDLs`: 現在ブロックされているDDLリスト。このDM-workerのすべての上流テーブルが「synced」ステータスの場合にのみ空でなくなります。この場合、実行されるべきまたはスキップされるシャーディングDDLステートメントを示します。
      - `unresolvedGroups`: 解決されていないシャーディンググループ。各グループには次のフィールドが含まれます。
        - `target`: レプリケートする下流のデータベーステーブル。
        - `DDLs`: DDLステートメントのリスト。
        - `firstPos`: シャーディングDDLステートメントの開始位置。
        - `synced`: `Sync`ユニットによって実行された上流のシャーディングテーブル。
...
- `Finished`:
  ...
  - 完了したサブタスクのステータス。
  - 完全なレプリケーションサブタスクが正常に終了した場合のみ、タスクはこのステータスに切り替わります。
