...
実行可能なMySQLインスタンスを2つ準備します。Dockerを使用してMySQLを素早く開始することもできます。コマンドは次のとおりです：
...
### 下流のTiDBを開始 {#start-downstream-tidb}
...
> **Note:**
>
> - データベースにパスワードがない場合、このステップをスキップできます。
> - DM v1.0.6以降のバージョンでは、平文のパスワードを使用してソース情報を構成できます。

安全上の理由から、暗号化されたパスワードを構成して使用することをお勧めします。MySQL/TiDBのパスワードを暗号化するには、dmctlを使用できます。パスワードが "123456" の場合を想定します。
...
DMクラスターにMySQL1のデータソース設定をロードするには、ターミナルで以下のコマンドを実行します。
...
[準備されたデータ](#prepare-data)をインポートした後、MySQL1とMySQL2の両方にいくつかのシャードテーブルがあります。これらのテーブルは同じ構造を持ち、テーブル名にはすべて "t" の接頭辞があります。これらのテーブルが存在するデータベースにはすべて "sharding" の接頭辞があり、プライマリキーまたはユニークキーには競合がありません（各シャードテーブルには、他のテーブルとは異なるプライマリキーまたはユニークキーがあります）。

さて、これらのシャードテーブルを `db_target.t_target` テーブルにTiDBに移行する必要があるとします。手順は次のとおりです。
...
上流のMySQLシャードテーブルのデータを変更できます。その後、[sync-diff-inspector](/sync-diff-inspector/shard-diff.md)を使用して、上流と下流のデータが一致しているかどうかを確認できます。一致するデータは、移行タスクが正常に機能していることを意味し、クラスターが正常に機能していることも示します。
