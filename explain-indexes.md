...
`IndexLookup` 操作符也可以用来高效优化对索引列的 `LIMIT` 查询：
...
在上述语句中，对每个 TiKV Region 执行 `IndexFullScan` 任务。尽管名为 `FullScan`，但实际上只需要读取第一行（`└─Limit_28`）。每个 TiKV Region 返回其 `MIN` 或 `MAX` 值，TiDB 再进行流式聚合以筛选出单行。使用 `MAX` 或 `MIN` 聚合函数的流式聚合还确保在表为空时返回 `NULL`。
...
在上述示例中，`IndexFullScan` 比 `TableFullScan` 更高效，因为 `(intkey + RowID)` 索引中的值宽度小于完整行的宽度。

以下语句不支持使用 `IndexFullScan` 操作符，因为需要从表中检索额外的列：
