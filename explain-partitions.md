...
* 每个协处理器请求返回一行数据到 TiDB 内部的 `└─TableReader_22`，然后在 `StreamAgg_21` 中进行流式聚合，最后将一行结果返回给客户端。
...
* TiDB 认为需要访问全部分区（`p2016..pMax`）。这是因为谓词 `YEAR(d) = 2017` 被认为是 [non-sargable](https://en.wikipedia.org/wiki/Sargable)。这个问题并非 TiDB 独有。
