...
TiDB 对 [多项优化](/subquery-optimization.md) 进行了优化，以提升子查询的性能。本文档描述了这些优化中的一些常见子查询优化方法，以及如何解读 `EXPLAIN` 的输出。
...
在以下示例中，`IN` 子查询搜索 `t2` 表中的一组 ID。为了语义正确性，TiDB 需要保证 `t1_id` 列是唯一的。使用 `EXPLAIN`，你可以看到用来去重并执行 `INNER JOIN` 操作的执行计划：
...
在前例中，为了确保 `t1_id` 的值唯一，进行了聚合操作后再与 `t1` 表连接。但在以下示例中，`t3.t1_id` 已经通过 `UNIQUE` 约束保证唯一：
...
在前两个示例中，TiDB 能在子查询中的数据经过 `StreamAgg` 去重或本身保证唯一后，执行 `INNER JOIN`。这两种连接都采用索引连接。
...
从结果可以看出，TiDB 使用了 _semi join_ 算法。semi join 与 inner join 的区别在于：semi join 只允许右侧键（`t2.t1_id`）的第一个值，意味着重复值在 join 操作中被去除。该 join 算法也是 Merge Join，类似于高效的拉链式合并，操作符会以排序的方式从左右两侧读取数据。
...
此查询先读取 `t3` 表，然后基于 `PRIMARY KEY` 进行探测 `t1` 表。连接类型为 _anti semi join_；anti 表示此示例用于判断值不存在（`NOT IN`），semi 表示只需匹配到第一个符合条件的行即可拒绝连接。

## Null-aware semi join (`IN` 和 `= ANY` 子查询)

`IN` 或 `= ANY` 集合操作符的值是三值（`true`、`false` 和 `NULL`）。对于由这两个操作符转换而来的连接类型，TiDB 需要意识到两边连接键上的 `NULL`，并以特殊方式处理。

包含 `IN` 和 `= ANY` 操作符的子查询会被转换为 semi join 和左外 semi join。前述 [Semi join](#semi-join-相关子查询) 示例中，由于两边的列 `test.t1.id` 和 `test.t2.t1_id` 都不是 `NULL`，因此 semi join 不需要考虑 null-aware（`NULL` 不会被特殊处理）。TiDB 以笛卡尔积和过滤的方式处理 null-aware semi join，不进行特殊优化。示例如下：
...
在第一个语句 `EXPLAIN SELECT (a,b) IN (SELECT * FROM s) FROM t;` 中，由于表 `t` 和 `s` 的列 `a` 和 `b` 是可为空的，`IN` 子查询转换的左外 semi join 是 null-aware。具体来说，先计算笛卡尔积，然后将连接 `IN` 或 `= ANY` 的列作为普通的等值条件加入其他条件进行过滤。

在第二个语句 `EXPLAIN SELECT * FROM t WHERE (a,b) IN (SELECT * FROM s);` 中，由于列 `a` 和 `b` 也是可为空的，`IN` 子查询本应转换为 null-aware semi join。但 TiDB 通过将 semi join 转为 inner join 和聚合进行优化。这是因为在非标量输出的 `IN` 子查询中，`NULL` 和 `false` 等价。推下过滤中的 `NULL` 行会导致 `WHERE` 子句的负语义，从而可以提前忽略这些行。

> **Note:**
>
> `Exists` 操作符也会被转换为 semi join，但它不是 null-aware。

## Null-aware anti semi join (`NOT IN` 和 `!= ALL` 子查询)

`NOT IN` 或 `!= ALL` 集合操作符的值是三值（`true`、`false` 和 `NULL`）。对于由这两个操作符转换而来的连接类型，TiDB 需要意识到两边连接键上的 `NULL`，并以特殊方式处理。

包含 `NOT IN` 和 `! = ALL` 操作符的子查询会被转换为 anti semi join 和 anti left outer semi join。前述 [Anti semi join](#anti-semi-join-not-in-subquery) 示例中，由于两边的列 `test.t3.t1_id` 和 `test.t1.id` 都不是 `NULL`，因此 anti semi join 不需要考虑 null-aware（`NULL` 不会被特殊处理）。

TiDB v6.3.0 对 null-aware anti join (NAAJ) 进行了如下优化：

- 使用 null-aware 等值条件（NA-EQ）构建哈希连接

    集合操作符引入了等值条件，要求对条件两边的 `NULL` 值进行特殊处理。需要 null-aware 的等值条件称为 NA-EQ。不同于早期版本，TiDB v6.3.0 不再像以前那样处理 NA-EQ，而是将其放在连接后其他条件中，然后在匹配笛卡尔积后判断结果的合法性。

    自 TiDB v6.3.0 起，NA-EQ 作为一种弱化的等值条件，仍用于构建哈希连接。这减少了需要遍历的匹配数据量，加快了匹配速度。当构建表的 `DISTINCT()` 值的总百分比几乎为 100% 时，优化效果更为明显。

- 利用 `NULL` 的特殊属性加快匹配结果的返回

    由于 anti semi join 是合取范式（CNF），连接两边的 `NULL` 会导致确定的结果。此特性可以用来加快整个匹配过程的返回速度。
...
在第一个语句 `EXPLAIN SELECT (a, b) NOT IN (SELECT * FROM s) FROM t;` 中，由于表 `t` 和 `s` 的列 `a` 和 `b` 是可为空的，`NOT IN` 子查询转换的左外 semi join 是 null-aware。不同之处在于 NAAJ 优化还将 NA-EQ 作为哈希连接条件，大大加快了连接计算。

在第二个语句 `EXPLAIN SELECT * FROM t WHERE (a, b) NOT IN (SELECT * FROM s);` 中，由于列 `a` 和 `b` 也是可为空的，`NOT IN` 子查询转换的反 semi join 也是 null-aware。不同之处在于 NAAJ 优化也将 NA-EQ 作为哈希连接条件，从而极大加快了连接计算。

目前，TiDB 仅支持对 anti semi join 和 anti left outer semi join 进行 null-aware 处理。且只支持哈希连接类型，且其构建表必须为右表。

> **Note:**
>
> `Not Exists` 操作符也会被转换为 anti semi join，但它不是 null-aware。
