---
title: Follower Read
summary: 本文档介绍了 Follower Read 的使用方法及实现机制。
---
...
为了在不违反线性一致性或影响 TiDB 的快照隔离的前提下允许 follower 节点读取数据，follower 节点需要使用 Raft 协议的 `ReadIndex`，以确保读请求能够读取到 leader 已提交的最新数据。在 TiDB 层面，Follower Read 功能只需根据负载均衡策略，将 Region 的读请求发送到 follower 副本即可。
