...
| `prefixItems`          | `array` | 应用于数组中按位置的元素的 schema |
...
如上输出所示，`fruits`、`vegetables` 和 `grains` 属性校验失败，因为 `grains` 不存在。
...
如上输出显示 `fruits` 是一个至少包含 3 个元素的数组。
...
如上输出显示 `fruits` **不是** 一个至少包含 4 个元素的数组，因为它不满足最小元素数量的要求。
