...
> **Note:**
>
> 目前，该功能在 [TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/select-cluster-tier#starter) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential) 集群中不可用。
...
你可以将其与 [CHECK](/constraints.md#check) 约束结合使用，在表被修改时自动进行 schema 校验。
...
| 校验关键字              | 适用类型         | 描述 |
|---|---|---|
| `type`                 | 任意类型 | 检查类型（如 `array` 和 `string`） |
| `enum`                 | 任意类型 | 检查值是否在指定的值数组中 |
| `const`                | 任意类型 | 类似于 `enum`，但只针对单个值 |
| `allOf`                | 任意类型 | 匹配所有指定的 schema |
| `anyOf`                | 任意类型 | 匹配任意一个指定的 schema |
...
如上输出所示，`fruits` 和 `vegetables` 属性存在，校验通过。
...
如上输出所示，`fruits`、`vegetables` 和 `grains` 属性的校验失败，因为 `grains` 不存在。
...
通过 `anyOf`，你可以组合多个要求，并校验是否满足其中任意一个。
