...
> **Note:**
>
> 目前，该功能在 [TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential) 集群中不可用。
...
可以与 [CHECK](/constraints.md#check) 约束结合使用，在表被修改时自动进行 schema 验证。
...
| `maxItems`             | `array` | 检查数组中的元素数量是否不超过指定值 |
| `minItems`             | `array` | 检查数组中的元素数量是否至少为指定值 |
| `uniqueItems`          | `array` | 检查数组中的元素是否唯一，`true`/`false`|
...
在上述输出中，可以看到 `fruits` 和 `vegetables` 属性存在的验证通过。
...
在上述输出中，可以看到 `fruits`、`vegetables` 和 `grains` 属性存在的验证失败，因为 `grains` 不存在。
...
上述输出显示 `fruits` 是一个包含至少 3 个元素的数组。
...
上述输出显示 `fruits` **不是** 一个包含至少 4 个元素的数组。这是因为它不满足最小元素数量的要求。
...
你可以检查某个值是否匹配指定的命名格式。可验证的格式包括 `ipv4`、`ipv6`、`time`、`date`、`duration`、`email`、`hostname`、`uuid` 和 `uri`。
...
通过 `anyOf`，你可以组合多个要求，并验证是否满足其中任意一个要求。
...
- 如果 `JSON_SCHEMA_VALID()` 中用于验证的 schema 无效（如 `{"type": "sting"}`），MySQL 可能会接受，但 TiDB 会返回错误。注意 `"sting"` 拼写错误，应该为 `"string"`。
