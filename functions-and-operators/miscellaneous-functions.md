...
`ANY_VALUE()` 函数返回一组值中的任意一个值。通常用于在 `SELECT` 语句中需要包含非聚合列且配合 `GROUP BY` 子句的场景。
...
在上述示例中，TiDB 对第一个 `SELECT` 语句返回错误，因为 `id` 列是非聚合列且未包含在 `GROUP BY` 中。为解决此问题，第二个 `SELECT` 查询使用 `ANY_VALUE()` 来从每个分组中获取任意值，并用 `GROUP_CONCAT()` 将每个分组中的所有 `id` 值连接成一个字符串。这种方式可以在不改变 SQL 模式的情况下，获取每个分组的一个值和所有值。
...
SELECT BIN_TO_UUID(0x9A17B457EB6D-11EE-BACF-5405DB7AAD56);
+-------------------------------------------------+
| BIN_TO_UUID(0x9A17B457EB6D-11EE-BACF-5405DB7AAD56) |
+-------------------------------------------------+
| 9a17b457-eb6d-11ee-bacf-5405db7aad56            |
+-------------------------------------------------+
1 行，耗时 (0.00 sec)
...
在上述示例中，`UPDATE` 语句将 `c1` 列的值设置为列的默认值（即 `5`）加 `3`，得到新值 `8`。
...
`INET_NTOA()` 函数将二进制 IPv4 地址转换为点分十进制格式。
...
`UUID()` 返回一个符合 [RFC 4122](https://datatracker.ietf.org/doc/html/rfc4122) 定义的版本 1 的 UUID。
...
| 名称 | 描述  |
|:------------|:-----------------------------------------------------------------------------------------------|
| [`UUID_SHORT()`](https://dev.mysql.com/doc/refman/8.0/en/miscellaneous-functions.html#function_uuid-short)            | 提供一个在特定假设下唯一的 UUID，TiDB 不支持 [TiDB #4620](https://github.com/pingcap/tidb/issues/4620)  |
