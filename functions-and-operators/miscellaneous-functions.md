...
| [`SLEEP()`](#sleep)                      | 休眠指定秒数。注意，对于 [TiDB Cloud Starter](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential) 集群，`SLEEP()` 函数有最大 300 秒的休眠时间限制。       |
...
`ANY_VALUE()` 函数从一组值中返回任意一个值。通常用于在 `SELECT` 语句中结合 `GROUP BY` 子句时，需要包含非聚合列的场景。
...
在上述示例中，TiDB 对第一个 `SELECT` 语句返回错误，因为 `id` 列是非聚合列且未包含在 `GROUP BY` 子句中。为了解决该问题，第二个 `SELECT` 查询使用 `ANY_VALUE()` 从每个分组中获取任意一个值，并使用 `GROUP_CONCAT()` 将每个分组内 `id` 列的所有值拼接为一个字符串。通过这种方式，你可以在不更改 SQL 模式的情况下，获取每个分组的一个值以及该分组的所有值。
