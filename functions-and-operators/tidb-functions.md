...
| [`TIDB_DECODE_KEY()`](#tidb_decode_key) | 将 TiDB 编码的 key 条目解码为包含 `_tidb_rowid` 和 `table_id` 的 JSON 结构。这些编码 key 可在部分系统表和日志输出中找到。 |
...
`TIDB_DECODE_KEY()` 函数将 TiDB 编码的 key 条目解码为包含 `_tidb_rowid` 和 `table_id` 的 JSON 结构。这些编码 key 存在于部分系统表和日志输出中。
...
该函数的作用在于，执行计划是在语句执行时捕获的。重新用 `EXPLAIN` 执行语句时，随着数据分布和统计信息的变化，可能会得到不同的结果。
