...
在下一个示例中，表的第一个 Region 以仅包含该表 `table_id` 的 key 开头。表的最后一个 Region 以 `table_id + 1` 结尾。中间的 Region key 更长，包含 `_tidb_rowid` 或 `handle`。
...
在上述示例中，参数为包含 3 个 SQL 摘要的 JSON 数组，查询结果中对应的 SQL 语句为三项。但第二个 SQL 摘要在集群中查不到对应的 SQL 语句，因此结果中第二项为 `null`。
