...
> **Note:**
>
> 当使用 Compaction Filter 机制时，GC 进度可能会延迟，进而影响 TiKV 的扫描性能。如果你的业务包含大量 coprocessor 请求，并且在 [**TiKV-Details > Coprocessor Detail**](/grafana-tikv-dashboard.md#coprocessor-detail) 面板中观察到 **Total Ops Details** 下的 `next()` 或 `prev()` 调用次数明显超过 `processed_keys` 调用次数的三倍，可以采取以下措施：
> 
> - 对于 v7.1.3 之前的 TiDB 版本，建议关闭 Compaction Filter 以加快 GC。
> - 对于 v7.1.3 至 v7.5.6 的 TiDB 版本，TiDB 会根据每个 Region 的冗余版本数量 [`region-compact-min-redundant-rows`](/tikv-configuration-file.md#region-compact-min-redundant-rows-new-in-v710) 和冗余版本百分比 [`region-compact-redundant-rows-percent`](/tikv-configuration-file.md#region-compact-redundant-rows-percent-new-in-v710) 自动触发 compaction，以提升 Compaction Filter GC 性能。此时建议调整这些配置项，而不是关闭 Compaction Filter。
