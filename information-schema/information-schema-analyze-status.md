...
summary: 了解 `ANALYZE_STATUS` information_schema 表格。
---

# ANALYZE_STATUS

`ANALYZE_STATUS` 表格提供关于正在运行的统计信息采集任务以及有限数量的历史任务的信息。

从 TiDB v6.1.0 版本开始，`ANALYZE_STATUS` 表支持显示集群级别的任务。即使在 TiDB 重启后，你仍然可以通过此表查看重启前的任务记录。在 TiDB v6.1.0 之前，`ANALYZE_STATUS` 表只能显示实例级别的任务，任务记录在重启后会被清除。

从 TiDB v6.1.0 版本开始，你可以通过系统表 `mysql.analyze_jobs` 查看最近 7 天内的历史任务。
...
* `JOB_INFO`：`ANALYZE` 任务的信息。如果分析的是索引，此信息会包含索引名。当 `tidb_analyze_version = 2` 时，此信息还会包括采样率等配置项。
