...
`CLUSTER_INFO` 集群拓扑表提供了当前集群的拓扑信息、每个实例的版本信息、实例版本对应的 Git Hash、每个实例的启动时间以及每个实例的运行时长。
...
* `STATUS_ADDRESS`：HTTP API 的服务地址。tikv-ctl、pd-ctl 或 tidb-ctl 中的一些命令可能会使用该 API 及其地址。你也可以通过该地址获取更多集群信息。详细信息可参考 [TiDB HTTP API 文档](https://github.com/pingcap/tidb/blob/release-8.5/docs/tidb_http_api.md)。
