...
`RESOURCE_GROUPS` 表显示所有资源组的信息。更多信息请参见 [Use Resource Control to Achieve Resource Group Limitation and Flow Control](/tidb-resource-control-ru-groups.md)。
...
* `PRIORITY`：在 TiKV 上待处理任务的绝对优先级。不同的资源根据 `PRIORITY` 设置进行调度。`PRIORITY` 高的任务优先调度。对于具有相同 `PRIORITY` 的资源组，任务将根据 `RU_PER_SEC` 配置按比例调度。如果未指定 `PRIORITY`，则默认优先级为 `MEDIUM`。
