...
* `PRIORITY`：在 TiKV 上待处理任务的绝对优先级。不同资源会根据 `PRIORITY` 设置进行调度，优先级高的任务会优先调度。对于优先级相同的资源组，任务会根据 `RU_PER_SEC` 配置按比例调度。如果未指定 `PRIORITY`，则默认优先级为 `MEDIUM`。
* `BURSTABLE`：是否允许资源组超额使用可用的系统资源。
