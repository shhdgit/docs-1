...
`SLOW_QUERY` 表提供当前节点的慢查询信息，这些信息是 TiDB [慢查询日志文件](/tidb-configuration-file.md#slow-query-file)的解析结果。表中的列名与慢查询日志中的字段名一一对应。

</CustomContent>

<CustomContent platform="tidb-cloud">

`SLOW_QUERY` 表提供当前节点的慢查询信息，这些信息是 TiDB [慢查询日志文件](https://docs.pingcap.com/tidb/stable/tidb-configuration-file#slow-query-file)的解析结果。表中的列名与慢查询日志中的字段名一一对应。

</CustomContent>

> **Note:**
>
> 该表在 [TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/select-cluster-tier#starter) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential) 集群中不可用。
...
`CLUSTER_SLOW_QUERY` 表提供整个集群所有节点的慢查询信息，这些信息是 TiDB 慢查询日志文件的解析结果。你可以像使用 `SLOW_QUERY` 表一样使用 `CLUSTER_SLOW_QUERY` 表。`CLUSTER_SLOW_QUERY` 表的表结构与 `SLOW_QUERY` 表的区别在于多了一个 `INSTANCE` 列。`INSTANCE` 列表示该慢查询信息所在的 TiDB 节点地址。
...
在输出结果中，可以在 `execution info` 部分查看以下字段（为便于阅读已格式化）：
...
| `read_file` | 读取慢查询日志文件所花费的时间 |
| `parse_log.time` | 解析慢查询日志文件所花费的时间 |
| `parse_log.concurrency` | 解析慢查询日志文件的并发度（由 [`tidb_distsql_scan_concurrency`](/system-variables.md#tidb_distsql_scan_concurrency) 设置） |
| `total_file` | 慢查询日志文件的总数 |
| `read_file` | 实际读取的慢查询日志文件数 |
