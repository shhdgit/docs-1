...
通过在 `SLOW_QUERY` 表上执行 [`EXPLAIN ANALYZE`](/sql-statements/sql-statement-explain-analyze.md) 查询，你可以获得数据库获取慢查询信息的详细过程。但在 `CLUSTER_SLOW_QUERY` 表上执行 `EXPLAIN ANALYZE` 时**不会**返回这些信息。
