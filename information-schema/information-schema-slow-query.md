...
在上述执行计划中，`user = u1` 条件被下推到其他（`cop`） TiDB 节点，聚合操作也被下推（图中的 `StreamAgg` 操作符）。
...
通过对 `SLOW_QUERY` 表运行 [`EXPLAIN ANALYZE`](/sql-statements/sql-statement-explain-analyze.md) 查询，可以获得数据库获取慢查询信息的详细过程。然而，在对 `CLUSTER_SLOW_QUERY` 表运行 `EXPLAIN ANALYZE` 时，此信息**不可用**。
...
在输出中，关注 `execution info` 部分的以下字段（为便于阅读而格式化）：
