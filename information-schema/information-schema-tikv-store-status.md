...
`TIKV_STORE_STATUS` 表通过 PD 的 API 展示 TiKV 节点的一些基本信息，例如集群中分配的 ID、地址和端口，以及当前节点的状态、容量和 Region leader 数量等。
...
* `LEADER_COUNT`：该 Store 上的 leader 数量。
* `LEADER_WEIGHT`：该 Store 的 leader 权重。
* `LEADER_SCORE`：该 Store 的 leader 分数。
* `LEADER_SIZE`：该 Store 上所有 leader 的数据总大小（MB，近似值）。
* `REGION_COUNT`：该 Store 上的 Region 数量。
* `REGION_WEIGHT`：该 Store 的 Region 权重。
* `REGION_SCORE`：该 Store 的 Region 分数。
* `REGION_SIZE`：该 Store 上所有 Region 的数据总大小（MB，近似值）。
