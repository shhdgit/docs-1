...
- 向 MySQL 同步数据： [TiCDC](/ticdc/ticdc-overview.md) 是一款通过拉取 TiKV 变更日志来同步 TiDB 增量数据的工具。TiCDC 使用 [MySQL sink](/ticdc/ticdc-overview.md#replication-consistency) 将 TiDB 的增量数据同步到 MySQL。
...
> **Note:**
>
> 本页面描述了 MySQL 与 TiDB 之间的一般性差异。关于安全性方面的兼容性，详见 [Security Compatibility with MySQL](/security-compatibility-with-mysql.md)。
...
+ `FULLTEXT` 语法和索引 [#1793](https://github.com/pingcap/tidb/issues/1793)
    ...
    >**Note:**
    >
    > 目前，仅部分 AWS 区域的 {{{ .starter }}} 和 {{{ .essential }}} 集群支持 [`FULLTEXT` 语法和索引](https://docs.pingcap.com/tidbcloud/vector-search-full-text-search-sql)。TiDB 自建版和 TiDB Cloud 专属集群支持解析 `FULLTEXT` 语法，但不支持使用 `FULLTEXT` 索引。
...
> **Note:**
>
> + 如果在建表时未指定主键，TiDB 会使用 `_tidb_rowid` 作为行标识。该值的分配与自增列（如存在）共用同一个分配器。如果指定自增列为主键，TiDB 会用该列作为行标识。在这种情况下，可能会出现如下情况：
...
> **Note:**
>
> `AUTO_INCREMENT` 属性在生产环境中可能导致热点问题。详见 [Troubleshoot HotSpot Issues](/troubleshoot-hot-spot-issues.md)。推荐使用 [`AUTO_RANDOM`](/auto-random.md) 替代。
...
> **Note:**
>
> `AUTO_INCREMENT` 属性在生产环境中可能导致热点问题。详见 [Troubleshoot HotSpot Issues](https://docs.pingcap.com/tidb/stable/troubleshoot-hot-spot-issues#handle-auto-increment-primary-key-hotspot-tables-using-auto_random)。推荐使用 [`AUTO_RANDOM`](/auto-random.md) 替代。
...
TiDB 结合使用 [Prometheus 和 Grafana](/tidb-monitoring-api.md) 来存储和查询性能监控指标。在 TiDB 中，大多数 [performance schema 表](/performance-schema/performance-schema.md) 不返回任何结果。
...
* 使用单条 `ALTER TABLE` 语句变更同一表的多个 schema 对象（如列或索引）时，不支持对同一对象进行多次变更。例如，执行 `ALTER TABLE t1 MODIFY COLUMN c1 INT, DROP COLUMN c1` 会报 `Unsupported operate same column/index` 错误。
* 不支持通过单条 `ALTER TABLE` 语句同时变更多个 TiDB 特有的 schema 对象，如 `TIFLASH REPLICA`、`SHARD_ROW_ID_BITS` 和 `AUTO_ID_CACHE`。
* TiDB 不支持通过 `ALTER TABLE` 变更某些数据类型。例如，不支持将 `DECIMAL` 类型变更为 `DATE` 类型。若数据类型变更不被支持，TiDB 会报 `Unsupported modify column: type %d not match origin %d` 错误。详见 [`ALTER TABLE`](/sql-statements/sql-statement-modify-column.md)。
* `ALGORITHM={INSTANT,INPLACE,COPY}` 语法在 TiDB 中仅作为断言存在，不会实际修改 `ALTER` 算法。详见 [`ALTER TABLE`](/sql-statements/sql-statement-alter-table.md)。
...
在 TiDB 中，[统计信息收集](/statistics.md#manual-collection) 与 MySQL 不同，会完全重建表的统计信息，因此资源消耗更大，耗时更长。而 MySQL/InnoDB 执行的是相对轻量、短时的操作。
...
- `SELECT .. GROUP BY expr` 不会像 MySQL 5.7 那样隐式等同于 `GROUP BY expr ORDER BY expr`。
...
如需通过 [`--store`](/command-line-flags-for-tidb-configuration.md#--store) 选项指定存储引擎，需启动 TiDB 实例。该存储引擎抽象特性与 MySQL 类似。
...
- 兼容模式（如 `Oracle` 和 `PostgreSQL`）会被解析但忽略。兼容模式在 MySQL 5.7 中已废弃，在 MySQL 8.0 中已移除。
- `ONLY_FULL_GROUP_BY` 模式与 MySQL 5.7 存在[语义差异](/functions-and-operators/aggregate-group-by-functions.md#differences-from-mysql)。
- MySQL 的 `NO_DIR_IN_CREATE` 和 `NO_ENGINE_SUBSTITUTION` SQL 模式为兼容性而接受，但在 TiDB 中无实际意义。
...
- `lower_case_table_names` 的默认值：
    ...
    - MySQL 默认值如下：
        - Linux：`0`。表示表名和数据库名在磁盘上按 `CREATE TABLE` 或 `CREATE DATABASE` 语句指定的大小写存储，名称比较区分大小写。
        - Windows：`1`。表示表名在磁盘上以小写存储，名称比较不区分大小写。MySQL 在存储和查找时会将所有表名转换为小写。该行为同样适用于数据库名和表别名。
        - macOS：`2`。表示表名和数据库名在磁盘上按 `CREATE TABLE` 或 `CREATE DATABASE` 语句指定的大小写存储，但 MySQL 在查找时会将其转换为小写，名称比较不区分大小写。
...
+ 当前 MySQL 默认使用本地时区，然后依赖系统内置的当前时区规则（如夏令时切换时）。若未[导入时区表数据](https://dev.mysql.com/doc/refman/8.0/en/time-zone-support.html#time-zone-installation)，MySQL 无法通过名称指定时区。
