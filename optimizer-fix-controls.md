...
每个 fix 是一个用于调整 TiDB 优化器某一特定行为的控制项。它由一个数字表示，该数字对应一个包含行为变更技术细节的 GitHub Issue。例如，对于 fix `44262`，你可以在 [Issue 44262](https://github.com/pingcap/tidb/issues/44262) 中查看它控制的内容。

[`tidb_opt_fix_control`](/system-variables.md#tidb_opt_fix_control-new-in-v653-and-v710) 系统变量可以接受多个 fix，使用逗号（`,`）分隔。格式为 `"<#issue1>:<value1>,<#issue2>:<value2>,...,<#issueN>:<valueN>"`，其中 `<#issueN>` 是 fix 编号。例如：
...
- `OFF`：不限制最小行数估算，保持 v8.4.0 之前的行为。在这种情况下，`estRows` 可能为 0。

### [`52592`](https://github.com/pingcap/tidb/issues/52592) <span class="version-mark">New in v8.4.0</span>

- 默认值：`OFF`
- 可选值：`ON`，`OFF`
- 此变量控制是否禁用查询执行中的 `Point Get` 和 `Batch Point Get` 算子。默认值 `OFF` 表示可以使用 `Point Get` 和 `Batch Point Get` 执行查询。如果设置为 `ON`，优化器会禁用 `Point Get` 和 `Batch Point Get`，强制选择 Coprocessor 执行查询。
- `Point Get` 和 `Batch Point Get` 不支持列裁剪（即无法只返回部分列），因此在某些场景下，其执行效率可能低于 Coprocessor，将此变量设置为 `ON` 可以提升查询性能。以下为推荐设置为 `ON` 的场景：
...
- 默认值：`OFF`
- 可选值：`ON`，`OFF`
- 如 [Explain Statements Using Index Merge](/explain-index-merge.md#examples) **Note** 所述，如果优化器可以为查询计划选择单一索引扫描方式（非全表扫描），则不会自动使用 index merge。
- 你可以通过启用此 fix control 移除该限制。移除该限制后，优化器可以在更多查询中自动选择 index merge，但也可能导致优化器忽略最优的执行计划。因此，建议在实际用例上充分测试，确保不会引起性能回退后再移除该限制。
...
- 你可以通过启用此 fix control 移除该限制，使优化器能够处理复杂的区间交集。但对于合取项数量较多（超过 10 个）的条件，优化时间可能会略有增加。
