...
Partition pruning 是一种适用于分区表的性能优化技术。它会分析查询语句中的过滤条件，在不包含任何所需数据的分区上进行(_裁剪_)，从而排除不必要的分区。通过排除不需要的分区，TiDB 能够减少需要访问的数据量，从而可能显著提升查询执行时间。
...
## Partition Pruning 的使用场景

分区裁剪的使用场景对于 Range 分区表和 Hash 分区表有所不同。

### 在 Hash 分区表中使用 partition pruning

本节描述 Hash 分区表中分区裁剪的适用和不适用场景。

#### Hash 分区表中的适用场景

分区裁剪仅适用于 Hash 分区表中等值比较的查询条件。
...
在上述 SQL 语句中，从条件 `x = 1` 可以判断所有结果都落在一个分区中。经过哈希分区后，可以确认值 `1` 在 `p1` 分区内。因此，只需要扫描 `p1` 分区，无需访问不会匹配的 `p2`、`p3` 和 `p4` 分区。从执行计划可以看到，只有一个 `TableFullScan` 操作，并且在 `access object` 中指定了 `p1` 分区，因此可以确认分区裁剪生效。
...
在此情况下，由于无法确认 `x > 2` 条件对应的哈希分区，因此分区裁剪不适用。

##### 场景二

由于分区裁剪的规则优化是在查询计划生成阶段进行的，不能用于过滤条件只能在执行阶段获得的场景。例如：
...
每次读取 `t2` 的一行时，都需要对 `t1` 的分区表进行查询。理论上，`t1.x > val` 的过滤条件在此时已满足，但实际上，分区裁剪只在查询计划的生成阶段生效，而非在执行阶段。

### 在 Range 分区表中使用 partition pruning

本节描述 Range 分区表中分区裁剪的适用和不适用场景。

#### Range 分区表中的适用场景

本节描述 Range 分区表中分区裁剪的三个适用场景。
...
在上述 SQL 中，从 `x in(1,13)` 条件可以判断所有结果只落在几个分区中。经过分析，发现 `x=1` 的所有记录都在 `p0` 分区，`x=13` 的所有记录都在 `p2` 分区，因此只需访问 `p0` 和 `p2` 分区。
...
分区裁剪适用于分区表达式为 `fn(col)` 简单形式，查询条件为 `>`, `<`, `=`, `>=`, `<=`，且 `fn` 函数为单调函数。

如果 `fn` 为单调函数，则对于任何 `x` 和 `y`，如果 `x > y`，则 `fn(x) > fn(y)`。在这种情况下，`fn` 可以被视为严格单调的函数。对于任何 `x` 和 `y`，如果 `x > y`，则 `fn(x) >= fn(y)`。此时，`fn` 也可以称为“单调函数”。理论上，所有单调函数（严格或非严格）都支持分区裁剪。目前，TiDB 仅支持以下单调函数：
...
由于分区裁剪的规则优化在查询计划生成阶段进行，不能用于过滤条件只能在执行阶段获得的场景。例如：
