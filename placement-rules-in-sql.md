...
SQL 中的放置规则是一项功能，允许你通过 SQL 语句指定数据在 TiKV 集群中的存储位置。通过该功能，你可以将集群、数据库、表或分区的数据调度到特定的区域、数据中心、机架或主机上。
...
2. 要将放置策略绑定到表或分区表，使用 `CREATE TABLE` 或 `ALTER TABLE` 语句为该表或分区表指定放置策略：
   ...
   `PLACEMENT POLICY` 不属于任何数据库 schema，可以全局绑定。因此，使用 `CREATE TABLE` 指定放置策略时，无需额外权限。
...
- 所有对象绑定的放置策略均为 *异步* 生效。要检查放置策略的调度进度，可以使用 [`SHOW PLACEMENT`](/sql-statements/sql-statement-show-placement.md) 语句：
...
- 绑定了 `multiaz` 放置策略的表，数据会以 3 个副本分布在不同 region，优先满足跨 region 的数据隔离生存目标，其次是跨 zone，最后是跨 host。
...
该放置策略创建并绑定到目标数据后，数据的 Raft Leader 副本会放置在 `LEADER_CONSTRAINTS` 指定的 `us-east-1` region，其余副本会放置在 `FOLLOWER_CONSTRAINTS` 指定的 region。注意，如果集群发生故障（如 `us-east-1` region 节点宕机），新 Leader 仍会从其他 region 选举，即使这些 region 只在 `FOLLOWER_CONSTRAINTS` 中指定。也就是说，保证服务可用性优先级最高。

如果 `us-east-1` region 故障时，你不希望在 `us-west-1` 选举新 Leader，可以配置特殊的 `evict-leader` 属性，将该 region 选出的 Leader 驱逐：
