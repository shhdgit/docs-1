...
| 层级            | 描述                                                                          |
|------------------|--------------------------------------------------------------------------------------|
| 集群          | 默认情况下，TiDB 为集群配置 3 个副本的策略。你可以为集群配置全局放置策略。详见 [为集群全局指定副本数](#specify-the-number-of-replicas-globally-for-a-cluster)。 |
| 数据库         | 你可以为特定数据库配置放置策略。详见 [为数据库指定默认放置策略](#specify-a-default-placement-policy-for-a-database)。 |
| 表            | 你可以为特定表配置放置策略。详见 [为表指定放置策略](#specify-a-placement-policy-for-a-table)。 |
| 分区        | 你可以为表中的不同行创建分区，并分别为分区配置放置策略。详见 [为分区表指定放置策略](#specify-a-placement-policy-for-a-partitioned-table)。 |
...
- 建议附加放置策略的表和分区总数控制在 10,000 个以内。为过多的表和分区附加策略会增加 PD 的计算负载，进而影响服务性能。
...
1. 要创建放置策略，使用 [`CREATE PLACEMENT POLICY`](/sql-statements/sql-statement-create-placement-policy.md) 语句：
    ...
    更多可配置的放置选项及其含义，详见 [放置选项参考](#placement-option-reference)。
...
2. 要将放置策略附加到表或分区表，使用 `CREATE TABLE` 或 `ALTER TABLE` 语句为该表或分区表指定放置策略：
   ...
   `PLACEMENT POLICY` 不与任何数据库 schema 关联，可以在全局范围内附加。因此，使用 `CREATE TABLE` 指定放置策略不需要额外的权限。
...
在该语句中，`FOLLOWERS=4` 选项表示为数据配置 5 个副本，包括 4 个 Follower 和 1 个 Leader。更多可配置的放置选项及其含义，详见 [放置选项参考](#placement-option-reference)。
...
> - 列表和字典格式均基于 YAML 解析器，但在某些情况下 YAML 语法可能被错误解析。例如，`"{+region=east:1,+region=west:2}"`（冒号后无空格）可能被错误解析为 `'{"+region=east:1": null, "+region=west:2": null}'`，这不是预期结果。而 `"{+region=east: 1,+region=west: 2}"`（冒号后有空格）可被正确解析为 `'{"+region=east": 1, "+region=west": 2}'`。因此，建议在冒号后加空格。
...
注意，由于 TiDB 默认 Leader 数为 `1`，`five replicas` 表示 4 个 Follower 和 1 个 Leader。
...
ALTER DATABASE test PLACEMENT POLICY=p2;  -- 修改数据库默认放置策略为 p2，不影响已存在的表 t1。
...
ALTER TABLE t PLACEMENT POLICY=default; -- 从表 t 移除 'five_replicas' 放置策略，重置为默认策略。
```
...
如果你对数据的具体分布没有特别要求，而更关注容灾需求，可以使用 `SURVIVAL_PREFERENCES` 选项指定数据生存优先级。
...
> **Note:**
>
> `SURVIVAL_PREFERENCES` 等价于 PD 的 `location-labels`。详见 [通过拓扑标签调度副本](/schedule-replicas-by-topology-labels.md)。
...
> **Note:**
>
> `SURVIVAL_PREFERENCES` 等价于 PD 的 `location-labels`。详见 [通过拓扑标签调度副本](https://docs.pingcap.com/tidb/stable/schedule-replicas-by-topology-labels)。

</CustomContent>

### 指定跨多数据中心 2:2:1 分布的 5 副本集群

如果你需要特定的数据分布，比如 5 副本以 2:2:1 的比例分布，可以通过配置 [字典格式](#constraints-格式) 的 `CONSTRAINTS`，为不同约束指定不同数量的副本：
...
如果你对 Raft Leader 在节点间的分布有特定要求，可以使用如下语句指定放置策略：
...
创建并附加该放置策略后，数据的 Raft Leader 副本会放置在 `LEADER_CONSTRAINTS` 选项指定的 `us-east-1` region，其余副本会放置在 `FOLLOWER_CONSTRAINTS` 指定的 region。注意，如果集群发生故障，比如 `us-east-1` region 节点宕机，新的 Leader 仍会从其他 region 选举产生，即使这些 region 是在 `FOLLOWER_CONSTRAINTS` 中指定的。也就是说，保证服务可用性优先级最高。

如果在 `us-east-1` region 故障时，不希望在 `us-west-1` 选举新的 Leader，可以配置特殊的 `evict-leader` 属性，将该 region 选出的 Leader 驱逐：
...
- `SCHEDULE` 选项指定 TiDB 如何平衡 Follower 的分布。
    ...
    - 如果希望确保足够数量的 Follower 副本放置在 `PRIMARY_REGION`（即 `us-east-1`），可以使用 `MAJORITY_IN_PRIMARY` 调度规则。该规则以牺牲部分可用性为代价，提供更低延迟的事务。如果主 region 故障，`MAJORITY_IN_PRIMARY` 不会自动切换。
...
在该示例中，约束通过列表格式指定，如 `[+app=order]`。你也可以使用字典格式指定，如 `{+app=order: 3}`。
...
| 工具名称 | 最低支持版本 | 说明 |
| --- | --- | --- |
| 备份恢复（BR） | 6.0 | v6.0 之前，BR 不支持备份和恢复放置策略。详见 [为什么恢复放置规则到集群时报错](/faq/backup-and-restore-faq.md#why-does-an-error-occur-when-i-restore-placement-rules-to-a-cluster)。 |
