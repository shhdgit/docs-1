...
> - 列表和字典格式均基于 YAML 解析器，但在某些情况下 YAML 语法可能被错误解析。例如，`"{+region=east:1,+region=west:2}"`（冒号后无空格）可能被错误解析为 `'{"+region=east:1": null, "+region=west:2": null}'`，这不是预期结果。而 `"{+region=east: 1,+region=west: 2}"`（冒号后有空格）会被正确解析为 `'{"+region=east": 1, "+region=west": 2}'`。因此，建议在冒号后加空格。
...
如果 `us-east-1` region 故障时，你不希望新 Leader 被放置在 `us-west-1`，可以配置特殊的 `evict-leader` 属性，将该 region 选出的 Leader 驱逐：
