...
对于 TiDB Cloud Dedicated 集群，TiKV 节点上的 label 已自动配置。
...
> - 列表和字典格式均基于 YAML 解析器，但 YAML 语法在某些情况下可能被错误解析。例如，`"{+region=east:1,+region=west:2}"`（冒号后无空格）可能被错误解析为 `'{"+region=east:1": null, "+region=west:2": null}'`，这不是预期结果。而 `"{+region=east: 1,+region=west: 2}"`（冒号后有空格）可以被正确解析为 `'{"+region=east": 1, "+region=west": 2}'`。因此，建议在冒号后添加空格。
...
创建并附加该放置策略后，数据的 Raft Leader 副本会放置在 `LEADER_CONSTRAINTS` 选项指定的 `us-east-1` region，其他副本会放置在 `FOLLOWER_CONSTRAINTS` 指定的 region。注意，如果集群发生故障，比如 `us-east-1` region 节点宕机，新的 Leader 仍会从其他 region 选举出来，即使这些 region 在 `FOLLOWER_CONSTRAINTS` 中指定。也就是说，保证服务可用性优先级最高。
...
| 工具名称 | 最低支持版本 | 说明 |
| --- | --- | --- |
| 备份恢复（BR） | 6.0 | v6.0 之前，BR 不支持备份和恢复放置策略。更多信息，参见 [为什么恢复 placement rules 到集群时报错](/faq/backup-and-restore-faq.md#why-does-an-error-occur-when-i-restore-placement-rules-to-a-cluster)。 |
