...
- セキュリティ強化モード（SEM）で [`require_secure_transport`](https://docs.pingcap.com/tidb/v6.5/system-variables#require_secure_transport-new-in-v610) を `ON` に設定することを禁止し、ユーザーの潜在的な接続問題を防ぐため [#47665](https://github.com/pingcap/tidb/issues/47665) @[tiancaiamao](https://github.com/tiancaiamao)
- オプティマイザがハッシュ結合をテーブルに選択するかどうかを制御する [`tidb_opt_enable_hash_join`](https://docs.pingcap.com/tidb/v6.5/system-variables#tidb_opt_enable_hash_join-new-in-v656) システム変数を導入 [#46695](https://github.com/pingcap/tidb/issues/46695) @[coderplay](https://github.com/coderplay)
- 追加のテストの後、TiCDC Changefeed 構成項目 [`case-sensitive`](/ticdc/ticdc-changefeed-config.md) のデフォルト値を `true` から `false` に変更しました。これにより、デフォルトで TiCDC 構成ファイルのテーブルおよびデータベース名は大文字と小文字を区別しないことになります [#10047](https://github.com/pingcap/tiflow/issues/10047) @[sdojjy](https://github.com/sdojjy)
- TiCDC Changefeed には、次の新しい構成項目が導入されました:
  - [`sql-mode`](https://docs.pingcap.com/tidb/v6.5/ticdc-changefeed-config): TiCDC が DDL ステートメントを解析する際に使用する[SQL モード](https://docs.pingcap.com/tidb/v6.5/ticdc-ddl#sql-mode)を設定することができます [#9876](https://github.com/pingcap/tiflow/issues/9876) @[asddongmen](https://github.com/asddongmen)
  - [`encoding-worker-num`](https://docs.pingcap.com/tidb/v6.5/ticdc-changefeed-config) および [`flush-worker-num`](https://docs.pingcap.com/tidb/v6.5/ticdc-changefeed-config): 異なるマシンの仕様に基づいてリドゥモジュールの並行性パラメータを設定することができます [#10048](https://github.com/pingcap/tiflow/issues/10048) @[CharlesCheung96](https://github.com/CharlesCheung96)
  - [`compression`](https://docs.pingcap.com/tidb/v6.5/ticdc-changefeed-config): リドゥログファイルの圧縮動作を構成することができます [#10176](https://github.com/pingcap/tiflow/issues/10176) @[sdojjy](https://github.com/sdojjy)
  - [`sink.cloud-storage-config`](https://docs.pingcap.com/tidb/v6.5/ticdc-changefeed-config): オブジェクトストレージにデータをレプリケートする際の過去データの自動クリーンアップを設定することができます [#10109](https://github.com/pingcap/tiflow/issues/10109) @[CharlesCheung96](https://github.com/CharlesCheung96)

## 改善点 {#improvements}

- TiDB

  - [`FLASHBACK CLUSTER TO TSO`](https://docs.pingcap.com/tidb/v6.5/sql-statement-flashback-cluster) 構文をサポート [#48372](https://github.com/pingcap/tidb/issues/48372) @[BornChanger](https://github.com/BornChanger)

- TiKV

  - OOM を防ぐために Resolver のメモリ使用量を最適化 [#15458](https://github.com/tikv/tikv/issues/15458) @[overvenus](https://github.com/overvenus)
  - 地域が分割されると、分割するキーがない場合、過剰な MVCC バージョンを排除するためにコンパクションがトリガーされるようにコンパクションメカニズムを最適化 [#15282](https://github.com/tikv/tikv/issues/15282) @[SpadeA-Tang](https://github.com/SpadeA-Tang)
  - ルーターオブジェクトの LRUCache を排除してメモリ使用量を減らし、OOM を防ぐ [#15430](https://github.com/tikv/tikv/issues/15430) @[Connor1996](https://github.com/Connor1996)
  - `apply_router` および `raft_router` メトリクスの `alive` および `leak` 監視次元を追加 [#15357](https://github.com/tikv/tikv/issues/15357) @[tonyxuqqi](https://github.com/tonyxuqqi)

- PD

  - Grafana ダッシュボードの `DR Auto-Sync` に `Status` および `Sync Progress` などの監視メトリクスを追加 [#6975](https://github.com/tikv/pd/issues/6975) @[disksing](https://github.com/disksing)

- ツール

  - バックアップ＆リストア（BR）

    - スナップショットバックアップを復元する際、特定のネットワークエラーに遭遇した場合、BR はリトライを行います [#48528](https://github.com/pingcap/tidb/issues/48528) @[Leavrth](https://github.com/Leavrth)
    - `delete range` シナリオでの Point-In-Time Recovery（PITR）のための新しい統合テストを導入し、PITR の安定性を向上させます [#47738](https://github.com/pingcap/tidb/issues/47738) @[Leavrth](https://github.com/Leavrth)
    - スナップショット復元中にリージョンのマージを自動的にリトライすることができるように、`merge-schedule-limit` 構成を `0` に設定することができます [#47236](https://github.com/pingcap/tidb/issues/47236) @[Leavrth](https://github.com/Leavrth)
    - BR は `merge-schedule-limit` 構成を `0` に設定することで、リージョンのマージを一時停止することができます [#7148](https://github.com/tikv/pd/issues/7148) @[BornChanger](https://github.com/3pointer)

  - TiCDC

    - `sink-uri` 構成で `content-compatible=true` を設定することで、TiCDC Canal-JSON コンテンツ形式を[公式 Canal 出力のコンテンツ形式と互換性があるようにサポート](https://docs.pingcap.com/tidb/v6.5/ticdc-canal-json#compatibility-with-the-official-canal) [#10106](https://github.com/pingcap/tiflow/issues/10106) @[3AceShowHand](https://github.com/3AceShowHand)
    - `ADD INDEX` DDL 操作をレプリケートする実行ロジックを最適化し、後続の DML ステートメントをブロックしないようにする [#9644](https://github.com/pingcap/tiflow/issues/9644) @[sdojjy](https://github.com/sdojjy)
    - TiCDC の増分スキャンが上流の TiKV に与える影響を軽減する [#11390](https://github.com/tikv/tikv/issues/11390) @[hicqu](https://github.com/hicqu)
