...
- 在数据库和数据表数量较多（例如超过 10 万个数据库和数据表）的场景，或者数据库和数据表数量已经影响到系统性能时，建议开启 schema 缓存特性。
...
- 当数据表被不规律地访问时，例如在 time1 访问一组表，在 time2 访问另一组表，并且 `tidb_schema_cache_size` 的值较小时，schema 信息可能会频繁被驱逐和缓存，导致性能波动。该特性更适用于频繁访问的数据库和数据表相对固定的场景。
- 统计信息可能无法及时收集。
- 访问部分元数据信息的速度可能变慢。
