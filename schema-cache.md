---
title: Schema Cache
summary: TiDB 采用基于 LRU（最近最少使用）的缓存机制来缓存元信息，在拥有大量数据库和数据表的场景下，显著降低了内存使用并提升了性能。
---

# Schema Cache

在一些多租户场景下，可能会有数十万甚至上百万个数据库和数据表。如果将所有数据库和数据表的元信息全部加载到内存中，不仅会消耗大量内存，还会导致访问性能下降。为了解决这个问题，TiDB 引入了类似 LRU（最近最少使用）的 schema 缓存机制。只有最近访问过的数据库和数据表的元信息会被缓存在内存中。

> **Note:**
>
> 目前，该特性不支持在 [TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential) 集群上使用。
...
- 当你使用带有 [`AUTO_INCREMENT`](/auto-increment.md) 或 [`AUTO_RANDOM`](/auto-random.md) 属性的数据表时，较小的 schema 缓存大小可能导致这些表的元信息频繁进出缓存，从而导致分配的 ID 区间在未完全使用前失效，出现 ID 跳号。在写入压力较大的场景下，甚至可能导致 ID 区间被耗尽。为尽量减少异常的 ID 分配行为并提升系统稳定性，建议采取以下措施：
