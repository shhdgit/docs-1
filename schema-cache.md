...
> **Note:**
>
> 目前，该特性在 [{{{ .starter }}}](https://docs.pingcap.com/tidbcloud/select-cluster-tier#tidb-cloud-serverless) 和 [{{{ .essential }}}](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential) 集群上不可用。
...
- 当数据表被不规律访问时，例如在 time1 访问一组表，在 time2 访问另一组表，且 `tidb_schema_cache_size` 的值较小，schema 信息可能会频繁被驱逐和重新缓存，导致性能波动。该特性更适用于频繁访问的数据库和数据表相对固定的场景。
- 统计信息可能无法及时收集。
- 访问部分元数据信息可能会变慢。
...
- 当你使用带有 [`AUTO_INCREMENT`](/auto-increment.md) 或 [`AUTO_RANDOM`](/auto-random.md) 属性的数据表时，较小的 schema 缓存大小可能导致这些表的元数据频繁进出缓存。这可能导致分配的 ID 区间在未完全使用前就失效，从而出现 ID 跳号。在写入压力较大的场景下，甚至可能耗尽 ID 区间。为尽量减少异常的 ID 分配行为并提升系统稳定性，建议采取以下措施：
