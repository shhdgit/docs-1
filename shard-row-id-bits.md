...
- 自增位的值存储在 TiKV 中，并按顺序分配。每次分配一个值，下一个值递增 1。自增位保证 `_tidb_rowid` 列的值在全局唯一。当自增位的值被耗尽（即达到最大值时），后续的自动分配会失败，并报错 `Failed to read auto-increment value from storage engine`。
