...
对于没有聚簇主键或没有主键的表，TiDB 会使用隐式自增行 ID。当执行大量 `INSERT` 操作时，数据会被写入到单个 Region，导致写入热点问题。
...
- 自增位的值存储在 TiKV 中，并按顺序分配。每次分配一个值，下一个值递增 1。自增位保证 `_tidb_rowid` 列的值在全局范围内唯一。当自增位的值被耗尽（即达到最大值时），后续的自动分配会失败，并报错 `Failed to read auto-increment value from storage engine`。
