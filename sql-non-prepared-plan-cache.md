...
v7.1.0からは、[`tidb_plan_cache_max_plan_size`](/system-variables.md#tidb_plan_cache_max_plan_size-new-in-v710)システム変数を使用して、キャッシュできる計画の最大サイズを制御できます。デフォルト値は2 MBです。計画のサイズがこの値を超える場合、計画はキャッシュされません。
...
これによりパフォーマンスの問題が発生する場合、`ignore_plan_cache()`ヒントを使用してキャッシュ内の計画を無視し、オプティマイザがSQLのたびに新しい実行計画を生成するようにすることができます。SQLを変更できない場合は、問題を解決するためにバインディングを作成することができます。たとえば、`CREATE BINDING FOR SELECT ... USING SELECT /*+ ignore_plan_cache() */ ...`。
...
- `ORDER BY`や`GROUP BY`の直後に数値や式が含まれるクエリはサポートされません。たとえば、`ORDER BY 1`や`GROUP BY a+1`などはサポートされません。`ORDER BY column_name`と`GROUP BY column_name`のみがサポートされます。
- `JSON`、`ENUM`、`SET`、または`BIT`型の列でフィルタリングするクエリはサポートされません。たとえば、`SELECT * FROM t WHERE json_col = '{}'`などが該当します。
- `NULL`値でフィルタリングするクエリはサポートされません。たとえば、`SELECT * FROM t WHERE a is NULL`などが該当します。
- パラメータ化後のパラメータが200個を超えるクエリはデフォルトでサポートされません。たとえば、`SELECT * FROM t WHERE a in (1, 2, 3, ... 201)`などが該当します。v7.1.1からは、[`tidb_opt_fix_control`](/system-variables.md#tidb_opt_fix_control-new-in-v710)システム変数を使用してこの制限を変更できます。
- パーティションテーブル、仮想列、一時テーブル、ビュー、またはメモリテーブルにアクセスするクエリはサポートされません。たとえば、`COLUMNS`がTiDBメモリテーブルである`INFORMATION_SCHEMA.COLUMNS`から`SELECT * FROM t`を実行するクエリが該当します。
- ヒントやバインディングを含むクエリはサポートされません。
- デフォルトでは、DMLステートメントまたは`FOR UPDATE`句を含む`SELECT`ステートメントはサポートされません。この制限を解除するには、`SET tidb_enable_non_prepared_plan_cache_for_dml = ON`を実行します。

この機能を有効にした後、オプティマイザはクエリを迅速に評価します。非準備実行計画キャッシュのサポート条件を満たさない場合、クエリは通常の最適化プロセスにフォールバックします。

## パフォーマンスの利点 {#performance-benefits}

内部テストでは、非準備実行計画キャッシュ機能を有効にすることで、ほとんどのTPシナリオで著しいパフォーマンスの利点を得ることができます。ただし、この機能はクエリがサポートされるかどうかを判断し、クエリをパラメータ化するという追加のパフォーマンスオーバーヘッドも導入します。この機能がワークロードの大部分のクエリをサポートできない場合、有効にすると実際にパフォーマンスに悪影響を及ぼす可能性があります。

この場合、Grafanaの**Queries Using Plan Cache OPS**パネルの`non-prepared`メトリクスと**Plan Cache Miss OPS**パネルの`non-prepared-unsupported`メトリクスを観察する必要があります。ほとんどのクエリがサポートされず、わずかなクエリのみが計画キャッシュにヒットする場合は、この機能を無効にできます。
...
準備されていないプランキャッシュを有効にした後、次のペインでメモリ使用量、キャッシュ内のプラン数、およびキャッシュヒット率をモニタリングできます：
