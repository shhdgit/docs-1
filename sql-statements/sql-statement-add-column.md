...
`ALTER TABLE.. ADD COLUMN` 语句在现有表中添加一列。在 TiDB 中，此操作是在线的，意味着添加列不会阻塞对表的读写。
...
* 在添加新列时，通过指定 **PRIMARY KEY** 或 **UNIQUE INDEX** 为 **GLOBAL** 来设置 [全局索引](/partitioned-table.md#global-indexes)，这是 TiDB 对 [分区表](/partitioned-table.md) 的扩展，不兼容 MySQL。
