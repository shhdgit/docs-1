...
> - 升级前，建议使用 [`ADMIN SHOW DDL`](/sql-statements/sql-statement-admin-show-ddl.md) 命令检查 TiDB 集群中是否有正在进行的 DDL 任务。如果集群中存在 DDL 任务，想要升级集群时，请等待 DDL 执行完成，或在升级前使用 [`ADMIN CANCEL DDL`](/sql-statements/sql-statement-admin-cancel-ddl.md) 命令取消 DDL 任务。
