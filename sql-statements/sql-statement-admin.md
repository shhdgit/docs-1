...
执行以下命令，查看 test 数据库中未完成的 DDL 任务。结果包括正在运行的 DDL 任务和最近完成但失败的 5 个 DDL 任务。
...
* `STATE`：DDL 操作的状态。常见状态包括：
    * `none`：表示操作任务已进入 DDL 任务队列但尚未执行，原因可能是正在等待前序任务完成。另一种情况是执行 drop 操作后变为 `none`，但很快会更新为 `synced`，表示所有 TiDB 实例已同步到该状态。
    * `running`：表示操作正在执行中。
