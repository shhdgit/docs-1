...
* `SCHEMA_STATE`：当前的 schema 状态。如果 `JOB_TYPE` 为 `add index`，则为索引的状态；如果为 `add column`，则为列的状态；如果为 `create table`，则为表的状态。常见状态包括：
    ...
    * `delete only`、`write only`、`delete reorganization`、`write reorganization`：这四种为中间状态。由于中间状态转换非常快，常规操作中一般不可见。只有在 `add index` 操作中可能看到 `write reorganization`，表示正在添加索引数据。
