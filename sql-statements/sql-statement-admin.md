...
运行以下命令，查看当前正在运行的 DDL 任务队列中最近完成的 10 个 DDL 任务。当未指定 `NUM` 时，默认只展示最近完成的 10 个 DDL 任务。
...
运行以下命令，查看当前正在运行的 DDL 任务队列中最近完成的 5 个 DDL 任务：
...
运行以下命令，查看表中某些特殊列的详细信息。输出内容与 [SHOW TABLE NEXT_ROW_ID](/sql-statements/sql-statement-show-table-next-rowid.md) 相同。
...
运行以下命令，查看 test 数据库中未完成的 DDL 任务。结果包括正在运行的 DDL 任务以及最近完成但失败的 5 个 DDL 任务。
