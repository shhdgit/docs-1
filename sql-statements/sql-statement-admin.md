...
上述语句可以根据出现多次的 `SELECT` 语句生成 SQL 执行计划绑定。
...
此处“不可信”指的是你需要手动确保原表的元数据可以被 `CREATE TABLE STATEMENT` 操作覆盖。要使用该 `REPAIR` 语句，需要开启 [`repair-mode`](/tidb-configuration-file.md#repair-mode) 配置项，并确保待修复的表已被列入 [`repair-table-list`](/tidb-configuration-file.md#repair-table-list)。
...
执行以下命令，查看当前 DDL 任务队列中最近完成的 10 个 DDL 任务。当未指定 `NUM` 时，默认只展示最近完成的 10 个 DDL 任务。
...
执行以下命令，查看当前 DDL 任务队列中最近完成的 5 个 DDL 任务：
...
* `STATE`：DDL 操作的状态。常见状态包括：
    * `none`：表示操作任务已进入 DDL 任务队列但尚未执行，正在等待前序任务完成。另一种情况是执行 drop 操作后变为 `none`，但很快会更新为 `synced`，表示所有 TiDB 实例已同步到该状态。
    * `running`：表示操作正在执行中。
