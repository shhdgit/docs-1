...
* `SCHEMA_STATE`：当前的 schema 状态。如果 `JOB_TYPE` 为 `add index`，则为索引的状态；如果为 `add column`，则为列的状态；如果为 `create table`，则为表的状态。常见状态包括：
    ...
    * `public`：表示存在且可用。当 `create table`、`add index/column` 等操作完成后，通常会变为 `public` 状态，表示新建的表/列/索引现在可以正常读写。
...
* `STATE`：DDL 操作的状态。常见状态包括：
    * `none`：表示操作任务已进入 DDL 任务队列但尚未执行，原因可能是正在等待前面的任务完成。另一种情况是执行 drop 操作后变为 `none` 状态，但很快会更新为 `synced`，表示所有 TiDB 实例已同步到该状态。
    * `running`：表示操作正在执行中。
