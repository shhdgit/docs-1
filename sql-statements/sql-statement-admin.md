...
* `SCHEMA_STATE`：当前的 schema 状态。如果 `JOB_TYPE` 为 `add index`，则为索引的状态；如果为 `add column`，则为列的状态；如果为 `create table`，则为表的状态。常见状态包括：
    ...
    * `delete only`、`write only`、`delete reorganization`、`write reorganization`：这四种状态为中间态。在常规操作中这些状态通常不可见，因为中间态的转换非常快。只有在 `add index` 操作中可能看到 `write reorganization` 状态，表示正在添加索引数据。
