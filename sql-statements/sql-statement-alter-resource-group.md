...
| `QUERY_LIMIT`| 当查询执行满足该条件时，该查询被识别为异常查询并执行相应操作。| `QUERY_LIMIT=(EXEC_ELAPSED='60s', ACTION=KILL, WATCH=EXACT DURATION='10m')` 表示当查询执行时间超过 60 秒时，该查询被识别为异常查询并被终止。所有 SQL 文本相同的 SQL 语句将在接下来的 10 分钟内被立即终止。`QUERY_LIMIT=()` 或 `QUERY_LIMIT=NULL` 表示未启用异常查询控制。详见 [Runaway Queries](/tidb-resource-control-runaway-queries.md)。|
...
> - 当前仅 `default` 资源组支持修改 `BACKGROUND` 配置。
