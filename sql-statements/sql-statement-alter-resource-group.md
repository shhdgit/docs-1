...
| 选项          | 描述                                                         | 示例                                                         |
|---------------|--------------------------------------------------------------|--------------------------------------------------------------|
| `RU_PER_SEC` | 每秒 RU 的回填速率                                           | `RU_PER_SEC = 500` 表示该资源组每秒回填 500 RUs                  |
| `PRIORITY`    | TiKV 上待处理任务的绝对优先级                                | `PRIORITY = HIGH` 表示优先级为高。若未指定，默认值为 `MEDIUM`。 |
| `BURSTABLE`   | 若设置 `BURSTABLE` 属性，TiDB 允许对应资源组在超出配额时使用系统可用资源。 |                                                              |
| `QUERY_LIMIT` | 当查询执行满足此条件时，查询被识别为 runaway 查询，并执行相应操作。 | `QUERY_LIMIT=(EXEC_ELAPSED='60s', ACTION=KILL, WATCH=EXACT DURATION='10m')` 表示当查询执行时间超过 60 秒时，识别为 runaway 查询，查询将被终止。所有具有相同 SQL 文本的 SQL 语句将在接下来的 10 分钟内立即终止。`QUERY_LIMIT=()` 或 `QUERY_LIMIT=NULL` 表示未启用 runaway 控制。详见 [Runaway Queries](/tidb-resource-control-runaway-queries.md)。 |
| `BACKGROUND`  | 配置后台任务。更多详情请参见 [Manage background tasks](/tidb-resource-control-background-tasks.md)。 | `BACKGROUND=(TASK_TYPES="br,stats", UTILIZATION_LIMIT=30)` 表示备份恢复和统计信息采集相关任务作为后台任务调度，后台任务最多可消耗 TiKV 资源的 30%。 |
...
MySQL 也支持 [ALTER RESOURCE GROUP](https://dev.mysql.com/doc/refman/8.0/en/alter-resource-group.html)，但接受的参数与 TiDB 不同，因此不兼容。
