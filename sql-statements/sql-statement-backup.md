...
一次只能执行一个 `BACKUP` 和 [`RESTORE`](/sql-statements/sql-statement-restore.md) 任务。如果在同一台 TiDB 服务器上已在执行 `BACKUP` 或 `RESTORE`，新的 `BACKUP` 执行将等待直到之前的任务完成。
...
上述示例中，将 `test` 数据库备份到本地文件系统。数据以 SST 文件形式存储在 `/mnt/backup/2020/04/` 目录中，分布在所有 TiDB 和 TiKV 节点上。
...
每个任务代表一个表范围或一个索引范围，具体取决于备份方案。对于一个有一个索引的表，使用两个任务来备份此表。`CONCURRENCY` 的默认值为 `4`。如果需要备份大量表或索引，可以增加其值。
