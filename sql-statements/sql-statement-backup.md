...
`BACKUP` 语句使用与 [BR 工具](https://docs.pingcap.com/tidb/stable/backup-and-restore-overview) 相同的引擎，不同之处在于备份过程由 TiDB 自身驱动，而不是独立的 BR 工具。BR 的所有优点和注意事项同样适用于该语句。
...
同一时间只能执行一个 `BACKUP` 或 [`RESTORE`](/sql-statements/sql-statement-restore.md) 任务。如果同一 TiDB 服务器上已经有 `BACKUP` 或 `RESTORE` 语句在执行，新的 `BACKUP` 执行会等待所有之前的任务完成。

`BACKUP` 只能与 "tikv" 存储引擎配合使用。与 "unistore" 引擎一起使用 `BACKUP` 会失败。
...
| `Destination` | 目标 URL |
| `Size` |  备份归档的总大小，单位为字节 |
| `BackupTS` | 创建备份时快照的 TSO（适用于 [增量备份](#incremental-backup)） |
...
在备份完成前，`BACKUP` 默认会对集群上的数据进行校验（checksum）以验证正确性。单表校验任务的默认并发数为 4，可以通过 `CHECKSUM_CONCURRENCY` 参数进行调整。如果你确信无需数据校验，可以将 `CHECKSUM` 参数设置为 `FALSE` 以关闭校验。

如需指定 BR 备份表和索引时可执行的并发任务数，可使用 `CONCURRENCY` 参数。该参数控制 BR 内部的线程池大小，从而优化备份操作的性能和效率。

一个任务代表一个表范围或一个索引范围，具体取决于备份的 schema。对于一个带有一个索引的表，会使用两个任务进行备份。`CONCURRENCY` 的默认值为 4。如果需要备份大量表或索引，可以适当增大该值。
