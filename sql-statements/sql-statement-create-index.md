...
>
> 表达式索引会隐式占用一个名称（例如 `_V$_{index_name}_{index_offset}`）。如果你尝试用已存在的列名创建新的表达式索引，会报错。此外，如果你添加了与之同名的新列，也会报错。
>
> 请确保表达式索引中函数的参数数量正确。
>
...
与 `CREATE INDEX` 语句相关的系统变量有 `tidb_ddl_enable_fast_reorg`、`tidb_ddl_reorg_worker_cnt`、`tidb_ddl_reorg_batch_size`、`tidb_enable_auto_increment_in_generated` 和 `tidb_ddl_reorg_priority`。详细信息参见 [系统变量](/system-variables.md#tidb_ddl_reorg_worker_cnt)。
