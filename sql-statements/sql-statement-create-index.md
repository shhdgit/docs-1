...
>
> 表达式索引会隐式占用一个名称（例如 `_V$_{index_name}_{index_offset}`）。如果你尝试用与已有列相同的名称创建新的表达式索引，会报错。此外，如果你添加了与之同名的新列，也会报错。
>
> 请确保表达式索引中函数参数的数量正确。
>
...
表达式索引的语法和限制与 MySQL 保持一致。其实现方式是基于不可见的虚拟生成列创建索引，因此受限于 [虚拟生成列的所有限制](/generated-columns.md#limitations)。
