...
`EXPLAIN` 语句用于展示查询的执行计划，但不会实际执行该查询。它与会实际执行查询的 `EXPLAIN ANALYZE` 语句互为补充。如果 `EXPLAIN` 的输出结果与预期不符，建议对查询中涉及的每个表执行 `ANALYZE TABLE`，以确保表的统计信息是最新的。
...
`DESC` 和 `DESCRIBE` 语句是 `EXPLAIN` 语句的别名。`EXPLAIN <tableName>` 的另一种用法已在 [`SHOW [FULL] COLUMNS FROM`](/sql-statements/sql-statement-show-columns-from.md) 文档中说明。
...
> **注意：**
>
> 当你使用 MySQL 客户端连接 TiDB 时，为了更清晰地阅读输出结果且不换行，可以使用 `pager less -S` 命令。然后在输出 `EXPLAIN` 结果后，可以通过键盘上的右箭头 <kbd>→</kbd> 水平滚动查看输出内容。

> **注意：**
>
> 在返回的执行计划中，自 v6.4.0 起，`IndexJoin` 和 `Apply` 算子的所有 probe 端子节点的 `estRows` 含义与 v6.4.0 之前不同。详情可参见 [TiDB 查询执行计划总览](/explain-overview.md#understand-explain-output)。
...
| estRows       | 算子预计输出的行数。该数值根据统计信息和算子逻辑估算得出。`estRows` 在 TiDB 4.0 早期版本中称为 `count`。 |
| task          | 算子所属的任务类型。目前执行计划分为两类任务：**root** 任务（在 tidb-server 上执行）和 **cop** 任务（在 TiKV 或 TiFlash 上并行执行）。执行计划的任务级拓扑结构为一个 root 任务后接多个 cop 任务。root 任务以 cop 任务的输出为输入。cop 任务指 TiDB 下推到 TiKV 或 TiFlash 的任务。每个 cop 任务分布在 TiKV 集群或 TiFlash 集群中，由多个进程执行。 |
| access object | 算子访问的数据项信息。包括 `table`、`partition` 和 `index`（如有）。仅直接访问数据的算子才有该信息。 |
...
| `brief`        | `EXPLAIN` 语句输出中的算子 ID 会被简化，相较于未指定 `FORMAT` 时更简洁。 |
...
如果你的电脑没有 `dot` 程序，可以将结果复制到 [此网站](http://www.webgraphviz.com/) 以获得树状图：
...
在输出中，`id`、`estRows`、`taskType`、`accessObject` 和 `operatorInfo` 与默认格式下的列含义相同。`subOperators` 是一个数组，存储子节点。子节点的字段及含义与父节点一致。如果某个字段缺失，表示该字段为空。
...
`EXPLAIN FOR CONNECTION` 用于获取某个连接当前正在执行的 SQL 查询或上一次执行的 SQL 查询的执行计划。输出格式与 `EXPLAIN` 相同。但 TiDB 中 `EXPLAIN FOR CONNECTION` 的实现与 MySQL 不同。除输出格式外，主要区别如下：
