...
| task          | 算子所属的任务类型。目前执行计划分为两类任务：**root** 任务（在 tidb-server 上执行）和 **cop** 任务（在 TiKV 或 TiFlash 上并行执行）。执行计划的任务级拓扑结构为 root 任务后接多个 cop 任务。root 任务以 cop 任务的输出为输入。cop 任务指 TiDB 下推到 TiKV 或 TiFlash 的任务。每个 cop 任务分布在 TiKV 集群或 TiFlash 集群中，由多个进程执行。 |
