...
summary: 关于 TiDB 数据库中 EXPLAIN 的用法概述。
---

# `EXPLAIN`

`EXPLAIN` 语句显示查询的执行计划，而不执行该查询。它补充了 `EXPLAIN ANALYZE` 语句，后者会执行该查询。如果 `EXPLAIN` 的输出与预期结果不符，建议对查询涉及的每个表执行 `ANALYZE TABLE`，以确保表统计信息是最新的。
...
> **注意：**
>
> 在返回的执行计划中，从 v6.4.0 起，`IndexJoin` 和 `Apply` 操作符的所有 probe 端子节点的 `estRows` 含义与 v6.4.0 之前不同。详细信息请参见 [TiDB 查询执行计划概述](/explain-overview.md#understand-explain-output)。

目前，TiDB 中的 `EXPLAIN` 输出 5 列：`id`、`estRows`、`task`、`access object`、`operator info`。每个执行计划中的操作符由这些属性描述，`EXPLAIN` 输出的每一行描述一个操作符。每个属性的说明如下：

| 属性名             | 描述                                                                                                              |
|:------------------|:------------------------------------------------------------------------------------------------------------------|
| id                | 操作符 ID 是整个执行计划中操作符的唯一标识。在 TiDB 2.1 中，ID 格式会显示操作符的树状结构。数据从子节点流向父节点。每个操作符只有一个父节点。 |
| estRows           | 预估该操作符输出的行数。此数字根据统计信息和操作符逻辑估算得出。在 TiDB 4.0 早期版本中，`estRows` 被称为 `count`。 |
| task              | 操作符所属的任务类型。目前，执行计划分为两类任务：**root** 任务，在 tidb-server 上执行；以及 **cop** 任务，在 TiKV 或 TiFlash 上并行执行。任务层级的拓扑结构为：一个 root 任务后跟多个 cop 任务。root 任务使用 cop 任务的输出作为输入。cop 任务指 TiDB 下推到 TiKV 或 TiFlash 的任务。每个 cop 任务在 TiKV 集群或 TiFlash 集群中分布，由多个进程执行。 |
| access object     | 操作符访问的数据项信息，包括 `table`、`partition` 和 `index`（如果有的话）。只有直接访问数据的操作符才有此信息。 |
| operator info     | 关于操作符的其他信息。每个操作符的 `operator info` 内容不同。以下示例可供参考。 |
...
| FORMAT | 描述 |
|:-------|:--------------------------------------------------------------|
| Not specified | 如果未指定格式，`EXPLAIN` 使用默认的 `row` 格式。 |
| `brief` | `EXPLAIN` 输出中的操作符 ID 被简化，与未指定 `FORMAT` 时相比。 |
| `dot` | `EXPLAIN` 输出 DOT 格式的执行计划，可通过 `dot` 程序（`graphviz` 包）生成 PNG 文件。 |
| `row` | `EXPLAIN` 以表格形式输出结果。详见 [理解查询执行计划](/explain-overview.md)。 |
| `tidb_json` | `EXPLAIN` 以 JSON 格式输出执行计划，并将操作符信息存储在 JSON 数组中。 |
| `verbose` | `EXPLAIN` 以 `row` 格式输出，结果中增加 `estCost` 列，表示查询的预估成本。关于此格式的更多信息，请参见 [SQL 计划管理](/sql-plan-management.md)。 |
| `plan_cache` | `EXPLAIN` 以 `row` 格式输出，结果中包含 [Plan Cache](/sql-non-prepared-plan-cache.md#diagnostics) 信息作为警告。 |
...
除了 MySQL 标准的结果格式外，TiDB 还支持 DotGraph 格式，你需要像下面示例一样指定 `FORMAT = "dot"`：
