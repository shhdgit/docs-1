...
目前，TiDB 中的 `EXPLAIN` 输出 5 列：`id`、`estRows`、`task`、`access object`、`operator info`。执行计划中的每个算子都由这些属性描述，`EXPLAIN` 输出的每一行描述一个算子。各属性说明如下：
...
| task          | 算子所属的任务类型。目前，执行计划分为两类任务：**root** 任务（在 tidb-server 上执行）和 **cop** 任务（在 TiKV 或 TiFlash 上并行执行）。执行计划在任务层面的拓扑结构为一个 root 任务后接多个 cop 任务。root 任务以 cop 任务的输出为输入。cop 任务指 TiDB 下推到 TiKV 或 TiFlash 的任务。每个 cop 任务分布在 TiKV 集群或 TiFlash 集群中，由多个进程执行。 |
| access object | 算子访问的数据项信息。包括 `table`、`partition` 和 `index`（如有）。只有直接访问数据的算子才有该信息。 |
...
| `plan_cache`   | `EXPLAIN` 语句以 `row` 格式输出结果，并以 warning 形式输出 [Plan Cache](/sql-non-prepared-plan-cache.md#diagnostics) 信息。 |
...
在输出中，`id`、`estRows`、`taskType`、`accessObject` 和 `operatorInfo` 与默认格式下的列含义相同。`subOperators` 是一个数组，存储子节点。子节点的字段及含义与父节点相同。如果某个字段缺失，表示该字段为空。
