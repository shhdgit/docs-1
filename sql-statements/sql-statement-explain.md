...
summary: TiDB 数据库中 EXPLAIN 的用法概述。
---
...
> **注意：**
>
> 当你使用 MySQL 客户端连接 TiDB 时，为了更清晰地阅读输出结果且不换行，可以使用 `pager less -S` 命令。然后在输出 `EXPLAIN` 结果后，可以按键盘上的右箭头 <kbd>→</kbd> 水平滚动查看输出内容。
...
| `verbose`      | `EXPLAIN` 语句以 `row` 格式输出结果，并在结果中增加 `estCost` 列，显示查询的估算成本。关于该格式的更多用法，参见 [SQL 执行计划管理](/sql-plan-management.md)。 |
...
除了 MySQL 标准结果格式外，TiDB 还支持 DotGraph，你需要像下面这样指定 `FORMAT = "dot"`：
...
在输出中，`id`、`estRows`、`taskType`、`accessObject` 和 `operatorInfo` 与默认格式下的列含义一致。`subOperators` 是一个数组，存储子节点。子节点的字段及含义与父节点相同。如果某个字段缺失，表示该字段为空。
