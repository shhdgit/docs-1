...
TiDB v6.4.0 引入了 `FLASHBACK CLUSTER TO TIMESTAMP` 语法。你可以使用该语法将集群恢复到某个特定的时间点。在指定时间戳时，可以设置一个 datetime 值或使用时间函数。datetime 的格式如 '2016-10-08 16:45:26.999'，最小时间单位为毫秒。但在大多数情况下，指定以秒为单位的时间戳就足够了，例如 '2016-10-08 16:45:26'。
...
>
> 如果你遇到了该问题，可以使用 [TiDB 快照备份与恢复](/br/br-snapshot-guide.md) 功能进行数据恢复。
...
* 如果 `FLASHBACK CLUSTER` 语句导致元数据（表结构、库结构）回滚，相关修改**不会**被 TiCDC 同步。因此，你需要手动暂停任务，等待 `FLASHBACK CLUSTER` 完成后，手动同步上下游的表结构定义，确保一致。之后需要重新创建 TiCDC changefeed。
