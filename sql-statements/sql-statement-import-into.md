...
指定数据文件中每个字段与目标表列的对应关系。你也可以用它将字段映射到变量，以跳过某些字段的导入，或在 `SetClause` 中使用。
...
指定目标列的值如何计算。在 `SET` 表达式右侧，可以引用 `ColumnNameOrUserVarList` 中指定的变量。
...
| `DISABLE_PRECHECK` | 所有文件格式及 `SELECT` 查询结果 | 设置该选项可禁用对非关键项的预检查，如是否存在 CDC 或 PITR 任务的检查。  |
...
Global Sort 会消耗大量内存资源。建议在数据导入前配置 [`tidb_server_memory_limit_gc_trigger`](/system-variables.md#tidb_server_memory_limit_gc_trigger-new-in-v640) 和 [`tidb_server_memory_limit`](/system-variables.md#tidb_server_memory_limit-new-in-v640) 变量，以避免频繁触发 golang GC 影响导入效率。
