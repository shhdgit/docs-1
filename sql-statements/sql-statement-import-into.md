...
> **Note:**
>
> 相较于 [TiDB Lightning](/tidb-lightning/tidb-lightning-overview.md)，`IMPORT INTO` 可以直接在 TiDB 节点上执行，支持自动化分布式任务调度和 [TiDB Global Sort](/tidb-global-sort.md)，在部署、资源利用率、任务配置便捷性、调用和集成的易用性、高可用性和可扩展性等方面有显著提升。建议你在合适的场景下优先考虑使用 `IMPORT INTO` 替代 TiDB Lightning。
...
- 如果未指定该参数，则数据文件每行字段数必须与目标表列数一致，字段会按顺序导入到对应列。
...
在 `fileLocation` 参数中，你可以指定单个文件，也可以使用 `*` 和 `[]` 通配符匹配多个文件进行导入。注意，通配符只能用于文件名部分，不能匹配目录或递归匹配子目录下的文件。以 Amazon S3 存储的文件为例，参数可配置如下：
...
| `SPLIT_FILE` | CSV | 将单个 CSV 文件拆分为多个约 256 MiB 的小块并行处理，以提升导入效率。该参数仅对**非压缩**的 CSV 文件生效，且使用限制与 TiDB Lightning 的 [`strict-format`](https://docs.pingcap.com/tidb/stable/tidb-lightning-data-source#strict-format) 相同。注意需显式指定 `LINES_TERMINATED_BY`。 |
...
- 对于存储在 Amazon S3 或 GCS 的数据文件，`IMPORT INTO ... FROM FILE` 支持在 [TiDB 分布式执行框架（DXF）](/tidb-distributed-execution-framework.md) 下运行。
    ...
    - 启用 DXF（[`tidb_enable_dist_task`](/system-variables.md#tidb_enable_dist_task-new-in-v710) 为 `ON`）时，`IMPORT INTO` 会将数据导入任务拆分为多个子任务，并分发到不同 TiDB 节点执行，以提升导入效率。
    - 未启用 DXF 时，`IMPORT INTO ... FROM FILE` 仅支持在当前用户连接的 TiDB 节点上运行。

- 对于 TiDB 本地存储的数据文件，`IMPORT INTO ... FROM FILE` 仅支持在当前用户连接的 TiDB 节点上运行。因此，数据文件需放置在当前用户连接的 TiDB 节点上。如果通过代理或负载均衡访问 TiDB，则无法导入本地存储的数据文件。
...
> **Note:**
>
> Global Sort 不支持在 [TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/select-cluster-tier#starter) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential) 集群上使用。
...
- 如果分配给各子任务的数据文件行存在主键范围重叠，则每个子任务编码生成的数据 KV 也会重叠。
...
如只需导入 `file-01.csv` 和 `file-03.csv`，可执行如下 SQL：
