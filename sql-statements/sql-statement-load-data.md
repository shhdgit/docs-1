...
你可以使用 `LOCAL` 指定要导入的客户端本地数据文件，此时文件参数必须是客户端文件系统上的路径。
...
如果未指定 `LOCAL`，则文件参数必须是有效的 S3 或 GCS 路径，具体格式详见 [外部存储](/br/backup-and-restore-storages.md)。

</CustomContent>

<CustomContent platform="tidb-cloud">

如果未指定 `LOCAL`，则文件参数必须是有效的 S3 或 GCS 路径，具体格式详见 [外部存储](https://docs.pingcap.com/tidb/stable/backup-and-restore-storages)。
