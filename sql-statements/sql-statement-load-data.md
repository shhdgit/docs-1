...
- `LINES TERMINATED BY`：指定行结束符，如果你希望以某个字符结束一行。
...
> - TiDB v4.0.0 到 v6.6.0 版本，默认将所有行在一个事务中提交。但如果你需要 `LOAD DATA` 语句每固定行数提交一次，可以设置 [`tidb_dml_batch_size`](/system-variables.md#tidb_dml_batch_size) 为期望的行数。
...
> - TiDB v7.6.0 之前的版本，无论每个事务提交多少行，`LOAD DATA` 都不会被显式事务中的 [`ROLLBACK`](/sql-statements/sql-statement-rollback.md) 语句回滚。
> - TiDB v7.6.0 之前的版本，`LOAD DATA` 语句始终以乐观事务模式执行，无论 TiDB 事务模式如何配置。
> - 从 v7.6.0 开始，TiDB 对事务中的 `LOAD DATA` 处理方式与其他 DML 语句一致：
