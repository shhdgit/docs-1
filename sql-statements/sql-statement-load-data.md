...
- 如果你使用 `DEFINED NULL BY`，如 `DEFINED NULL BY 'my-null'`，则 `my-null` 被视为 NULL 值。
- 如果你使用 `DEFINED NULL BY ... OPTIONALLY ENCLOSED`，如 `DEFINED NULL BY 'my-null' OPTIONALLY ENCLOSED`，则 `my-null` 和 `"my-null"`（假设 `ENCLOSED BY '"'`）都被视为 NULL 值。
- 如果未使用 `DEFINED NULL BY` 或 `DEFINED NULL BY ... OPTIONALLY ENCLOSED`，但使用了 `ENCLOSED BY`，如 `ENCLOSED BY '"'`，则 `NULL` 被视为 NULL 值。此行为与 MySQL 一致。
- 其他情况下，不会被视为 NULL 值。
...
如果你想提取 `bob`、`20` 和 `street 1`，可以将字段分隔符指定为 `','`，包裹字符指定为 `'\"'`：
