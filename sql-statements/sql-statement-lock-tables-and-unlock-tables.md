...
> - 对于 [TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/select-cluster-tier#starter) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential)，不支持将 [`enable-table-lock`](https://docs.pingcap.com/tidb/stable/tidb-configuration-file#enable-table-lock-new-in-v400) 设置为 `true`。
...
- 持有该锁的会话可以读写表。
- 只有持有该锁的会话可以访问该表。在锁被释放之前，其他会话无法访问该表。
...
你不能在同一个 `LOCK TABLES` 语句中多次获取同一张表的锁。
...
- 在 TiDB 中，如果会话 A 已经持有表锁，当会话 B 尝试写入该表时会返回错误。而在 MySQL 中，会话 B 的写请求会被阻塞，直到会话 A 释放表锁，并且其他会话对该表的加锁请求也会被阻塞，直到当前会话释放 `WRITE` 锁。
- 在 TiDB 中，如果 `LOCK TABLES` 语句需要的锁已被其他会话持有，则 `LOCK TABLES` 语句必须等待，并在执行该语句时返回错误。而在 MySQL 中，该语句会被阻塞，直到获取到锁为止。
