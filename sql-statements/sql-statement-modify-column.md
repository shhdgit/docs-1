...
自 v5.1.0 版本起，TiDB 支持对 Reorg 数据进行数据类型的变更，内容包括但不限于：
...
> **Note:**
>
> - 当变更的数据类型与现有数据行冲突时，TiDB 会返回错误。在上述示例中，TiDB 返回如下错误：
>
>    ```
...
>    ```
>
> - 为了兼容 Async Commit 功能，DDL 语句会等待一段时间（大约 2.5 秒）后，才开始处理成 Reorg 数据。
>
>    ```
>    Query OK, 0 rows affected (2.52 sec)
>    ```
...
* 不支持修改主键列的 Reorg-Data 类型，但支持修改 Meta-Only 类型。例如：
...
* 不支持将某些数据类型（例如部分 TIME 类型、BIT、SET、ENUM、JSON）修改为其他类型，原因是 TiDB 和 MySQL 之间的 `cast` 函数行为存在兼容性问题。
