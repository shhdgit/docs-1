...
同一时间只能执行一个 `BACKUP` 或 `RESTORE` 任务。如果同一 TiDB 服务器上已有 `BACKUP` 或 `RESTORE` 任务在运行，新的 `RESTORE` 执行将会等待所有前序任务完成后再开始。
...
如果统计信息已被备份，恢复时默认会一并恢复统计信息。如果你不需要恢复统计信息，可以将 `LOAD_STATS` 参数设置为 `FALSE`。
