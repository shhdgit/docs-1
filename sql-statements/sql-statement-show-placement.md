...
`SHOW PLACEMENT` 汇总了所有来自 placement policy 的 placement 选项，并以规范形式展示。
...
该语句返回的结果集中，`Scheduling_State` 字段表示 Placement Driver（PD）在调度 placement 时的当前进度：

* `PENDING`：PD 尚未开始调度 placement。这可能表示 placement 规则在语义上是正确的，但当前集群无法满足。例如，如果 `FOLLOWERS=4`，但只有 3 个 TiKV 节点可作为 follower。
* `INPROGRESS`：PD 当前正在调度 placement。
* `SCHEDULED`：PD 已成功调度 placement。
