...
* `SCATTER_FINISH_RATIO` 表示新拆分 Region 被成功调度的比例。`1.0` 表示所有 Region 均已调度完成。
...
* Region `102` 的 `START_KEY` 和 `END_KEY` 表示记录数据在 `[-inf, 20000)` 范围内的数据被存储在该 Region。类似地，可以计算出 Region (`106`、`110`、`114`、`3`) 的数据存储范围。
...
使用 `SPLIT TABLE REGION` 可以将索引数据拆分为多个 Region。如下例所示，将表 t 的索引数据 `name` 在 `[a,z]` 范围内拆分为 2 个 Region。
