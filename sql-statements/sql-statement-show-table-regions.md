...
上述输出显示 Region 96 被拆分，生成了新的 Region 98。表中的其他 Region 未受分裂操作影响。可以通过输出统计信息确认：
...
* Region `98` 存储索引数据。表 t 的索引数据起始键为 `t_43_i`，该范围属于 Region `98`。
...
可以使用 `SPLIT TABLE REGION` 将索引数据拆分为多个 Region。如下例，将表 t 的索引数据 `name` 在 `[a,z]` 范围内拆分为 2 个 Region。
