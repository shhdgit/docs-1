...
* `APPROXIMATE_SIZE(MB)`：Region 内数据的估算量，单位为兆字节（MB）。
...
TiDB 会根据需要自动进行 Region 的负载均衡。如需手动均衡，可使用 `SPLIT TABLE REGION` 语句：
...
上述输出显示 Region 96 被拆分，生成了新的 Region 98。表中的其他 Region 未受拆分操作影响。输出统计信息说明如下：

* `TOTAL_SPLIT_REGION` 表示新拆分出来的 Region 数量。本例中为 1。
* `SCATTER_FINISH_RATIO` 表示新拆分 Region 被成功调度（scatter）的比例。`1.0` 表示全部 Region 已调度完成。
...
使用 `SPLIT TABLE REGION` 可以将索引数据拆分为多个 Region。如下例，将表 t 的索引数据 `name` 在 `[a,z]` 范围内拆分为 2 个 Region。
