...
* `APPROXIMATE_SIZE(MB)`：Region 中数据的估算量，单位为 MB。
* `APPROXIMATE_KEYS`：Region 中 key 的估算数量。
...
在上面的输出中，`START_KEY` 为 `t_75_r_31717`，`END_KEY` 为 `t_75_r_63434`，表示主键在 `31717` 到 `63434` 之间的数据存储在该 Region 中。前缀 `t_75_` 表示该 Region 属于内部表 ID 为 `75` 的表（`t`）。`START_KEY` 或 `END_KEY` 为空时，分别表示负无穷或正无穷。

TiDB 会根据需要自动进行 Region 的负载均衡。若需手动均衡，可使用 `SPLIT TABLE REGION` 语句：
...
* 在 Region `102` 中，`START_KEY` 和 `END_KEY` 表示存储主键范围为 `[-inf, 20000)` 的记录数据。类似地，可以计算出 Region (`106`、`110`、`114`、`3`) 的数据存储范围。
* Region `98` 存储索引数据。表 t 的索引数据起始 key 为 `t_43_i`，该范围的数据存储在 Region `98` 中。

要查看 store 1 上与表 t 对应的 Region，可使用 `WHERE` 子句：
...
此时，表 t 对应 7 个 Region。其中 5 个（`102`、`106`、`110`、`114`、`3`）存储表 t 的记录数据，另外 2 个（`135`、`98`）存储索引数据 `name`。
