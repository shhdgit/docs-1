...
* `SCHEDULING_STATE`：带有 placement policy 的 Region 的调度状态。
...
TiDB 会根据需要自动进行 Region 的负载均衡。如需手动进行 Region 分裂，可以使用 `SPLIT TABLE REGION` 语句：
...
上述输出显示 Region 96 被拆分，生成了新的 Region 98。表中的其他 Region 未受分裂操作影响。输出统计信息说明如下：

* `TOTAL_SPLIT_REGION` 表示新拆分出来的 Region 数量。本例中为 1。
...
可以使用 `SPLIT TABLE REGION` 将索引数据拆分为多个 Region。如下例所示，将表 t 的索引数据 `name` 在 `[a,z]` 范围内拆分为 2 个 Region。
