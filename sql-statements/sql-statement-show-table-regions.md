...
在上面的输出中，`START_KEY` 为 `t_75_r_31717` 且 `END_KEY` 为 `t_75_r_63434`，表示主键在 `31717` 到 `63434` 之间的数据存储在该 Region 中。前缀 `t_75_` 表示该 Region 属于内部表 ID 为 `75` 的表（`t`）。`START_KEY` 或 `END_KEY` 为空时，分别表示负无穷或正无穷。
...
* `SCATTER_FINISH_RATIO` 表示新拆分 Region 被成功调度（scatter）的比例。`1.0` 表示全部已调度完成。
...
* Region `98` 存储索引数据。表 t 的索引数据起始 key 为 `t_43_i`，该 key 位于 Region `98` 的范围内。
