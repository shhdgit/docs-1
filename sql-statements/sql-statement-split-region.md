...
- 均匀拆分的语法：
    ...
    `BETWEEN lower_value AND upper_value REGIONS region_num` 用于定义上下边界和 Region 数量。然后当前 Region 会在上下边界之间被均匀拆分为 `region_num` 个 Region。
...
- 非均匀拆分的语法：
    ...
    `BY value_list…` 用于手动指定一系列拆分点，当前 Region 会根据这些点进行拆分。适用于数据分布不均匀的场景。
...
由于 `row_id` 是整数，可以根据指定的 `lower_value`、`upper_value` 和 `region_num` 计算出要拆分的 key 值。TiDB 首先计算步长（`step = (upper_value - lower_value)/region_num`），然后在 `lower_value` 和 `upper_value` 之间按步长均匀拆分，生成 `region_num` 个 Region。
...
该语句会将表 t 在 minInt64 和 maxInt64 之间拆分为 16 个 Region。如果主键范围比上述范围小，例如 0~1000000000，可以用 0 和 1000000000 替换 minInt64 和 maxInt64 进行拆分：
...
如果已知数据分布不均匀，并且你希望在 key 范围 -inf ~ 10000、10000 ~ 90000 和 90000 ~ +inf 各拆分一个 Region，可以通过设置固定拆分点实现，如下所示：
...
`upper` 和 `lower` 的值首先会被编码为字节数组。去除 `lower` 和 `upper` 字节数组的最长公共前缀后，将剩余部分的前 8 个字节分别转换为 uint64 格式，然后计算 `step = (upper - lower)/num`。之后，将计算出的步长编码为字节数组，并拼接到 `lower` 和 `upper` 字节数组的最长公共前缀后，用于索引拆分。示例如下：

如果 `idx` 索引的列为整数类型，可以使用如下 SQL 语句拆分索引数据：

```sql
SPLIT TABLE t INDEX idx BETWEEN (-9223372036854775808) AND (9223372036854775807) REGIONS 16;
```

该语句会将表 t 的 idx 索引 Region 从 `minInt64` 拆分到 `maxInt64`，共 16 个 Region。

如果 idx1 索引的列为 varchar 类型，并且你希望按前缀字母拆分索引数据：

```sql
SPLIT TABLE t INDEX idx1 BETWEEN ("a") AND ("z") REGIONS 25;
```

该语句会将 idx1 索引从 a~z 拆分为 25 个 Region。Region 1 的范围为 `[minIndexValue, b)`，Region 2 的范围为 `[b, c)`，……，Region 25 的范围为 `[y, minIndexValue]`。对于 idx 索引，前缀为 a 的数据会写入 Region 1，前缀为 b 的数据会写入 Region 2。
...
如果 idx2 索引的列为时间类型（如 timestamp/datetime），并且你希望按年份拆分索引 Region：

```sql
SPLIT TABLE t INDEX idx2 BETWEEN ("2010-01-01 00:00:00") AND ("2020-01-01 00:00:00") REGIONS 10;
```

该语句会将表 `t` 的 idx2 索引 Region 从 `2010-01-01 00:00:00` 拆分到 `2020-01-01 00:00:00`，共 10 个 Region。Region 1 的范围为 `[minIndexValue, 2011-01-01 00:00:00)`，Region 2 的范围为 `[2011-01-01 00:00:00, 2012-01-01 00:00:00)`。

如果你希望按天拆分索引 Region，可参考以下示例：
...
例如，索引 `idx3 (a, b)` 包含 2 列，a 列为 timestamp 类型，b 列为 int。如果你只想按 a 列的时间范围拆分，可以使用单列时间索引的拆分 SQL 语句，此时不需要在 `lower_value` 和 `upper_value` 中指定 b 列的值：

```sql
SPLIT TABLE t INDEX idx3 BETWEEN ("2010-01-01 00:00:00") AND ("2020-01-01 00:00:00") REGIONS 10;
```

在同一时间范围内，如果你还想按 b 列再拆分一次，只需在拆分时指定 b 列的值：
...
2. 使用 `SPLIT` 语法为每个分区拆分 Region。假设你希望将每个分区 `[0,10000]` 范围的数据拆分为 4 个 Region，示例语句如下：
    ...
    在上述语句中，`0` 和 `10000` 分别代表你希望分散的热点数据的上下边界 `row_id`。

    > **Note:**
    >
    > 此示例仅适用于热点数据均匀分布的场景。如果热点数据在指定数据范围内分布不均匀，请参考 [拆分分区表 Region](#split-regions-for-partitioned-tables) 中的非均匀拆分语法。
...
当创建带有 `AUTO_RANDOM` 或 `SHARD_ROW_ID_BITS` 属性的表时，如果你希望在建表后立即将表均匀预拆分为多个 Region，可以指定 `PRE_SPLIT_REGIONS` 选项。表的预拆分 Region 数量为 `2^(PRE_SPLIT_REGIONS)`。

> **Note:**
>
> `PRE_SPLIT_REGIONS` 的值必须小于等于 `SHARD_ROW_ID_BITS` 或 `AUTO_RANDOM` 的值。

[`tidb_scatter_region`](/system-variables.md#tidb_scatter_region) 全局变量会影响 `PRE_SPLIT_REGIONS` 的行为。该变量用于控制建表后是否等待 Region 预拆分并调度完成后再返回结果。如果建表后有大量写入操作，需要将该变量设置为 `global`，此时 TiDB 会根据整个集群的数据分布调度新建表的 Region。否则，TiDB 会在调度未完成前写入数据，这会对写入性能产生较大影响。
...
> **Note:**
>
> 通过 Split Region 语句拆分出来的 Region 受 PD 中 [Region merge](/best-practices/pd-scheduling-best-practices.md#region-merge) 调度器控制。为避免 PD 在短时间内重新合并新拆分的 Region，需要使用 [表属性](/table-attributes.md) 或 [动态修改](/pd-control.md) 与 Region merge 相关的配置项。
