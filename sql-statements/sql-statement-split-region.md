...
对于在 TiDB 中创建的每个新表，默认会以一个 [Region](/tidb-storage.md#region) 来存储该表的数据。此默认行为由 TiDB 配置文件中的 `split-table` 控制。当该 Region 中的数据超过默认的 Region 大小限制时，该 Region 会开始拆分成两个。

在上述情况下，由于一开始只有一个 Region，所有写请求都发生在该 Region 所在的 TiKV 上。如果新创建的表有大量写入操作，就会造成热点问题。
...
- 均匀拆分的语法：
    ...
    `BETWEEN lower_value AND upper_value REGIONS region_num` 定义了上边界、下边界和 Region 数量。然后当前 Region 会在上下边界之间均匀拆分成指定数量（`region_num`）的 Region。
...
>
> - `tidb_wait_split_region_finish`：散布 Region 可能需要一段时间。此时长取决于 PD 调度和 TiKV 负载。此变量用于控制在执行 `SPLIT REGION` 时，是否等待所有 Region 散布完成后再返回结果给客户端。若其值设为 `1`（默认），TiDB 仅在散布完成后返回结果；若设为 `0`，TiDB 无论散布状态如何都立即返回结果。
...
同一表中的行数据具有相同的 `table_id`，但每行具有唯一的 `row_id`，可用于 Region 拆分。

#### 均匀拆分

由于 `row_id` 为整数，待拆分的 key 值可以根据指定的 `lower_value`、`upper_value` 和 `region_num` 计算。TiDB 首先计算步长值（`step = (upper_value - lower_value)/region_num`），然后在 `lower_value` 和 `upper_value` 之间每个“步长”均匀拆分，生成指定数量（`region_num`）的 Region。

例如，若要将表 t 的 key 范围从 `minInt64` 到 `maxInt64` 均匀拆分成 16 个 Region，可以使用以下语句：
...
首先将 `upper` 和 `lower` 转换为字节数组。去除 `lower` 和 `upper` 字节数组的最长公共前缀后，将 `lower` 和 `upper` 的前 8 个字节转换为 uint64 格式。然后计算 `step = (upper - lower)/num`。之后将计算得到的步长编码为字节数组，附加到 `lower` 和 `upper` 字节数组的最长公共前缀后，用于索引拆分。示例：

如果 `idx` 索引列为整数类型，可以使用以下 SQL 语句拆分索引数据：

```sql
SPLIT TABLE t INDEX idx BETWEEN (-9223372036854775808) AND (9223372036854775807) REGIONS 16;
```

此语句会将表 t 中索引 idx 的 Region 从 `minInt64` 拆分到 `maxInt64`，共 16 个 Region。
...
对于 `a` 前缀的数据会写入 Region 1，`b` 前缀的数据写入 Region 2。

在上述拆分方法中，`y` 和 `z` 前缀的数据都写入第 25 个 Region，因为上界不是 `z`，而是 ASCII 中 `z` 后的字符 `{`。因此，更精确的拆分方法如下：
...
此语句会将表 t 中索引 `idx2` 的 Region 拆分成 10 个，从 `2010-01-01 00:00:00` 到 `2020-01-01 00:00:00`。每个 Region 的范围如下：
...
此语句会将索引 `idx2` 在 2020 年 6 月的数据拆分成 30 个 Region，每个 Region 表示 1 天。

其他类型索引列的 Region 拆分方法类似。

对于联合索引的索引 Region 拆分，唯一不同的是可以指定多个列的值。

例如，索引 `idx3 (a, b)` 包含 2 列，列 `a` 为 timestamp 类型，列 `b` 为 int。如果只想根据列 `a` 的时间范围拆分，可以使用单列时间索引的拆分语句。在这种情况下，不需要在 `lower_value` 和 `upper_value` 中指定列 `b` 的值。
...
此语句会在列 `a` 的时间范围内，按照列 `b` 的值将 10 个 Region 拆分出来，且时间前缀与列 `a` 相同。如果为列 `a` 指定的值不同，列 `b` 的值可能不会被用到。

如果表的主键是 [non-clustered index](/clustered-indexes.md)，在拆分 Region 时需要用反引号 ``` ` ``` 转义 `PRIMARY` 关键字，例如：
...
分区表的 Region 拆分与普通表相同。唯一的区别是对每个分区都执行相同的拆分操作。
...
1. 创建一个分区表 `t`。假设你想将其拆分为两个哈希分区，示例语句如下：
...
4. 你也可以对每个分区的索引 Region 进行拆分。例如，将 `idx` 索引在 `[1000, 10000]` 范围内的 Region 拆分成两个 Region，示例语句如下：
...
1. 创建一个分区表，假设你想将其拆分为三个范围分区，示例语句如下：
...
2. 假设你想将 `p1` 分区的 `[0,10000]` 范围内的数据拆分成两个 Region，示例语句如下：

    ```sql
    SPLIT PARTITION TABLE t PARTITION (p1) BETWEEN (0) AND (10000) REGIONS 2;
    ```

3. 假设你想将 `p2` 分区的 `[10000,20000]` 范围内的数据拆分成两个 Region，示例语句如下：
