...
在 TiDB 中，每创建一张新表，默认会划分出一个 [Region](/tidb-storage.md#region) 用于存储该表的数据。这个默认行为由 TiDB 配置文件中的 `split-table` 控制。当该 Region 中的数据超过默认的 Region 大小限制时，该 Region 会开始拆分为两个。

在上述情况下，由于一开始只有一个 Region，所有写入请求都会集中在该 Region 所在的 TiKV 上。如果对新建表有大量写入操作，就会造成热点问题。
...
- 均匀拆分的语法：
    ...
    `BETWEEN lower_value AND upper_value REGIONS region_num` 用于定义上边界、下边界以及 Region 的数量。然后当前 Region 会在上下边界之间，均匀拆分为 `region_num` 个 Region。
...
>
> - `tidb_wait_split_region_finish`：Region 的调度可能需要一段时间，具体时长取决于 PD 调度和 TiKV 负载。该变量用于控制执行 `SPLIT REGION` 语句时，是否等到所有 Region 调度完成后再将结果返回给客户端。如果该变量值为 `1`（默认值），TiDB 会等到调度完成后再返回结果；如果为 `0`，则无论调度是否完成都会立即返回结果。
...
同一张表的行数据具有相同的 `table_id`，但每一行有唯一的 `row_id`，可用于 Region 拆分。
...
2. 使用 `SPLIT` 语法为每个分区拆分 Region。假设你想将每个分区的 `[0,10000]` 范围的数据拆分为 4 个 Region，示例如下：
    ...
    > **Note:**
    >
    > 该示例仅适用于热点数据均匀分布的场景。如果热点数据在指定数据范围内分布不均，请参考 [拆分分区表 Region](#split-regions-for-partitioned-tables) 中的非均匀拆分语法。
...
[`tidb_scatter_region`](/system-variables.md#tidb_scatter_region) 全局变量会影响 `PRE_SPLIT_REGIONS` 的行为。该变量用于控制建表后是否等待 Region 预拆分并调度完成后再返回结果。如果建表后有大量写入操作，需要将该变量设置为 `global`，此时 TiDB 会根据整个集群的数据分布调度新建表的 Region。否则，TiDB 会在调度完成前写入数据，这会对写入性能产生较大影响。
...
建表后，该语句会为表 t 拆分出 `4 + 1` 个 Region。`4 (2^2)` 个 Region 用于存储表的行数据，1 个 Region 用于存储 `idx1` 的索引数据。
