...
- 均匀拆分语法：
    ...
    `BETWEEN lower_value AND upper_value REGIONS region_num` 用于定义上边界、下边界和 Region 数量。然后当前 Region 会在上下边界之间被均匀拆分为 `region_num` 个 Region。
...
>
> - `tidb_wait_split_region_finish`：Region 分散可能需要一段时间，具体取决于 PD 调度和 TiKV 负载。该变量用于控制执行 `SPLIT REGION` 语句时，是否等所有 Region 分散完成后再将结果返回给客户端。若值为 `1`（默认），TiDB 会在分散完成后才返回结果；若值为 `0`，则无论分散状态如何都会立即返回结果。
> - `tidb_wait_split_region_timeout`：该变量用于设置 `SPLIT REGION` 语句的执行超时时间（单位：秒），默认值为 300 秒。如果在该时间内拆分操作未完成，TiDB 会返回超时错误。
...
该语句会将表 t 在 minInt64 和 maxInt64 之间拆分为 16 个 Region。如果主键范围比上述范围小，比如 0~1000000000，可以用 0 和 1000000000 替换 minInt64 和 maxInt64 进行拆分。
...
如果已知数据分布不均匀，想要在 key 范围 -inf ~ 10000、10000 ~ 90000、90000 ~ +inf 各拆分一个 Region，可以通过设置固定拆分点实现，如下所示：
...
`upper` 和 `lower` 的值会先编码为字节数组。去除 `lower` 和 `upper` 字节数组的最长公共前缀后，将剩余部分的前 8 个字节分别转为 uint64 格式，然后计算 `step = (upper - lower)/num`。之后，将计算出的步长编码为字节数组，并拼接回最长公共前缀，作为索引拆分点。示例如下：
...
在上述拆分方式中，前缀为 y 和 z 的数据都会写入 Region 25，因为上界不是 z，而是 `{`（ASCII 中 z 的下一个字符）。因此，更准确的拆分方式如下：
...
例如，索引 `idx3 (a, b)` 包含 2 列，a 为 timestamp 类型，b 为 int。如果只想按 a 列的时间范围拆分，可以使用单列时间索引的拆分 SQL，此时不需要在 `lower_value` 和 `upper_value` 中指定 b 列的值。
...
该语句指定 3 个值，将拆分为 4 个 Region。每个 Region 的范围如下：
...
1. 创建分区表 `t`。假设要创建一个分为两个分区的 Hash 表，示例如下：
    ...
    创建表 `t` 后，每个分区会拆分出一个 Region。可使用 [`SHOW TABLE REGIONS`](/sql-statements/sql-statement-show-table-regions.md) 语法查看该表的 Region：
...
> **Note:**
>
> 通过 Split Region 语句拆分出来的 Region 受 [Region merge](/best-practices/pd-scheduling-best-practices.md#region-merge) 调度器控制。为避免 PD 在拆分后很快将新拆分的 Region 合并，需要使用 [表属性](/table-attributes.md) 或 [动态修改](/pd-control.md) 与 Region merge 相关的配置项。
