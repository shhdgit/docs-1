...
> - 从 v7.5.1 和 v7.6.0 版本开始，对于启用 [resource control](/tidb-resource-control-ru-groups.md) 的集群，`statements_summary` 将按资源组进行聚合，例如，在不同资源组中执行的相同语句会作为不同的记录收集。
...
`statements_summary_evicted` 表记录了逐出发生的时间段以及在此期间被逐出的 SQL digest 数量。此表有助于评估 `tidb_stmt_summary_max_stmt_count` 是否合理配置。如果该表中有记录，说明在某个时间点 SQL digest 数量超出了限制。
...
在 [诊断页面的 SQL 语句标签页](/tidb-cloud/tune-performance.md#statement-analysis)，逐出语句的信息会显示在 `Others` 行。
...
- `tidb_stmt_summary_max_stmt_count`：限制 `statements_summary` 和 `statements_summary_history` 表在内存中存储的 SQL digest 数量。默认值为 `3000`。
    ...
    > **Note:**
    >
    > - 当 SQL digest 被逐出时，所有时间范围的相关摘要数据会从 `statements_summary` 和 `statements_summary_history` 表中移除。因此，即使某个时间范围内的 SQL digest 数量未超出限制，`statements_summary_history` 表中的 SQL digest 数量也可能少于实际数量。如果此情况影响性能，建议增加 `tidb_stmt_summary_max_stmt_count` 的值。
...
配置生效后，`statements_summary` 表每 30 分钟清理一次，`statements_summary_history` 表最多存储 3000 种 SQL 语句。每种类型的 SQL 语句，`statements_summary_history` 表存储最近 24 个周期的数据。`statements_summary_evicted` 表记录最近 24 个周期内被逐出的 SQL 语句。`statements_summary_evicted` 表每 30 分钟更新一次。

> **Note:**
>
> - 如果某个 SQL 类型每分钟出现一次，`statements_summary_history` 会存储最近 12 小时的数据；如果某个 SQL 类型每天只在 00:00 到 00:30 出现一次，`statements_summary_history` 会存储最近 24 个周期（每个周期 1 天），即最近 24 天的数据。
...
从结果可以看出，最多有 59 个 SQL 类别被逐出。建议将 `statement_summary` 表的大小至少增加到 3059 条记录，以容纳这些逐出类别。

## 限制

默认情况下，语句摘要表存储在内存中。TiDB 重启后，所有数据会丢失。

<CustomContent platform="tidb">

为解决此问题，TiDB 在 v6.6.0 版本中实验性引入了 [statement summary persistence](#persist-statements-summary) 功能，默认关闭。启用后，历史数据不再存储在内存中，而是直接写入磁盘。这样，即使 TiDB 重启，历史数据依然可用。
...
本节内容仅适用于 TiDB 自托管版本。对于 TiDB Cloud，`tidb_stmt_summary_enable_persistent` 参数的默认值为 `false`，且不支持动态修改。
...
如 [限制](#limitation) 部分所述，语句摘要表默认存储在内存中。TiDB 重启后，所有语句摘要信息会丢失。从 v6.6.0 开始，TiDB 实验性提供了配置项 [`tidb_stmt_summary_enable_persistent`](/tidb-configuration-file.md#tidb_stmt_summary_enable_persistent-new-in-v660)，允许用户启用或禁用语句摘要持久化。

</CustomContent>

<CustomContent platform="tidb-cloud">

如 [限制](#limitation) 部分所述，语句摘要表默认存储在内存中。TiDB 重启后，所有语句摘要信息会丢失。从 v6.6.0 开始，TiDB 实验性提供了配置项 `tidb_stmt_summary_enable_persistent`，允许用户启用或禁用语句摘要持久化。
...
在此示例中，客户端表现出对 `employee` 表的点查询较慢。可以对 SQL 文本进行模糊搜索：
...
- `STMT_TYPE`: SQL 语句类型。
- `SCHEMA_NAME`: 当前执行此类别 SQL 语句的 schema。
- `DIGEST`: 此类别 SQL 语句的 digest。
- `DIGEST_TEXT`: 规范化的 SQL 语句。
- `QUERY_SAMPLE_TEXT`: 此类别 SQL 语句的原始 SQL，只取一个。
- `TABLE_NAMES`: SQL 语句涉及的所有表，多个用逗号分隔。
- `INDEX_NAMES`: SQL 语句使用的所有索引，多个用逗号分隔。
- `SAMPLE_USER`: 执行此类别 SQL 语句的用户，只取一个。
...
- `PLAN_CACHE_HITS`: 此类别 SQL 语句命中计划缓存的总次数。
- `PLAN_IN_CACHE`: 表示此类别 SQL 语句的上次执行是否命中计划缓存。
- `PLAN_CACHE_UNQUALIFIED`: 此类别 SQL 语句未命中计划缓存的次数。
...
- `FIRST_SEEN`: 此类别 SQL 语句首次出现的时间。
- `LAST_SEEN`: 此类别 SQL 语句最后一次出现的时间。

<CustomContent platform="tidb">

与 TiDB 服务器相关的字段：

- `EXEC_COUNT`: 此类别 SQL 语句的总执行次数。
- `SUM_ERRORS`: 执行过程中发生的错误总数。
- `SUM_WARNINGS`: 执行过程中发生的警告总数。
- `SUM_LATENCY`: 此类别 SQL 语句的总执行延迟。
- `MAX_LATENCY`: 此类别 SQL 语句的最大执行延迟。
- `MIN_LATENCY`: 此类别 SQL 语句的最小执行延迟。
- `AVG_LATENCY`: 此类别 SQL 语句的平均执行延迟。
...
- `AVG_TIDB_CPU_TIME`: 此类别 SQL 语句消耗的 TiDB 服务器 CPU 时间的平均值。仅在启用 [Top SQL](/dashboard/top-sql.md) 功能时显示有意义的值，否则始终为 `0`。
...
- `AVG_DISK`: 平均使用的磁盘空间（字节）。
- `MAX_DISK`: 最大使用的磁盘空间（字节）。
- `AVG_TIDB_CPU_TIME`: 此类别 SQL 语句消耗的 TiDB 服务器 CPU 时间的平均值，仅在启用 Top SQL 时有效，否则为 `0`。

字段与 TiKV Coprocessor 任务相关：
...
- `MAX_BACKOFF_TIME`: 重试前的最大等待时间。
- `AVG_TOTAL_KEYS`: Coprocessor 扫描的平均键数。
- `MAX_TOTAL_KEYS`: Coprocessor 扫描的最大键数。
- `AVG_PROCESSED_KEYS`: Coprocessor 处理的平均键数。与 `avg_total_keys` 比较，`avg_processed_keys` 不包括旧版本的 MVCC。两者差异大说明存在大量旧版本。
...
- `MAX_COMMIT_BACKOFF_TIME`: 重试前的最大等待时间。
...
- `AVG_PREWRITE_REGIONS`: 预写阶段涉及的 Region 数量的平均值。
- `MAX_PREWRITE_REGIONS`: 预写阶段涉及的最大 Region 数量。
- `AVG_TXN_RETRY`: 事务重试的平均次数。
- `MAX_TXN_RETRY`: 事务重试的最大次数。
- `SUM_BACKOFF_TIMES`: 遇到需要重试的错误时的重试总次数。
- `BACKOFF_TYPES`: 需要重试的所有错误类型及每种类型的重试次数，格式为 `type:number`，多个用逗号分隔，例如 `txnLock:2,pdRPC:1`。
- `AVG_AFFECTED_ROWS`: 影响的平均行数。
- `PREV_SAMPLE_TEXT`: 当前 SQL 语句为 `COMMIT` 时，`PREV_SAMPLE_TEXT` 为 `COMMIT` 前的语句。此时，SQL 语句会按 digest 和 `prev_sample_text` 分组。不同 `prev_sample_text` 的 `COMMIT` 语句会分到不同的行。当当前 SQL 语句不是 `COMMIT` 时，`PREV_SAMPLE_TEXT` 为空字符串。

与资源控制相关字段：

- `AVG_REQUEST_UNIT_WRITE`: SQL 语句消耗的写 RUs 平均值。
- `MAX_REQUEST_UNIT_WRITE`: SQL 语句消耗的最大写 RUs。
- `AVG_REQUEST_UNIT_READ`: SQL 语句消耗的读 RUs 平均值。
- `MAX_REQUEST_UNIT_READ`: SQL 语句消耗的最大读 RUs。
...
- `BEGIN_TIME`: 记录开始时间。
- `END_TIME`: 记录结束时间。
- `EVICTED_COUNT`: 在此期间被逐出的 SQL 类别数量。
