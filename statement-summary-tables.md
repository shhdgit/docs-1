...
本文档将详细介绍这些表，并介绍如何使用它们来排查 SQL 性能问题。
...
“执行计划摘要”指的是通过规范化执行计划计算得到的唯一标识。规范化过程会忽略常量。相同的 SQL 语句可能因为执行计划不同而被分到不同的类别。属于同一类别的 SQL 语句具有相同的执行计划。
...
> **注意：**
>
> - 在 TiDB 中，语句概要表字段的时间单位为纳秒（ns），而在 MySQL 中为皮秒（ps）。
...
[`tidb_stmt_summary_max_stmt_count`](/system-variables.md#tidb_stmt_summary_max_stmt_count-new-in-v40) 系统变量限制了 `statements_summary` 和 `statements_summary_history` 两张表在内存中总共能存储的 SQL 摘要数量。一旦超过该限制，TiDB 会将最久未使用的 SQL 摘要从 `statements_summary` 和 `statements_summary_history` 表中淘汰。
...
`statements_summary_evicted` 表记录了淘汰发生的时间段以及该时间段内被淘汰的 SQL 摘要数量。该表有助于你评估 `tidb_stmt_summary_max_stmt_count` 是否为你的业务配置得当。如果该表有记录，说明某一时刻 SQL 摘要数量超过了 `tidb_stmt_summary_max_stmt_count`。
...
- `tidb_stmt_summary_max_stmt_count`：限制 `statements_summary` 和 `statements_summary_history` 两张表在内存中总共能存储的 SQL 摘要数量。默认值为 `3000`。
    ...
    一旦超过该限制，TiDB 会将最久未使用的 SQL 摘要从 `statements_summary` 和 `statements_summary_history` 表中淘汰。这些被淘汰的摘要会被计入 [`statements_summary_evicted`](#statements_summary_evicted) 表。

    > **注意：**
    >
    > - 当某个 SQL 摘要被淘汰时，其所有时间段的相关概要数据都会从 `statements_summary` 和 `statements_summary_history` 表中移除。因此，即使某个时间段内的 SQL 摘要数量未超限，`statements_summary_history` 表中的 SQL 摘要数量也可能少于实际数量。如果出现这种情况且影响性能，建议适当增大 `tidb_stmt_summary_max_stmt_count` 的值。
    > - 对于 TiDB 自建版，当启用 [`tidb_stmt_summary_enable_persistent`](#persist-statements-summary) 后，`statements_summary_history` 表中的数据会持久化到磁盘。此时，`tidb_stmt_summary_max_stmt_count` 只限制 `statements_summary` 表在内存中能存储的 SQL 摘要数量，超出时 TiDB 只会从 `statements_summary` 表中淘汰最久未使用的 SQL 摘要。
...
上述配置生效后，`statements_summary` 表每 30 分钟清空一次，`statements_summary_history` 表最多存储 3000 类 SQL 语句，每类存储最近 24 个周期的数据。`statements_summary_evicted` 表记录最近 24 个周期内被淘汰的 SQL 语句类别。`statements_summary_evicted` 表每 30 分钟更新一次。
...
- `AVG_TIDB_CPU_TIME`：该类别 SQL 消耗的 TiDB 实例 CPU 平均时间。仅在启用 [Top SQL](/dashboard/top-sql.md) 功能时有意义，否则值始终为 `0`。
...
- `AVG_DISK`：平均使用磁盘空间（字节）。
- `MAX_DISK`：最大使用磁盘空间（字节）。
- `AVG_TIDB_CPU_TIME`：该类别 SQL 消耗的 TiDB 实例 CPU 平均时间。仅在启用 Top SQL 功能时有意义，否则值始终为 `0`。
...
- `SUM_BACKOFF_TIMES`：该类别 SQL 遇到需重试错误的总重试次数。
- `BACKOFF_TYPES`：所有需重试错误类型及各类型重试次数，格式为 `type:number`，多个类型用逗号分隔，如 `txnLock:2,pdRPC:1`。
- `AVG_AFFECTED_ROWS`：平均影响行数。
- `PREV_SAMPLE_TEXT`：当前 SQL 为 `COMMIT` 时，`PREV_SAMPLE_TEXT` 为上一个 SQL 语句。此时，SQL 按摘要和 `prev_sample_text` 分组，即不同 `prev_sample_text` 的 `COMMIT` 语句分到不同行。当前 SQL 非 `COMMIT` 时，`PREV_SAMPLE_TEXT` 为空字符串。
