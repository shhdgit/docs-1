...
> **注意：**
>
> 上述表在 [TiDB Cloud Serverless](https://docs.pingcap.com/tidbcloud/select-cluster-tier#starter) 和 [TiDB Cloud Essential](https://docs.pingcap.com/tidbcloud/select-cluster-tier#essential) 集群中不可用。

本文档详细介绍了这些表，并介绍了如何使用它们来排查 SQL 性能问题。

## `statements_summary`

`statements_summary` 是 `information_schema` 中的一个系统表。`statements_summary` 按资源组、SQL digest 和 plan digest 对 SQL 语句进行分组，并为每类 SQL 提供统计信息。
...
> **注意：**
>
> 当启用 [`tidb_stmt_summary_enable_persistent`](#persist-statements-summary) 后，`statements_summary_history` 表中的数据会持久化到磁盘。此时，`tidb_stmt_summary_max_stmt_count` 只限制 `statements_summary` 表在内存中能存储的 SQL digest 数量，超出时 TiDB 只会从 `statements_summary` 表中淘汰最久未使用的 SQL digest。

</CustomContent>

`statements_summary_evicted` 表记录了淘汰发生的时间段以及该时间段内被淘汰的 SQL digest 数量。该表有助于你评估 `tidb_stmt_summary_max_stmt_count` 是否为你的业务场景配置得当。如果该表有记录，说明某一时刻 SQL digest 的数量超过了 `tidb_stmt_summary_max_stmt_count`。
...
- `tidb_enable_stmt_summary`：是否启用 statement summary 功能。`1` 表示启用，`0` 表示禁用。默认启用。禁用后系统表中的统计信息会被清空；再次启用时会重新统计。测试表明启用该功能对性能影响很小。
...
- `tidb_stmt_summary_max_stmt_count`：限制 `statements_summary` 和 `statements_summary_history` 表在内存中总共能存储的 SQL digest 数量。默认值为 `3000`。
    ...
    > **注意：**
    >
    > - 当某个 SQL digest 被淘汰时，其所有时间段相关的汇总数据都会从 `statements_summary` 和 `statements_summary_history` 表中移除。因此，即使某个时间段内 SQL digest 的数量未超限，`statements_summary_history` 表中的 SQL digest 数量也可能小于实际数量。如果出现这种情况且影响性能，建议你增大 `tidb_stmt_summary_max_stmt_count` 的值。
    > - 对于 TiDB 自建集群，当启用 [`tidb_stmt_summary_enable_persistent`](#persist-statements-summary) 后，`statements_summary_history` 表中的数据会持久化到磁盘。此时，`tidb_stmt_summary_max_stmt_count` 只限制 `statements_summary` 表在内存中能存储的 SQL digest 数量，超出时 TiDB 只会从 `statements_summary` 表中淘汰最久未使用的 SQL digest。
...
> **注意：**
>
> - 如果某种 SQL 类型每分钟出现一次，`statements_summary_history` 会保存最近 12 小时的数据。如果某种 SQL 类型每天只在 00:00 到 00:30 出现一次，`statements_summary_history` 会保存最近 24 个周期的数据，每个周期为 1 天，因此会保存最近 24 天的数据。
> - `tidb_stmt_summary_history_size`、`tidb_stmt_summary_max_stmt_count` 和 `tidb_stmt_summary_max_sql_length` 配置项会影响内存使用量。建议你根据实际需求、SQL 大小、SQL 数量和机器配置调整这些参数，不建议设置过大。你可以通过 `tidb_stmt_summary_history_size` \* `tidb_stmt_summary_max_stmt_count` \* `tidb_stmt_summary_max_sql_length` \* `3` 计算内存占用。
...
从上述结果可以看到，最多有 59 种 SQL 类型被淘汰。此时建议你将 `statement_summary` 表的大小至少增加 59 条记录，即增大到至少 3059 条。
...
为了解决这个问题，TiDB v6.6.0 实验性地引入了 [statements summary 持久化](#persist-statements-summary) 功能，默认关闭。启用后，历史数据不再保存在内存中，而是直接写入磁盘。这样即使 TiDB 实例重启，历史数据依然可用。
...
> **警告：**
>
> statements summary 持久化是实验性功能。不建议在生产环境中使用。该功能可能会在未来的版本中变更或移除，且不会提前通知。如果你发现 bug，可以在 GitHub 上提交 [issue](https://github.com/pingcap/tidb/issues)。
...
本节通过两个示例说明如何利用 statement summary 功能排查 SQL 性能问题。
...
- `SAMPLE_USER`：执行该类别 SQL 语句的用户，仅取一个用户。
- `PLAN_DIGEST`：执行计划的 digest。
- `PLAN`：原始执行计划，若有多条语句，仅取一条的计划。
- `BINARY_PLAN`：二进制编码的原始执行计划，若有多条语句，仅取一条的计划。可通过执行 [`SELECT tidb_decode_binary_plan('xxx...')`](/functions-and-operators/tidb-functions.md#tidb_decode_binary_plan) 解析具体执行计划。
- `PLAN_CACHE_HITS`：该类别 SQL 语句命中执行计划缓存的总次数。
- `PLAN_IN_CACHE`：上一次执行该类别 SQL 语句是否命中执行计划缓存。
- `PLAN_CACHE_UNQUALIFIED`：该类别 SQL 语句未命中执行计划缓存的次数。
- `PLAN_CACHE_UNQUALIFIED_LAST_REASON`：该类别 SQL 语句上一次未命中执行计划缓存的原因。
...
- `AVG_TIDB_CPU_TIME`：该类别 SQL 语句在 TiDB 实例上消耗的平均 CPU 时间。仅在启用 [Top SQL](/dashboard/top-sql.md) 功能时有意义，否则值始终为 `0`。
...
- `AVG_DISK`：平均使用磁盘空间（字节）。
- `MAX_DISK`：最大使用磁盘空间（字节）。
- `AVG_TIDB_CPU_TIME`：该类别 SQL 语句在 TiDB 实例上消耗的平均 CPU 时间。仅在启用 Top SQL 功能时有意义，否则值始终为 `0`。
...
- `AVG_BACKOFF_TIME`：SQL 语句遇到需重试错误时的平均重试等待时间。
- `MAX_BACKOFF_TIME`：SQL 语句遇到需重试错误时的最大重试等待时间。
- `AVG_TOTAL_KEYS`：Coprocessor 扫描的平均 key 数。
- `MAX_TOTAL_KEYS`：Coprocessor 扫描的最大 key 数。
- `AVG_PROCESSED_KEYS`：Coprocessor 处理的平均 key 数。与 `avg_total_keys` 相比，`avg_processed_keys` 不包含 MVCC 的旧版本。如果两者差异较大，说明存在大量旧版本。
- `MAX_PROCESSED_KEYS`：Coprocessor 处理的最大 key 数。
...
- `AVG_COMMIT_BACKOFF_TIME`：提交阶段遇到需重试错误时的平均重试等待时间。
- `MAX_COMMIT_BACKOFF_TIME`：提交阶段遇到需重试错误时的最大重试等待时间。
...
- `AVG_REQUEST_UNIT_WRITE`：SQL 语句消耗的平均写 RU 数量。
- `MAX_REQUEST_UNIT_WRITE`：SQL 语句消耗的最大写 RU 数量。
- `AVG_REQUEST_UNIT_READ`：SQL 语句消耗的平均读 RU 数量。
- `MAX_REQUEST_UNIT_READ`：SQL 语句消耗的最大读 RU 数量。
