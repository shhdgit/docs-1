...
- TiDBが自動的に`ANALYZE`ステートメントを実行する場合、自動分析が有効になっているため、`DROP STATS`ステートメントを生成する次のステートメントを実行します。
...
- テーブルの特定のパーティションの特定のカラムの[統計情報を収集](/statistics.md#collect-statistics-on-some-columns)する場合は、次の構文を使用します。
