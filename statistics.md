...
| [`tidb_enable_auto_analyze_priority_queue`](/system-variables.md#tidb_enable_auto_analyze_priority_queue-new-in-v800) | `ON` | 控制是否启用优先队列调度自动收集统计信息的任务。启用该变量后，TiDB 优先为更有价值的表收集统计信息，如新建索引和分区发生变化的分区表。此外，TiDB 会优先处理健康度较低的表，将其排在队列前面。 |
...
直方图统计信息被优化器用于估算区间或范围谓词的选择性，也可能用于统计信息版本 2 中等值/IN 谓词的不同值个数估算（参见 [统计信息版本](#versions-of-statistics)）。
...
> **注意：**
>
> Count-Min Sketch 仅在统计信息版本 1 中用于等值/IN 谓词选择性估算。在版本 2 中，由于难以管理 Count-Min Sketch 以避免哈希冲突（见下文），已改用直方图统计信息。
...
- 修改 `WITH NUM TOPN` 参数。TiDB 会将高频（前 x 个）数据单独存储，其余数据存储在 Count-Min Sketch 中。因此，为防止较大值和较小值被哈希到一起，可以增大 `WITH NUM TOPN` 的值。在 TiDB 中，默认值为 20，最大值为 1024。关于该参数，参见 [手动收集](#manual-collection)。
...
如果表中有很多列，收集所有列的统计信息会带来较大开销。为减少开销，你可以只为特定列（你选择的）或 `PREDICATE COLUMNS`（优化器会用到的谓词列）收集统计信息。要持久化任意子集列的列表以便后续复用，参见 [持久化列配置](#persist-column-configurations)。
...
- 要收集 `PREDICATE COLUMNS` 的统计信息，使用以下语法：
    ...
    > - 被排除在收集之外的列（无论是手动列举还是使用 `PREDICATE COLUMNS`）不会被覆盖其统计信息。当执行新类型的 SQL 查询时，优化器会使用这些列的旧统计信息（如果存在），或伪列统计信息（如果从未收集过）。下次使用 `PREDICATE COLUMNS` 执行 ANALYZE 时会收集这些列的统计信息。
...
- 当触发全局统计信息更新且 [`tidb_skip_missing_partition_stats`](/system-variables.md#tidb_skip_missing_partition_stats-new-in-v730) 为 `OFF` 时：
    ...
    - 如果部分分区没有统计信息（如新建分区从未分析过），全局统计信息生成会中断，并显示警告，提示分区上无统计信息。

    - 如果某些分区缺少特定列的统计信息（这些分区分析时指定了不同的列），在聚合这些列的统计信息时全局统计信息生成会中断，并显示警告，提示某些分区缺少某些列的统计信息。

- 当触发全局统计信息更新且 [`tidb_skip_missing_partition_stats`](/system-variables.md#tidb_skip_missing_partition_stats-new-in-v730) 为 `ON` 时：

    - 如果部分分区缺少全部或部分列的统计信息，TiDB 在生成全局统计信息时会跳过这些缺失的分区统计信息，不影响全局统计信息的生成。
...
当前采样率基于自适应算法计算。当你可以通过 [`SHOW STATS_META`](/sql-statements/sql-statement-show-stats-meta.md) 观察表的行数时，可以用该行数计算对应 10 万行的采样率。如果无法观察该行数，可以用 [`SHOW TABLE REGIONS`](/sql-statements/sql-statement-show-table-regions.md) 结果中 `APPROXIMATE_KEYS` 列的所有值之和作为参考，计算采样率。
...
TiDB 会用最新一次 `ANALYZE` 语句指定的新配置覆盖之前记录的持久化配置。例如，执行 `ANALYZE TABLE t WITH 200 TOPN;` 会设置 `ANALYZE` 语句的 Top-N 为 200。随后执行 `ANALYZE TABLE t WITH 0.1 SAMPLERATE;`，则自动 `ANALYZE` 语句会同时设置 Top-N 为 200 和采样率为 0.1，等价于 `ANALYZE TABLE t WITH 200 TOPN, 0.1 SAMPLERATE;`。
...
推荐使用版本 2，未来也将持续增强并最终完全替代版本 1。与版本 1 相比，版本 2 在大数据量下提升了许多统计信息的准确性。版本 2 还通过不再为谓词选择性估算收集 Count-Min Sketch 统计信息，以及支持仅自动收集部分列，提升了收集性能（参见 [收集部分列的统计信息](#collect-statistics-on-some-columns)）。
...
建议确保所有表/索引（及分区）都使用同一版本的统计信息收集。推荐使用版本 2，但不建议无正当理由（如当前版本出现问题）随意切换版本。切换版本期间，直到所有表都用新版本分析完毕前，可能会出现无统计信息的情况，进而影响优化器的执行计划选择。
...
- 如果上述语句结果过长无法直接复制粘贴，可以将结果导出到临时文本文件，然后从文件执行：
...
自 TiDB v6.1.0 起，`SHOW ANALYZE STATUS` 支持显示集群级任务。即使 TiDB 重启后，仍可通过该语句查看重启前的任务记录。TiDB v6.1.0 之前，`SHOW ANALYZE STATUS` 只能显示实例级任务，且重启后任务记录会被清空。
...
- 对于占用内存较大的统计信息（如直方图、TopN 和 Count-Min Sketch），为保证 SQL 执行性能，TiDB 会按需异步加载这些统计信息。例如，TiDB 仅在优化器需要使用某列的直方图统计信息时，才将该列的直方图统计信息加载到内存。按需异步加载统计信息不会影响 SQL 执行性能，但可能导致 SQL 优化时统计信息不完整。
...
- 要控制 SQL 优化等待超时后的行为，可修改 [`tidb_stats_load_pseudo_timeout`](/system-variables.md#tidb_stats_load_pseudo_timeout-new-in-v540) 系统变量。该变量默认值为 `ON`，表示超时后 SQL 优化过程不会使用任何列的直方图、TopN 或 CMSketch 统计信息。若该变量设为 `OFF`，超时后 SQL 执行失败。
...
`lite-init-stats` 默认值为 `true`，即启用轻量级统计信息初始化。将 `lite-init-stats` 设为 `true` 可加快统计信息初始化速度，并通过避免不必要的统计信息加载减少 TiDB 内存使用。

</CustomContent>

<CustomContent platform="tidb-cloud">

启用同步加载统计信息特性后，你可以通过修改 [`tidb_stats_load_pseudo_timeout`](/system-variables.md#tidb_stats_load_pseudo_timeout-new-in-v540) 系统变量的值，控制 SQL 优化等待超时后的行为。该变量默认值为 `ON`，表示超时后 SQL 优化过程不会使用任何列的直方图、TopN 或 CMSketch 统计信息。若该变量设为 `OFF`，超时后 SQL 执行失败。
...
2. 终止正在后台运行的 `ANALYZE` 任务。
    ...
    - 若 `enable-global-kill` 为 `false`，需使用客户端连接到正在执行后台 `ANALYZE` 任务的 TiDB 实例，然后执行 `KILL TIDB ${id};`。如果使用客户端连接到其他 TiDB 实例，或客户端与 TiDB 集群之间有代理，则 `KILL` 语句无法终止后台 `ANALYZE` 任务。
