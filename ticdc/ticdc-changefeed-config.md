...
# TiCDC ChangefeedのCLIとコンフィグレーションパラメーター {#cli-and-configuration-parameters-of-ticdc-changefeeds}

## Changefeed CLIパラメーター {#changefeed-cli-parameters}

このセクションでは、レプリケーション（changefeed）タスクを作成する方法を説明することで、TiCDC changefeedのコマンドラインパラメーターを紹介します。
...
- `--changefeed-id`: レプリケーションタスクのID。フォーマットは `^[a-zA-Z0-9]+(\-[a-zA-Z0-9]+)*$` の正規表現に一致する必要があります。このIDが指定されていない場合、TiCDCは自動的にUUID（バージョン4のフォーマット）をIDとして生成します。

- `--sink-uri`: レプリケーションタスクの下流アドレス。次のフォーマットに従って `--sink-uri` を設定します。現在、スキームは `mysql`、`tidb`、および `kafka` をサポートしています。

      [scheme]://[userinfo@][host]:[port][/path]?[query_parameters]

  `--sink-uri` に `! * ' ( ) ; : @ & = + $ , / ? % # [ ]` などの特殊文字が含まれる場合、特殊文字をエスケープする必要があります。例えば、[URIエンコーダー](https://www.urlencoder.org/)でエスケープすることができます。

- `--start-ts`: チェンジフィードの開始TSOを指定します。このTSOから、TiCDCクラスターはデータを取得し始めます。デフォルト値は現在時刻です。

- `--target-ts`: チェンジフィードの終了TSOを指定します。このTSOまで、TiCDCクラスターはデータの取得を停止します。デフォルト値は空で、TiCDCは自動的にデータの取得を停止しません。

- `--config`: チェンジフィードの設定ファイルを指定します。

## チェンジフィードの設定パラメーター {#changefeed-configuration-parameters}
