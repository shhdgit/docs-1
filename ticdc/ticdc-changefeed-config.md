...
- `--sink-uri`: レプリケーションタスクのダウンストリームアドレス。`--sink-uri`は以下のフォーマットに従って構成します。現在、スキームは `mysql`、`tidb`、`kafka` をサポートしています。

  ```
  [scheme]://[userinfo@][host]:[port][/path]?[query_parameters]
  ```

  シンクURIに `! * ' ( ) ; : @ & = + $ , / ? % # [ ]` などの特殊文字が含まれている場合、特殊文字をエスケープする必要があります。例えば、[URIエンコーダー](https://www.urlencoder.org/)を使用します。

- `--start-ts`: changefeedの開始TSOを指定します。このTSOから、TiCDCクラスターはデータを取得し始めます。デフォルト値は現在時刻です。

- `--target-ts`: changefeedの終了TSOを指定します。このTSOまで、TiCDCクラスターはデータを取得を停止します。デフォルト値は空で、つまりTiCDCは自動的にデータの取得を停止しません。
