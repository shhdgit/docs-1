...
TiDB 兼容 MySQL，使得迁移和对接现有应用变得简单，同时具备无缝扩展性，能够从小型工作负载扩展到大规模高性能集群。它在一个系统中同时支持事务型（OLTP）和分析型（OLAP）工作负载，简化运维并实现实时洞察。
...
Starter 集群方案非常适合初次使用 TiDB Cloud 的用户。它为开发者和小型团队提供以下特性：

- **免费**：该方案完全免费，无需信用卡即可开始使用。
- **存储**：提供初始 5 GiB 行存储和 5 GiB 列存储。
...
- 默认情况下，采用 Zonal High Availability 选项的集群，其所有组件位于同一可用区，带来更低的网络延时。
...
你可以部署多个 TiDB 节点以实现水平扩展，处理更高的工作负载。这些节点通常与负载均衡器（如 TiProxy 或 HAProxy）配合使用，提供无缝接口。TiDB 节点本身不存储数据——它们会将数据请求转发给 TiKV 节点（行存储）或 TiFlash 节点（列存储）。
