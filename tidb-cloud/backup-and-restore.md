...
4. 按如下方式配置备份计划：
    ...
    - 在 **Backup Retention** 中，配置备份数据的最小保留周期。默认周期为 7 天。为尽量减少对业务的影响，建议在业务低峰期安排自动备份。
        ...
        > **注意**
        >
        > - 除最新的自动备份外，所有超出保留周期的自动备份都会被删除。最新的自动备份不会被删除，除非你手动删除。这样可以确保在意外删除时仍可恢复集群数据。
...
TiDB Cloud Dedicated 通过将备份从集群所在区域复制到另一个不同区域，实现双区域备份。开启该功能后，所有备份会自动复制到指定区域。这为数据跨区域保护和灾备提供了能力。预计约 99% 的数据可在一小时内复制到次级区域。
...
手动备份是用户主动发起的备份，可以根据需要将数据备份到已知状态，并可随时恢复到该状态。
