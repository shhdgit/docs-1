...
> **提示**
>
> 如需了解如何在 {{{ .starter }}} 或 {{{ .essential }}} 集群上备份和恢复数据，请参见 [在 {{{ .starter }}} 或 Essential 集群上备份和恢复数据](/tidb-cloud/backup-and-restore-serverless.md)。
...
- 如果你多次开启和关闭 Point-in-time Restore，只能选择最近一次开启后可恢复范围内的时间点，之前的可恢复范围无法访问。
...
4. 按如下方式配置备份计划：
    ...
    - 在 **Backup Retention** 中，配置备份数据的最小保留周期。默认周期为 7 天。为尽量减少对业务的影响，建议在业务低峰期安排自动备份。
        ...
        > **注意**
        >
        > - 除最新备份外，所有超出保留周期的自动备份都会被删除。最新的自动备份不会被删除，除非你手动删除。这确保了在意外删除时可以恢复集群数据。
...
TiDB Cloud 专属集群支持通过将备份从集群所在区域复制到另一个不同区域，实现双区域备份。开启该功能后，所有备份会自动复制到指定区域，实现跨区域数据保护和灾备能力。预计约 99% 的数据可在一小时内复制到次级区域。
...
手动备份是由用户发起的备份，可以根据需要将数据备份到已知状态，并可随时恢复到该状态。

> **注意**
>
> - 手动备份会被无限期保留，直到你手动删除或账户关闭。
> - 删除 TiDB Cloud 专属集群后，已有的手动备份会被移入回收站，直至手动删除或账户关闭。
...
4. 在 **Restore Mode** 中，选择将任意时间点的数据或指定备份恢复到新集群。
...
4. 在 **Backups** 页面，找到你想恢复的备份时间，在 **Action** 列点击 **...**，然后点击 **Restore**。
