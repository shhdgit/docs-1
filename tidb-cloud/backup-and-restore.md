...
TiDB Cloud Dedicated 支持 [snapshot backup](https://docs.pingcap.com/tidb/stable/br-snapshot-guide) 和 [log backup](https://docs.pingcap.com/tidb/stable/br-pitr-guide)。快照备份允许你将数据恢复到备份时的状态。默认情况下，快照备份会自动执行，并根据你的备份保留策略进行存储。你可以随时关闭自动备份。
...
TiDB Cloud Dedicated 支持通过将备份从集群所在区域复制到另一个不同区域，实现双区域备份。启用该功能后，所有备份会自动复制到指定区域。这为数据提供了跨区域保护和灾备能力。预计约 99% 的数据可在一小时内复制到次级区域。
...
> - 删除 TiDB Cloud Dedicated 集群后，现有的手动备份会被移至回收站，直至你手动删除或账户关闭。
