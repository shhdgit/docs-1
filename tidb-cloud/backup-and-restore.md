...
4. 按如下方式配置备份计划：
    ...
    - 在 **Backup Retention** 中，配置备份数据的最小保留周期。默认周期为 7 天。为尽量减少对业务的影响，建议在业务低峰期安排自动备份。
        ...
        > **Note**
        >
        > - 除最新一次外，所有超出保留周期的自动备份将被删除。最新的自动备份不会被删除，除非你手动删除。这样可以确保在误删情况下仍可恢复集群数据。
        > - 删除集群后，保留周期内的自动备份将被移入回收站。
...
> - 托管在 Google Cloud 上的 TiDB Cloud 专属集群可无缝对接 Google Cloud Storage。与 Google Cloud Storage 类似，**TiDB Cloud 专属集群仅支持在与 Google 双区域存储相同的 multi-region code 内进行双区域配对**。例如，在亚洲，目前必须将东京和大阪配对为双区域存储。更多信息请参见 [Dual-regions](https://cloud.google.com/storage/docs/locations#location-dr)。
...
> - 删除 TiDB Cloud 专属集群后，已有的手动备份会被移入回收站，直至你手动删除或账户关闭。
