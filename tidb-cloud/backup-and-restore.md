...
- 对于 v6.2.0 或更高版本的集群，TiDB Cloud 专属集群默认支持从备份中恢复用户账户和 SQL 绑定。
...
- 如果你多次开启和关闭 Point-in-time Restore，只能选择最近一次开启 Point-in-time Restore 后的可恢复时间范围，之前的可恢复范围无法访问。
...
> **注意**
>
> Point-in-time Restore 功能仅支持 v6.4.0 或更高版本的 TiDB Cloud 专属集群。
...
4. 按如下方式配置备份计划：
    ...
    - 在 **Backup Retention** 中，配置最小备份数据保留周期。默认周期为 7 天。为尽量减少对业务的影响，建议在业务低峰期安排自动备份。
        ...
        > **注意**
        >
        > - 除最新备份外，所有自动备份在超出保留周期后会被删除。最新自动备份不会被删除，除非你手动删除。这样可以确保在意外删除时仍可恢复集群数据。
...
> - 托管在 Google Cloud 上的 TiDB Cloud 专属集群可无缝对接 Google Cloud Storage。与 Google Cloud Storage 类似，**TiDB Cloud 专属集群仅支持在同一 multi-region code 下的 Google 双区域存储配对**。例如，在亚洲，目前必须将东京和大阪配对用于双区域存储。更多信息请参见 [Dual-regions](https://cloud.google.com/storage/docs/locations#location-dr)。
