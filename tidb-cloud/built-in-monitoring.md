...
| 指标名称  | 标签 | 描述                                   |
| :------------| :------| :-------------------------------------------- |
| Request Units | RU per second | Request Unit（RU）是用于衡量查询或事务资源消耗的单位。除了你运行的查询外，后台活动也会消耗 Request Units，因此当 QPS 为 0 时，每秒 Request Units 可能也不为 0。 |
...
| Average Query Duration | All, {SQL type} | 从客户端发起请求到 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群，直到集群执行请求并返回结果给客户端的持续时间。 |
...
| Average Query Duration Per DB | All, {Database name} | 从客户端发起请求到某个数据库，直到该数据库执行请求并返回结果给客户端的持续时间。|
