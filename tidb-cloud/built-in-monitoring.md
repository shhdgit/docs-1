...
| Query Duration | avg-{SQL type}, 99-{SQL type} | 从 TiDB 接收到客户端请求到 TiDB 执行请求并返回结果给客户端的持续时间。通常，客户端请求以 SQL 语句形式发送；但该持续时间也可能包含如 `COM_PING`、`COM_SLEEP`、`COM_STMT_FETCH`、`COM_SEND_LONG_DATA` 等命令的执行时间。TiDB 支持 Multi-Query，即客户端可以一次发送多个 SQL 语句，如 `select 1; select 1; select 1;`。此时，该查询的总执行时间包含所有 SQL 语句的执行时间。|
...
| Query Duration | Avg, P99, P99-{SQL type} | 从客户端请求到 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群接收请求、执行并返回结果给客户端的持续时间。|
| Failed Query | All | 每秒 SQL 语句执行错误的数量。|
| Transaction Per Second | All | 每秒执行的事务数量。|
| Transaction Duration | Avg, P99 | 事务的执行持续时间。|
| Lock-wait | P95, P99 | 事务等待获取悲观锁所花费的时间。较高的值表示对同一行或键的竞争。|
...
| Average Query Duration Per DB | All, {Database name} | 从客户端请求到数据库接收请求、执行并返回结果给客户端的持续时间。|
...
- 对应集群的负载未触发该指标。例如，在没有失败查询的情况下，失败查询指标始终为空。
