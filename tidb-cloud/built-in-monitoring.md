...
| 指标名称  | 标签 | 描述                                   |
| :------------| :------| :-------------------------------------------- |
| Database Time by SQL types | database time, {SQL type} | database time：每秒的总数据库时间。<br/> {SQL type}：每秒由不同 SQL 类型（如 `SELECT`、`INSERT`、`UPDATE` 等）消耗的数据库时间。 |
| Query Per Second | {SQL type} | 所有 TiDB 实例每秒执行的 SQL 语句数量，按 SQL 类型（如 `SELECT`、`INSERT`、`UPDATE` 等）统计。 |
| Query Duration | avg-{SQL type}, 99-{SQL type} | 从 TiDB 接收到客户端请求到 TiDB 执行请求并将结果返回给客户端的持续时间。通常，客户端请求以 SQL 语句的形式发送；但该持续时间也可能包括如 `COM_PING`、`COM_SLEEP`、`COM_STMT_FETCH` 和 `COM_SEND_LONG_DATA` 等命令的执行时间。TiDB 支持 Multi-Query，即客户端可以一次发送多条 SQL 语句，如 `select 1; select 1; select 1;`。此时，该查询的总执行时间包含所有 SQL 语句的执行时间。 |
...
## TiDB Cloud Serverless 和 TiDB Cloud Essential 集群的指标
...
| 指标名称  | 标签 | 描述                                   |
| :------------| :------| :-------------------------------------------- |
| Request Units | RU per second | Request Unit（RU）是用于衡量 TiDB Cloud Serverless 集群中查询或事务资源消耗的单位。除了用户查询，后台活动也会消耗 RU，因此即使 QPS 为 0，每秒 RU 使用量也可能不为零。|
| Capacity vs Usage (RU/s) | Provisioned capacity (RCU), Consumed RU/s | TiDB Cloud Essential 集群中 Request Capacity Units（RCU）和每秒消耗的 Request Units（RU）。 |
...
| Query Duration | Avg, P99, P99-{SQL type} | 从客户端请求到 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群接收请求、执行并返回结果的持续时间。 |
| Failed Query | All | 每秒 SQL 语句执行错误的数量。 |
| Transaction Per Second | All | 每秒执行的事务数量。 |
| Transaction Duration | Avg, P99 | 事务的执行持续时间。 |
| Lock-wait | P95, P99 | 事务等待获取悲观锁的时间。较高的值表示对同一行或键存在竞争。 |
| Total Connection | All | 连接到 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群的连接数。 |
| Idle Connection Duration | P99, P99(in-txn), P99(not-in-txn) | 连接在开启事务时处于空闲状态的时长。较长的持续时间通常表示应用逻辑较慢或存在长事务。 |
