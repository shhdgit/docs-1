...
TiDB Cloud 会在 **Metrics** 页面收集并展示集群的全套标准指标。通过查看这些指标，你可以轻松识别性能问题，并判断当前的数据库部署是否满足你的需求。

## 查看 Metrics 页面

要在 **Metrics** 页面查看指标，请按照以下步骤操作：

1. 在你的项目的 [**Clusters**](https://tidbcloud.com/project/clusters) 页面，点击目标集群的名称，进入该集群的概览页面。
...
| Query Duration | avg-{SQL type}, 99-{SQL type} | 从客户端发送请求到 TiDB，直到 TiDB 执行请求并返回结果给客户端的持续时间。通常，客户端请求以 SQL 语句形式发送；但该持续时间也可能包括如 `COM_PING`、`COM_SLEEP`、`COM_STMT_FETCH` 和 `COM_SEND_LONG_DATA` 等命令的执行时间。TiDB 支持 Multi-Query，即客户端可以一次发送多条 SQL 语句，如 `select 1; select 1; select 1;`。此时，该查询的总执行时间包含所有 SQL 语句的执行时间。|
...
| Average Query Duration | All, {SQL type} | 从客户端发送请求到 {{{ .starter }}} 或 {{{ .essential }}} 集群，直到集群执行请求并返回结果给客户端的持续时间。|
