...
| 指标名称  | 标签 | 描述                                   |
| :------------| :------| :-------------------------------------------- |
| Database Time by SQL types | database time, {SQL type} | database time：每秒的总数据库时间。<br/> {SQL type}：按 SQL 类型（如 `SELECT`、`INSERT`、`UPDATE` 等）统计的每秒数据库时间。|
...
**Metrics** 页面为 TiDB Cloud Serverless 和 TiDB Cloud Essential 集群提供了两个指标选项卡：
...
下表展示了 **Cluster Status** 选项卡下的集群级别主要指标。
...
下表展示了 **Database Status** 选项卡下的数据库级别主要指标。
...
- 对应集群的负载没有触发该指标。例如，如果没有失败的查询，则失败查询指标面板始终为空。
