...
> - 对于 [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 和 [TiDB Cloud Essential](/tidb-cloud/select-cluster-tier.md#essential) 集群，暂不支持 changefeed 功能。
...
你可以通过扩容或缩容 changefeed 来调整 TiCDC Replication Capacity Units（RCU）。
...
扩缩容过程大约需要 10 分钟（期间 changefeed 可正常工作），切换到新规格大约需要几秒钟（切换期间 changefeed 会自动暂停并恢复）。
...
关于 TiDB Cloud 中 changefeed 的计费方式，详见 [Changefeed billing](/tidb-cloud/tidb-cloud-billing-ticdc-rcu.md)。
