...
4. 在 **Data Format** 区域，选择你期望的 Kafka 消息格式。
    ...
    - Open Protocol 是一种行级数据变更通知协议，为监控、缓存、全文索引、分析引擎以及不同数据库间主从复制等场景提供数据源。详见 [Open Protocol data format](https://docs.pingcap.com/tidb/stable/ticdc-open-protocol)。
...
5. 如果你希望在 Kafka 消息体中添加 TiDB 扩展字段，可启用 **TiDB Extension** 选项。
    ...
    关于 TiDB 扩展字段的更多信息，参见 [Avro data format 中的 TiDB 扩展字段](https://docs.pingcap.com/tidb/stable/ticdc-avro-protocol#tidb-extension-fields) 和 [Canal-JSON data format 中的 TiDB 扩展字段](https://docs.pingcap.com/tidb/stable/ticdc-canal-json#tidb-extension-field)。
...
8. 在 **Partition Distribution** 区域，你可以决定 Kafka 消息发送到哪个分区。你可以为所有表定义 **单一分区分发器**，也可以为不同表定义 **不同的分区分发器**。TiDB Cloud 提供四种分发器类型：
    ...
    - **Distribute changelogs by table to Kafka partition**
        ...
        如果你希望将某个表的 Kafka 消息全部发送到同一个分区，选择此方式。行变更日志的表名将决定其发送到哪个分区。该方式保证表级有序，但可能导致分区不均衡。
