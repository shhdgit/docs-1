...
5. 在 **Topic Distribution** 区域，选择分发模式，并根据所选模式填写 topic 名称配置。
    ...
    由于 Pulsar 支持多租户，如果租户和命名空间与默认值不同，你还可以设置 **Pulsar Tenant** 和 **Pulsar Namespace**。
...
7. 在 **Split Event** 区域，选择是否将 `UPDATE` 事件拆分为独立的 `DELETE` 和 `INSERT` 事件，或保持为原始的 `UPDATE` 事件。更多信息请参见 [为非 MySQL sink 拆分主键或唯一键 UPDATE 事件](https://docs.pingcap.com/tidb/stable/ticdc-split-update-behavior/#split-primary-or-unique-key-update-events-for-non-mysql-sinks)。
