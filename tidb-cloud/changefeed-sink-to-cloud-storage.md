...
summary: 本文档介绍如何创建变更订阅（changefeed），将 TiDB Cloud 的数据流式同步到 Amazon S3 或 GCS。内容包括限制、目标端配置、同步配置与规范说明，以及启动同步流程。
---
...
进入目标 TiDB 集群的集群总览页面。在左侧导航栏点击 **Data** > **Changefeed**，点击 **Create Changefeed**，并选择 **Amazon S3** 或 **GCS** 作为目标端。不同目标端的配置流程有所不同。
...
1. 自定义 **Table Filter**，筛选你希望同步的表。规则语法请参考 [table filter rules](https://docs.pingcap.com/tidb/stable/ticdc-filter#changefeed-log-filters)。
    ...
    - **Case Sensitive**：可设置过滤规则中数据库和表名的匹配是否区分大小写。默认情况下，匹配不区分大小写。
    - **Filter Rules**：可在此列设置过滤规则。默认有一条规则 `*.*`，表示同步所有表。添加新规则后，TiDB Cloud 会查询 TiDB 中所有表，并在右侧仅显示符合规则的表。最多可添加 100 条过滤规则。
...
2. 自定义 **Event Filter**，筛选你希望同步的事件。
    ...
    - **Tables matching**：可设置事件过滤器应用于哪些表。规则语法与前述 **Table Filter** 区域相同。每个变更订阅最多可添加 10 条事件过滤规则。
    - **Event Filter**：可使用以下事件过滤器，从变更订阅中排除特定事件：
...
5. 在 **Flush Parameters** 区域，可配置以下两项：
