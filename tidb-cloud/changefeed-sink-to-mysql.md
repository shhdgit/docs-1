...
1. 在 MySQL 服务所在的 VPC 与 TiDB 集群的 VPC 之间 [建立 VPC 对等连接](/tidb-cloud/set-up-vpc-peering-connections.md)。
...
2. 在 MySQL 服务所在的 VPC 与 TiDB 集群的 VPC 之间 [建立 VPC 对等连接](/tidb-cloud/set-up-vpc-peering-connections.md)。
...
5. 自定义 **Table Filter**，筛选你希望同步的表。规则语法可参考 [table filter rules](/table-filter.md)。
    ...
    - **Tables without valid keys**：此列显示缺少主键或唯一键的表。这些表在同步过程中存在挑战，因为缺少唯一标识符可能导致下游处理重复事件时数据不一致。为保证数据一致性，建议在同步前为这些表添加唯一键或主键，或者通过添加过滤规则排除这些表。例如，可以使用规则 `"!test.tbl1"` 排除表 `test.tbl1`。
...
7. 在 **Start Replication Position** 中，配置 MySQL sink 的起始同步位置。
    ...
    - 如果你已使用 Dumpling [加载了现有数据](#load-existing-data-optional)，请选择 **Start replication from a specific TSO**，并填写从 Dumpling 导出 metadata 文件中获取的 TSO。
...
11. 如果你已使用 Dumpling [加载了现有数据](#load-existing-data-optional)，在 sink 创建完成后，需要将 GC 时间恢复为原始值（默认值为 `10m`）：
