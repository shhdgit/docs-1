...
> **注意：**
>
> 本文档适用于 [**TiDB Cloud Dedicated**](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated)。关于为 **{{{ .starter }}}** 或 **{{{ .essential }}}** 配置 IP 访问列表的说明，请参见 [为 {{{ .starter }}} 或 Essential 公网端点配置防火墙规则](/tidb-cloud/configure-serverless-firewall-rules-for-public-endpoints.md)。
...
4. 在弹出的对话框中，选择以下选项之一：
    ...
    - **Use IP addresses**（推荐）：你可以添加允许通过 SQL 客户端访问 TiDB Cloud 的 IP 和 CIDR 地址列表。
