...
- 可以关闭维护时间窗口吗？
    ...
    不可以。维护时间窗口默认开启且无法关闭。你可以修改维护时间窗口的开始时间，或在截止时间前重新安排维护任务。详情请参见 [查看和配置维护时间窗口](#view-and-configure-maintenance-windows)。
...
- 维护任务会在任意状态的集群上执行吗？
    ...
    - 如果集群正在进行自动或手动备份，维护将被延迟，待当前备份成功完成后再触发。请注意，对于数据量较大的集群，备份过程可能需要较长时间，例如 12 小时。为尽量减少对集群的影响，建议合理设置备份和维护时间窗口的开始时间。
