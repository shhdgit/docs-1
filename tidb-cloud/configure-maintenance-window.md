...
在维护时间窗口期间，TiDB Cloud 会依次对专属集群执行维护操作，因此整体影响较小。虽然可能会出现临时的连接中断或 QPS 波动，但集群始终保持可用，现有的数据导入、备份、恢复、迁移和同步任务仍可正常运行。
...
> **Note:**
>
> - 对于首次注册 TiDB Cloud 时自动创建的默认项目，维护时间窗口将在每周三凌晨 03:00（以你的 TiDB Cloud 组织的时区为准）开始。
> - 对于你新创建的项目，可以在项目设置期间自定义维护时间窗口的开始时间。

你可以按如下方式将开始时间修改为你期望的时间，或在截止时间前重新安排维护任务：
...
3. 在 **Maintenance** 页面，查看维护信息。
     ...
     - 如果有维护任务显示，请查看描述、计划开始时间和截止时间。维护任务将在指定时间开始执行。
...
- 维护任务会在任意状态的集群上执行吗？
    ...
    - 如果集群正在进行自动或手动备份，维护将被延迟，直到当前备份成功完成后再触发。请注意，对于数据量较大的集群，备份过程可能需要较长时间，例如 12 小时。为尽量减少对集群的影响，建议合理设置备份和维护时间窗口的开始时间。
