...
在维护时间窗口期间，TiDB Cloud 会依次对专属集群执行维护操作，从而将整体影响降到最低。虽然可能会出现临时的连接中断或 QPS 波动，但集群依然可用，现有的数据导入、备份、恢复、迁移和同步任务仍可正常运行。
...
- 不允许的操作：
    ...
    - 创建私有链接或配置 VPC 对等连接
    - 创建导入任务、迁移任务或变更数据同步（changefeed）
    - 调整迁移任务或变更数据同步（changefeed）的规格

## 获取维护时间窗口的通知

为避免潜在的中断，了解维护计划并据此安排你的操作非常重要。

每个维护时间窗口，TiDB Cloud 会在以下时间点向所有项目成员发送四封邮件通知：
...
定期维护可确保执行重要更新，保护 TiDB Cloud 免受安全威胁、性能问题和不可靠性的影响。因此，维护时间窗口默认启用且无法禁用。

> **Note:**
>
> - 对于首次注册 TiDB Cloud 时自动创建的默认项目，维护时间窗口默认在每周三凌晨 03:00（以你的 TiDB Cloud 组织的时区为准）开始。
> - 对于你新创建的项目，可以在项目设置期间自定义维护时间窗口的开始时间。
...
5. 若需重新安排某个具体的维护任务，在 **Action** 列点击 **...** > **Reschedule**，并在截止时间前选择新的时间。

    如果你需要将维护任务重新安排到截止时间之后，请联系 [TiDB Cloud Support](/tidb-cloud/tidb-cloud-support.md#tidb-cloud-support) 获取帮助。
...
- 可以禁用维护时间窗口吗？
    ...
    不可以。维护时间窗口默认启用且无法禁用。你可以修改维护时间窗口的开始时间，或在截止时间前重新安排维护任务。更多信息请参见 [查看和配置维护时间窗口](#view-and-configure-maintenance-windows)。
...
- 维护任务会在任意状态的集群上执行吗？
    ...
    - 如果集群正在执行自动或手动备份，维护将被延迟，直到当前备份成功完成后再触发。请注意，对于数据量较大的集群，备份过程可能需要较长时间，例如 12 小时。为尽量减少对集群的影响，建议你合理设置备份和维护时间窗口的开始时间。
    - 如果集群处于其他状态，维护任务将按计划开始执行。