...
在创建 Data App 之前，请确保你已经创建了一个托管在 AWS 上的 [{{{ .starter }}}](/tidb-cloud/select-cluster-tier.md) 集群。如果还没有，请按照 [Create a {{{ .starter }}} or Essential Cluster](/tidb-cloud/create-tidb-cluster-serverless.md) 的步骤创建。

> **Note:**
>
> Data Service 仅适用于托管在 AWS 上的 [{{{ .starter }}}](/tidb-cloud/select-cluster-tier.md) 集群。如需在 TiDB Cloud Dedicated 集群中使用 Data Service，请联系 [TiDB Cloud Support](/tidb-cloud/tidb-cloud-support.md)。

## 使用示例 Data App 快速入门

创建一个示例 Data App 是体验 Data Service 的最佳方式。如果你的项目还没有任何 Data App，可以按照 **Data Service** 页面上的引导创建一个示例 Data App，并用它来探索 Data Service 的功能。
...
4. 示例 Data App 自动创建完成后，你可以在左侧看到 App 名称和端点列表，在中间看到端点的 SQL 语句，在右侧看到使用示例 Data App 的说明。
...
4. （可选）如需将 Data App 的端点自动部署到你指定的 GitHub 仓库和分支，请启用 **Connect to GitHub**，然后执行以下操作：
    ...
    > **Note:**
    >
    > - 目录必须以斜杠（`/`）开头。例如，`/mydata`。如果你指定的目录在目标仓库和分支中不存在，会自动创建。
...
在右侧面板点击 **Properties** 标签，设置端点属性，例如：
...
1. 选择集群。
    ...
    在 SQL 编辑器上方，从下拉列表选择你希望执行 SQL 语句的集群。然后，你可以在右侧面板的 **Schema** 标签下查看该集群的所有数据库。
...
2. 编写 SQL 语句。
    ...
    如需定义参数，可以在 SQL 语句中以变量占位符的形式插入，例如 `${ID}`。例如，`SELECT * FROM table_name WHERE id = ${ID}`。然后，你可以点击右侧面板的 **Params** 标签修改参数定义和测试值。
...
3. 运行 SQL 语句。
    ...
    如果 SQL 语句中包含参数，请确保你已在右侧 **Params** 标签下为参数设置了测试值或默认值，否则会返回错误。
    ...
    运行后，你可以在页面底部的 **Result** 标签下立即看到查询结果。
...
然后，你可以在页面底部的 **HTTP Response** 标签下看到响应。关于响应的更多信息，参见 [Response of an endpoint](/tidb-cloud/data-service-manage-endpoint.md#response)。
...
2. 点击 **Deploy** 确认部署。如果端点部署成功，会提示 **Endpoint has been deployed**。
    ...
    如需查看部署历史，可以点击左侧 Data App 名称，再点击右侧的 **Deployments** 标签。
