...
4. （可选）如需将 Data App 的端点自动部署到你指定的 GitHub 仓库和分支，请启用 **Connect to GitHub**，然后执行以下操作：
    ...
    2. 返回 TiDB Cloud 控制台，点击 **Authorize** 授权访问 GitHub 上的应用。
...
- **Endpoint URL**：（只读）URL 会根据对应集群所在区域、Data App 的服务 URL 以及端点路径自动生成。例如，若端点路径为 `/my_endpoint/get_id`，则端点 URL 为 `https://<region>.data.tidbcloud.com/api/v1beta/app/<App ID>/endpoint/my_endpoint/get_id`。
...
1. 在 [**Data Service**](https://tidbcloud.com/project/data-service) 页面左侧点击你的 Data App 名称，查看详情。
...
1. 在 [**Data Service**](https://tidbcloud.com/project/data-service) 页面左侧点击你的端点名称，然后点击右上角 **...** > **Code Example**。会弹出 **Code Example** 对话框。
