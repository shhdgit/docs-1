...
Data Service（测试版）使你能够通过自定义 API 端点以 HTTPS 请求访问 TiDB Cloud 数据，并允许你无缝集成到任何兼容 HTTPS 的应用或服务中。
...
在创建 Data App 之前，请确保你已经创建了一个托管在 AWS 上的 [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md) 集群。如果还没有，请按照 [Create a TiDB Cloud Serverless or Essential Cluster](/tidb-cloud/create-tidb-cluster-serverless.md) 的步骤创建。

> **Note:**
>
> Data Service 仅适用于托管在 AWS 上的 [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md) 集群。如需在 TiDB Cloud Dedicated 集群中使用 Data Service，请联系 [TiDB Cloud Support](/tidb-cloud/tidb-cloud-support.md)。
...
4. 示例 Data App 自动创建完成后，你可以在左侧看到 App 名称和端点列表，中间看到某个端点的 SQL 语句，右侧则有关于如何使用示例 Data App 的说明。

5. 按照右侧的说明，选择一个端点并使用 curl 命令调用该端点。

## 使用你自己的 Data App 快速入门

你也可以创建自己的 Data App，然后按照以下步骤开发、测试、部署和调用端点，快速上手 Data Service。
...
2. 在项目的 [**Data Service**](https://tidbcloud.com/project/data-service) 页面左侧点击 <MDSvgIcon name="icon-create-data-app" /> **Create DataApp**。
...
6. 如果你已配置连接 GitHub，请检查你指定的 GitHub 目录。你会发现 [Data App 配置文件](/tidb-cloud/data-service-app-config-files.md) 已由 `tidb-cloud-data-service` 提交到该目录，说明 Data App 已成功连接到 GitHub。

    对于新建的 Data App，**Auto Sync & Deployment** 和 **Review Draft** 默认开启，便于你在 TiDB Cloud 控制台和 GitHub 之间同步 Data App 变更，并在部署前审查变更。关于 GitHub 集成的更多信息，参见 [Deploy your Data App changes with GitHub automatically](/tidb-cloud/data-service-manage-github-connection.md)。
...
- **Endpoint URL**：（只读）URL 会根据对应集群所在区域、Data App 的服务 URL 及端点路径自动生成。例如，若端点路径为 `/my_endpoint/get_id`，则端点 URL 为 `https://<region>.data.tidbcloud.com/api/v1beta/app/<App ID>/endpoint/my_endpoint/get_id`。
...
1. 选择集群。
    ...
    > **Note:**
    >
    > 只有已关联到 Data App 的集群会显示在下拉列表中。如何管理关联集群，参见 [Manage linked clusters](/tidb-cloud/data-service-manage-data-app.md#manage-linked-data-sources)。

    在 SQL 编辑器上方，从下拉列表选择你希望执行 SQL 语句的集群。然后你可以在右侧面板的 **Schema** 标签页查看该集群的所有数据库。
...
2. 编写 SQL 语句。
    ...
    如需定义参数，可以在 SQL 语句中以变量占位符的形式插入，例如 `${ID}`。例如，`SELECT * FROM table_name WHERE id = ${ID}`。然后你可以点击右侧面板的 **Params** 标签页修改参数定义和测试值。
...
3. 运行 SQL 语句。
    ...
    运行后，你可以在页面底部的 **Result** 标签页立即看到查询结果。
...
然后你可以在页面底部的 **HTTP Response** 标签页看到响应。关于响应的更多信息，参见 [Response of an endpoint](/tidb-cloud/data-service-manage-endpoint.md#response)。
...
1. 在 [**Data Service**](https://tidbcloud.com/project/data-service) 页左侧点击你的 Data App 名称，查看其详情。
2. 在 **Authentication** 区域点击 **Create API Key**。
3. 在 **Create API Key** 对话框中，完成以下操作：
...
1. 在 [**Data Service**](https://tidbcloud.com/project/data-service) 页左侧点击你的端点名称，然后点击右上角 **...** > **Code Example**。会弹出 **Code Example** 对话框。
...
2. 在对话框中，选择你希望用于调用端点的集群和数据库，然后复制代码示例。
    ...
    你必须先部署端点，才能查看线上环境下的代码示例。

    如需调用端点的当前线上版本，使用如下命令：
