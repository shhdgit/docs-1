...
在创建 Data App 之前，请确保你已经创建了一个托管在 AWS 上的 [TiDB Cloud Starter](/tidb-cloud/select-cluster-tier.md) 集群。如果还没有，请按照 [Create a TiDB Cloud Starter or Essential Cluster](/tidb-cloud/create-tidb-cluster-serverless.md) 的步骤创建。
...
创建一个示例 Data App 是快速上手 Data Service 的最佳方式。如果你的项目还没有任何 Data App，可以按照 **Data Service** 页面上的引导，创建一个示例 Data App，并用它来体验 Data Service 的功能。
...
5. 按照右侧的说明，选择一个端点，并使用 curl 命令调用该端点。
...
在右侧面板，点击 **Properties** 标签页，为端点设置属性，例如：
...
2. 编写 SQL 语句。
    ...
    如需定义参数，可以在 SQL 语句中以变量占位符的形式插入，例如 `${ID}`。例如，`SELECT * FROM table_name WHERE id = ${ID}`。然后，你可以点击右侧面板的 **Params** 标签页，修改参数定义和测试值。
...
3. 运行 SQL 语句。
    ...
    - 如果编辑器中有多条语句，要顺序运行其中一条或多条，将光标放在目标语句上，或用光标选中目标语句的行，然后按 **⌘ + Enter** 或点击 **Run**。
    ...
    - 如果编辑器中有多条语句，要顺序运行其中一条或多条，将光标放在目标语句上，或用光标选中目标语句的行，然后按 **Ctrl + Enter** 或点击 **Run**。
...
2. 点击 **Deploy** 确认部署。如果端点成功部署，你会看到 **Endpoint has been deployed** 的提示。
...
1. 在 [**Data Service**](https://tidbcloud.com/project/data-service) 页面左侧，点击你的 Data App 名称，查看其详情。
...
TiDB Cloud 会生成代码示例，帮助你调用端点。要获取代码示例，请执行以下操作：
