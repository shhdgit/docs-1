...
创建一个示例 Data App 是快速上手 Data Service 的最佳方式。如果你的项目还没有任何 Data App，可以按照 **Data Service** 页面上的引导创建一个示例 Data App，并用它体验 Data Service 的功能。
...
你也可以通过创建自己的 Data App，按照以下步骤开发、测试、部署和调用端点，快速上手 Data Service。
...
在右侧面板点击 **Properties** 标签页，设置端点属性，例如：
...
1. 选择集群。
    ...
    > **Note:**
    >
    > 只有已关联到 Data App 的集群会显示在下拉列表中。要管理已关联集群，参见 [Manage linked clusters](/tidb-cloud/data-service-manage-data-app.md#manage-linked-data-sources)。

    在 SQL 编辑器上方，从下拉列表选择你希望执行 SQL 语句的集群。然后，你可以在右侧 **Schema** 标签页查看该集群的所有数据库。

2. 编写 SQL 语句。

    在查询或修改数据前，需要先在 SQL 语句中指定数据库。例如，`USE database_name;`。

    在 SQL 编辑器中，你可以编写如表连接查询、复杂查询、聚合函数等语句。你也可以直接输入 `--` 加指令，让 AI 自动生成 SQL 语句。
    ...
    如需定义参数，可以在 SQL 语句中以变量占位符的形式插入，例如 `${ID}`。如：`SELECT * FROM table_name WHERE id = ${ID}`。然后，你可以点击右侧 **Params** 标签页修改参数定义和测试值。
...
3. 运行 SQL 语句。
    ...
    如果 SQL 语句中包含参数，请确保已在右侧 **Params** 标签页为参数设置了测试值或默认值，否则会返回错误。
    ...
    - 如果编辑器中有多条语句，要顺序运行其中一条或多条，将光标放在目标语句上或用光标选中目标语句行，然后按 **⌘ + Enter** 或点击 **Run**。

    - 要顺序运行编辑器中所有语句，按 **⇧ + ⌘ + Enter**，或用光标选中所有语句行后点击 **Run**。
    ...
    - 如果编辑器中有多条语句，要顺序运行其中一条或多条，将光标放在目标语句上或用光标选中目标语句行，然后按 **Ctrl + Enter** 或点击 **Run**。

    - 要顺序运行编辑器中所有语句，按 **Shift + Ctrl + Enter**，或用光标选中所有语句行后点击 **Run**。

    </div>
    </SimpleTab>

    运行后，你可以在页面底部 **Result** 标签页立即看到查询结果。
