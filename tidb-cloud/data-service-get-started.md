...
4. 示例 Data App 自动创建完成后，你可以在左侧看到 App 名称和端点列表，在中间看到某个端点的 SQL 语句，在右侧看到使用示例 Data App 的说明。

5. 按照右侧的说明，选择一个端点并使用 curl 命令调用该端点。

## 使用自定义 Data App 快速入门

你也可以通过创建自己的 Data App，并按照以下步骤开发、测试、部署和调用端点，来快速上手 Data Service。
...
4. （可选）如需将 Data App 的端点自动部署到你指定的 GitHub 仓库和分支，请启用 **Connect to GitHub**，然后执行以下操作：
    ...
    > - 仓库、分支和目录的组合唯一标识配置文件路径，在 Data App 之间必须唯一。如果你指定的路径已被其他 Data App 使用，需要重新指定路径。否则，当前 Data App 在 TiDB Cloud 控制台配置的端点会覆盖你指定路径下的文件。
...
2. 编写 SQL 语句。
    ...
    在 SQL 编辑器中，你可以编写如表连接查询、复杂查询、聚合函数等语句。你也可以直接输入 `--` 加上你的指令，让 AI 自动生成 SQL 语句。
...
1. 在 [**Data Service**](https://tidbcloud.com/project/data-service) 页左侧点击你的 Data App 名称，查看详情。
