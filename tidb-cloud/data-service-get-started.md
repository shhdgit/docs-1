...
你也可以通过创建自己的 Data App，然后按照以下步骤开发、测试、部署和调用端点，来快速上手 Data Service。
...
4. （可选）如需将 Data App 的端点自动部署到你指定的 GitHub 仓库和分支，请启用 **Connect to GitHub**，然后执行以下操作：
    ...
    > - 仓库、分支和目录的组合唯一标识配置文件路径，在所有 Data App 中必须唯一。如果你指定的路径已被其他 Data App 使用，需要重新指定路径。否则，在 TiDB Cloud 控制台配置的当前 Data App 端点会覆盖你指定路径下的文件。
