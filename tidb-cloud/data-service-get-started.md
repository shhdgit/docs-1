...
在创建 Data App 之前，请确保你已经创建了托管在 AWS 上的 [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md) 集群。如果还没有，请按照 [Create a TiDB Cloud Serverless or Essential Cluster](/tidb-cloud/create-tidb-cluster-serverless.md) 的步骤创建。
...
5. 按照右侧的说明选择一个端点，并使用 curl 命令调用该端点。
...
4. （可选）如需将 Data App 的端点自动部署到你指定的 GitHub 仓库和分支，请启用 **Connect to GitHub**，然后执行以下操作：
    ...
    > - 仓库、分支和目录的组合唯一标识配置文件路径，在所有 Data App 中必须唯一。如果你指定的路径已被其他 Data App 使用，需要重新指定路径，否则当前 Data App 在 TiDB Cloud 控制台配置的端点会覆盖你指定路径下的文件。
...
2. 编写 SQL 语句。
    ...
    > **注意：**
    >
    > 如需体验 TiDB Cloud 的 AI 能力，你需要允许 PingCAP 和 Amazon Bedrock 使用你的代码片段进行研究和服务改进。详细信息参见 [Enable or disable AI to generate SQL queries](/tidb-cloud/explore-data-with-chat2query.md#enable-or-disable-ai-to-generate-sql-queries)。
    ...
    - 详细信息参见 [Configure parameters](/tidb-cloud/data-service-manage-endpoint.md#configure-parameters)。
...
然后，你可以在页面底部的 **HTTP Response** 标签页看到响应。关于响应的详细信息，参见 [Response of an endpoint](/tidb-cloud/data-service-manage-endpoint.md#response)。
...
2. 点击 **Deploy** 确认部署。如果端点部署成功，你会看到 **Endpoint has been deployed** 的提示。
...
关于响应的详细信息，参见 [Response of an endpoint](/tidb-cloud/data-service-manage-endpoint.md#response)。
