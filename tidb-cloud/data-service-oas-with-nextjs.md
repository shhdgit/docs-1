...
- 一个 TiDB 集群。更多信息，参见 [创建 TiDB Cloud Serverless 或 Essential 集群](/tidb-cloud/create-tidb-cluster-serverless.md) 或 [创建 TiDB Cloud Dedicated 集群](/tidb-cloud/create-tidb-cluster.md)。
...
数据插入完成后，进入 [TiDB Cloud 控制台](https://tidbcloud.com)的 [**Data Service**](https://tidbcloud.com/project/data-service) 页面。创建一个关联到你的 TiDB 集群的 Data App，为该 Data App 创建一个 API key，然后在 Data App 中创建一个 `GET /repositories` 的 endpoint。该 endpoint 对应的 SQL 语句如下，用于查询 `test.repository` 表中的所有行：
...
1. 在 `hello-repos` 项目目录下，创建 `.env.local` 文件，并添加如下变量，然后将变量值设置为你的 Data App 的 public key 和 private key。
