...
数据插入完成后，前往 [TiDB Cloud 控制台](https://tidbcloud.com)的 [**Data Service**](https://tidbcloud.com/project/data-service) 页面。创建一个关联到你的 TiDB 集群的 Data App，为该 Data App 创建一个 API key，并在 Data App 中创建一个 `GET /repositories` 的接口。该接口对应的 SQL 语句如下，用于查询 `test.repository` 表中的所有行：
