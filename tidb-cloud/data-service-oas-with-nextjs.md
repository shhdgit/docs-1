...
在将 OpenAPI 规范与 Next.js 一起使用之前，请确保你具备以下条件：
...
数据插入完成后，进入 [TiDB Cloud 控制台](https://tidbcloud.com)的 [**Data Service**](https://tidbcloud.com/project/data-service) 页面。创建一个关联到你的 TiDB 集群的 Data App，为该 Data App 创建一个 API key，然后在 Data App 中创建一个 `GET /repositories` 的 endpoint。该 endpoint 对应的 SQL 语句如下，用于获取 `test.repository` 表中的所有行：
