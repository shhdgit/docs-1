...
3. 在 AWS 管理控制台中，为 TiDB Cloud 创建访问角色并获取 role ARN。
    ...
    1. 在 IAM 控制台 [https://console.aws.amazon.com/iam/](https://console.aws.amazon.com/iam/)，在左侧导航栏点击 **Roles**，然后点击 **Create role**。
...
1. 创建一个具有以下策略的 IAM 用户：
   ...
   更多信息请参见 [Creating an IAM user](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_users_create.html#id_users_create_console)。

2. 使用你的 AWS 账号 ID 或账号别名，以及 IAM 用户名和密码登录 [IAM 控制台](https://console.aws.amazon.com/iam)。

3. 创建访问密钥。详细步骤请参见 [Creating an access key for an IAM user](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html#Using_CreateAccessKey)。
...
要允许 TiDB Cloud 访问你 GCS 存储桶中的源数据，需要为该存储桶配置 GCS 访问权限。项目下任意一个 TiDB 集群完成配置后，该项目下所有 TiDB 集群都可以访问该 GCS 存储桶。
