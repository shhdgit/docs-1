...
按如下步骤为 TiDB Cloud 配置 bucket 访问权限并获取 Role ARN：
...
2. 在 AWS 管理控制台中，为你的 Amazon S3 bucket 创建托管策略。
    ...
    5. 复制以下访问策略模板，并粘贴到策略文本框中。
        ...
        - 如果你启用了 AWS Key Management Service key (SSE-KMS) 并使用了客户自管密钥加密，请确保策略中包含以下配置。`"arn:aws:kms:ap-northeast-1:105880447796:key/c3046e91-fdfc-4f3a-acff-00597dd3801f"` 是 bucket 的一个示例 KMS key。
            ...
            如果你的 bucket 中的对象是从另一个加密 bucket 拷贝过来的，KMS key 的值需要同时包含两个 bucket 的 key。例如，`"Resource": ["arn:aws:kms:ap-northeast-1:105880447796:key/c3046e91-fdfc-4f3a-acff-00597dd3801f","arn:aws:kms:ap-northeast-1:495580073302:key/0d7926a7-6ecc-4bf7-a9c1-a38f0faec0cd"]`。
...
3. 在 AWS 管理控制台中，为 TiDB Cloud 创建访问角色并获取 role ARN。
    ...
    4. 在 **Role details** 下为角色设置名称，然后点击右下角的 **Create role**。角色创建完成后，会显示角色列表。
...
1. 创建一个具有以下策略的 IAM 用户：
   ...
   详细信息参见 [创建 IAM 用户](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_users_create.html#id_users_create_console)。

2. 使用你的 AWS 账号 ID 或账号别名，以及 IAM 用户名和密码登录 [IAM 控制台](https://console.aws.amazon.com/iam)。

3. 创建访问密钥。详细步骤参见 [为 IAM 用户创建访问密钥](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html#Using_CreateAccessKey)。
...
要允许 TiDB Cloud 访问你 GCS bucket 中的源数据，需要为该 bucket 配置 GCS 访问权限。项目下任意一个 TiDB 集群完成配置后，该项目下所有 TiDB 集群都可以访问该 GCS bucket。
...
1. 在 [Azure Storage account](https://portal.azure.com/#browse/Microsoft.Storage%2FStorageAccounts) 页面，点击包含该容器的存储账户。
...
> **Note:**
>
> 在开始数据导入前，请测试连接和权限，确保 TiDB Cloud Dedicated 能访问指定的 Azure Blob 容器及文件。