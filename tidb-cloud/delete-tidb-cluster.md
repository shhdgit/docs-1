...
4. 在集群删除窗口中，确认删除操作：
    ...
    > **注意：**
    >
    > [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 和 [TiDB Cloud Essential](/tidb-cloud/select-cluster-tier.md#essential) 集群在删除后不支持数据恢复。如果你希望删除 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群并在未来恢复其数据，请参见 [Export Data from TiDB Cloud Serverless or Essential](/tidb-cloud/serverless-export.md) 将你的数据导出作为备份。
