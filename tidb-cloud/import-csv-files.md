...
- 方式二：在存放 CSV 文件的 Amazon S3、GCS 或 Azure Blob Storage 目录下，为源数据创建目标表结构文件，具体如下：
    ...
    2. 为源数据创建表结构文件。
        ...
        每个表结构文件必须采用 `${db_name}.${table_name}-schema.sql` 格式，并包含 `CREATE TABLE` DDL 语句。通过该文件，TiDB Cloud 会在导入数据时在 `${db_name}` 数据库中创建 `${db_table}` 表。
...
为了让 TiDB Cloud 能够访问 Amazon S3、GCS 或 Azure Blob Storage 中的 CSV 文件，请按以下方式之一操作：
