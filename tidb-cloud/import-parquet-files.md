...
3. 在 **Import Data from Azure Blob Storage** 页面，填写以下信息：
    ...
    - **SAS Token**：输入账户 SAS token，以允许 TiDB Cloud 访问 Azure Blob Storage 容器中的源文件。如果你还没有 SAS token，可以点击 **Click here to create a new one with Azure ARM template**，按照屏幕上的指引使用提供的 Azure ARM 模板创建。或者，你也可以手动创建账户 SAS token。更多信息请参见 [配置 Azure Blob Storage 访问权限](/tidb-cloud/dedicated-external-storage.md#configure-azure-blob-storage-access)。
