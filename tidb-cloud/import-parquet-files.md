...
- 为确保数据一致性，TiDB Cloud 仅允许将 Parquet 文件导入到空表中。若需将数据导入已存在数据的表，可以按照本文档将数据导入到一个临时空表，然后使用 `INSERT SELECT` 语句将数据复制到目标表。
...
3. 在 **Import Data from Amazon S3** 页面，填写以下信息：
    ...
    - **Bucket Access**：你可以使用 AWS IAM Role ARN 或 AWS 访问密钥访问存储桶。
        - **AWS Role ARN**（推荐）：输入 AWS IAM Role ARN。如果你还没有为存储桶创建 IAM 角色，可以点击 **Click here to create new one with AWS CloudFormation**，按照屏幕上的指引使用 AWS CloudFormation 模板创建。或者，你也可以手动为存储桶创建 IAM Role ARN。
