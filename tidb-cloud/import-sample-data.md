...
本文档介绍了如何通过 UI 将示例数据（SQL 文件）导入到 TiDB Cloud Dedicated。所用的示例数据为 Capital Bikeshare 的系统数据，根据 Capital Bikeshare Data License Agreement 发布。在导入示例数据之前，你需要拥有一个 TiDB 集群。
...
3. 在 **Import Data from GCS** 页面，配置以下源数据信息：
    ...
    - **Bucket Access**：你可以使用 GCS IAM Role 访问你的 bucket。更多信息请参见 [Configure GCS access](/tidb-cloud/dedicated-external-storage.md#configure-gcs-access)。
...
3. 在 **Import Data from Azure Blob Storage** 页面，配置以下源数据信息：
    ...
    - **SAS Token**：
        ...
        - 对于你自己的数据，可以使用 SAS token 访问你的 Azure Blob Storage。更多信息请参见 [Configure Azure Blob Storage access](/tidb-cloud/dedicated-external-storage.md#configure-azure-blob-storage-access)。
