...
[Airbyte](https://airbyte.com/) 是一个开源的数据集成引擎，用于构建提取、加载、转换（ELT）数据管道，并将你的数据整合到数据仓库、数据湖和数据库中。本文档介绍了如何将 Airbyte 作为源或目标连接到 TiDB Cloud。
...
2. 填写以下参数。
    ...
    你可以在集群的连接对话框中获取这些参数值。要打开该对话框，请前往项目的 [**Clusters**](https://tidbcloud.com/project/clusters) 页面，点击目标集群名称进入概览页面，然后点击右上角的 **Connect**。
...
4. 点击 **Set up source** 或 **destination** 完成连接器的创建。下图展示了将 TiDB 作为源的配置示例。

![TiDB source configuration](/media/tidb-cloud/integration-airbyte-parameters.jpg)

你可以使用任意源和目标的组合，例如 TiDB 到 Snowflake，或 CSV 文件到 TiDB。
...
在设置好源和目标后，你可以构建并配置连接。

以下步骤以 TiDB 同时作为源和目标为例。其他连接器可能有不同的参数。
...
6. 选择 **Sync mode** 为 **Full refresh | Overwrite**。
    ...
    > - 在全量刷新模式下，Airbyte 会在每次同步任务中读取源端的所有记录并复制到目标端。你可以为 Airbyte 中每个名为 **Namespace** 的表单独设置同步模式。
...
- 在默认标准化模式下，TiDB 目标会将 `timestamp` 类型转换为 `varchar` 类型。这是因为 Airbyte 在传输过程中会将 timestamp 类型转换为字符串，而 TiDB 不支持 `cast ('2020-07-28 14:50:15+1:00' as timestamp)`。
