...
2. 填写以下参数。
    ...
    你可以在集群的连接对话框中获取这些参数值。打开该对话框的方法是，进入项目的 [**Clusters**](https://tidbcloud.com/project/clusters) 页面，点击目标集群名称进入概览页，然后点击右上角的 **Connect**。
...
4. 点击 **Set up source** 或 **destination** 完成连接器的创建。下图展示了将 TiDB 作为数据源的配置界面。

![TiDB source configuration](/media/tidb-cloud/integration-airbyte-parameters.jpg)

你可以任意组合数据源和目标端，例如 TiDB 到 Snowflake，或 CSV 文件到 TiDB。
...
2. 选择之前建立的数据源和目标端。
3. 进入 **Set up** 连接面板，并为该连接创建一个名称，例如 `${source_name} - ${destination-name}`。
...
6. 选择 **Sync mode** 为 **Full refresh | Overwrite**。
    ...
    >
    > - 在增量模式下，Airbyte 只会读取自上次同步作业以来新增的记录。首次使用增量模式同步等同于全量刷新模式。
