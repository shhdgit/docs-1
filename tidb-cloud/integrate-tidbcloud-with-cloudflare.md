...
[Cloudflare Workers](https://workers.cloudflare.com/) 是一个平台，允许你在特定事件（如 HTTP 请求或数据库变更）发生时运行代码。Cloudflare Workers 易于使用，可用于构建各种应用，包括自定义 API、无服务器函数和微服务。它对于需要低延迟性能或需要快速扩展的应用尤其有用。
...
> **注意：**
>
> TiDB Cloud serverless driver 仅可用于 TiDB Cloud Serverless 和 TiDB Cloud Essential。
...
- 一个 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群。如果你还没有集群，请参阅 [创建 TiDB Cloud Serverless 或 Essential 集群](/tidb-cloud/create-tidb-cluster-serverless.md)。
