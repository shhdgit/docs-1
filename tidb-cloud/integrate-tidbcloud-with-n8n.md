...
- **Delete Key**：决定数据库中哪些行被删除的项属性名。item 是从一个节点发送到另一个节点的数据。节点会对每个输入数据项执行操作。关于 n8n 中的 item 更多信息请参见 [n8n 文档](https://docs.n8n.io/workflows/items/)。
...
- **Database**：数据库名称。
- **Table**：表名。你可以使用 `From list` 模式选择，或使用 `Name` 模式手动输入表名。
- **Columns**：以逗号分隔的输入项属性列表，这些属性将作为新行的列。item 是从一个节点发送到另一个节点的数据。节点会对每个输入数据项执行操作。关于 n8n 中的 item 更多信息请参见 [n8n 文档](https://docs.n8n.io/workflows/items/)。
...
- **Database**：数据库名称。
- **Table**：表名。你可以使用 `From list` 模式选择，或使用 `Name` 模式手动输入表名。
- **Update Key**：决定数据库中哪些行被更新的项属性名。item 是从一个节点发送到另一个节点的数据。节点会对每个输入数据项执行操作。关于 n8n 中的 item 更多信息请参见 [n8n 文档](https://docs.n8n.io/workflows/items/)。
