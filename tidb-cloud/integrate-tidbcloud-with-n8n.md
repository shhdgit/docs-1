...
summary: 了解在 n8n 中使用 TiDB Cloud 节点。
---
...
本文档介绍如何构建一个自动化工作流：创建 TiDB Cloud Starter 集群，收集 Hacker News RSS，将其存储到 TiDB，并发送简报邮件。
...
有两种方式可以安装自托管的 n8n。你可以选择任意一种适合你的方式。
...
然后你可以在 **Workflow** > 搜索栏中搜索 **TiDB Cloud** 节点，并将其拖拽到工作区使用。

## 步骤 3：构建你的工作流

在此步骤中，你将创建一个新的工作流，当你点击 **Execute** 按钮时会向 TiDB 插入一些数据。
...
如果你还没有 TiDB Cloud Starter 集群，可以使用该节点创建一个。否则可以跳过此操作。
...
6. 选择集群。如果你还没有在列表中看到新集群，需要等待几分钟直到集群创建完成。
7. 在 **User** 列表中选择一个用户。TiDB Cloud 总是会创建一个默认用户，因此你无需手动创建。
...
如果工作流按预期运行，你将收到 Hacker News 简报邮件。这些新闻内容会被记录到你的 TiDB Cloud Starter 集群中，因此你无需担心丢失。

现在你可以在 **Workflows** 面板中激活该工作流。此工作流将帮助你每天获取 Hacker News 的头条文章。

## TiDB Cloud 节点核心

### 支持的操作

TiDB Cloud 节点作为一个 [常规节点](https://docs.n8n.io/workflows/nodes/#regular-nodes)，仅支持以下五种操作：
...
- **Credential for TiDB Cloud API**：仅支持 TiDB Cloud API 密钥。如何创建 API 密钥请参考 [获取 TiDB Cloud API 密钥](#prerequisites-get-tidb-cloud-api-key)。
- **Project**：TiDB Cloud 项目名称。
- **Operation**：该节点的操作。所有支持的操作请参考 [支持的操作](#supported-operations)。
...
- **Delete Key**：决定数据库中哪些行被删除的条目属性名。item 是从一个节点发送到另一个节点的数据。节点会对每个输入数据项执行操作。关于 n8n 中的 item 更多信息，请参见 [n8n 文档](https://docs.n8n.io/workflows/items/)。
...
- **Database**：数据库名称。
- **Table**：表名。你可以使用 `From list` 模式选择一个表，或使用 `Name` 模式手动输入表名。
- **Columns**：以逗号分隔的输入数据项属性列表，这些属性将作为新行的列。item 是从一个节点发送到另一个节点的数据。节点会对每个输入数据项执行操作。关于 n8n 中的 item 更多信息，请参见 [n8n 文档](https://docs.n8n.io/workflows/items/)。
...
- **Database**：数据库名称。
- **Table**：表名。你可以使用 `From list` 模式选择一个表，或使用 `Name` 模式手动输入表名。
- **Update Key**：决定数据库中哪些行被更新的条目属性名。item 是从一个节点发送到另一个节点的数据。节点会对每个输入数据项执行操作。关于 n8n 中的 item 更多信息，请参见 [n8n 文档](https://docs.n8n.io/workflows/items/)。
- **Columns**：以逗号分隔的输入数据项属性列表，这些属性将作为要更新行的列。
...
- 对于 **Insert** 和 **Update** 操作，你需要在 **Columns** 字段中指定以逗号分隔的列表，且字段名必须与输入数据项的属性名一致。