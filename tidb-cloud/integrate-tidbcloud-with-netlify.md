...
1. 在 [TiDB Cloud 控制台](https://tidbcloud.com/) 中，进入你的项目的 [**Clusters**](https://tidbcloud.com/project/clusters) 页面，点击目标集群名称进入概览页，然后点击右上角的 **Connect**。在弹出的对话框中，你可以从连接字符串中获取以下连接参数。
...
2. 启动自动化设置。此步骤会将你的仓库连接到持续部署，因此 Netlify CLI 需要访问权限以在仓库中创建 deploy key 和 webhook。
...
3. 设置环境变量。为了让你自己的空间和 Netlify 空间都能连接到 TiDB Cloud 集群，你需要将 `DATABASE_URL` 设置为在 [第 1 步](#step-1-get-the-example-project-and-the-connection-string) 获取到的连接字符串。
...
然后你可以前往 Netlify 控制台查看部署状态。部署完成后，你可以通过 `https://<netlify-host>/api/hello` 这个 URL 访问 edge function。