...
你需要在 TiDB Cloud 上拥有一个账号和一个集群。如果还没有，请参考以下内容创建：
...
TiDB Cloud Serverless 和 TiDB Cloud Essential 集群默认允许所有 IP 地址连接，因此无需配置流量过滤器。
...
2. 按提示选择你的集群、客户端和操作系统。注意本文档使用的客户端是 `Prisma`。
...
2. 启动自动化设置。此步骤会连接你的仓库以实现持续部署，因此 Netlify CLI 需要访问权限以在仓库中创建 deploy key 和 webhook。
    ...
    在提示时，选择 **Create & configure a new site**，并授权 GitHub 访问。其他选项均使用默认值。
...
3. 设置环境变量。为了让你的本地空间和 Netlify 空间都能连接到 TiDB Cloud 集群，你需要将 `DATABASE_URL` 设置为在 [第 1 步](#step-1-get-the-example-project-and-the-connection-string) 获取到的连接字符串。
...
上文提到的示例应用运行在 Netlify 的无服务器函数上。本节将介绍如何将 edge function 与 [TiDB Cloud serverless driver](/tidb-cloud/serverless-driver.md) 一起使用。edge function 是 Netlify 提供的一个特性，允许你在 Netlify CDN 的边缘运行无服务器函数。

要使用 edge function，请按照以下步骤操作：

1. 在你的项目根目录下创建名为 `netlify/edge-functions` 的目录。
...
然后你可以前往 Netlify 控制台检查部署状态。部署完成后，你可以通过 `https://<netlify-host>/api/hello` URL 访问 edge function。