...
3. 设置环境变量。为了让你的本地空间和 Netlify 空间都能连接到 TiDB Cloud 集群，你需要将 `DATABASE_URL` 设置为在 [第 1 步](#step-1-get-the-example-project-and-the-connection-string) 获取的连接字符串。
...
6. 将应用部署到 Netlify。当你对本地预览满意后，可以使用以下命令将站点部署到 Netlify。`--trigger` 表示不上传本地文件进行部署。如果你有本地更改，请确保已将其提交到 GitHub 仓库。
