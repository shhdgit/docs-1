...
对于上述两种方式，TiDB Cloud 都为你的数据库提供了以下编程连接选项：
...
> **Note:**
>
> 此方法仅适用于 {} 和 {} 集群。如果你想连接 TiDB Cloud 专属集群，请使用[手动方法](#connect-via-manually-setting-environment-variables)。
...
6. 回到 Vercel 控制台，进入你的 Vercel 项目，点击 **Settings** > **Environment Variables**，检查目标 TiDB 集群的环境变量是否已自动添加。
...
6. 回到 Vercel 控制台，进入你的 Vercel 项目，点击 **Settings** > **Environment Variables**，检查目标 Data App 的环境变量是否已自动添加。
...
Vercel 的 [Preview Deployments](https://vercel.com/docs/deployments/preview-deployments) 功能允许你在不将更改合并到 Git 项目的生产分支的情况下，预览应用的实时部署变更。结合 [TiDB Cloud Branching](/tidb-cloud/branch-overview.md)，你可以为 Vercel 项目的每个分支创建一个新的实例，从而在不影响生产数据的情况下预览应用变更。
