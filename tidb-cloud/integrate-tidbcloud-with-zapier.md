...
由于我们不希望当你的 API 中某项数据在多次不同轮询中都存在时，多次触发动作，TiDB Cloud 触发器会用 `id` 字段进行数据去重。

`New Cluster` 和 `New Table` 触发器直接用 `cluster_id` 或 `table_id` 作为 `id` 字段进行去重。你无需为这两个触发器做任何操作。

**New Row Trigger**

`New Row` 触发器每次获取限制为 10000 条结果。因此，如果某些新行未包含在这 10000 条结果中，则无法触发 Zapier。
...
`New Row (Custom Query)` 触发器每次获取限制为 1,000,000 条结果。1,000,000 是一个很大的数字，仅用于保护整个系统。建议你的查询包含 `ORDER BY` 和 `LIMIT`。
