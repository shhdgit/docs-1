...
在 Zapier 上使用 [TiDB Cloud app](https://zapier.com/apps/tidb-cloud/integrations) 可以：
...
本节将以 **Add new Github global events to TiDB rows** 模板为例，创建一个工作流。在该工作流中，每当你的 GitHub 账户产生新的全局事件（任何 [GitHub event](https://docs.github.com/en/developers/webhooks-and-events/events/github-event-types) 发生在你或由你发起，针对任意仓库），Zapier 会向你的 TiDB Cloud 集群添加一行新数据。
...
- 一个 [TiDB Cloud 账户](https://tidbcloud.com/signup) 以及在 TiDB Cloud 上创建的 TiDB Cloud Starter 集群。更多详情请参见 [TiDB Cloud 快速入门](https://docs.pingcap.com/tidbcloud/tidb-cloud-quickstart#step-1-create-a-tidb-cluster)。
...
下表列出了 TiDB Cloud App 支持的动作。注意部分动作需要额外资源，使用前需准备相应资源。

| Action | Description | Resource |
|---|---|---|
| Find Cluster | 查找已存在的 TiDB Cloud Starter 或 TiDB Cloud Dedicated 集群。 | None |
| Create Cluster | 创建新集群。仅支持创建 TiDB Cloud Starter 集群。 | None |
| Find Database | 查找已存在的数据库。 | 一个 TiDB Cloud Starter 集群 |
| Create Database | 创建新数据库。 | 一个 TiDB Cloud Starter 集群 |
| Find Table | 查找已存在的表。 | 一个 TiDB Cloud Starter 集群和一个数据库 |
| Create Table | 创建新表。 | 一个 TiDB Cloud Starter 集群和一个数据库 |
| Create Row | 创建新行。 | 一个 TiDB Cloud Starter 集群、一个数据库和一个表 |
| Update Row | 更新已存在的行。 | 一个 TiDB Cloud Starter 集群、一个数据库和一个表 |
| Find Row | 通过查找列在表中查找行。 | 一个 TiDB Cloud Starter 集群、一个数据库和一个表 |
| Find Row (Custom Query) | 通过你提供的自定义查询在表中查找行。 | 一个 TiDB Cloud Starter 集群、一个数据库和一个表 |
...
一种避免方法是在触发器中指定 `Order By` 配置。例如，一旦你按创建时间排序，新行总会包含在 10000 条结果中。
...
为实现去重，你的查询结果必须有唯一的 id 字段。否则会收到 `You must return the results with id field` 错误。
