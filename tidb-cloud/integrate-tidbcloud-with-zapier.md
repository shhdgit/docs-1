...
在本节中，我们将以 **Add new Github global events to TiDB rows** 模板为例，创建一个工作流。在该工作流中，每当你的 GitHub 账户产生新的全局事件（任何 [GitHub 事件](https://docs.github.com/en/developers/webhooks-and-events/events/github-event-types)，无论是你发起还是针对你的，发生在任意仓库），Zapier 都会向你的 TiDB Cloud 集群添加一行新数据。
...
由于我们不希望当你的 API 中某项数据在多次轮询中都存在时多次触发动作，TiDB Cloud 触发器会用 `id` 字段进行去重。
...
为实现去重，你的查询结果必须有唯一的 id 字段。否则你会收到 `You must return the results with id field` 错误。
