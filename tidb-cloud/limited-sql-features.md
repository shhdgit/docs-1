...
| 语句 | TiDB Cloud 专属集群 | TiDB Cloud Serverless 与 TiDB Cloud Essential |
|:-|:-|:-|
...
| `ADMIN RESET TELEMETRY_ID` | 支持 | TiDB Cloud Serverless 与 TiDB Cloud Essential 不支持 Telemetry。 |
| `ADMIN SHOW TELEMETRY` | 不支持 [^4] | 不支持 [^4] |
| `ADMIN SHOW SLOW` | 支持 | 不支持 [^5] |
| `ADMIN PLUGINS ENABLE` | 支持 | 不支持 [^8] |
| `ADMIN PLUGINS DISABLE` | 支持 | 不支持 [^8] |
| `ALTER INSTANCE RELOAD TLS` | 支持 | TiDB Cloud Serverless 与 TiDB Cloud Essential 会自动刷新 TLS 证书。 |
| `LOAD DATA INFILE` | 支持 `LOAD DATA LOCAL INFILE`，以及从 Amazon S3 或 Google Cloud Storage 加载的 `LOAD DATA INFILE` | 仅支持 `LOAD DATA LOCAL INFILE` |
...
| 函数与运算符 | TiDB Cloud 专属集群 | TiDB Cloud Serverless 与 TiDB Cloud Essential |
|:-|:-|:-|
| `SLEEP` | 无限制 | [`SLEEP()` 函数](https://docs.pingcap.com/tidbcloud/miscellaneous-functions)有最大 300 秒的休眠时间限制。|

## 系统表

| 数据库 | 表 | TiDB Cloud 专属集群 | TiDB Cloud Serverless 与 TiDB Cloud Essential |
|:-|:-|:-|:-|
...
| 变量 | TiDB Cloud 专属集群 | TiDB Cloud Serverless 与 TiDB Cloud Essential |
|:-|:-|:-|
| `datadir` | 无限制 | 不支持 [^1] |
...
[^1]: TiDB Cloud Serverless 与 TiDB Cloud Essential 不支持数据副本放置相关配置。

[^2]: TiDB Cloud Serverless 与 TiDB Cloud Essential 不支持资源组相关配置。

[^3]: 在 TiDB Cloud Serverless 或 TiDB Cloud Essential 上进行 [备份与恢复](/tidb-cloud/backup-and-restore-serverless.md) 操作时，你可以使用 TiDB Cloud 控制台。

[^4]: 该功能在 [安全增强模式（SEM）](/system-variables.md#tidb_enable_enhanced_security) 下不可用。

[^5]: 在 TiDB Cloud Serverless 或 TiDB Cloud Essential 上追踪 [慢查询](/tidb-cloud/tune-performance.md#slow-query) 时，你可以使用 TiDB Cloud 控制台。

[^6]: 在 TiDB Cloud Serverless 或 TiDB Cloud Essential 上进行 [SQL 语句分析](/tidb-cloud/tune-performance.md#statement-analysis) 时，你可以使用 TiDB Cloud 控制台。

[^7]: TiDB Cloud 不支持 Drainer 和 Pump。

[^8]: TiDB Cloud Serverless 与 TiDB Cloud Essential 不支持插件功能。

[^9]: TiDB Cloud Serverless 与 TiDB Cloud Essential 强制执行强密码策略。

[^10]: 该变量在 TiDB Cloud Serverless 与 TiDB Cloud Essential 上为只读。

[^11]: TiDB Cloud Serverless 与 TiDB Cloud Essential 不支持通过 `${tidb-server-status-port}` 下载 `PLAN REPLAYER` 导出的文件，如[示例](https://docs.pingcap.com/tidb/stable/sql-plan-replayer#examples-of-exporting-cluster-information)所示。取而代之的是，TiDB Cloud Serverless 与 TiDB Cloud Essential 会为你生成一个 [预签名 URL](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ShareObjectPreSignedURL.html) 用于下载文件。请注意，该 URL 在生成后 10 小时内有效。
