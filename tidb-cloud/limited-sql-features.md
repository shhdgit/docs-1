...
[^3]: 在 TiDB Cloud Serverless 或 TiDB Cloud Essential 上进行 [备份与恢复](/tidb-cloud/backup-and-restore-serverless.md) 操作时，可以使用 TiDB Cloud 控制台。

[^4]: 该功能在 [安全增强模式 (SEM)](/system-variables.md#tidb_enable_enhanced_security) 下不可用。

[^5]: 在 TiDB Cloud Serverless 或 TiDB Cloud Essential 上追踪 [慢查询](/tidb-cloud/tune-performance.md#slow-query) 时，可以使用 TiDB Cloud 控制台。

[^6]: 在 TiDB Cloud Serverless 或 TiDB Cloud Essential 上进行 [SQL 语句分析](/tidb-cloud/tune-performance.md#statement-analysis) 时，可以使用 TiDB Cloud 控制台。
