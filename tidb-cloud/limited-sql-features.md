...
| `ADMIN RESET TELEMETRY_ID` | 支持 | TiDB Cloud Starter 与 TiDB Cloud Essential 不支持遥测。 |
| `ADMIN SHOW TELEMETRY` | 不支持 [^4] | 不支持 [^4] |
| `ADMIN SHOW SLOW` | 支持 | 不支持 [^5] |
| `ADMIN PLUGINS ENABLE` | 支持 | 不支持 [^8] |
| `ADMIN PLUGINS DISABLE` | 支持 | 不支持 [^8] |
| `ALTER INSTANCE RELOAD TLS` | 支持 | TiDB Cloud Starter 与 TiDB Cloud Essential 会自动刷新 TLS 证书。 |
...
| 函数与操作符 | TiDB Cloud 专属版 | TiDB Cloud Starter 与 TiDB Cloud Essential |
|:-|:-|:-|
| `SLEEP` | 无限制 | [`SLEEP()` 函数](https://docs.pingcap.com/tidbcloud/miscellaneous-functions)有最大 300 秒的休眠时间限制。|

## 系统表

| 数据库 | 表 | TiDB Cloud 专属版 | TiDB Cloud Starter 与 TiDB Cloud Essential |
|:-|:-|:-|:-|
...
| 变量 | TiDB Cloud 专属版 | TiDB Cloud Starter 与 TiDB Cloud Essential |
|:-|:-|:-|
| `datadir` | 无限制 | 不支持 [^1] |
