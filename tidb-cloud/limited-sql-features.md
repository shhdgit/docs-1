...
| `ADMIN RESET TELEMETRY_ID` | 支持 | TiDB Cloud Serverless 与 TiDB Cloud Essential 不支持遥测。 |
...
| 函数与操作符 | TiDB Cloud 专属集群 | TiDB Cloud Serverless 与 TiDB Cloud Essential |
|:-|:-|:-|
...
[^3]: 在 TiDB Cloud Serverless 或 TiDB Cloud Essential 上进行 [备份与恢复](/tidb-cloud/backup-and-restore-serverless.md) 操作时，你可以使用 TiDB Cloud 控制台来实现。

[^4]: 该功能在 [安全增强模式 (SEM)](/system-variables.md#tidb_enable_enhanced_security) 下不可用。

[^5]: 在 TiDB Cloud Serverless 或 TiDB Cloud Essential 上追踪 [慢查询](/tidb-cloud/tune-performance.md#slow-query) 时，你可以使用 TiDB Cloud 控制台来实现。

[^6]: 在 TiDB Cloud Serverless 或 TiDB Cloud Essential 上进行 [SQL 语句分析](/tidb-cloud/tune-performance.md#statement-analysis) 时，你可以使用 TiDB Cloud 控制台来实现。
