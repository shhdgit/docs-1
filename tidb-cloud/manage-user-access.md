...
本文档介绍了如何在 TiDB Cloud 中管理对组织、项目、角色和用户资料的访问。

在访问 TiDB Cloud 之前，请先[创建一个 TiDB Cloud 账号](https://tidbcloud.com/free-trial)。你可以使用邮箱和密码注册，这样你可以[通过 TiDB Cloud 管理你的密码](/tidb-cloud/tidb-cloud-password-authentication.md)，也可以选择使用 Google、GitHub 或 Microsoft 账号进行单点登录（SSO）访问 TiDB Cloud。
...
- 要访问某个组织，用户必须是该组织的成员。
- 要访问组织中的某个项目，用户至少需要拥有该组织下该项目的只读访问权限。
...
TiDB Cloud 在组织层面进行计费，并为每个项目提供详细的账单信息。

如果你是组织所有者，你在组织中拥有最高权限。

例如，你可以执行以下操作：

- 为不同目的（如开发、测试、生产）创建不同的项目。
- 为不同用户分配不同的组织角色和项目角色。
- 配置组织设置。例如，为你的组织配置时区。
...
> **Note:**
>
> - `Organization Console Audit Manager` 角色（由 `Organization Console Audit Admin` 重命名）用于管理 TiDB Cloud 控制台的审计日志，而不是数据库审计日志。要管理数据库审计，请在项目层面使用 `Project Owner` 角色。
...
> - 每个项目角色默认都拥有 <code>Organization Viewer</code> 的所有权限。
...
| 管理项目中 TiDB Cloud Serverless 和 TiDB Cloud Essential 集群的分支，如分支创建、连接和删除。 | ✅ | ❌ | ❌ | ❌ |
| 管理项目中 TiDB Cloud Dedicated 集群的 [恢复组](/tidb-cloud/recovery-group-overview.md)，如恢复组的创建和删除。 | ✅ | ❌ | ❌ | ❌ |
...
1. 在 [TiDB Cloud 控制台](https://tidbcloud.com)中，点击左上角的下拉框，会显示你所属的组织和项目列表。
    ...
    > **Tip:**
    >
    > - 如果你当前在某个集群页面，点击左上角下拉框后，还需要点击下拉框中的 ← 返回到组织和项目列表。
    > - 如果你属于多个组织，可以在下拉框中点击目标组织名称，在组织之间切换账号。
...
5. 输入要邀请用户的邮箱地址，然后为该用户选择一个组织角色。
    ...
    > - 被邀请的用户默认不属于任何项目。要邀请用户加入项目，请参见 [邀请项目成员](#invite-a-project-member)。

6. 点击 **Confirm**。新用户会被成功添加到用户列表中，同时会向被邀请邮箱发送一封带有验证链接的邮件。

7. 用户收到邮件后，需要点击邮件中的链接进行身份验证，随后会显示一个新页面。

8. 如果被邀请的邮箱地址尚未注册 TiDB Cloud 账号，用户会被引导至注册页面创建账号；如果邮箱地址已注册 TiDB Cloud 账号，用户会被引导至登录页面，登录后账号会自动加入该组织。

> **Note:**
>
> 邮件中的验证链接 24 小时内有效。如果你邀请的用户未收到邮件，可以点击 **Resend** 重新发送。

### 修改组织角色

如果你拥有 `Organization Owner` 角色，可以修改组织内所有成员的组织角色。

要修改成员的组织角色，请按照以下步骤操作：
...
如果你拥有 `Organization Owner` 角色，可以在你的组织中创建项目。
...
8. 如果被邀请的邮箱地址尚未注册 TiDB Cloud 账号，用户会被引导至注册页面创建账号；如果邮箱地址已注册 TiDB Cloud 账号，用户会被引导至登录页面。登录后账号会自动加入该项目。

> **Note:**
>
> 邮件中的验证链接 24 小时内有效。如果你的用户未收到邮件，可以点击 **Resend** 重新发送。

### 修改项目角色

如果你拥有 `Organization Owner` 角色，可以修改你组织下所有项目成员的项目角色。如果你拥有 `Project Owner` 角色，可以修改你项目下所有成员的项目角色。

要修改成员的项目角色，请按照以下步骤操作：
