...
在访问 TiDB Cloud 之前，请先[创建一个 TiDB Cloud 账号](https://tidbcloud.com/free-trial)。你可以选择使用邮箱和密码注册，这样可以[通过 TiDB Cloud 管理你的密码](/tidb-cloud/tidb-cloud-password-authentication.md)，也可以选择使用 Google、GitHub 或 Microsoft 账号进行单点登录（SSO）到 TiDB Cloud。
...
8. 如果被邀请邮箱尚未注册 TiDB Cloud 账号，用户会被引导至注册页面创建账号；如果邮箱已注册 TiDB Cloud 账号，用户会被引导至登录页面，登录后账号会自动加入该组织。

> **Note:**
>
> 邮件中的验证链接 24 小时内有效。如果你要邀请的用户未收到邮件，可以点击 **Resend** 重新发送。
...
8. 如果被邀请邮箱尚未注册 TiDB Cloud 账号，用户会被引导至注册页面创建账号；如果邮箱已注册 TiDB Cloud 账号，用户会被引导至登录页面，登录后账号会自动加入该项目。
...
4. 在目标成员所在行，点击 **Role** 列中的角色，然后在下拉列表中选择新角色。
