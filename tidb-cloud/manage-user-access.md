...
在访问 TiDB Cloud 之前，请先[创建一个 TiDB Cloud 账号](https://tidbcloud.com/free-trial)。你可以使用邮箱和密码注册账号，这样你可以[通过 TiDB Cloud 管理你的密码](/tidb-cloud/tidb-cloud-password-authentication.md)，也可以选择使用 Google、GitHub 或 Microsoft 账号通过单点登录（SSO）访问 TiDB Cloud。
...
| 管理项目中所有 {{{ .starter }}} 集群的 [消费限额](/tidb-cloud/manage-serverless-spend-limit.md)。 | ✅ | ❌ | ❌ | ❌ |
| 管理项目中的集群操作，如集群创建、修改和删除。 | ✅ | ❌ | ❌ | ❌ |
| 管理项目中 {{{ .starter }}} 和 {{{ .essential }}} 集群的分支，如分支创建、连接和删除。 | ✅ | ❌ | ❌ | ❌ |
| 管理项目中 TiDB Cloud 专属集群的 [恢复组](/tidb-cloud/recovery-group-overview.md)，如恢复组的创建和删除。 | ✅ | ❌ | ❌ | ❌ |
...
8. 如果被邀请邮箱尚未注册 TiDB Cloud 账号，用户会跳转到注册页面创建账号；如果邮箱已注册 TiDB Cloud 账号，用户会跳转到登录页面，登录后账号会自动加入该组织。
...
8. 如果被邀请邮箱尚未注册 TiDB Cloud 账号，用户会跳转到注册页面创建账号；如果邮箱已注册 TiDB Cloud 账号，用户会跳转到登录页面。登录后账号会自动加入该项目。
...
在 TiDB Cloud 中，你可以轻松管理你的个人资料，包括你的名字、姓氏和手机号。
