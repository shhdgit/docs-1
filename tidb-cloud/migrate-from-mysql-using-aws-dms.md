...
- TiDB 默认包含以下系统数据库：`INFORMATION_SCHEMA`、`PERFORMANCE_SCHEMA`、`mysql`、`sys` 和 `test`。在创建 AWS DMS 迁移任务时，需要过滤掉这些系统数据库，而不是使用默认的 `%` 选择迁移对象。否则，AWS DMS 会尝试将这些系统数据库从源数据库迁移到目标 TiDB，导致任务失败。为避免此问题，建议填写具体的数据库和表名。
...
- 建议使用 AWS DMS `dms.t3.large`（2 vCPUs 和 8 GiB 内存）或更高的实例规格。小规格实例可能会导致内存溢出（OOM）错误。
...
5. 在 **Allocated storage (GiB)** 字段配置存储，使用默认配置即可。
