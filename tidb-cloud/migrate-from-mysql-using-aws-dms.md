...
- 建议使用 AWS DMS `dms.t3.large`（2 vCPUs 和 8 GiB 内存）或更高规格的实例。小规格实例可能会导致 OOM（内存溢出）错误。
...
2. 配置 Amazon RDS 的安全组规则。本例中，将 AWS DMS 实例的公网和私网 IP 地址添加到安全组中。
...
4. 本例中，点击 **Select RDS DB instance**，然后选择源 RDS 实例。如果源数据库为自建 MySQL，可以跳过此步骤，在后续步骤中填写相关信息。
...
5. 配置以下信息：
   ...
   - **Server name**：填写数据提供方的数据服务器名称。可从数据库控制台复制。如果上游为 Amazon RDS 或 Amazon Aurora，名称会自动填充。如果是无域名的自建 MySQL，可填写 IP 地址。
...
2. 配置以下信息：
    ...
    - **Migration type**：根据需要选择迁移类型。本例选择 **Migrate existing data and replicate ongoing changes**。
...
3. 配置以下信息：
    ...
    - **Target table preparation mode**：选择 **Do nothing** 或其他选项。本例选择 **Do nothing**。
...
- 如果你想了解如何将 AWS DMS 连接到 TiDB Cloud Serverless 或 TiDB Cloud Dedicated，请参见 [Connect AWS DMS to TiDB Cloud clusters](/tidb-cloud/tidb-cloud-connect-aws-dms.md)。
