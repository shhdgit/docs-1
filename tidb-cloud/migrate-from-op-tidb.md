...
本文档介绍如何通过 Dumpling 和 TiCDC，将数据从 TiDB 自建集群迁移到 TiDB Cloud（基于 AWS）。
...
- 一个 [TiDB Cloud 账号](/tidb-cloud/tidb-cloud-quickstart.md)，并对目标托管在 AWS 上的 TiDB Cloud 集群至少拥有 [`Project Data Access Read-Write`](/tidb-cloud/manage-user-access.md#user-roles) 权限
...
2. 将其解压到目标机器。你也可以通过运行 `tiup install dumpling` 使用 TiUP 获取 Dumpling。之后，可以通过 `tiup dumpling ...` 命令运行 Dumpling。更多信息参见 [Dumpling 介绍](https://docs.pingcap.com/tidb/stable/dumpling-overview#dumpling-introduction)。
...
4. 创建并运行增量同步任务。在上游集群执行如下命令：
    ...
    - `--sink-uri`：同步任务的下游地址。`--sink-uri` 格式如下。目前支持的 scheme 有 `mysql`、`tidb`、`kafka`、`s3` 和 `local`。
...
9. 备份上游集群的用户及权限信息，并在下游集群恢复。你可以使用以下脚本备份用户及权限信息。注意需将占位符替换为实际值。
    ...
    获取用户及权限信息后，在下游集群执行生成的 SQL 语句以恢复用户及权限信息。