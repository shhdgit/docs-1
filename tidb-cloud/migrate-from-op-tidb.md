---
title: 从 TiDB 自建版迁移到 TiDB Cloud
summary: 了解如何将数据从 TiDB 自建版迁移到 TiDB Cloud。
---

# 从 TiDB 自建版迁移到 TiDB Cloud

本文档介绍如何通过 Dumpling 和 TiCDC，将数据从你的 TiDB 自建集群迁移到 TiDB Cloud（AWS）。

整体流程如下：

1. 搭建环境并准备工具。
2. 迁移全量数据。流程如下：
   1. 使用 Dumpling 将数据从 TiDB 自建版导出到 Amazon S3。
...
- [一个 TiDB Cloud 账号](/tidb-cloud/tidb-cloud-quickstart.md)，并对目标 AWS 上托管的 TiDB Cloud 集群至少拥有 [`Project Data Access Read-Write`](/tidb-cloud/manage-user-access.md#user-roles) 权限
...
[Dumpling](https://docs.pingcap.com/tidb/dev/dumpling-overview) 是一个可以将 TiDB 或 MySQL 数据导出为 SQL 或 CSV 文件的工具。你可以使用 Dumpling 从 TiDB 自建版导出全量数据。
...
为确保增量迁移期间新写入的数据不会丢失，在开始迁移前，需要关闭上游集群的垃圾回收（GC）机制，防止系统清理历史数据。
...
2. 从 AWS 控制台获取 S3 bucket 的 URI 和 region 信息。详见 [创建 bucket](https://docs.aws.amazon.com/AmazonS3/latest/userguide/create-bucket-overview.html)。
...
4. 检查导出数据。通常导出的数据包括以下内容：
    ...
    - `metadata`：包含导出起始时间和主 binary log 位置
...
1. 在 [TiDB Cloud 控制台](https://tidbcloud.com/)中，根据以下文档获取目标集群的 Account ID 和 External ID：
   ...
   - 对于 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群，参见 [通过 Role ARN 配置 Amazon S3 访问权限](/tidb-cloud/serverless-external-storage.md#configure-amazon-s3-access-using-a-role-arn)。
...
2. 配置 Amazon S3 访问权限。通常需要以下只读权限：
    ...
    如果 S3 bucket 使用了服务端加密 SSE-KMS，还需添加 KMS 权限：

    - kms:Decrypt

3. 配置访问策略。进入 [AWS 控制台 > IAM > Access Management > Policies](https://console.aws.amazon.com/iamv2/home#/policies)，切换到你的区域，检查是否已存在 TiDB Cloud 的访问策略。如果不存在，按照 [在 JSON 选项卡上创建策略](https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies_create-console.html) 创建策略。
    ...
    ##<Your AWS KMS ARN>: 填写 S3 bucket KMS 密钥的 ARN。可在 S3 bucket > Properties > Default encryption > AWS KMS Key ARN 获取。详见 https://docs.aws.amazon.com/AmazonS3/latest/userguide/viewing-bucket-key-settings.html
...
5. 获取 Role-ARN。进入 [AWS 控制台 > IAM > Access Management > Roles](https://console.aws.amazon.com/iamv2/home#/roles)，切换到你的区域，点击你创建的角色，记录 ARN。导入数据到 TiDB Cloud 时会用到。

6. 导入数据到 TiDB Cloud。

    - 对于 TiDB Cloud Dedicated 集群，参见 [从云存储导入 CSV 文件到 TiDB Cloud Dedicated](/tidb-cloud/import-csv-files.md)。
    - 对于 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群，参见 [从云存储导入 CSV 文件到 TiDB Cloud Serverless 或 Essential](/tidb-cloud/import-csv-files-serverless.md)。
...
2. 授权 TiCDC 连接 TiDB Cloud。
    ...
    1. 在 [TiDB Cloud 控制台](https://tidbcloud.com/project/clusters)中，进入 [**Clusters**](https://tidbcloud.com/project/clusters) 页面，点击目标集群名称进入概览页。
    2. 在左侧导航栏点击 **Settings** > **Networking**。
    3. 在 **Networking** 页面点击 **Add IP Address**。
    4. 在弹窗中选择 **Use IP addresses**，点击 **+**，在 **IP Address** 字段填写 TiCDC 组件的公网 IP 地址，然后点击 **Confirm**。此时 TiCDC 可访问 TiDB Cloud。更多信息参见 [配置 IP 访问列表](/tidb-cloud/configure-ip-access-list.md)。
...
4. 创建并运行增量同步任务。在上游集群运行如下命令：
    ...
    - `--changefeed-id`：同步任务的 ID，格式需符合 ^[a-zA-Z0-9]+(\-[a-zA-Z0-9]+)*$ 正则表达式。如果未指定，TiCDC 会自动生成一个 UUID（version 4 格式）作为 ID。
...
5. 在上游集群重新开启 GC 机制。如果增量同步无报错或延迟，可开启 GC 机制，恢复集群的垃圾回收。
...
6. 校验增量同步任务。
    ...
    - 校验同步效果。向上游集群写入新记录，然后检查该记录是否同步到下游 TiDB Cloud 集群。

7. 设置上下游集群相同的时区。TiDB Cloud 默认时区为 UTC。如果上下游集群时区不同，需要将两者设置为相同的时区。
...
8. 备份上游集群的 [查询绑定](/sql-plan-management.md)，并在下游集群恢复。你可以使用以下 SQL 备份查询绑定：
    ...
    如果没有输出，说明上游集群未使用查询绑定，可跳过此步骤。

    获取查询绑定后，在下游集群执行这些 SQL 以恢复查询绑定。
...
9. 备份上游集群的用户和权限信息，并在下游集群恢复。你可以使用以下脚本备份用户和权限信息。注意需将占位符替换为实际值。
    ...
    获取用户和权限信息后，在下游集群执行生成的 SQL 语句以恢复用户和权限信息。