...
4. 创建并运行增量同步任务。在上游集群执行如下命令：
    ...
    更多信息参见 [TiCDC Changefeed 的 CLI 和配置参数](https://docs.pingcap.com/tidb/dev/ticdc-changefeed-config)。
...
7. 设置上下游集群的时区一致。TiDB Cloud 默认时区为 UTC。如果上下游集群时区不同，需要将两者设置为相同的时区。
