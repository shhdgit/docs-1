...
在 AWS 控制台创建访问密钥。详细步骤参见 [创建访问密钥](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html#Using_CreateAccessKey)。
...
3. 点击 **Create access key** 创建访问密钥。然后选择 **Download .csv file**，将访问密钥 ID 和密钥保存为 CSV 文件到本地。请妥善保存该文件，关闭对话框后将无法再次获取密钥。下载 CSV 文件后，点击 **Close**。创建访问密钥后，密钥对默认处于激活状态，可以立即使用。
...
2. 从 AWS 控制台获取 S3 bucket 的 URI 和区域信息。详细步骤参见 [创建 bucket](https://docs.aws.amazon.com/AmazonS3/latest/userguide/create-bucket-overview.html)。

    下图展示了如何获取 S3 bucket 的 URI 信息：
