...
如果存在跨多个实例的分片，可以为每个数据库创建一个一级目录，为每个分片表创建一个二级目录，然后为每个 MySQL 实例创建一个三级目录以便管理。例如，如果你希望将 MySQL instance1 和 MySQL instance2 的 `stock_N.product_N` 表迁移并合并到 TiDB Cloud 的 `stock.products` 表，可以创建如下目录：
...
5. 在 **Destination** 部分，选择目标数据库和表。
    ...
    导入多个文件时，可以通过 **Advanced Settings** > **Mapping Settings** 为每个目标表及其对应的 CSV 文件定义自定义映射规则。之后，数据源文件将根据你提供的自定义映射规则重新扫描。
...
4. 编辑任务配置文件 `test-task1`，为每个数据源配置增量同步模式及同步起始点。
