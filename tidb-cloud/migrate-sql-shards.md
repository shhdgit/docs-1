---
title: 将大型数据集的 MySQL 分片迁移并合并到 TiDB Cloud
summary: 了解如何将大型数据集的 MySQL 分片迁移并合并到 TiDB Cloud。
---

# 将大型数据集的 MySQL 分片迁移并合并到 TiDB Cloud
...
2. 将 MySQL instance1 的数据导出到 Amazon S3 存储桶的 `s3://dumpling-s3/store/sales/instance01/` 目录。
...
3. 将 MySQL instance2 的数据导出到 Amazon S3 存储桶的 `s3://dumpling-s3/store/sales/instance02/` 目录。
...
3. 在 **Import Data from Amazon S3** 页面，填写以下信息：
    ...
    TiDB Cloud 会开始验证是否能访问你指定的存储桶 URI。验证通过后，TiDB Cloud 会尝试使用默认文件命名模式扫描数据源中的所有文件，并在下一页左侧返回扫描摘要结果。如果遇到 `AccessDenied` 错误，参见 [排查 S3 数据导入时的 Access Denied 错误](/tidb-cloud/troubleshoot-import-access-denied-error.md)。
...
要将上游集群指定位置的 binlog 数据变更同步到 TiDB Cloud，可以使用 TiDB Data Migration (DM) 进行增量同步。
...
1. 新建数据源文件 `dm-source1.yaml`，配置一个上游数据源到 DM，内容如下：
