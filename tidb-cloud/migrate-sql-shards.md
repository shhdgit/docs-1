...
3. 在 **Import Data from Amazon S3** 页面，填写以下信息：
    ...
    TiDB Cloud 会开始验证是否能访问指定的 bucket URI。验证通过后，TiDB Cloud 会尝试使用默认文件命名模式扫描数据源中的所有文件，并在下一页左侧返回扫描摘要结果。如果遇到 `AccessDenied` 错误，参见 [Troubleshoot Access Denied Errors during Data Import from S3](/tidb-cloud/troubleshoot-import-access-denied-error.md)。
...
6. 如有需要，编辑 CSV 配置。
    ...
    > **Note:**
    >
    > 分隔符、定界符和 null 的配置既可以使用字母数字，也可以使用部分特殊字符。支持的特殊字符包括 `\t`、`\b`、`\n`、`\r`、`\f` 和 `\u0001`。
