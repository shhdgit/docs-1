...
2. 根据你的 [Datadog 集成版本](#datadog-集成版本)，执行以下操作之一：
   ...
   >
   > - 在 Datadog 合并待处理的 [PR](https://github.com/DataDog/integrations-extras/pull/2751) 之前，**Dashboard** 链接会跳转到旧版仪表盘，该仪表盘不包含预览版中引入的最新指标。
...
| tidb_cloud.db_average_query_duration| gauge | sql_type: Select\|Insert\|...<br/>cluster_name: `<cluster name>`<br/>instance: tidb-0\|tidb-1…<br/>component: `tidb` | 客户端网络请求发送到 TiDB 的时间与 TiDB 执行后返回给客户端的时间之间的持续时间。 |
...
对于 Datadog integration (Preview)，还可用以下额外指标：
...
| tidbcloud.kv_request_duration | histogram | instance: `tidb-0\|tidb-1\|...`<br/>component: `tikv`<br/>cluster_name: `<cluster name>`<br/>`type`: `BatchGet\|Commit\|Prewrite\|...` | 按类型统计的 TiKV 请求持续时间（秒）。 |
