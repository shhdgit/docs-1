...
TiDB Cloud 支持 New Relic 集成（预览版）。你可以配置 TiDB Cloud，将 TiDB 集群的监控指标发送到 [New Relic](https://newrelic.com/)。之后，你可以直接在 New Relic 仪表盘中查看这些指标。

## New Relic 集成版本

自 2023 年 4 月 11 日起，TiDB Cloud 支持 New Relic 集成（Beta）。从 2025 年 7 月 31 日起，TiDB Cloud 推出了增强的预览版集成。
...
1. 准备新仪表盘的 JSON 文件。
    ...
    3. 在 JSON 文件的所有 `"accountIds": []` 字段中，添加你的 New Relic 账号 ID。
        ...
        > **注意**：
        >
        > 为避免集成错误，请确保你的账号 ID 已添加到 JSON 文件中所有 `"accountIds"` 字段。
...
| tidb_cloud.cdc_changefeed_latency (AKA cdc_changefeed_checkpoint_ts_lag) | gauge | changefeed_id: `<changefeed-id>`<br/><br/>cluster_name: `<cluster name>`| changefeed owner 的检查点时间戳延迟（秒）。 |
