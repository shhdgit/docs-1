...
1. 准备新仪表盘的 JSON 文件。
    ...
    3. 在 JSON 文件的所有 `"accountIds": []` 字段中，添加你的 New Relic 账号 ID。
        ...
        > **注意**：
        >
        > 为避免集成出错，请确保你的账号 ID 已添加到 JSON 文件中所有 `"accountIds"` 字段。
...
| tidb_cloud.dm_syncer_replication_lag_bucket | gauge | instance: `instance`<br/><br/>cluster_name: `<cluster name>` | 数据迁移的复制延迟（桶）。 |
