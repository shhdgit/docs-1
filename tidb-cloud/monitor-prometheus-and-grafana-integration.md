...
- 你无法在 [TiDB Cloud Starter](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 或 [TiDB Cloud Essential](/tidb-cloud/select-cluster-tier.md#essential) 集群中使用 Prometheus 和 Grafana 集成。
...
4. 在项目的 **Integrations** 页面，删除对应的旧 `scrape_config` 文件，防止他人使用其访问 TiDB Cloud Prometheus 端点。
...
- 为什么同一指标在 Grafana 和 TiDB Cloud 控制台上显示的数值不同？

    Grafana 和 TiDB Cloud 的聚合计算逻辑不同，因此显示的聚合值可能存在差异。你可以调整 Grafana 中的 `mini step` 配置，以获得更细粒度的指标数值。