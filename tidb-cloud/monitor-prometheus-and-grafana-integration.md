...
- 要编辑 TiDB Cloud 的第三方集成设置，你必须拥有组织的 **Organization Owner** 权限，或在 TiDB Cloud 中目标项目的 **Project Member** 权限。
...
4. 在项目的 **Integrations** 页面，删除对应的旧 `scrape_config` 文件，防止其他人使用它从 TiDB Cloud Prometheus 端点读取数据。
...
| tidbcloud_kv_request_duration | histogram | instance: `tidb-0\|tidb-1\|...`<br/>component: `tikv`<br/>cluster_name: `<cluster name>`<br/>`type`: `BatchGet\|Commit\|Prewrite\|...` | 按类型统计的 TiKV 请求耗时（秒） |
