...
TiDB Cloud 提供了一个 [Prometheus](https://prometheus.io/) API 端点（Beta 版）。如果你有 Prometheus 服务，可以轻松地通过该端点监控 TiDB Cloud 的关键指标。
...
要获取 Prometheus 的 `scrape_config` 文件，请按以下步骤操作：
...
5. 复制 `scrape_config` 文件的内容，供后续使用。
...
2. [将该 JSON 导入到你自己的 Grafana GUI](https://grafana.com/docs/grafana/v8.5/dashboards/export-import/#import-dashboard) 以可视化指标。
    ...
    > **注意：**
    >
    > 如果你已经在使用 Prometheus 和 Grafana 监控 TiDB Cloud，并希望引入新开放的指标，建议新建一个仪表盘，而不是直接更新现有仪表盘的 JSON。
...
4. 在项目的 **Integrations** 页面，删除对应的旧 `scrape_config` 文件，防止他人使用该文件从 TiDB Cloud Prometheus 端点读取数据。

## Prometheus 可用的指标

Prometheus 会为你的 TiDB 集群跟踪以下指标数据。
...
| tidbcloud_db_query_duration_seconds | histogram | sql_type: `Select\|Insert\|...`<br/>cluster_name: `<cluster name>`<br/>instance: `tidb-0\|tidb-1…`<br/>component: `tidb` | 语句执行时延直方图 |
...
| tidbcloud_node_storage_used_bytes | gauge | cluster_name: `<cluster name>`<br/>instance: `tikv-0\|tikv-1…\|tiflash-0\|tiflash-1…`<br/>component: `tikv\|tiflash` | TiKV/TiFlash 节点的磁盘已用空间（字节） |
| tidbcloud_node_storage_capacity_bytes | gauge | cluster_name: `<cluster name>`<br/>instance: `tikv-0\|tikv-1…\|tiflash-0\|tiflash-1…`<br/>component: `tikv\|tiflash` | TiKV/TiFlash 节点的磁盘总容量（字节） |
...
| tidbcloud_node_memory_used_bytes | gauge | cluster_name: `<cluster name>`<br/>instance: `tidb-0\|tidb-1…\|tikv-0…\|tiflash-0…`<br/>component: `tidb\|tikv\|tiflash` | TiDB/TiKV/TiFlash 节点的已用内存（字节） |
| tidbcloud_node_memory_capacity_bytes | gauge | cluster_name: `<cluster name>`<br/>instance: `tidb-0\|tidb-1…\|tikv-0…\|tiflash-0…`<br/>component: `tidb\|tikv\|tiflash` | TiDB/TiKV/TiFlash 节点的内存总容量（字节） |
...
- 为什么同一指标在 Grafana 和 TiDB Cloud 控制台上同时显示的数值不同？
    ...
    因为 Grafana 和 TiDB Cloud 的聚合计算逻辑不同，所以显示的聚合值可能会有差异。你可以调整 Grafana 中的 `mini step` 配置，以获得更细粒度的指标数值。