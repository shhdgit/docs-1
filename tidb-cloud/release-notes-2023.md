...
- 优化 [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 的连接体验。
    ...
    优化 **Connect** 对话框界面，为 TiDB Cloud Serverless 用户带来更流畅高效的连接体验。此外，TiDB Cloud Serverless 新增更多客户端类型，并允许你选择所需分支进行连接。
...
- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持从备份中恢复 SQL 绑定。
    ...
    现在，TiDB Cloud 专属集群在从备份恢复时默认恢复用户账号和 SQL 绑定。该增强适用于 v6.2.0 及以上版本的集群，简化了数据恢复流程。SQL 绑定的恢复确保查询相关配置和优化的顺利回归，为你提供更全面高效的恢复体验。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 支持监控 SQL 语句的 RU 消耗。
    ...
    TiDB Cloud Serverless 现在可为每条 SQL 语句提供详细的 [Request Units (RUs)](/tidb-cloud/tidb-cloud-glossary.md#request-unit) 消耗信息。你可以查看每条 SQL 语句的 **Total RU** 和 **Mean RU**。该功能有助于识别和分析 RU 消耗，为运维成本优化提供参考。
...
- [数据迁移](/tidb-cloud/migrate-from-mysql-using-data-migration.md) 支持 Google Cloud 上部署的 TiDB 集群的高速物理模式。
    ...
    现在，你可以在 AWS 和 Google Cloud 上部署的 TiDB 集群使用物理模式。物理模式的迁移速度可达 110 MiB/s，是逻辑模式的 2.4 倍。该性能提升适合大规模数据集的快速迁移至 TiDB Cloud。
...
- 从 TiDB Cloud 专属集群恢复数据时，默认行为由“仅恢复数据不包含用户账号”改为“恢复所有用户账号”。
...
- 新增以下资源使用率告警，默认关闭。你可按需开启。
    ...
    - TiDB 节点最大内存使用率连续 10 分钟超过 70%
    - TiKV 节点最大内存使用率连续 10 分钟超过 70%
    - TiDB 节点最大 CPU 使用率连续 10 分钟超过 80%
    - TiKV 节点最大 CPU 使用率连续 10 分钟超过 80%
...
- 支持 [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 在 AWS 上的 [双区域备份（Beta）](/tidb-cloud/backup-and-restore.md#turn-on-dual-region-backup)。
...
- [数据服务（Beta）](https://tidbcloud.com/project/data-service) 支持为每个 API key 自定义限流，满足不同场景下的限流需求。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群新增 **Events** 页面，记录集群主要变更。
    ...
    你可在该页面查看近 7 天的事件历史，追踪触发时间、操作用户等重要信息。
...
- 支持通过 changefeed 将 [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的数据流式写入 [Google Cloud Storage (GCS)](https://cloud.google.com/storage)。
    ...
    你可以使用自有账号的 bucket 并精确配置权限，将数据流式同步到 GCS。同步后可按需分析数据变更。
...
- [数据服务（Beta）](https://tidbcloud.com/project/data-service) 支持 `GET` 请求分页，提升开发体验。

    对于 `GET` 请求，你可在 **Advance Properties** 启用 **Pagination**，并在调用接口时通过查询参数指定 `page` 和 `page_size`。例如，获取第 2 页且每页 10 条数据：
...
- [数据服务（Beta）](https://tidbcloud.com/project/data-service) 支持为 `GET` 请求的接口响应设置缓存 TTL。
    ...
    对于 `GET` 请求接口，可在 **Advance Properties** 启用 **Cache Response** 并配置缓存 TTL。

    详情参见 [高级属性](/tidb-cloud/data-service-manage-endpoint.md#advanced-properties)。

- 禁用 2023 年 8 月 15 日后在 AWS 上新建的 [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的负载均衡改进，包括：
...
- [数据服务（Beta）](https://tidbcloud.com/project/data-service) 现支持 Basic 认证。
    ...
    你可在请求中将公钥作为用户名、私钥作为密码，使用 ['Basic' HTTP 认证](https://datatracker.ietf.org/doc/html/rfc7617)。相比 Digest 认证，Basic 认证更简单，便于调用数据服务接口。
...
- TiDB Cloud [数据服务](https://tidbcloud.com/project/data-service) 支持 OpenAPI 规范。
    ...
    TiDB Cloud 数据服务为每个 Data App 自动生成 OpenAPI 文档。你可在文档中查看接口、参数、响应，并直接试用接口。

    你还可以下载 Data App 及其已部署接口的 OpenAPI 规范（YAML 或 JSON 格式）。OAS 提供标准化 API 文档、简化集成和便捷代码生成，加速开发与协作。
...
- 支持在 [Postman](https://www.postman.com/) 运行 Data App。

    Postman 集成支持将 Data App 的接口作为集合导入工作区，便于协作和 API 测试，支持 Postman Web 和桌面应用。
...
- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 新增 **Pausing** 状态，暂停期间无需付费。
    ...
    点击 **Pause** 后，集群先进入 **Pausing** 状态，暂停完成后转为 **Paused**。

    只有在状态变为 **Paused** 后才能恢复，解决了因频繁点击 **Pause** 和 **Resume** 导致的异常恢复问题。
...
- TiDB Cloud [数据服务](https://tidbcloud.com/project/data-service) 支持 `PUT` 和 `DELETE` 请求方法。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群支持 [分支功能（Beta）](/tidb-cloud/branch-overview.md)。
    ...
    你可为 TiDB Cloud Serverless 集群创建分支。分支为独立实例，包含原集群数据的分叉副本，提供隔离环境，便于自由实验而不影响原集群。
    ...
    若你使用 GitHub 进行应用开发，可将 TiDB Cloud Serverless 分支集成至 GitHub CI/CD 流水线，实现 PR 自动测试且不影响生产库。详情参见 [与 GitHub 集成 TiDB Cloud Serverless 分支（Beta）](/tidb-cloud/branch-github-integration.md)。

- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持每周备份。详情参见 [开启自动备份](/tidb-cloud/backup-and-restore.md#turn-on-auto-backup)。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群支持时间点恢复（PITR）（Beta）。
    ...
    你可将 TiDB Cloud Serverless 集群恢复到近 90 天内任意时间点，提升数据恢复能力。例如，数据写入出错时可用 PITR 恢复到早期状态。
...
- 支持通过 changefeed 将数据流式写入 Amazon S3。
    ...
    实现 TiDB Cloud 与 Amazon S3 的无缝集成，支持将 [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的数据实时捕获并同步到 Amazon S3，确保下游应用和分析系统获取最新数据。
...
- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 [**Key Visualizer**](/tidb-cloud/tune-performance.md#key-visualizer) 页面发布全新原生 Web 架构。
    ...
    新架构下，**Key Visualizer** 页面导航更便捷，信息获取更高效，用户体验显著提升，SQL 诊断过程更友好。

## 2023 年 6 月 6 日

**通用变更**

- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 引入 Index Insight（Beta），为慢查询提供索引推荐，优化查询性能。

    使用 Index Insight 可提升数据库操作和应用整体性能，具体优势包括：

    - 查询性能提升：识别慢查询并推荐合适索引，加速查询执行，降低响应时间，提升用户体验。
    - 成本优化：通过索引优化减少额外计算资源消耗，更高效利用现有基础设施，降低运维成本。
    - 优化流程简化：自动识别并推荐索引，无需手动分析和猜测，节省时间和精力。
    - 应用效率提升：优化数据库性能后，应用可承载更大负载和更多并发用户，提升扩展能力。
...
- 推出 [TiDB Playground](https://play.tidbcloud.com/?utm_source=docs&utm_medium=tidb_cloud_release_notes)，无需注册或安装即可体验 TiDB 全功能的交互平台。
    ...
    TiDB Playground 提供一站式体验，支持扩展性、MySQL 兼容性、实时分析等能力。
...
- 支持将 [Data App](/tidb-cloud/tidb-cloud-glossary.md#data-app) 连接至 GitHub。
    ...
    [连接 Data App 至 GitHub](/tidb-cloud/data-service-manage-github-connection.md) 后，可将所有配置作为 [代码文件](/tidb-cloud/data-service-app-config-files.md) 管理，实现与系统架构和 DevOps 流程的无缝集成。
...
- 增强 TiDB Cloud 数据迁移功能对增量数据迁移的支持。
    ...
    你现在可以指定 binlog 位置或全局事务标识（GTID），仅同步指定位置之后产生的增量数据至 TiDB Cloud。该增强为数据选择和同步提供更大灵活性，满足个性化需求。
...
- 上传 CSV 文件至 TiDB 时，除英文和数字外，还可用中文、日文等字符定义列名。但特殊字符仅支持下划线（`_`）。
...
- 专属集群和 Serverless 集群均引入按功能分类的左侧导航入口。
...
- 专属集群 **Diagnosis** 页的以下两个标签页发布全新原生 Web 架构：
    ...
    新架构下，页面导航更便捷，信息获取更高效，用户体验显著提升，SQL 诊断过程更友好。

## 2023 年 5 月 9 日

**通用变更**

- 支持变更 2023 年 4 月 26 日后创建的 GCP 集群节点规格。
...
- [**Event**](/tidb-cloud/tidb-cloud-events.md) 页面新增事件类型，记录专属集群的备份、恢复、changefeed 操作。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 的 [**SQL Diagnosis**](/tidb-cloud/tune-performance.md) 页面新增 **SQL Statement** 标签。
    ...
    **SQL Statement** 标签提供：
    ...
    - 全面展示 TiDB 数据库执行的所有 SQL 语句，便于识别和诊断慢查询。
    - 每条 SQL 语句的详细信息，如查询时间、执行计划、数据库响应等，助力性能优化。
    - 友好的界面，便于排序、筛选和搜索大量数据，聚焦关键查询。

  详情参见 [语句分析](/tidb-cloud/tune-performance.md#statement-analysis)。
...
- 支持直接访问 TiDB [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 所在区域的 [Data Service endpoint](/tidb-cloud/tidb-cloud-glossary.md#endpoint)。
    ...
    新建 Serverless 集群的 endpoint URL 现包含集群区域信息。通过 `<region>.data.tidbcloud.com` 可直接访问集群所在区域的 endpoint。

    也可通过 `data.tidbcloud.com` 全局域名访问，系统会自动重定向至目标区域，但可能增加延迟。若采用此方式，调用 endpoint 时需在 curl 命令中添加 `--location-trusted` 选项。
...
- 组织下前 5 个 [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 均享有免费额度：
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 提供维护窗口功能，便于你安排和管理计划性维护。
    ...
    维护窗口为自动执行操作系统更新、安全补丁、基础设施升级等计划性维护的指定时段，保障 TiDB Cloud 服务的可靠性、安全性和性能。

    维护期间可能出现短暂连接中断或 QPS 波动，但集群保持可用，SQL 操作、数据导入、备份、恢复、迁移、同步等任务可正常运行。详见 [维护期间允许与禁止的操作](/tidb-cloud/configure-maintenance-window.md#allowed-and-disallowed-operations-during-a-maintenance-window)。

    我们将尽量减少维护频率。若有维护计划，默认开始时间为目标周三 03:00（以组织时区为准）。请关注维护计划，合理安排操作。

    - TiDB Cloud 会为每次维护窗口发送三封邮件通知：维护前、开始时、结束后。
    - 你可在 **Maintenance** 页面修改维护开始时间或延后维护，以降低影响。

  详情参见 [配置维护窗口](/tidb-cloud/configure-maintenance-window.md)。

- 优化 AWS 上新建的 [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 TiDB 负载均衡，缩扩容时减少连接中断。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 [Monitoring](/tidb-cloud/built-in-monitoring.md#view-the-metrics-page) 页面发布全新原生 Web 架构。
    ...
    新架构下，页面导航更便捷，信息获取更高效，用户体验显著提升，监控过程更友好。

## 2023 年 4 月 18 日

**通用变更**

- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持扩缩 [数据迁移作业规格](/tidb-cloud/tidb-cloud-billing-dm.md#specifications-for-data-migration)。
...
- 全新 UI 设计优化 [集群创建](https://tidbcloud.com/clusters/create-cluster) 体验，几步即可完成集群创建与配置。
...
- 优化 AWS 上 [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 TiDB 负载均衡，缩扩容时减少连接中断。

    - 扩容 TiDB 节点时，支持自动迁移现有连接到新节点。
    - 缩容 TiDB 节点时，支持自动迁移现有连接到可用节点。

  目前仅适用于 AWS `Oregon (us-west-2)` 区域的专属集群。

- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持 [New Relic](https://newrelic.com/) 集成。

    配置后可将 TiDB 集群的指标数据发送至 [New Relic](https://newrelic.com/)，实现应用与数据库性能统一监控和分析，便于快速定位和解决问题。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 Prometheus 集成新增以下 [changefeed](/tidb-cloud/changefeed-overview.md) 指标：
    ...
    已 [集成 Prometheus](/tidb-cloud/monitor-prometheus-and-grafana-integration.md) 后，可实时监控 changefeed 性能与健康，并可基于这些指标创建告警。

**控制台变更**

- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 [Monitoring](/tidb-cloud/built-in-monitoring.md#view-the-metrics-page) 页面采用 [节点级资源指标](/tidb-cloud/built-in-monitoring.md#server)。

    节点级指标更准确反映资源消耗，便于了解实际服务使用情况。
...
- [数据服务（Beta）](/tidb-cloud/data-service-overview.md) 支持 Data App 更细粒度的访问控制。
    ...
    在 Data App 详情页，可关联集群并为每个 API key 指定角色。角色控制 API key 是否可对关联集群读写数据，可设为 `ReadOnly` 或 `ReadAndWrite`。该功能实现 Data App 的集群级和权限级访问控制，灵活满足业务需求。
...
- 支持对 2023 年 3 月 28 日后创建的 [changefeed](/tidb-cloud/changefeed-overview.md) 扩缩规格。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 [数据迁移](/tidb-cloud/migrate-from-mysql-using-data-migration.md) 功能新增支持 GCP 区域：`Singapore (asia-southeast1)` 和 `Oregon (us-west1)`。
    ...
    新增区域为数据迁移提供更多选择，靠近数据源时可获得更快更可靠的迁移体验。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 推出 [数据服务（Beta）](https://tidbcloud.com/project/data-service)，支持通过自定义 API endpoint 以 HTTPS 请求访问数据。
    ...
    访问方式：左侧导航栏进入 [**Data Service**](https://tidbcloud.com/project/data-service) 页面。详情参见：
...
- 支持对 2022 年 12 月 31 日后在 AWS 上创建的 [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 缩小 TiDB、TiKV、TiFlash 节点规格。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 新增 **Events** 页面，记录集群主要变更。
    ...
    可查看近 7 天事件历史，追踪触发时间、操作用户等信息，如集群暂停、规格变更等。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 的 **Monitoring** 页面新增 **Database Status** 标签，展示以下数据库级指标：
  ...
  通过这些指标可监控单个数据库性能，便于数据驱动决策和性能优化。
...
- 上传带表头的本地 CSV 文件到 [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 时，支持修改 TiDB Cloud 自动创建目标表的列名。

    若表头列名不符合 TiDB Cloud 命名规范，将在对应列名旁显示警告图标。可将鼠标悬停查看提示，并按需编辑或输入新列名。
...
- 支持使用 [控制台审计日志](/tidb-cloud/tidb-cloud-console-auditing.md) 跟踪组织成员在 [TiDB Cloud 控制台](https://tidbcloud.com/) 的各类操作。
    ...
    仅 `Owner` 或 `Audit Admin` 角色可见该功能，默认关闭。启用方法：在 [TiDB Cloud 控制台](https://tidbcloud.com/) 右上角点击 <MDSvgIcon name="icon-top-organization" /> **Organization** > **Console Audit Logging**。
...
- 导入数据到 TiDB Cloud 时，支持使用 IAM 用户的 AWS 访问密钥访问 Amazon S3 bucket。
...
- 监控指标保留期由 2 天延长：
  ...
  保留期延长后，可访问更多历史数据，有助于识别趋势、优化决策和加速故障排查。
...
- [TiDB Cloud CLI](/tidb-cloud/cli-reference.md) 新增 [`ticloud connect`](/tidb-cloud/ticloud-serverless-shell.md) 命令。
    ...
    `ticloud connect` 支持本地直接连接 TiDB Cloud 集群，无需安装 SQL 客户端。连接后可在 CLI 中执行 SQL 语句。

## 2023 年 2 月 14 日

**通用变更**

- 支持缩减 TiKV 和 TiFlash 节点数量，实现 TiDB [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的缩容。
...
- 推出 TiDB Cloud CLI 客户端 [`ticloud`](/tidb-cloud/cli-reference.md)。
    ...
    使用 `ticloud`，你可通过终端或自动化流程便捷管理 TiDB Cloud 资源。针对 GitHub Actions，已提供 [`setup-tidbcloud-cli`](https://github.com/marketplace/actions/set-up-tidbcloud-cli) 便捷集成。
...
- 每个集群新增 **Get Support** 选项，简化针对特定集群的支持请求流程。
    ...
    - 在项目 [**Clusters**](https://tidbcloud.com/project/clusters) 页面，点击集群行的 **...** 并选择 **Get Support**。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 的 SQL Editor（Beta）更名为 Chat2Query（Beta），并支持 AI 生成 SQL 查询。

  在 Chat2Query 中，你可让 AI 自动生成 SQL 查询，也可手动编写 SQL 查询，并直接在数据库上运行，无需终端。
...
- 支持通过增加 **Node Size(vCPU + RAM)** 扩容 AWS 上新建的 TiDB Cloud 专属集群的 TiDB、TiKV、TiFlash 节点。

    可通过 [TiDB Cloud 控制台](/tidb-cloud/scale-tidb-cluster.md#change-vcpu-and-ram) 或 [TiDB Cloud API (beta)](https://docs.pingcap.com/tidbcloud/api/v1beta#tag/Cluster/operation/UpdateCluster) 扩容节点。
...
- 支持自定义 Prometheus 集成的 Grafana dashboard JSON。

    已 [集成 Prometheus](/tidb-cloud/monitor-prometheus-and-grafana-integration.md) 后，可导入预置 Grafana dashboard 并自定义，便捷监控 TiDB Cloud 集群，快速发现性能问题。
...
- 简化 [**Clusters**](https://tidbcloud.com/project/clusters) 页面和集群概览页展示。
    ...
    - 可在 [**Clusters**](https://tidbcloud.com/project/clusters) 页面点击集群名进入概览页并开始操作。
    - 从概览页移除 **Connection** 和 **Import** 面板。可在右上角点击 **Connect** 获取连接信息，或在左侧导航栏点击 **Import** 导入数据。