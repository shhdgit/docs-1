...
- [数据迁移](/tidb-cloud/migrate-from-mysql-using-data-migration.md) 支持 Google Cloud 上部署的 TiDB 集群的高速物理模式。
    ...
    现在，你可以在 AWS 和 Google Cloud 上部署的 TiDB 集群使用物理模式。物理模式的迁移速度可达 110 MiB/s，是逻辑模式的 2.4 倍。该性能提升适用于大数据集的快速迁移至 TiDB Cloud。
...
- [将数据变更日志流式同步到 Apache Kafka](/tidb-cloud/changefeed-sink-to-apache-kafka.md) 功能现已正式 GA。
    ...
    经过 10 个月的 beta 试用，该功能现已正式发布。将 TiDB Cloud 的数据变更日志流式同步到 Apache Kafka 是数据集成场景的常见需求。你可以通过 Kafka sink 集成其他数据处理系统（如 Snowflake）或支持业务消费。
...
- 发布多组 TiDB Cloud API 接口，用于管理 [AWS PrivateLink](https://aws.amazon.com/privatelink/?privatelink-blogs.sort-by=item.additionalFields.createdDate&privatelink-blogs.sort-order=desc) 或 [Google Cloud Private Service Connect](https://cloud.google.com/vpc/docs/private-service-connect) 的 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群：
...
- 支持通过 changefeed 将 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群数据流式同步到 [Google Cloud Storage (GCS)](https://cloud.google.com/storage)。
...
- 对 2023 年 8 月 15 日后在 AWS 上新建的 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群，禁用负载均衡改进，包括：
    ...
    - 扩容 TiDB 节点时，不再自动迁移现有连接到新节点。
    - 缩容 TiDB 节点时，不再自动迁移现有连接到可用节点。

  该变更避免混合部署资源争用，不影响已启用该功能的现有集群。如需为新集群启用负载均衡改进，请联系 [TiDB Cloud 支持](/tidb-cloud/tidb-cloud-support.md)。

## 2023 年 8 月 8 日

**通用变更**

- [Data Service (beta)](https://tidbcloud.com/project/data-service) 现支持 Basic 认证。

    你可以在请求中将公钥作为用户名、私钥作为密码，使用 ['Basic' HTTP 认证](https://datatracker.ietf.org/doc/html/rfc7617)。与 Digest 认证相比，Basic 认证更简单，便于调用 Data Service 接口。
...
- 支持在 [Postman](https://www.postman.com/) 运行 Data App。
    ...
    Postman 集成支持将 Data App 的接口作为集合导入你的工作区，便于协作和无缝 API 测试，支持 Postman Web 和桌面应用。
...
- TiDB Cloud [Data Service](https://tidbcloud.com/project/data-service) 引入自动生成接口功能。

    开发者现在可以通过极少的点击和配置，快速创建 HTTP 接口，省去重复样板代码，简化并加速接口创建，减少潜在错误。
...
- TiDB Cloud [Data Service](https://tidbcloud.com/project/data-service) 支持 `POST`、`PUT`、`DELETE` 请求方法的 **Batch Operation**。
    ...
    启用 **Batch Operation** 后，你可以在一次请求中操作多行数据。例如，使用单个 `POST` 请求插入多行数据。
...
- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群（AWS）支持客户自管加密密钥（CMEK）（beta）。
    ...
    你可以基于 AWS KMS 创建 CMEK，对 EBS 和 S3 存储的数据进行加密，密钥由客户自主管理，提升安全性。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群支持 [分支功能（beta）](/tidb-cloud/branch-overview.md)。
    ...
    你可以为 TiDB Cloud Serverless 集群创建分支。分支是原集群数据的分离副本，提供隔离环境，便于自由实验而不影响原集群。
    ...
    如果你使用 GitHub 进行应用开发，可将分支集成到 CI/CD 流水线，实现自动测试 PR，且不影响生产库。详情参见 [与 GitHub 集成 TiDB Cloud Serverless 分支（Beta）](/tidb-cloud/branch-github-integration.md)。
...
- 支持通过 changefeed 将数据流式同步到 Amazon S3。
    ...
    该功能实现 TiDB Cloud 与 Amazon S3 的无缝集成，支持将 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的数据实时捕获并同步到 Amazon S3，确保下游应用和分析系统获取最新数据。

    详情参见 [同步到云存储](/tidb-cloud/changefeed-sink-to-cloud-storage.md)。

- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的 16 vCPU TiKV 节点最大存储由 4 TiB 提升至 6 TiB。

    该增强提升了集群的数据存储能力，提高了工作负载扩展效率，满足不断增长的数据需求。
...
- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群引入 Index Insight（beta），为慢查询提供索引优化建议，提升查询性能。
    ...
    - 降低成本：优化查询性能，减少额外计算资源消耗，更高效利用现有基础设施，节省运维成本。
...
- 上传 CSV 文件到 TiDB 时，列名除英文和数字外，还支持中文、日文等字符。特殊字符仅支持下划线（`_`）。
...
- 为 Dedicated 和 Serverless 两类集群引入按功能分类的左侧导航入口。
...
- Dedicated Tier 集群 **Diagnosis** 页的以下两个标签页发布全新原生 Web 架构：
    ...
    新架构让你更便捷地浏览和获取信息，提升 SQL 诊断体验和用户友好性。
...
- 支持为 2023 年 4 月 26 日后创建的 GCP 集群变更节点规格。
    ...
    你可以根据需求升级高性能节点或降级低性能节点，灵活调整集群容量，优化成本。
...
- [Serverless Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群支持基于 AWS PrivateLink 的端点连接，作为新的网络访问管理选项。
    ...
    私有端点连接不会暴露数据到公网，且支持 CIDR 重叠，便于网络管理。

    详情参见 [设置私有端点连接](/tidb-cloud/set-up-private-endpoint-connections.md)。

**控制台变更**

- [**Event**](/tidb-cloud/tidb-cloud-events.md) 页面新增 Dedicated Tier 集群的备份、恢复、changefeed 操作事件类型。
...
- 支持直接访问 TiDB [Serverless Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群所在区域的 [Data Service endpoint](/tidb-cloud/tidb-cloud-glossary.md#endpoint)。
    ...
    新建 Serverless Tier 集群的 endpoint URL 现包含集群区域信息。通过请求区域域名 `<region>.data.tidbcloud.com`，可直接访问对应区域的 endpoint。

    你也可以请求全局域名 `data.tidbcloud.com`，此时 TiDB Cloud 会内部重定向到目标区域，但可能增加延迟。如采用此方式，调用接口时请确保 curl 命令加上 `--location-trusted` 选项。
...
- 你的组织下前 5 个 [Serverless Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群，每个均享有免费额度：
...
- 优化 AWS 上新建 [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的 TiDB 负载均衡，缩容/扩容 TiDB 节点时减少连接中断。
...
- [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的 [Monitoring](/tidb-cloud/built-in-monitoring.md#view-the-metrics-page) 页面现采用 [节点级资源指标](/tidb-cloud/built-in-monitoring.md#server)。
    ...
    访问方法：进入集群 [Monitoring](/tidb-cloud/built-in-monitoring.md#view-the-metrics-page) 页面，在 **Metrics** 标签下选择 **Server** 类别。
...
- [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群新增 [Alerts](/tidb-cloud/monitor-built-in-alerting.md) 页面，展示每个集群的活跃和已关闭告警。
    ...
    **Alerts** 页面提供：
    ...
    - 高级筛选，按严重性、状态等属性快速查找和排序，并支持查看最近 7 天历史，便于追踪。
...
- [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的数据迁移功能新增支持 GCP 区域：`Singapore (asia-southeast1)` 和 `Oregon (us-west1)`。
    ...
    新增区域为你提供更多数据迁移选择，靠近上游数据时可获得更快、更可靠的迁移体验。
...
- 支持为 2022 年 12 月 31 日后在 AWS 上创建的 [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群缩容 TiDB、TiKV、TiFlash 节点规格。
...
- 支持在上传带表头的本地 CSV 文件时，修改 TiDB Cloud 自动创建目标表的列名。
    ...
    导入带表头的本地 CSV 文件到 [Serverless Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群时，如列名不符合命名规范，将在对应列名旁显示警告图标。你可悬停图标，根据提示编辑或输入新列名。
...
- [Serverless Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群新增 [SQL Diagnosis](/tidb-cloud/tune-performance.md) 功能。
    ...
    使用方法：在集群页面左侧导航栏点击 **SQL Diagnosis**。
...
- 支持使用 [控制台审计日志](/tidb-cloud/tidb-cloud-console-auditing.md) 跟踪组织成员在 [TiDB Cloud 控制台](https://tidbcloud.com/) 的各类操作。
    ...
    该功能仅对 `Owner` 或 `Audit Admin` 可见，默认关闭。开启方法：在 [TiDB Cloud 控制台](https://tidbcloud.com/) 右上角点击 <MDSvgIcon name="icon-top-organization" /> **Organization** > **Console Audit Logging**。
...
- 优化本地 CSV 文件导入 TiDB 的体验，提升 [Serverless Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群的易用性。
    ...
    - 你现在可以直接拖拽 CSV 文件到 **Import** 页上传区域。
    - 创建导入任务时，如目标数据库或表不存在，可输入名称自动创建。新建目标表时可指定主键或选择多字段组成复合主键。
...
- 支持为 2022 年 12 月 31 日后在 AWS 上创建的 TiDB Cloud Dedicated 集群扩容 TiDB、TiKV、TiFlash 节点（提升 **Node Size(vCPU + RAM)**）。
    ...
    你可以通过 [TiDB Cloud 控制台](/tidb-cloud/scale-tidb-cluster.md#change-vcpu-and-ram) 或 [TiDB Cloud API (beta)](https://docs.pingcap.com/tidbcloud/api/v1beta#tag/Cluster/operation/UpdateCluster) 扩大节点规格。

- [**Monitoring**](/tidb-cloud/built-in-monitoring.md) 页的指标保留期延长至两天。

    你现在可访问近两天的指标数据，更灵活地分析集群性能和趋势。

    该提升无需额外费用，可在集群 [**Monitoring**](/tidb-cloud/built-in-monitoring.md) 页的 **Diagnosis** 标签访问，有助于更高效地定位和排查性能问题，监控集群健康。

- 支持为 Prometheus 集成自定义 Grafana dashboard JSON。

    已 [集成 Prometheus](/tidb-cloud/monitor-prometheus-and-grafana-integration.md) 的用户可导入预置 Grafana dashboard 并自定义，便于快速监控 TiDB Cloud 集群，及时发现性能问题。
...
- 简化 [**Clusters**](https://tidbcloud.com/project/clusters) 页和集群概览页展示：
    ...
    - 从集群概览页移除 **Connection** 和 **Import** 面板。你可在右上角点击 **Connect** 获取连接信息，或在左侧导航栏点击 **Import** 导入数据。