...
- 为 changefeed 引入事件过滤器。
    ...
    该增强支持你直接通过 [TiDB Cloud 控制台](https://tidbcloud.com/) 管理 changefeed 的事件过滤器，简化了排除特定事件的流程，并提升了下游数据同步的控制能力。
...
- 新增 TiDB Cloud Billing API 端点，可获取指定组织某月账单。
    ...
    该 Billing API 端点已在 TiDB Cloud API v1beta1（最新版本）中发布。详细信息参见 [API 文档 (v1beta1)](https://docs.pingcap.com/tidbcloud/api/v1beta1#tag/Billing)。

## 2023 年 9 月 19 日

**通用变更**

- 从 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群中移除 2 vCPU 的 TiDB 和 TiKV 节点选项。
...
- 发布 [TiDB Cloud serverless driver (beta)](/tidb-cloud/serverless-driver.md) for JavaScript。
    ...
    TiDB Cloud serverless driver for JavaScript 支持通过 HTTPS 连接 [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#starter) 集群。该驱动特别适用于 TCP 连接受限的边缘环境，如 [Vercel Edge Function](https://vercel.com/docs/functions/edge-functions) 和 [Cloudflare Workers](https://workers.cloudflare.com/)。

    详细信息参见 [TiDB Cloud serverless driver (beta)](/tidb-cloud/serverless-driver.md)。

**控制台变更**

- 对于 [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#starter) 集群，你可以在 **Usage This Month** 面板或设置消费上限时获得费用预估。
...
- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群支持 Google Cloud [Private Service Connect](https://cloud.google.com/vpc/docs/private-service-connect)。
    ...
    你现在可以创建私有端点，并与部署在 Google Cloud 上的 TiDB Cloud Dedicated 集群建立安全连接。
...
- 支持通过 changefeed 将 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的数据流式写入 [Google Cloud Storage (GCS)](https://cloud.google.com/storage)。
    ...
    你可以使用自己的 GCS 存储桶并精确配置权限，将数据流式同步到 GCS。数据复制到 GCS 后，你可按需分析数据变更。
...
- [Data Service (beta)](https://tidbcloud.com/project/data-service) 支持 `GET` 请求的分页，提升开发体验。
    ...
    详细信息参见 [调用端点](/tidb-cloud/data-service-manage-endpoint.md#call-an-endpoint)。

- [Data Service (beta)](https://tidbcloud.com/project/data-service) 支持为 `GET` 请求的端点配置缓存响应的 TTL。
...
- 对 2023 年 8 月 15 日后在 AWS 上新建的 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群，禁用负载均衡改进，包括：
  ...
  此变更避免混合部署时的资源争用，不影响已启用该改进的现有集群。如需为新集群启用负载均衡改进，请联系 [TiDB Cloud 支持](/tidb-cloud/tidb-cloud-support.md)。
...
- TiDB Cloud [Data Service](https://tidbcloud.com/project/data-service) 引入自动生成端点功能。
    ...
    开发者可通过极少的点击和配置，快速创建 HTTP 端点。无需重复编写样板代码，简化并加速端点创建，减少潜在错误。

    详细用法参见 [自动生成端点](/tidb-cloud/data-service-manage-endpoint.md#generate-an-endpoint-automatically)。
...
- TiDB Cloud [Data Service](https://tidbcloud.com/project/data-service) 支持 `POST`、`PUT`、`DELETE` 请求方法的 **批量操作**。
...
- 优化 TiDB Cloud 的导入功能，提升数据导入体验，具体改进如下：
    ...
    - 源文件映射改进：支持自定义源文件与目标表的映射关系，解决需修改源文件名的问题。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#starter) 集群支持 [分支功能（beta）](/tidb-cloud/branch-overview.md)。
    ...
    TiDB Cloud 支持为 Serverless 集群创建分支。分支是原集群数据的分离副本，提供隔离环境，便于自由实验而不影响原集群。
    ...
    如果你使用 GitHub 进行应用开发，可将分支集成到 CI/CD 流程，实现 PR 自动测试且不影响生产库。详细信息参见 [将 TiDB Cloud Serverless 分支（Beta）集成到 GitHub](/tidb-cloud/branch-github-integration.md)。

- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群支持每周备份。详细信息参见 [开启自动备份](/tidb-cloud/backup-and-restore.md#turn-on-auto-backup)。
...
- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群引入 Index Insight（beta），为慢查询提供索引推荐，优化查询性能。

    通过 Index Insight，你可提升数据库操作的整体性能和效率，具体包括：

    - 查询性能提升：Index Insight 识别慢查询并推荐合适索引，加速查询执行，降低响应时间，提升用户体验。
    - 成本优化：通过索引优化减少额外计算资源需求，更高效利用现有基础设施，降低运维成本。
    - 优化流程简化：Index Insight 简化索引改进的识别和实施，无需手动分析和猜测，节省时间和精力。
...
- 为 Dedicated 集群的 **Diagnosis** 页的以下两个标签页发布全新原生 Web 架构：
...
- 支持为 2023 年 4 月 26 日后创建的 GCP 集群变更节点规格。
    ...
    你可以根据需求升级高性能节点或降级低性能节点，实现弹性扩容和成本优化。
...
- [TiDB Cloud Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群新增支持基于 AWS PrivateLink 的私有端点连接作为网络访问管理选项。
...
- [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群的 [**SQL Diagnosis**](/tidb-cloud/tune-performance.md) 页面新增 **SQL Statement** 标签。
    ...
    **SQL Statement** 标签提供：
    ...
    - 友好的界面，支持排序、筛选和搜索，便于聚焦关键查询。

  详细信息参见 [语句分析](/tidb-cloud/tune-performance.md#statement-analysis)。
...
- 支持直接访问 TiDB [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群所在区域的 [Data Service endpoint](/tidb-cloud/tidb-cloud-glossary.md#endpoint)。
    ...
    也可通过 `data.tidbcloud.com` 全局域名访问，系统会自动重定向到目标区域，但可能增加延迟。若采用此方式，调用端点时需在 curl 命令中添加 `--location-trusted`。
...
- 组织下前五个 [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群可享受如下免费额度：

    - 行存储：5 GiB
    - [Request Units (RUs)](/tidb-cloud/tidb-cloud-glossary.md#request-unit)：每月 5000 万 RUs

  2023 年 5 月 31 日前，Serverless Tier 集群全部免费。之后超出免费额度部分将计费。
...
- [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群新增维护窗口功能，便于你安排和管理计划性维护。
...
- 优化 AWS 上新建的 [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的 TiDB 负载均衡，缩放 TiDB 节点时减少连接中断。
...
- 优化 [集群创建](https://tidbcloud.com/clusters/create-cluster) UI，提升易用性，几步即可完成集群创建和配置。
...
- 优化 AWS 上 [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的 TiDB 负载均衡，缩放 TiDB 节点时减少连接中断。
...
- [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群支持集成 [New Relic](https://newrelic.com/)。

    集成后，你可将 TiDB 集群的指标数据发送到 [New Relic](https://newrelic.com/)，实现应用与数据库性能的统一监控和分析，便于快速定位和解决问题。
...
- [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群引入 [Data Service (beta)](https://tidbcloud.com/project/data-service)，支持通过自定义 API endpoint 以 HTTPS 请求访问数据。
...
- 支持缩小 TiDB、TiKV、TiFlash 节点规格，实现 AWS 上 2022 年 12 月 31 日后创建的 [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的缩容。
...
- [Dedicated Tier](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群新增 **Events** 页面，记录集群主要变更。
    ...
    你可查看最近 7 天的事件历史，追踪触发时间、操作用户等详情。例如，可查看集群暂停、规格变更等事件。
...
- [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群的 **Monitoring** 页面新增 **Database Status** 标签，展示以下数据库级指标：
  ...
  详细信息参见 [Serverless Tier 集群监控指标](/tidb-cloud/built-in-monitoring.md)。
...
- 上传带表头的本地 CSV 文件到 [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群时，支持修改 TiDB Cloud 自动创建目标表的列名。
    ...
    若表头列名不符合 TiDB Cloud 命名规范，将在对应列名旁显示警告图标。你可悬停查看提示并按需修改或输入新列名。
...
- [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群新增 [SQL Diagnosis](/tidb-cloud/tune-performance.md) 功能。
    ...
    通过 SQL Diagnosis，你可深入了解 SQL 运行状态，提升 SQL 性能调优效率。目前仅提供慢查询数据。
...
- 新注册用户将自动创建一个免费的 [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群，便于快速开启数据探索之旅。
...
- 优化本地 CSV 文件导入 TiDB 的体验，提升 [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群的易用性。
    ...
    - 创建导入任务时，若目标数据库或表不存在，可输入名称自动创建。新建目标表时可指定主键或选择多个字段组成复合主键。
...
- [Serverless Tier](/tidb-cloud/select-cluster-tier.md#starter) 集群将 SQL Editor（beta）重命名为 Chat2Query（beta），并支持 AI 生成 SQL 查询。
  ...
  访问方法：在项目的 [**Clusters**](https://tidbcloud.com/project/clusters) 页面点击集群名，再点击左侧导航栏的 **Chat2Query**。
...
- 支持为 Prometheus 集成自定义 Grafana dashboard JSON。
    ...
    已集成 Prometheus 的集群可导入预置 Grafana dashboard 并自定义，便于快速监控和定位性能问题。
