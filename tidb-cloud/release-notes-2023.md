...
- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持 Google Cloud 上的双区域备份（beta）。
    ...
    托管于 Google Cloud 的 TiDB Cloud 专属集群可无缝对接 Google Cloud Storage。与 Google Cloud Storage 的 [Dual-regions](https://cloud.google.com/storage/docs/locations#location-dr) 功能类似，TiDB Cloud 专属集群的双区域需位于同一多区域内。例如，东京和大阪同属 `ASIA` 多区域，可共同用于双区域存储。
...
- 发布 [JavaScript 版 TiDB Cloud serverless driver（beta）](/tidb-cloud/serverless-driver.md)。
    ...
    JavaScript 版 TiDB Cloud serverless driver 支持通过 HTTPS 连接 [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群，适用于 TCP 连接受限的边缘环境，如 [Vercel Edge Function](https://vercel.com/docs/functions/edge-functions) 和 [Cloudflare Workers](https://workers.cloudflare.com/)。
...
- [数据服务（beta）](https://tidbcloud.com/project/data-service) 支持为每个 API key 自定义限流，以满足不同场景下的速率限制需求。
...
- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持 Google Cloud [Private Service Connect](https://cloud.google.com/vpc/docs/private-service-connect)。
    ...
    你现在可以为托管于 Google Cloud 的 TiDB Cloud 专属集群创建私有端点并建立安全连接。
...
- 支持通过 changefeed 将 [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的数据流式同步到 [Google Cloud Storage (GCS)](https://cloud.google.com/storage)。
    ...
    详细信息参见 [同步到云存储](/tidb-cloud/changefeed-sink-to-cloud-storage.md)。
...
- 禁用 2023 年 8 月 15 日后在 AWS 上新建的 [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的负载均衡优化，包括：
  ...
  此变更避免混合部署资源争用，不影响已启用该优化的现有集群。如需为新集群启用负载均衡优化，请联系 [TiDB Cloud 支持](/tidb-cloud/tidb-cloud-support.md)。
...
- [数据服务](https://tidbcloud.com/project/data-service) 支持 Data App 的 OpenAPI 规范。
...
- 支持在 [Postman](https://www.postman.com/) 中运行 Data App。
    ...
    Postman 集成支持将 Data App 的接口导入为集合到你的工作区，便于协作和 API 测试，支持 Postman 网页端和桌面端。
...
- 优化 TiDB Cloud 的导入功能，提升数据导入体验，具体包括：
    ...
    - Serverless 导入入口统一：整合本地文件和 Amazon S3 文件导入入口，便于切换。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群支持 [分支功能（beta）](/tidb-cloud/branch-overview.md)。
    ...
    如果你使用 GitHub 进行应用开发，可将分支集成到 GitHub CI/CD 流水线，实现自动测试 PR，避免影响生产库。详细信息参见 [与 GitHub 集成 TiDB Cloud Serverless 分支（Beta）](/tidb-cloud/branch-github-integration.md)。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群支持时间点恢复（PITR）（beta）。
    ...
    你可以将 Serverless 集群恢复到过去 90 天内的任意时间点，提升数据恢复能力。例如，数据写入出错时可用 PITR 恢复到早期状态。
...
- 优化整体导航体验，具体包括：
    ...
    - 将左侧导航栏 <svg ...></svg> **Admin** 合并到 <MDSvgIcon name="icon-left-projects" /> **Project**，并移除左上角 ☰ 悬浮菜单。现在可通过 <MDSvgIcon name="icon-left-projects" /> 切换项目及修改项目设置。
    - 将所有帮助与支持信息整合到右下角 **?** 菜单，包括文档、交互式教程、自学培训和支持入口。
...
- 推出 [TiDB Playground](https://play.tidbcloud.com/?utm_source=docs&utm_medium=tidb_cloud_release_notes)，无需注册或安装即可体验 TiDB 全功能的交互式平台。
    ...
    TiDB Playground 提供一站式体验，支持扩展性、MySQL 兼容性、实时分析等功能。

    你可在受控环境下实时试用 TiDB 功能，无需复杂配置，便于理解 TiDB 特性。

    立即体验，请访问 [**TiDB Playground**](https://play.tidbcloud.com/?utm_source=docs&utm_medium=tidb_cloud_release_notes) 页面，选择你想探索的功能开始体验。
...
- 增强 TiDB Cloud 数据迁移功能对增量数据迁移的支持。
    ...
    你现在可以指定 binlog 位置或全局事务标识（GTID），仅同步指定位置之后产生的增量数据到 TiDB Cloud。该增强为你按需选择和同步数据提供更大灵活性。
...
- 支持变更 2023 年 4 月 26 日后创建的 GCP 托管集群的节点规格。
...
- 支持直接访问 [Data Service endpoint](/tidb-cloud/tidb-cloud-glossary.md#endpoint) 所在区域的 TiDB [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless)。
    ...
    也可通过 `data.tidbcloud.com` 全局域名访问，系统会自动重定向到目标区域，但可能增加延迟。若采用此方式，调用接口时请确保 curl 命令加上 `--location-trusted`。
...
- 组织下前五个 [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 均享有免费额度：
  ...
  2023 年 5 月 31 日前，Serverless 集群仍免费。之后超出免费额度部分将计费。
...
- 提供维护窗口功能，便于你为 [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 安排和管理计划性维护。

    维护窗口为自动执行操作系统更新、安全补丁、基础设施升级等计划性维护的指定时间段，保障 TiDB Cloud 服务的可靠性、安全性和性能。

    维护期间可能出现短暂连接中断或 QPS 波动，但集群始终可用，SQL 操作、数据导入、备份、恢复、迁移、同步等任务可正常运行。维护期间允许与禁止的操作详见 [维护窗口操作列表](/tidb-cloud/configure-maintenance-window.md#allowed-and-disallowed-operations-during-a-maintenance-window)。
...
- 优化 AWS 上新建的 [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 TiDB 负载均衡，缩容/扩容时减少连接中断：

    - 扩容时，支持自动迁移现有连接到新 TiDB 节点。
    - 缩容时，支持自动迁移现有连接到可用 TiDB 节点。

  目前该功能适用于所有 AWS 托管的专属集群。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持扩缩容 [数据迁移作业规格](/tidb-cloud/tidb-cloud-billing-dm.md#specifications-for-data-migration)。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持集成 [New Relic](https://newrelic.com/)。

    你可将 TiDB 集群的监控数据发送到 [New Relic](https://newrelic.com/)，实现应用与数据库的统一监控和分析，便于快速定位和解决问题。
...
- 为避免误报，从 [TiDB Cloud 内置告警](/tidb-cloud/monitor-built-in-alerting.md#tidb-cloud-built-in-alert-conditions) 移除以下两项告警。因单节点临时离线或 OOM 不会显著影响集群整体健康。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 新增 [Alerts](/tidb-cloud/monitor-built-in-alerting.md) 页面，展示每个集群的活跃和已关闭告警。
    ...
    **Alerts** 页面提供：
    ...
    - 直观友好的界面，未订阅邮件也可查看告警。
    - 高级筛选，按严重性、状态等属性快速查找和排序，并支持 7 天历史数据，便于追溯。
    - **Edit Rule** 功能，可自定义告警规则。
...
- 推出 [Getting Started](https://tidbcloud.com/getting-started) 页面，帮助你快速了解 TiDB Cloud。
    ...
    **Getting Started** 页面提供交互式教程、基础指南和实用链接。通过交互式教程，你可轻松体验 TiDB Cloud 功能和 HTAP 能力，内置行业数据集（Steam Game、S&P 500）。

    访问方式：在 [TiDB Cloud 控制台](https://tidbcloud.com/) 左侧导航栏点击 <svg ...></svg> **Getting Started**。可点击 **Query Sample Dataset** 进入交互式教程，或点击其他链接探索 TiDB Cloud。也可在右下角 **?** 菜单点击 **Interactive Tutorials**。
...
- 支持对 2023 年 3 月 28 日后创建的 [changefeed](/tidb-cloud/changefeed-overview.md) 扩缩容。

    你可通过选择更高规格提升同步性能，或选择更低规格降低成本。

    详细信息参见 [扩缩容 changefeed](/tidb-cloud/changefeed-overview.md#scale-a-changefeed)。
...
- 支持将 AWS 上 [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的增量数据实时同步到同项目同区域的 [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless)。
    ...
    详细信息参见 [同步到 TiDB Cloud](/tidb-cloud/changefeed-sink-to-tidb-cloud.md)。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 推出 [数据服务（beta）](https://tidbcloud.com/project/data-service)，支持通过自定义 API endpoint 以 HTTPS 请求访问数据。
    ...
    访问方式：左侧导航栏进入 [**Data Service**](https://tidbcloud.com/project/data-service) 页面。详细文档：
...
- 支持对 2022 年 12 月 31 日后 AWS 托管的 [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 缩小 TiDB、TiKV、TiFlash 节点规格。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 [数据迁移](/tidb-cloud/migrate-from-mysql-using-data-migration.md) 功能新增 GCP 区域：`Tokyo (asia-northeast1)`。

    该功能便于将 GCP 上 MySQL 兼容数据库迁移至 TiDB 集群。
...
- 支持在上传带表头的本地 CSV 文件时，修改 TiDB Cloud 自动创建目标表的列名。
    ...
    导入带表头的本地 CSV 文件到 [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 时，若表头列名不符合 TiDB Cloud 命名规范，将在对应列名旁显示警告图标。你可悬停查看提示并编辑列名。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 新增 [SQL Diagnosis](/tidb-cloud/tune-performance.md) 功能。
    ...
    SQL Diagnosis 可深入洞察 SQL 运行状态，提升 SQL 性能调优效率。目前 Serverless 集群仅提供慢查询数据。
...
- 支持使用 IAM 用户的 AWS 访问密钥导入数据到 TiDB Cloud 时访问 Amazon S3 存储桶。
...
- 支持缩减 TiKV 和 TiFlash 节点数量，实现 [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的集群缩容。
...
- 推出 TiDB Cloud CLI 客户端 [`ticloud`](/tidb-cloud/cli-reference.md)。
    ...
    详细信息参见 [TiDB Cloud CLI 快速上手](/tidb-cloud/get-started-with-cli.md) 及 [TiDB Cloud CLI 参考](/tidb-cloud/cli-reference.md)。
...
- 简化 [**Clusters**](https://tidbcloud.com/project/clusters) 页面和集群概览页展示：
    ...
    - 点击 [**Clusters**](https://tidbcloud.com/project/clusters) 页的集群名可进入集群概览页并开始操作。
