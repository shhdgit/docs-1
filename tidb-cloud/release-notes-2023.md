...
- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持从备份中恢复 SQL 绑定。
    ...
    现在，TiDB Cloud 专属集群在从备份恢复时默认恢复用户账号和 SQL 绑定。该增强适用于 v6.2.0 及以上版本的集群，简化了数据恢复流程。SQL 绑定的恢复确保查询相关配置和优化的顺利回归，为你提供更全面、高效的恢复体验。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 支持监控 SQL 语句的 RU 消耗。
    ...
    现在，TiDB Cloud Serverless 提供每条 SQL 语句的 [Request Units (RUs)](/tidb-cloud/tidb-cloud-glossary.md#request-unit) 详细信息。你可以查看每条 SQL 语句的 **Total RU** 和 **Mean RU** 消耗。该功能有助于你识别和分析 RU 消耗，为运维成本优化提供参考。
...
- 从 TiDB Cloud 专属集群恢复数据时，默认行为由“仅恢复数据不包含用户账号”调整为“恢复所有用户账号”。
...
- 为 [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群引入 **Events** 页面，记录集群主要变更。
    ...
    你可以在该页面查看最近 7 天的事件历史，追踪触发时间、操作用户等重要信息。
...
- 对 2023 年 8 月 15 日后在 AWS 上创建的 [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 禁用负载均衡改进，包括：

    - 禁用 TiDB 节点扩容时自动迁移现有连接到新节点。
    - 禁用 TiDB 节点缩容时自动迁移现有连接到可用节点。

  此变更避免混合部署资源争用，不影响已启用该功能的现有集群。如需为新集群启用负载均衡改进，请联系 [TiDB Cloud 支持](/tidb-cloud/tidb-cloud-support.md)。
...
- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 新增 **Pausing** 状态，暂停期间无需付费。
    ...
    只有在状态变为 **Paused** 后才能恢复集群，解决了因频繁点击 **Pause** 和 **Resume** 导致的异常恢复问题。
...
- TiDB Cloud [数据服务](https://tidbcloud.com/project/data-service) 引入自动生成接口功能。
...
- 优化 TiDB Cloud 的导入功能，提升数据导入体验，具体包括：

    - Serverless 集群统一导入入口：本地文件与 Amazon S3 文件导入入口合并，便于切换。
    - 配置简化：Amazon S3 文件导入仅需一步配置，节省时间。
    - CSV 配置增强：CSV 配置项移至文件类型选项下，便于快速设置参数。
    - 目标表选择优化：支持通过勾选选择目标表，无需复杂表达式，简化操作。
    - 展示信息优化：修复导入过程中的信息不准确问题，移除预览功能，避免数据不全和误导。
    - 源文件映射改进：支持自定义源文件与目标表的映射关系，解决需修改文件名的难题。
...
- 推出 TiDB Bot（Beta），基于 OpenAI 的智能聊天机器人，支持多语言、7x24 实时响应、集成文档访问。
    ...
    - 提高效率：自动回复减少等待，提升运维效率。
    - 无缝文档访问：可直接访问 TiDB Cloud 文档，便于信息检索和问题解决。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群支持 [分支功能（Beta）](/tidb-cloud/branch-overview.md)。
    ...
    若你使用 GitHub 进行应用开发，可将分支集成到 CI/CD 流程，实现 PR 自动测试且不影响生产库。详情参见 [与 GitHub 集成 TiDB Cloud Serverless 分支（Beta）](/tidb-cloud/branch-github-integration.md)。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 集群支持时间点恢复（PITR）（Beta）。
    ...
    你可以将 Serverless 集群恢复到过去 90 天内任意时间点，提升数据恢复能力。例如，数据写入出错时可用 PITR 恢复到早期状态。
...
- 优化整体导航体验，具体包括：
    ...
    - 将所有帮助与支持信息整合到右下角 **?** 菜单，包括文档、交互式教程、自助培训和支持入口。
...
- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 [**Key Visualizer**](/tidb-cloud/tune-performance.md#key-visualizer) 页面发布全新原生 Web 架构。
    ...
    新架构提升了页面导航和信息获取的直观性和效率，优化了用户体验，使 SQL 诊断过程更友好。
...
- [TiDB Cloud 专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 引入 Index Insight（Beta），为慢查询提供索引推荐，优化查询性能。
  ...
  使用方法：进入专属集群 **Diagnosis** 页面，点击 **Index Insight BETA** 标签。
...
- 支持将 [Data App](/tidb-cloud/tidb-cloud-glossary.md#data-app) 连接到 GitHub。
    ...
    [连接 Data App 到 GitHub](/tidb-cloud/data-service-manage-github-connection.md) 后，你可将所有配置作为 [代码文件](/tidb-cloud/data-service-app-config-files.md) 管理，实现与系统架构和 DevOps 流程的无缝集成。
...
- 增强 TiDB Cloud 数据迁移功能对增量数据迁移的支持。
    ...
    你现在可以指定 binlog 位置或全局事务标识（GTID），仅同步指定位置之后产生的增量数据到 TiDB Cloud。该增强为你提供更灵活的数据选择与同步能力，满足个性化需求。
...
- 上传 CSV 文件到 TiDB 时，除英文和数字外，还可使用中文、日文等字符定义列名。但特殊字符仅支持下划线（`_`）。
...
- 支持导入压缩文件。可导入 `.gzip`、`.gz`、`.zstd`、`.zst`、`.snappy` 格式的 CSV 和 SQL 文件，提升数据导入效率，降低传输成本。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 的 [**SQL Diagnosis**](/tidb-cloud/tune-performance.md) 页面新增 **SQL Statement** 标签页。
    ...
    **SQL Statement** 标签页提供：
    ...
    - TiDB 数据库所有 SQL 语句的全面概览，便于识别和诊断慢查询。
...
- 支持直接访问 TiDB [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 所在区域的 [Data Service endpoint](/tidb-cloud/tidb-cloud-glossary.md#endpoint)。
    ...
    也可通过 `data.tidbcloud.com` 全局域名访问，系统会自动重定向到目标区域，但可能增加延迟。若采用此方式，调用接口时需为 curl 命令添加 `--location-trusted` 选项。
...
- 组织下前五个 [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 均享有免费额度：
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 提供维护窗口功能，便于你安排和管理计划性维护。
    ...
    维护期间可能出现临时连接中断或 QPS 波动，但集群保持可用，SQL 操作、数据导入、备份、恢复、迁移、同步等任务可正常运行。详见 [维护期间允许与禁止的操作](/tidb-cloud/configure-maintenance-window.md#allowed-and-disallowed-operations-during-a-maintenance-window)。
...
- 优化 AWS 上新建 [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 TiDB 负载均衡，缩放 TiDB 节点时减少连接中断。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 [Monitoring](/tidb-cloud/built-in-monitoring.md#view-the-metrics-page) 页面发布全新原生 Web 架构。
    ...
    新架构提升了页面导航和信息获取的直观性和效率，优化了用户体验，使监控过程更友好。
...
- 优化 [集群创建](https://tidbcloud.com/clusters/create-cluster) UI，提升易用性，几步即可创建和配置集群。
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 支持 [New Relic](https://newrelic.com/) 集成。
    ...
    你可将 TiDB 集群的监控数据发送至 [New Relic](https://newrelic.com/)，实现应用与数据库性能的统一监控和分析，便于快速定位和解决问题。
...
- 为专属集群的 Prometheus 集成新增以下 [changefeed](/tidb-cloud/changefeed-overview.md) 指标：
    ...
    已 [集成 Prometheus](/tidb-cloud/monitor-prometheus-and-grafana-integration.md) 的用户可实时监控 changefeed 性能和健康状况，并可基于这些指标创建告警。

**控制台变更**

- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 的 [Monitoring](/tidb-cloud/built-in-monitoring.md#view-the-metrics-page) 页面现采用 [节点级资源指标](/tidb-cloud/built-in-monitoring.md#server)。
...
- 将 TiDB Cloud 的帮助相关信息和操作整合至同一入口。
    ...
    现在可通过 [TiDB Cloud 控制台](https://tidbcloud.com/) 右下角 **?** 获取所有 [TiDB Cloud 帮助信息](/tidb-cloud/tidb-cloud-support.md) 并联系支持。

- 推出 [Getting Started](https://tidbcloud.com/getting-started) 页面，帮助你快速了解 TiDB Cloud。

    **Getting Started** 页面提供交互式教程、基础指南和实用链接。通过交互式教程，你可轻松体验 TiDB Cloud 功能和 HTAP 能力，内置行业数据集（Steam 游戏、S&P 500）。

    访问方式：在 [TiDB Cloud 控制台](https://tidbcloud.com/) 左侧导航栏点击 <svg ...></svg> **Getting Started**。可点击 **Query Sample Dataset** 打开交互式教程，或点击其他链接探索 TiDB Cloud。也可通过右下角 **?** 进入 **Interactive Tutorials**。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 推出 [数据服务（Beta）](https://tidbcloud.com/project/data-service)，支持通过自定义 API endpoint 以 HTTPS 请求访问数据。
    ...
    - [数据服务快速上手](/tidb-cloud/data-service-get-started.md)
    - [Chat2Query API 快速上手](/tidb-cloud/use-chat2query-api.md)
...
- [专属集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 新增 **Events** 页面，记录集群主要变更。
    ...
    你可查看最近 7 天的事件历史，追踪触发时间、操作用户等重要信息。例如，查看集群暂停、规格变更等事件。
...
- 上传带表头的本地 CSV 文件到 [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 时，支持修改 TiDB Cloud 自动创建目标表的列名。
    ...
    若表头列名不符合 TiDB Cloud 命名规范，将在对应列名旁显示警告图标。你可悬停查看提示并按需编辑或输入新列名。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 新增 [SQL Diagnosis](/tidb-cloud/tune-performance.md) 功能。
    ...
    通过 SQL Diagnosis 可深入了解 SQL 运行状态，提升 SQL 性能调优效率。目前仅提供慢查询数据。
...
- 支持使用 [控制台审计日志](/tidb-cloud/tidb-cloud-console-auditing.md) 跟踪组织成员在 [TiDB Cloud 控制台](https://tidbcloud.com/) 的各类操作。
    ...
    仅 `Owner` 或 `Audit Admin` 角色可见，默认关闭。开启方式：在 [TiDB Cloud 控制台](https://tidbcloud.com/) 右上角点击 <MDSvgIcon name="icon-top-organization" /> **Organization** > **Console Audit Logging**。
...
- [TiDB Cloud CLI](/tidb-cloud/cli-reference.md) 新增 `ticloud cluster connect-info` 命令。
    ...
    该命令用于获取集群连接字符串。需将 `ticloud` 升级至 v0.3.2 或更高版本，升级方法见 [更新 ticloud](/tidb-cloud/ticloud-upgrade.md)。

## 2023 年 2 月 21 日

**通用变更**

- 导入数据到 TiDB Cloud 时，支持使用 IAM 用户的 AWS 访问密钥访问 Amazon S3 存储桶。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 新增 **Monitoring** 页面。
    ...
    该页面提供 SQL 每秒执行数、平均查询时长、失败查询数等多项指标，帮助你全面了解 Serverless 集群 SQL 性能。
...
- 发布 TiDB Cloud CLI 客户端 [`ticloud`](/tidb-cloud/cli-reference.md)。
    ...
    通过 `ticloud`，你可在终端或自动化流程中便捷管理 TiDB Cloud 资源。针对 GitHub Actions，已提供 [`setup-tidbcloud-cli`](https://github.com/marketplace/actions/set-up-tidbcloud-cli) 便捷集成。

    详情参见 [TiDB Cloud CLI 快速上手](/tidb-cloud/get-started-with-cli.md) 及 [TiDB Cloud CLI 参考](/tidb-cloud/cli-reference.md)。
...
- [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 的 SQL Editor（Beta）更名为 Chat2Query（Beta），并支持 AI 生成 SQL 查询。
  ...
  在 Chat2Query 中，你可让 AI 自动生成 SQL 查询，也可手动编写 SQL，并直接在数据库上运行，无需终端。
...
- 支持为 Prometheus 集成自定义 Grafana dashboard JSON。
    ...
    已 [集成 Prometheus](/tidb-cloud/monitor-prometheus-and-grafana-integration.md) 的用户可导入预置 Grafana dashboard 并自定义，便捷监控 TiDB Cloud 集群，快速发现性能问题。
...
- 所有 [Serverless 集群](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 的默认 TiDB 版本由 [v6.3.0](https://docs.pingcap.com/tidb/v6.3/release-6.3.0) 升级至 [v6.4.0](https://docs.pingcap.com/tidb/v6.4/release-6.4.0)。升级后已解决 Serverless 集群 v6.4.0 的冷启动问题。
