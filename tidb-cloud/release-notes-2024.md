...
- 为部署在 AWS 上的 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群引入灾备恢复组（Recovery Group）功能（测试版）。
    ...
    更多信息，参见 [开始使用恢复组](/tidb-cloud/recovery-group-get-started.md)。
...
- [TiDB Cloud Serverless 分支（测试版）](/tidb-cloud/branch-overview.md)对分支管理引入以下改进：
  ...
  更多信息，参见 [管理 TiDB Cloud Serverless 分支](/tidb-cloud/branch-manage.md) 和 [将 TiDB Cloud Serverless 分支（测试版）集成到 GitHub](/tidb-cloud/branch-github-integration.md)。
...
- [Datadog 集成（测试版）](/tidb-cloud/monitor-datadog-integration.md) 新增对 `AP1`（日本）区域的支持。
...
- 新增指标：为 Prometheus 集成添加 `tidbcloud_changefeed_checkpoint_ts`。
    ...
    该指标用于追踪 changefeed 的检查点时间戳，表示已成功写入下游的最大 TSO（时间戳 Oracle）。更多可用指标信息，参见 [将 TiDB Cloud 集成到 Prometheus 和 Grafana（测试版）](/tidb-cloud/monitor-prometheus-and-grafana-integration.md#metrics-available-to-prometheus)。
...
- 优化 [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群的连接体验。
    ...
    - 调整 **Connect** 对话框界面，为 TiDB Cloud Dedicated 用户提供更简洁高效的连接体验。
    - 新增集群级别的 **Networking** 页面，简化集群的网络配置。
...
- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 在 AWS 上的负载均衡计费变更。
    ...
    自 2024 年 8 月 1 日起，TiDB Cloud Dedicated 账单将包含新的 AWS 公网 IPv4 地址费用，与 [2024 年 2 月 1 日起生效的 AWS 价格变更](https://aws.amazon.com/blogs/aws/new-aws-public-ipv4-address-charge-public-ip-insights/) 保持一致。每个公网 IPv4 地址的费用为 $0.005/小时，每个托管在 AWS 上的 TiDB Cloud Dedicated 集群每月约 $10。
...
- [数据服务（测试版）](https://tidbcloud.com/project/data-service) 支持自动生成向量检索端点。

    如果你的表包含 [向量数据类型](/vector-search/vector-search-data-types.md)，你可以自动生成基于所选距离函数计算向量距离的向量检索端点。

    该功能可与 [Dify](https://docs.dify.ai/guides/tools) 和 [GPTs](https://openai.com/blog/introducing-gpts) 等 AI 平台无缝集成，为你的应用带来更复杂的自然语言处理和 AI 能力，助力智能解决方案。
...
- [数据服务（测试版）](https://tidbcloud.com/project/data-service) 提供端点库，内置预定义系统端点，可直接添加到你的数据应用中，减少端点开发工作量。

    当前库中仅包含 `/system/query` 端点，你可以通过传递预定义的 `sql` 参数直接执行任意 SQL 语句。该端点便于即时执行 SQL 查询，提升灵活性和效率。
...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 支持向量检索（测试版）。

    向量检索（测试版）功能为文档、图片、音频和视频等多种数据类型提供语义相似性检索的高级解决方案。该功能让开发者可以用熟悉的 MySQL 技能轻松构建具备生成式人工智能（AI）能力的可扩展应用。主要特性包括：
  ...
  更多信息，参见 [向量检索（测试版）概览](/vector-search/vector-search-overview.md)。

- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 现为组织所有者提供每周邮件报告。

    这些报告可帮助你了解集群的性能和活动。通过自动每周更新，你可以及时掌握集群动态，基于数据做出优化决策。

- 发布 Chat2Query API v3 端点，并弃用 Chat2Query API v1 端点 `/v1/chat2data`。

    通过 Chat2Query API v3 端点，你可以基于会话开启多轮 Chat2Query。

    更多信息，参见 [开始使用 Chat2Query API](/tidb-cloud/use-chat2query-api.md)。

**控制台变更**

- 将 Chat2Query（测试版）重命名为 SQL Editor（测试版）。
...
- 为 [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 列式存储 [TiFlash](/tiflash/tiflash-overview.md) 引入计费与计量（测试版）。

    截至 2024 年 6 月 30 日，TiDB Cloud Serverless 集群中的列式存储仍享受 100% 折扣免费。此日期后，每个 TiDB Cloud Serverless 集群将包含 5 GiB 列式存储免费额度，超出部分将按量计费。
...
- [数据服务（测试版）](https://tidbcloud.com/project/data-service) 支持路径参数与查询参数。
...
- [TiDB Cloud Dedicated](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 为 [节点级资源指标](/tidb-cloud/built-in-monitoring.md#server) 显示 **limit** 标签。
    ...
    **limit** 标签展示了集群中各组件的 CPU、内存和存储等资源最大使用量。该增强简化了集群资源使用率的监控流程。
...
- 支持在 Google Cloud 上创建更多 TiDB Cloud 节点。
    ...
    - 如果你希望在项目的任意区域内创建超过 124 个节点，可联系 [TiDB Cloud 支持](/tidb-cloud/tidb-cloud-support.md) 协助自定义 `/16` 到 `/18` 的 IP 范围大小。
...
- [数据服务（测试版）](https://tidbcloud.com/project/data-service) 支持为 Data App 配置自定义域名访问端点。
...
- 支持 [组织 SSO](https://tidbcloud.com/org-settings/authentication)，简化企业认证流程。
    ...
    通过该功能，你可以使用 [安全断言标记语言（SAML）](https://en.wikipedia.org/wiki/Security_Assertion_Markup_Language) 或 [OpenID Connect（OIDC）](https://openid.net/developers/how-connect-works/) 将 TiDB Cloud 与任意身份提供商（IdP）无缝集成。
