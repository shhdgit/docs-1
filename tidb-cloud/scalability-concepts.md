...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 非常适合原型开发、开发环境和早期阶段的工作负载。它为你提供了一种简化且具备成本效益的方式来快速使用 TiDB Cloud，并内置了自动扩缩容功能。
...
> **Tip:**
>
> 关于如何确定 TiDB Cloud Dedicated 集群的规格，请参见 [Determine Your TiDB Size](/tidb-cloud/size-your-cluster.md)。
...
TiKV 负责存储行存数据。你可以为 TiKV 配置节点数量、vCPU、内存（RAM）和存储空间。TiKV 节点数量至少为 1 组（即 3 个节点，分布在 3 个不同的可用区），并且每次扩容需增加 3 个节点。
