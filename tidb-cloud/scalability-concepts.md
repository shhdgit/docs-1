...
- [TiDB Cloud Serverless](/tidb-cloud/select-cluster-tier.md#tidb-cloud-serverless) 非常适合原型开发、开发环境和早期阶段的工作负载。它为你提供了一种简化且具备成本效益的方式来快速开始使用 TiDB Cloud，并内置了自动扩缩容功能。
- [TiDB Cloud Essential](/tidb-cloud/select-cluster-tier.md#essential) 适用于需要更强可扩展性和在流量或数据量增长时具备可预测性能的生产级工作负载。
...
- 水平扩展是指向你的专属集群中添加节点，以分担工作负载。
- 垂直扩展是指提升你的专属集群的 vCPU 和内存（RAM）配置。
...
TiKV 负责存储行存数据。你可以为 TiKV 配置节点数量、vCPU、内存（RAM）以及存储空间。TiKV 节点数量至少为 1 组（即 3 个节点，分布在 3 个不同的可用区），并以 3 个节点为单位进行扩展。
...
TiFlash 负责存储列存数据。TiFlash 会实时从 TiKV 同步数据，并原生支持实时分析型工作负载。你可以为 TiFlash 配置节点数量、vCPU、内存（RAM）以及存储空间。

TiDB Cloud 会将 TiFlash 节点均匀部署在一个区域内的不同可用区。建议你在每个 TiDB Cloud 集群中至少配置 2 个 TiFlash 节点，并为生产环境中的数据创建至少 2 个副本，以实现高可用性。