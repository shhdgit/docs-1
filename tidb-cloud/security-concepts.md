...
TiDB Cloud 提供了一个强大且灵活的安全框架，旨在保护数据、实施访问控制，并满足现代合规标准。该框架将先进的安全特性与高效的运维相结合，以支持大规模组织的需求。
...
TiDB Cloud 用户账户是管理资源身份与访问的基础。每个账户代表平台内的个人或实体，并支持多种认证方式以适应组织需求：
...
**Best practices:** 生产环境建议使用私有终端节点以最小化公网暴露，并定期检查配置。
...
**Best practices:** 建议关键业务使用，避免公网暴露，并监控性能。
...
- 对于未启用 CMEK 的 TiDB Cloud Dedicated 集群，TiDB Cloud 使用托管密钥；{{{ .starter }}} 和 {{{ .essential }}} 集群仅使用托管密钥。
