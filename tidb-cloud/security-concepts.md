...
TiDB Cloud 提供了强大且灵活的安全框架，旨在保护数据、实施访问控制，并满足现代合规标准。该框架将先进的安全特性与高效的运维相结合，以支持大规模组织的需求。
...
- **Token-based:** SQL Proxy 账户使用安全的 JWT token 替代密码，确保通过 TiDB Cloud Data Service 或 SQL Editor 实现无缝且受限的访问。
...
- 使你在虚拟私有云 (VPC) 内的 SQL 客户端能够安全连接到 TiDB Cloud Dedicated 集群。
