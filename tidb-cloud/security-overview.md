...
TiDB Cloud 提供私有终端节点、VPC Peering 以及 IP 访问列表，用于网络隔离和访问控制。

你可以通过 TLS 加密所有通信，确保数据在传输过程中的机密性和完整性。网络访问控制确保只有授权来源才能访问集群资源，从而提升整体安全性。

## 数据访问控制

对于支持客户自管加密密钥（CMEK）的集群类型，TiDB Cloud 为静态数据和备份数据都提供加密保护。
...
TiDB Cloud 提供基于用户和角色的访问控制机制，结合静态和动态权限。你可以为用户分配角色，以更细粒度地管理和分配权限。
...
TiDB Cloud 针对控制台和数据库操作均提供审计日志，以支持活动追踪、合规监控和安全事件调查。
