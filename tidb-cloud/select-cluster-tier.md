...
TiDB Cloud Starter 是一款全托管的多租户 TiDB 服务。它提供了一个即时、自动弹性扩缩的 MySQL 兼容数据库，并在超出免费额度后，提供基于消耗的计费方式。
...
对于 TiDB Cloud 中的每个组织，默认最多可以创建 5 个免费的 TiDB Cloud Serverless 集群。若需创建更多 TiDB Cloud Serverless 集群，你需要添加信用卡并指定消费上限。
...
对于工作负载不断增长、需要实时扩展的应用，Essential 集群方案通过以下功能为你的业务增长提供灵活性和性能保障：

- **增强能力**：包含 Starter 方案的所有功能，并具备处理更大、更复杂工作负载的能力，以及高级安全特性。
- **自动扩缩**：自动调整存储和计算资源，高效应对不断变化的工作负载需求。
- **高可用性**：内置容错和冗余机制，确保你的应用即使在基础设施故障时也能保持可用和弹性。
- **可预测的定价**：基于存储和计算资源的 Request Capacity Units (RCUs) 计费，提供透明、按用量计费的定价方式，随需扩展，让你只为实际使用的资源付费，无额外意外支出。
...
- 连接到你的集群：
    ...
    > **注意：**
    >
    > TiDB Cloud Serverless 和 TiDB Cloud Essential 要求使用 TLS 连接。要查找你系统上的 CA 根证书路径，请参见 [Root certificate default path](/tidb-cloud/secure-connections-to-serverless-clusters.md#root-certificate-default-path)。
...
TiDB Cloud Dedicated 适用于生产环境，具备跨可用区高可用性、水平扩展能力以及 [HTAP](https://en.wikipedia.org/wiki/Hybrid_transactional/analytical_processing) 优势。
