...
[Kysely](https://kysely.dev/docs/intro) 是一个类型安全且支持自动补全的 TypeScript SQL 查询构建器。TiDB Cloud 提供了 [@tidbcloud/kysely](https://github.com/tidbcloud/kysely)，使你能够通过 [TiDB Cloud serverless driver](/tidb-cloud/serverless-driver.md) 在 HTTPS 上使用 Kysely。与传统的 TCP 方式相比，[ @tidbcloud/kysely ](https://github.com/tidbcloud/kysely) 带来了以下优势：
...
3. 在项目根目录下，找到 `package.json` 文件，并通过添加 `"type": "module"` 指定为 ES 模块：
...
1. 在 TiDB Cloud Serverless 集群的概览页面，点击右上角的 **Connect**，然后在弹出的对话框中获取数据库的连接字符串。连接字符串格式如下：
...
1. 在 TiDB Cloud Serverless 集群中创建数据表并插入一些数据。

    你可以使用 [TiDB Cloud 控制台中的 SQL Editor](/tidb-cloud/explore-data-with-chat2query.md) 执行 SQL 语句。以下为示例：
...
在 TiDB Cloud Serverless 集群的概览页面，点击右上角的 **Connect**，然后在弹出的对话框中获取数据库的连接字符串。连接字符串格式如下：
...
2. 访问 `${Your-URL}/api/edge-function-example` 页面获取该路由的响应。
