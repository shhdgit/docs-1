...
> **Tip:**
>
> - 如果你想了解如何通过 AWS 的私有终端节点连接 TiDB Cloud 专属集群，请参见 [Connect to a TiDB Cloud Dedicated Cluster via AWS PrivateLink](/tidb-cloud/set-up-private-endpoint-connections.md)。
> - 如果你想了解如何通过 Google Cloud 的私有终端节点连接 TiDB Cloud 专属集群，请参见 [Connect to a TiDB Cloud Dedicated Cluster via Google Cloud Private Service Connect](/tidb-cloud/set-up-private-endpoint-connections-on-google-cloud.md)
> - 如果你想了解如何通过私有终端节点连接 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群，请参见以下文档：
>     - [Connect to TiDB Cloud Serverless or Essential via AWS PrivateLink](/tidb-cloud/set-up-private-endpoint-connections-serverless.md)
...
TiDB Cloud 支持通过 [Azure Private Link](https://learn.microsoft.com/en-us/azure/private-link/private-link-overview) 在 Azure 虚拟网络中对托管的 TiDB Cloud 服务进行高度安全的单向访问，就像服务部署在你自己的虚拟网络中一样。你可以在自己的虚拟网络中创建一个私有终端节点，然后通过该终端节点并获得权限后连接 TiDB Cloud 服务。
...
关于私有终端节点和终端节点服务的详细定义，请参见以下 Azure 文档：
...
1. 在 **Create Azure Private Endpoint Connection** 对话框中，复制私有链路服务的 TiDB Cloud 资源 ID，并保持对话框打开以便后续使用。
    ...
    > **Note:**
    >
    > 对于每个 TiDB Cloud 专属集群，系统会在集群创建后 3 到 4 分钟自动创建对应的终端节点服务。
...
- **Discovered**：TiDB Cloud 可以在你接受请求前自动检测到与你的终端节点服务关联的私有终端节点，避免重复创建。
- **Pending**：等待处理。
...
在你打开 **Create Azure Private Endpoint** 页面并选择 TiDB 集群后，终端节点服务会自动创建。如果显示为失败或长时间处于 **Creating** 状态，请提交 [support ticket](/tidb-cloud/tidb-cloud-support.md) 获取帮助。
