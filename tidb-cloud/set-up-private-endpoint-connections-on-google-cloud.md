...
> **提示：**
>
> - 如果你想了解如何通过 AWS 的私有端点连接 TiDB Cloud 专属集群，请参见 [通过 AWS PrivateLink 连接 TiDB Cloud 专属集群](/tidb-cloud/set-up-private-endpoint-connections.md)。
> - 如果你想了解如何通过 Azure 的私有端点连接 TiDB Cloud 专属集群，请参见 [通过 Azure Private Link 连接 TiDB Cloud 专属集群](/tidb-cloud/set-up-private-endpoint-connections-on-azure.md)。
> - 如果你想了解如何通过私有端点连接 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群，请参见以下文档：
>     - [通过 AWS PrivateLink 连接 TiDB Cloud Serverless](/tidb-cloud/set-up-private-endpoint-connections-serverless.md)
>     - [通过阿里云私有端点连接 TiDB Cloud Serverless 或 Essential](/tidb-cloud/set-up-private-endpoint-connections-on-alibaba-cloud.md)
...
TiDB Cloud 支持通过 [Private Service Connect](https://cloud.google.com/vpc/docs/private-service-connect) 实现对托管在 Google Cloud VPC 中的 TiDB Cloud 服务的高度安全、单向访问。你可以创建一个端点，并使用它连接到 TiDB Cloud 服务。

借助 Google Cloud Private Service Connect，端点连接是安全且私有的，不会将你的数据暴露在公网上。此外，端点连接支持 CIDR 重叠，便于网络管理。
...
如果你有多个集群，需要对每个希望通过 Google Cloud Private Service Connect 连接的集群重复以上步骤。
...
- 准备好具备创建端点所需权限的 [IAM 角色](https://cloud.google.com/iam/docs/understanding-roles)。
    ...
    - 任务：
        - 创建端点
        - 自动或手动为端点配置 [DNS 条目](https://cloud.google.com/vpc/docs/configure-private-service-connect-services#dns-endpoint)
    - 所需 IAM 角色：
...
- **Deleting**：私有端点正在被删除。
- **Failed**：私有端点创建失败。你可以点击该行的 **Edit** 重新尝试创建。
...
已取消操作的未保存草稿不会被保留或显示。下次在 TiDB Cloud 控制台创建新的私有端点时，需要重新执行每一步。
