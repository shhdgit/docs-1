...
> - 如果你想了解如何通过私有端点连接 TiDB Cloud Starter 或 TiDB Cloud Essential 集群，请参见以下文档：
...
借助 Google Cloud Private Service Connect，端点连接安全且私密，不会将你的数据暴露在公网。此外，端点连接支持 CIDR 重叠，便于网络管理。
...
- 自 2025 年 8 月 12 日起，你可以为 Google Cloud 上的 TiDB Cloud 专属集群在每个区域创建的 Google Private Service Connect (PSC) 连接数上限取决于 NAT 子网 CIDR 块的大小：
...
- 你正在使用 [TiCDC](https://docs.pingcap.com/tidb/stable/ticdc-overview) 集群将数据从源 TiDB 集群跨区域同步到目标 TiDB 集群，以实现高可用。目前，私有端点不支持跨区域连接。
...
- 准备以下 [IAM 角色](https://cloud.google.com/iam/docs/understanding-roles)，以获得创建端点所需的权限。
...
1. 提供以下信息以生成创建私有端点的命令：
    ...
    - **Google Cloud VPC Name**：指定项目中 VPC 的名称。你可以在 [Google Cloud **VPC networks** 页面](https://console.cloud.google.com/networking/networks/list)找到。
    - **Google Cloud Subnet Name**：指定 VPC 中子网的名称。你可以在 **VPC network details** 页面找到。
...
2. 在 **Connect With** 下拉列表中选择你偏好的连接方式。对话框底部会显示相应的连接字符串。
3. 使用该连接字符串连接到你的集群。
...
要排查该问题，你需要查看在 Google Cloud Shell 执行私有端点创建命令后返回的错误信息。如果是权限相关错误，必须授予所需权限后重试。
...
而直接复制服务附件（即不是通过 TiDB Cloud 控制台生成命令创建的端点）生成的端点不会在 TiDB Cloud 控制台显示。
