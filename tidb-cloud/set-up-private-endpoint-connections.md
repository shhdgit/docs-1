...
> **提示：**
>
> - 如果你想了解如何通过 AWS PrivateLink 连接到 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群，请参见 [通过 AWS PrivateLink 连接到 TiDB Cloud Serverless 或 Essential](/tidb-cloud/set-up-private-endpoint-connections-serverless.md)。
...
TiDB Cloud 支持通过 [AWS PrivateLink](https://aws.amazon.com/privatelink) 实现对托管在 AWS VPC 中的 TiDB Cloud 服务的高度安全且单向的访问，就像服务部署在你自己的 VPC 中一样。你的 VPC 中会暴露一个私有终端节点，你可以通过该终端节点并具备权限后连接到 TiDB Cloud 服务。
...
> **提示：**
>
> - 在运行命令前，你需要安装并配置好 AWS CLI。详见 [AWS CLI 配置基础](https://docs.aws.amazon.com/cli/latest/userguide/cli-configure-quickstart.html)。
>
> - 如果你的服务跨越了三个以上的可用区（AZ），你会收到一条错误消息，提示 VPC 终端节点服务不支持该子网的 AZ。该问题通常是因为你选择的区域中存在额外的 AZ，而你的 TiDB 集群并未部署在该 AZ 上。此时，你可以联系 [PingCAP 技术支持](https://docs.pingcap.com/tidbcloud/tidb-cloud-support)。
...
7. 在 **Subnets** 区域，选择你的 TiDB 集群所在的可用区。
    ...
    > **提示：**
    >
    > 如果你的服务跨越了三个以上的可用区（AZ），你可能无法在 **Subnets** 区域选择 AZ。该问题通常是因为你选择的区域中存在额外的 AZ，而你的 TiDB 集群并未部署在该 AZ 上。此时，请联系 [PingCAP 技术支持](https://docs.pingcap.com/tidbcloud/tidb-cloud-support)。
