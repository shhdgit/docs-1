...
TiDB Cloud 支持通过 [AWS PrivateLink](https://aws.amazon.com/privatelink) 在 AWS VPC 中对托管的 TiDB Cloud 服务进行高度安全的单向访问，就像服务部署在你自己的 VPC 中一样。你的 VPC 中会暴露一个私有终端节点，你可以通过该终端节点并具备权限后连接到 TiDB Cloud 服务。
...
- 你正在使用 [TiCDC](https://docs.pingcap.com/tidb/stable/ticdc-overview) 集群将数据从源 TiDB 集群跨区域同步到目标 TiDB 集群，以实现高可用。目前，私有终端节点不支持跨区域连接。
...
> **Note:**
>
> 对于 2023 年 3 月 28 日之后创建的每个 TiDB Cloud 专属集群，对应的终端节点服务会在集群创建后 3 到 4 分钟内自动创建。
...
> **Tip:**
>
> 如果无法连接到集群，可能是因为 AWS 中 VPC 终端节点的安全组设置不正确。解决方法请参见 [此常见问题](#troubleshooting)。
