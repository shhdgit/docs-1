...
4. 运行以下命令。
    ...
    > **Note:**
    >
    > 有时，即使路由表规则已成功创建，你仍可能收到 `An error occurred (MissingParameter) when calling the CreateRoute operation: The request must contain the parameter routeTableId` 错误。此时可以检查已创建的规则并忽略该错误。
