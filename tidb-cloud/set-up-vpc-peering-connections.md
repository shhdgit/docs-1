...
目前，同一项目同一区域下的 TiDB 集群会创建在同一个 VPC 中。因此，一旦在某个项目的某个区域设置了 VPC Peering，该项目在同一区域内创建的所有 TiDB 集群都可以通过你的 VPC 进行连接。不同云服务商的 VPC Peering 设置方式不同。
...
3. 确保你的 VPC 已启用私有 DNS 托管区域支持。
    ...
    4. 在设置下拉列表中点击 **Edit DNS hostnames**，启用 DNS hostnames 并点击 **Save**。

    5. 在设置下拉列表中点击 **Edit DNS resolution**，启用 DNS resolution 并点击 **Save**。
