...
假设你的 MySQL dump 文件大小为 20 TB，TiKV 压缩比为 40%。则可以按如下方式计算推荐的 TiKV 节点数：
...
你可以为 TiFlash 配置节点数、vCPU 和内存、存储空间。
...
TiDB Cloud 会将 TiFlash 节点均匀部署到一个区域内的不同可用区。建议你为每个 TiDB Cloud 集群至少配置两个 TiFlash 节点，并在生产环境中为数据创建至少两个副本，以实现高可用性。
...
例如，如果你将每个 TiFlash 节点在 AWS 上的节点存储配置为 1024 GiB，并为表 A 设置 2 个副本（压缩后大小为 800 GiB），为表 B 设置 1 个副本（压缩后大小为 100 GiB），则所需 TiFlash 节点数如下：

TiFlash 节点最小数量：`min((800 GiB * 2 + 100 GiB * 1) / 1024 GiB, max(2, 1)) ≈ 2`

### TiFlash 节点存储空间

不同 TiFlash vCPU 支持的节点存储空间如下：
