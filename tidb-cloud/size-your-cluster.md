...
例如，你的整体期望性能为 110,000 QPS，负载类型为混合，P95 延迟约为 100 ms，并且希望使用 8 vCPU, 16 GiB TiDB 节点。你可以从上表获取 8 vCPU, 16 GiB TiDB 节点的预估性能（即 `15,500`），然后计算大致的 TiDB 节点数量如下：
...
你可以为 TiKV 配置节点数量、vCPU 和内存（RAM）、存储空间。
...
假设你的 MySQL dump 文件大小为 20 TB，TiKV 压缩比为 40%。则可以按如下方式计算推荐的 TiKV 节点数量：
...
例如，你的整体期望性能为 110,000 QPS，负载类型为混合，P95 延迟约为 100 ms，并且希望使用 8 vCPU, 32 GiB TiKV 节点。你可以从上表获取 8 vCPU, 32 GiB TiKV 节点的预估性能（即 `17,800`），然后计算大致的 TiKV 节点数量如下：
...
标准存储类型会自动应用于使用 TiDB v7.5.5、v8.1.2、v8.5.0 或更高版本创建的 AWS 新集群。
...
你可以为 TiFlash 配置节点数量、vCPU 和内存（RAM）、存储空间。
...
TiDB Cloud 会将 TiFlash 节点均匀部署到一个区域内的不同可用区。建议你在每个 TiDB Cloud 集群中至少配置两个 TiFlash 节点，并为生产环境中的数据创建至少两个副本以实现高可用。
