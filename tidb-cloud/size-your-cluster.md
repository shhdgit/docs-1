...
例如，你的整体期望性能为 110,000 QPS，负载类型为混合，P95 延迟约为 100 ms，并且你希望使用 8 vCPU, 16 GiB 的 TiDB 节点。你可以从上表中获得 8 vCPU, 16 GiB TiDB 节点的预估性能（即 `15,500`），然后计算大致的 TiDB 节点数如下：
...
通常建议 TiKV 存储的使用率保持在 80% 以下。TiDB Cloud 的副本数默认为 3。8 vCPU, 64 GiB TiKV 节点的最大存储容量为 4096 GiB。
...
例如，你的整体期望性能为 110,000 QPS，负载类型为混合，P95 延迟约为 100 ms，并且你希望使用 8 vCPU, 32 GiB 的 TiKV 节点。你可以从上表中获得 8 vCPU, 32 GiB TiKV 节点的预估性能（即 `17,800`），然后计算大致的 TiKV 节点数如下：
...
标准存储适用于大多数负载，在性能和成本之间实现平衡。与基础存储相比，标准存储为 Raft 日志预留了充足的磁盘资源，从而减少 Raft I/O 对数据盘 I/O 的影响，提高 TiKV 的读写性能。
