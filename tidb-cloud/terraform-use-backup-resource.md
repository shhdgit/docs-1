...
你可以通过本文档了解如何使用 `tidbcloud_backup` 资源为 TiDB Cloud 集群创建备份。
...
- 本文介绍的备份与恢复功能不适用于 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群。要使用 `tidbcloud_backup` 资源，请确保你已创建 TiDB Cloud 专属集群。
...
当状态变为 `SUCCESS` 时，表示你已为集群创建了备份。请注意，备份在创建后无法更新。
