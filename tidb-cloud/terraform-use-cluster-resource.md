...
1. 在你 [获取 TiDB Cloud Terraform Provider](/tidb-cloud/terraform-get-tidbcloud-provider.md) 时创建的 `main.tf` 文件中，添加如下 `data` 和 `output` 块：
   ...
   - 使用 `data` 块定义 TiDB Cloud 的数据源，包括数据源类型和数据源名称。
      ...
      - 对于 `tidbcloud_projects` 数据源，可以使用 `page` 和 `page_size` 属性限制你想要查询的最大项目数量。
...
5. 使用 `terraform show` 或 `terraform state show tidbcloud_cluster.${resource-name}` 命令检查资源状态。前者会显示所有资源和数据源的状态。
    ...
    此时集群的状态为 `CREATING`。你需要等待状态变为 `AVAILABLE`，通常至少需要 10 分钟。
...
当集群状态为 `AVAILABLE` 时可以暂停集群，当状态为 `PAUSED` 时可以恢复集群。
...
例如，你可以导入一个不是通过 Terraform 创建的集群，或者导入一个 [通过 `tidbcloud_restore` 资源创建的集群](/tidb-cloud/terraform-use-restore-resource.md#create-a-restore-task)。
...
4. 若要通过 Terraform 管理该集群，可以将上一步的输出内容复制到你的配置文件中。注意需要删除 `id` 和 `status` 两行，因为它们将由 Terraform 控制：
