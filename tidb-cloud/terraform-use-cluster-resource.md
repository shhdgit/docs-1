...
1. 在你 [获取 TiDB Cloud Terraform Provider](/tidb-cloud/terraform-get-tidbcloud-provider.md) 时创建的 `main.tf` 文件中，添加如下 `data` 和 `output` 块：
   ...
   - 使用 `data` 块定义 TiDB Cloud 的数据源，包括数据源类型和数据源名称。
      ...
      - 对于 `tidbcloud_projects` 数据源，可以使用 `page` 和 `page_size` 属性限制你想要查询的最大项目数。
   ...
   要获取所有资源和数据源的可用配置，参见此 [配置文档](https://registry.terraform.io/providers/tidbcloud/tidbcloud/latest/docs)。
...
现在，你已经使用 Terraform 创建并管理了 TiDB Cloud Dedicated 集群。接下来，你可以尝试通过 [`tidbcloud_backup`](/tidb-cloud/terraform-use-backup-resource.md) 资源为集群创建备份。
