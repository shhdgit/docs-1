...
每个 TiDB 集群都属于一个项目。在创建 TiDB 集群前，你需要获取要创建集群的项目 ID。
...
在创建集群前，你需要获取集群规格信息，其中包含所有可用的配置值（如支持的云服务商、区域和节点规格）。
...
- `node_size` 表示节点规格。
- `storage_size_gib_range` 显示你可为节点设置的最小和最大存储容量。
...
6. 若要查看最新状态，运行 `terraform refresh` 命令以更新状态，然后运行 `terraform state show tidbcloud_cluster.${resource-name}` 命令显示状态。
...
1. 在 [创建集群](#create-a-cluster-using-the-cluster-resource) 时使用的 `cluster.tf` 文件中，编辑 `components` 配置。
    ...
    例如，若要为 TiDB 增加 1 个节点，为 TiKV 增加 3 个节点（TiKV 节点数需为 3 的倍数，因为步长为 3。你可以 [从集群规格获取此信息](#get-cluster-specification-information-using-the-tidbcloud_cluster_specs-data-source)），为 TiFlash 增加 1 个节点，可按如下方式配置：
...
5. 运行 `terraform apply` 命令并输入 `yes` 进行确认。使用 `terraform state show tidbcloud_cluster.${resource-name}` 命令检查状态，会发现状态变为 `RESUMING`：
...
6. 为确保配置与状态一致，可以执行 `terraform plan` 或 `terraform apply`。如果看到 `No changes`，则导入成功。
