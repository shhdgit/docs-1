...
本文档介绍如何使用 `tidbcloud_dedicated_cluster` 资源管理 [TiDB Cloud 专属](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 集群。

你还将学习如何通过 `tidbcloud_projects` 数据源获取所需信息，并使用 `tidbcloud_dedicated_node_group` 资源管理 TiDB Cloud 专属集群的 TiDB 节点组。
...
1. 在你 [获取 TiDB Cloud Terraform Provider](/tidb-cloud/terraform-get-tidbcloud-provider.md) 时创建的 `main.tf` 文件中，添加如下 `data` 和 `output` 块：
    ...
    - 使用 `data` 块定义 TiDB Cloud 的数据源，包括数据源类型和数据源名称。
        ...
        - 数据源名称可按需自定义，例如 `"example_project"`。
        - 对于 `tidbcloud_projects` 数据源，可以通过 `page` 和 `page_size` 属性限制你要查询的最大项目数。

    - 使用 `output` 块定义要在输出中显示的数据源信息，并将其暴露给其他 Terraform 配置使用。

        `output` 块的作用类似于编程语言中的返回值。更多信息请参见 [Terraform 官方文档](https://www.terraform.io/language/values/outputs)。

    若要获取所有资源和数据源的可用配置，请参见 [Terraform provider 配置文档](https://registry.terraform.io/providers/tidbcloud/tidbcloud/latest/docs)。

2. 运行 `terraform apply` 命令以应用配置。在确认提示时需要输入 `yes` 以继续。

    若要跳过确认提示，可使用 `terraform apply --auto-approve`：
...
> **Note:**
>
> - 在开始之前，请确保你已在 [TiDB Cloud 控制台](https://tidbcloud.com) 设置了 CIDR。详细信息请参见 [设置 CIDR](/tidb-cloud/set-up-vpc-peering-connections.md#prerequisite-set-a-cidr-for-a-region)。
...
2. 创建一个 `cluster.tf` 文件。
    ...
    使用 `resource` 块定义 TiDB Cloud 的资源，包括资源类型、资源名称和资源详细配置。

    - 若要使用 `tidbcloud_dedicated_cluster` 资源，需将资源类型设置为 `tidbcloud_dedicated_cluster`。
    - 资源名称可按需自定义，例如 `example_cluster`。
    - 资源详细配置可根据项目 ID 及 [`tidbcloud_dedicated_cluster` 规范](https://registry.terraform.io/providers/tidbcloud/tidbcloud/latest/docs/resources/dedicated_cluster) 进行设置。
...
5. 运行 `terraform apply` 命令并在确认时输入 `yes`。稍等片刻，状态最终会变为 `ACTIVE`。
...
要删除集群的 TiDB 节点组，可以删除 `dedicated_node_group` 资源的配置，然后使用 `terraform apply` 命令销毁该资源：
...
现在，如果你运行 `terraform show` 命令，将不会有任何资源输出，因为该资源已被清除：
...
对于未被 Terraform 管理的 TiDB 集群，你可以通过导入将其纳入 Terraform 管理。

导入未由 Terraform 创建的集群，步骤如下：
...
现在你可以使用 Terraform 管理导入的集群。

## 删除 TiDB Cloud 专属集群

要删除 TiDB Cloud 专属集群，可以删除 `tidbcloud_dedicated_cluster` 资源的配置，然后使用 `terraform apply` 命令销毁该资源：
...
现在，如果你运行 `terraform show` 命令，将不会有任何受管资源输出，因为该资源已被清除：
