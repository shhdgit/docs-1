...
> **Note:**
>
> - 在开始之前，请确保你已在 [TiDB Cloud 控制台](https://tidbcloud.com) 设置了 CIDR。更多信息参见 [设置 CIDR](/tidb-cloud/set-up-vpc-peering-connections.md#prerequisite-set-a-cidr-for-a-region)。
...
3. 运行 `terraform apply` 命令。应用资源时不建议使用 `terraform apply --auto-approve`。
    ...
    - 你可以检查配置与当前状态的差异。
    - 你也可以看到本次 `apply` 的结果。将会新增一个资源，没有资源被更改或销毁。
