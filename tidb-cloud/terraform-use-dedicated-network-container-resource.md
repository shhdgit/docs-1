...
以下示例展示了如何创建 TiDB Cloud Dedicated 网络容器。
...
5. 使用 `terraform show` 或 `terraform state show tidbcloud_dedicated_network_container.${resource-name}` 命令检查资源状态。前者会显示所有资源和数据源的状态。
...
1. 为新的 `tidbcloud_dedicated_network_container` 资源添加 import 块。
    ...
    在你的 `.tf` 文件中添加如下 import 块，将 `example` 替换为你期望的资源名称，将 `${id}` 替换为 `cluster_id,network_container_id` 格式：
...
2. 生成新的配置文件。
    ...
    根据 import 块为新的 `tidbcloud_dedicated_network_container` 资源生成配置文件：
...
现在你可以使用 Terraform 管理被导入的 TiDB Cloud Dedicated 网络容器。
