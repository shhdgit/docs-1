...
summary: 了解如何使用 `tidbcloud_dedicated_network_container` 资源来创建和修改 TiDB Cloud 专属网络容器。
---

# 使用 `tidbcloud_dedicated_network_container` 资源

本文档介绍了如何使用 `tidbcloud_dedicated_network_container` 资源来管理 [TiDB Cloud 专属](/tidb-cloud/select-cluster-tier.md#tidb-cloud-dedicated) 网络容器。

网络容器是一个逻辑网络资源，允许你为特定项目和区域定义和管理一个 CIDR 块（IP 地址范围）。该 CIDR 块用于为 TiDB Cloud 专属集群创建 VPC，并且在该区域设置 VPC 对等连接之前是必需的。

在向某个区域添加 VPC 对等连接请求之前，你必须先为该区域设置 CIDR 块并创建首个 TiDB Cloud 专属集群。创建第一个集群后，TiDB Cloud 会自动创建关联的 VPC，从而使你能够与应用的 VPC 建立对等连接。

`tidbcloud_dedicated_network_container` 资源的功能包括：

- 创建 TiDB Cloud 专属网络容器。
- 导入 TiDB Cloud 专属网络容器。
- 删除 TiDB Cloud 专属网络容器。

> **注意：**
>
> 如果 TiDB Cloud 专属网络容器的状态为 `ACTIVE`，则无法修改或删除。请确保在应用 `tidbcloud_network_container` 资源前，其配置是正确的。
...
你可以使用 `tidbcloud_dedicated_network_container` 资源来创建 TiDB Cloud 专属网络容器。

以下示例展示了如何创建一个 TiDB Cloud 专属网络容器。

1. 为 TiDB Cloud 专属网络容器创建一个目录并进入该目录。
...
2. 创建一个 `network_container.tf` 文件：
    ...
    - 有关 TiDB Cloud 专属网络容器规范的更多信息，请参见 [tidbcloud_dedicated_network_container (Resource)](https://registry.terraform.io/providers/tidbcloud/tidbcloud/latest/docs/resources/dedicated_network_container)。
...
4. 如果你的计划内容无误，输入 `yes` 继续：
    ...
    该资源的状态会保持为 `INACTIVE`，直到你在该 TiDB Cloud 专属网络容器所在区域创建 TiDB Cloud 专属集群。之后，状态会变为 `ACTIVE`。

5. 使用 `terraform show` 或 `terraform state show tidbcloud_dedicated_network_container.${resource-name}` 命令检查资源的状态。前者会显示所有资源和数据源的状态。
...
对于未被 Terraform 管理的 TiDB Cloud 专属网络容器，你可以通过导入操作将其纳入 Terraform 管理。

例如，你可以导入一个不是通过 Terraform 创建的网络容器。

1. 为新的 `tidbcloud_dedicated_network_container` 资源添加 import 块。

    在你的 `.tf` 文件中添加如下 import 块，将 `example` 替换为你想要的资源名称，将 `${id}` 替换为 `cluster_id,network_container_id` 的格式：
...
现在你可以使用 Terraform 管理被导入的 TiDB Cloud 专属网络容器了。

## 删除 TiDB Cloud 专属网络容器

要删除 TiDB Cloud 专属集群，你可以删除 `tidbcloud_dedicated_cluster` 资源的配置，然后使用 `terraform apply` 命令销毁该资源。但你必须确保 TiDB Cloud 专属网络容器的状态不是 `ACTIVE`。如果状态为 `ACTIVE`，则无法删除。
...
此时，如果你运行 `terraform show` 命令，将不会显示任何被管理的资源，因为该资源已被清除：
