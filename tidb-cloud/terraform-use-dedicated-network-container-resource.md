...
在向某个区域添加 VPC 对等连接请求之前，你必须先为该区域设置 CIDR 块，并创建一个初始的 TiDB Cloud Dedicated 集群。创建第一个集群后，TiDB Cloud 会自动创建关联的 VPC，从而使你能够与你应用的 VPC 建立对等连接。
...
> **注意：**
>
> 如果 TiDB Cloud Dedicated 网络容器的状态为 `ACTIVE`，则无法修改或删除。请确保在应用 `tidbcloud_network_container` 资源之前，其配置是正确的。
...
5. 使用 `terraform show` 或 `terraform state show tidbcloud_dedicated_network_container.${resource-name}` 命令来检查你的资源状态。前者会显示所有资源和数据源的状态。
