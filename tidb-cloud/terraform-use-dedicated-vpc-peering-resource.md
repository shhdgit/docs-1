...
2. 创建一个 `vpc_peering.tf` 文件：
    ...
    - 如需获取 TiDB Cloud 专属 VPC Peering 规范信息，请参见 [tidbcloud_dedicated_vpc_peering (Resource)](https://registry.terraform.io/providers/tidbcloud/tidbcloud/latest/docs/resources/dedicated_vpc_peering)。
...
对于未被 Terraform 管理的 TiDB Cloud 专属 VPC Peering，你可以通过导入操作将其纳入 Terraform 管理。
...
现在你可以使用 Terraform 管理被导入的 TiDB Cloud 专属 VPC Peering 了。
