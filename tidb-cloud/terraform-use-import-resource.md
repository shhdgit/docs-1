...
> **Note:**
>
> 本地文件导入仅支持 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群，不支持 TiDB Cloud Dedicated 集群。
...
2. 创建一个 `import` 目录，并在其中创建 `main.tf` 文件。例如：
   ...
   将文件中的资源值（如 project ID、cluster ID 和 CSV 路径）替换为你自己的。你可以在 [配置页面](https://registry.terraform.io/providers/tidbcloud/tidbcloud/latest/docs/resources/import#nested-schema-for-csv_format) 查看 `csv_format` 的详细信息。
...
> **Note:**
>
> 为了让 TiDB Cloud 能够访问你在 Amazon S3 存储桶中的文件，你需要先[配置 Amazon S3 访问权限](/tidb-cloud/dedicated-external-storage.md#configure-amazon-s3-access)。
