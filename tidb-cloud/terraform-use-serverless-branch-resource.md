...
1. 为新的 TiDB Cloud Serverless branch 资源添加一个 import 块。
    ...
    在你的 `.tf` 文件中添加如下 import 块，将 `example` 替换为你想要的资源名称，将 `${id}` 替换为 `cluster_id,branch_id` 的格式：
...
现在，如果你运行 `terraform show` 命令，将不会显示任何被管理的资源，因为该资源已经被清除：
