...
每个 TiDB 集群都属于一个项目。在创建 TiDB Cloud Serverless 集群之前，你需要获取要创建集群所在项目的 ID。如果未指定 `project_id`，则会使用默认项目。
...
1. 在你 [获取 TiDB Cloud Terraform Provider](/tidb-cloud/terraform-get-tidbcloud-provider.md) 时创建的 `main.tf` 文件中，添加如下的 `data` 和 `output` 块：
    ...
    - 使用 `output` 块定义要在输出中显示的数据源信息，并将信息暴露给其他 Terraform 配置使用。
        ...
        `output` 块的作用类似于编程语言中的返回值。更多细节可参考 [Terraform 文档](https://www.terraform.io/language/values/outputs)。
