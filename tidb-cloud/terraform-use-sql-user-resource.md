...
2. 创建一个 `sql_user.tf` 文件：
    ...
    - 对于 TiDB Cloud Starter 或 TiDB Cloud Essential 集群中的 SQL 用户，`user_name` 以及内置角色 `role_readonly` 和 `role_readwrite` 必须以用户前缀开头，你可以通过运行 `tidbcloud_serverless_cluster` 数据源获取用户前缀。
...
5. 使用 `terraform show` 或 `terraform state show tidbcloud_sql_user.${resource-name}` 命令来查看资源的状态。前者会显示所有资源和数据源的状态。
...
2. 生成新的配置文件。
    ...
    然后，`generated.tf` 文件会在当前目录下生成，其中包含了被导入资源的配置信息。但 provider 会因为没有设置必需参数 `password` 而报错。你可以在生成的配置文件中为 `tidbcloud_sql_user` 资源的 `password` 参数设置值。
