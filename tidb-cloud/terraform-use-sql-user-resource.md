...
本文档介绍了如何使用 `tidbcloud_sql_user` 资源来管理 TiDB Cloud SQL 用户。
...
2. 创建一个 `sql_user.tf` 文件：
    ...
    - 对于 TiDB Cloud Serverless 或 TiDB Cloud Essential 集群中的 SQL 用户，`user_name` 以及内置角色 `role_readonly` 和 `role_readwrite` 必须以用户前缀开头，你可以通过运行 `tidbcloud_serverless_cluster` 数据源获取用户前缀。
...
5. 使用 `terraform show` 或 `terraform state show tidbcloud_sql_user.${resource-name}` 命令来检查你的资源状态。前者会显示所有资源和数据源的状态。
...
例如，你可以按如下方式导入一个不是通过 Terraform 创建的 SQL 用户：

1. 为新的 `tidbcloud_sql_user` 资源添加一个 import 块。
...
2. 生成新的配置文件。
    ...
    然后，`generated.tf` 文件会在当前目录下生成，其中包含了被导入资源的配置信息。但 provider 会抛出错误，因为没有设置必需的 `password` 参数。你可以在生成的配置文件中为 `tidbcloud_sql_user` 资源替换 `password` 参数的值。
...
现在，如果你运行 `terraform show` 命令，将不会有任何输出，因为资源已经被清除：
