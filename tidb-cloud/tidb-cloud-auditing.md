...
- 你正在使用 TiDB Cloud 专属集群。TiDB Cloud Starter 或 TiDB Cloud Essential 集群不支持审计日志功能。
- 你拥有组织的 `Organization Owner` 或 `Project Owner` 角色。否则，你无法在 TiDB Cloud 控制台中看到数据库审计相关选项。更多信息，参见 [用户角色](/tidb-cloud/manage-user-access.md#user-roles)。
...
3. 进入 **IAM** > **Access Management** > **Roles**，检查是否已存在信任实体为你之前记录的 TiDB Cloud Account ID 和 External ID 的角色。
    ...
    - 如果有，记录匹配的角色以备后用。
    - 如果没有，点击 **Create role**，选择 **Another AWS account** 作为信任实体类型，然后在 **Account ID** 字段中输入 TiDB Cloud Account ID。接着，选择 **Require External ID** 选项，并在 **External ID** 字段中输入 TiDB Cloud External ID。

4. 在 **IAM** > **Access Management** > **Roles**，点击上一步中的角色名称进入 **Summary** 页面，然后按以下步骤操作：
...
1. 在 **Bucket URI** 字段中，输入你 S3 存储桶的 URI，即审计日志文件将被写入的位置。
2. 在 **Bucket Region** 下拉列表中，选择存储桶所在的 AWS 区域。
3. 在 **Role ARN** 字段中，填写你在 [步骤 2. 配置 Amazon S3 访问权限](#step-2-configure-amazon-s3-access) 中复制的 Role ARN。
...
在 TiDB Cloud 控制台，返回你获取 TiDB Cloud account ID 的 **Enable Database Audit Logging** 对话框，然后按以下步骤操作：
...
1. 在 [Azure 门户](https://portal.azure.com/) 中，创建用于存储数据库审计日志的容器。
    ...
    1. 在 Azure 门户左侧导航栏中，点击 **Storage Accounts**，然后点击用于存储数据库审计日志的存储账户。
...
3. 为目标容器生成 SAS token。
    ...
    4. 在 **Start** 和 **Expiry** 字段中，指定 SAS token 的有效期。
...
4. 在 **Enable Database Audit Logging** 对话框中，填写你在 [步骤 2. 配置 Azure Blob 访问权限](#step-2-configure-azure-blob-storage-access) 获取的 blob URL 和 SAS token：
    ...
    - 在 **SAS Token** 字段中，输入访问该容器的 SAS token。
...
启用审计日志后，你必须指定审计过滤规则，以控制要捕获并写入审计日志的用户访问事件。如果未指定过滤规则，TiDB Cloud 不会记录任何日志。

要为集群指定审计过滤规则，请按以下步骤操作：

1. 在 **DB Audit Logging** 页面，**Log Filter Rules** 区域点击 **Add Filter Rule**，添加一条审计过滤规则。
