...
- 你只能为你的组织启用和禁用控制台审计日志。你只能追踪你所在组织内用户的操作。
...
> **注意：**
>
> 目前，TiDB Cloud 控制台的大多数事件类型都可以被审计，你可以在下表中找到它们。对于尚未覆盖的事件类型，TiDB Cloud 会持续完善并逐步纳入审计范围。
...
| UpdateUserRoleInOrganization        | 更新组织内用户的角色                                                 |
...
| trace_id              | string     | 操作人发起请求的 Trace ID。该字段当前为空，未来版本将支持。           |
