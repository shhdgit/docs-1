...
TiDB Cloud 提供了控制台审计日志功能，帮助你追踪用户在 [TiDB Cloud 控制台](https://tidbcloud.com) 上的各种行为和操作。例如，你可以追踪邀请用户加入组织、创建集群等操作。
...
> - 如果控制台审计日志被禁用超过 90 天，你将无法看到任何日志。
...
> **注意：**
>
> 目前，TiDB Cloud 控制台的大多数事件类型都可以被审计，你可以在下表中找到这些事件类型。对于尚未覆盖的事件类型，TiDB Cloud 会持续完善并逐步纳入审计范围。
...
| SetSpendLimit                     | 编辑 {{{ .starter }}} 集群的消费上限                                   |
| UpdateMaintenanceWindow           | 修改维护窗口开始时间                                                  |
| DeferMaintenanceTask              | 延迟维护任务                                                          |
| CreateBranch                      | 为 {{{ .starter }}} 或 {{{ .essential }}} 集群创建分支                 |
| DeleteBranch                      | 删除 {{{ .starter }}} 或 {{{ .essential }}} 集群的分支                 |
| SetBranchRootPassword             | 为 {{{ .starter }}} 或 {{{ .essential }}} 集群的分支设置 root 密码      |
...
| trace_id              | string     | 操作人发起请求的 Trace ID。该字段当前为空，后续版本将支持。                             |
