...
> **Note**:
>
> TiDB 节点组功能 **不适用于** TiDB Cloud Starter 或 TiDB Cloud Essential 集群。
...
- TiDB 节点组：TiDB 节点组用于管理 TiDB 节点的分组，并维护端点与 TiDB 节点之间的映射关系。
    ...
    - 当你删除 TiDB 节点组时，相关的网络设置（如私有链路和 IP 访问列表）也会被删除。
...
> **Note**:
>
> 在集群创建过程中无法创建 TiDB 节点组。你需要在集群创建完成并处于 **Available** 状态后再添加节点组。
...
4. 在 **Modify Cluster** 页面，点击 **+** 以添加新的 TiDB 节点组。你也可以直接使用默认组。
    ...
    - TiDB
        ...
        - **Node Groups**：点击 **+** 创建新的 TiDB 节点组。你也可以使用默认组，并在 **DefaultGroup** 字段中输入 TiDB 节点数量。
    
    - TiKV
        - **vCPU + RAM**：选择你需要的 [TiKV 规格](/tidb-cloud/size-your-cluster.md#size-tikv)。
        - **Storage x Nodes**：选择存储大小和 TiKV 节点数量。
    
    - TiFlash（可选）
        - **vCPU + RAM**：选择你需要的 [TiFlash 规格](/tidb-cloud/size-your-cluster.md#size-tiflash)。
        - **Storage x Nodes**：选择存储大小和 TiFlash 节点数量。
...
如果你创建了 TiDB 节点组但仍然使用默认组的端点连接集群，则该 TiDB 节点组中的 TiDB 节点不会承担任何负载，造成资源浪费。你需要为新 TiDB 节点组创建新的连接。参见 [连接到 TiDB 节点组](#connect-to-a-tidb-node-group)。

## 连接到 TiDB 节点组

### 通过公网连接

新建的 TiDB 节点组默认禁用公网连接。你需要先启用它。
...
4. 在左侧导航栏点击 **Settings** > **Networking**。
5. 在 **Networking** 页面右上角的 **TiDB Node Group** 列表中选择你的 TiDB 节点组。
6. 点击 **Create Private Endpoint Connection**，为该节点组创建新的连接。
...
由于所有 TiDB 节点组与集群共享同一个 VPC，你只需为该集群创建一个 VPC Peering 连接，即可为所有节点组提供访问能力。
...
> **Note**:
>
> 当你删除 TiDB 节点组时，其节点和网络配置也会被移除，包括私有端点连接和公网访问的 IP 列表。
