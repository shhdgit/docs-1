...
- 从 v7.2.0 版本开始，资源控制功能引入了对 runaway queries 的管理。你可以为资源组设置条件，以识别 runaway queries，并自动采取措施防止它们耗尽资源，影响其他查询。你可以在 [`CREATE RESOURCE GROUP`](/sql-statements/sql-statement-create-resource-group.md) 或 [`ALTER RESOURCE GROUP`](/sql-statements/sql-statement-alter-resource-group.md) 中包含 `QUERY_LIMIT` 字段来管理 runaway queries。
...
关于资源控制功能的更多信息，请参见 [使用资源控制实现资源组限制与流控](/tidb-resource-control-ru-groups.md)。

## `QUERY_LIMIT` 参数

如果查询超过以下任一限制，即被识别为 runaway query：

- `EXEC_ELAPSED`：检查查询执行时间是否超过限制。此规则适用于读写 DML 语句。
- `PROCESSED_KEYS`：检查协处理器处理的键数量是否超过限制。此规则仅适用于读语句。
- `RU`：检查该语句消耗的读写 RUs 总数是否超过限制。此规则仅适用于读语句。

支持的操作（`ACTION`）：

- `DRYRUN`：不采取任何操作。将记录追加到 runaway queries 的记录中，主要用于观察条件设置是否合理。
- `COOLDOWN`：将查询的执行优先级降至最低。查询继续以最低优先级执行，不占用其他操作的资源。
- `KILL`：自动终止识别出的查询，并报告错误 `Query execution was interrupted, identified as runaway query`。
- `SWITCH_GROUP`：在 v8.4.0 引入，将识别出的查询切换到指定的资源组以继续执行。该查询完成后，后续 SQL 语句在原资源组中执行。如果指定的资源组不存在，查询将保持在原资源组中。

为了避免大量并发 runaway queries 耗尽系统资源，资源控制功能引入了快速识别机制，可以快速识别并隔离 runaway 查询。你可以通过 `WATCH` 子句使用此功能。当查询被识别为 runaway query 时，该机制会提取匹配的特征（由 `WATCH` 后的参数定义）。在接下来的时间段（由 `DURATION` 定义），将匹配的 runaway 查询特征加入监控列表，TiDB 实例会匹配监控列表中的查询。匹配的查询会被直接标记为 runaway 查询，并根据对应的操作进行隔离，而不是等待条件识别。`KILL` 操作会终止查询并报告错误 `Quarantined and interrupted because of being in runaway watch list`。

`WATCH` 快速识别的三种匹配方式：

- `EXACT` 表示只快速识别完全相同 SQL 文本的语句。
- `SIMILAR` 表示匹配所有具有相同模式的 SQL Digest，字面值会被忽略。
...
| 参数             | 描述                                                                 | 备注                                                                                     |
|------------------|----------------------------------------------------------------------|------------------------------------------------------------------------------------------|
| `EXEC_ELAPSED`   | 当查询执行时间超过此值时，识别为 runaway query                        | `EXEC_ELAPSED='60s'` 表示如果查询执行时间超过 60 秒，则识别为 runaway query。             |
| `PROCESSED_KEYS` | 当协处理器处理的键数量超过此值时，识别为 runaway query                  | `PROCESSED_KEYS=1000` 表示如果协处理器处理的键数超过 1000，则识别为 runaway query。     |
| `RU`             | 当查询消耗的读写 RUs 总数超过此值时，识别为 runaway query                | `RU=1000` 表示如果查询消耗的总读写 RUs 超过 1000，则识别为 runaway query。             |
| `ACTION`         | 识别为 runaway query 后的处理动作                                       | 可选值为 `DRYRUN`、`COOLDOWN`、`KILL` 和 `SWITCH_GROUP`。                                |
...
> **Note:**
>
> 如果你想严格限制 runaway 查询在特定资源组中，可以结合使用 `SWITCH_GROUP` 和 [`QUERY WATCH`](#query-watch-parameters) 语句。因为 `QUERY_LIMIT` 只在查询满足条件时触发对应的 `ACTION` 操作，在此类场景下，`SWITCH_GROUP` 可能无法及时将查询切换到目标资源组。
...
- `ACTION` 的含义与 `QUERY LIMIT` 相同。此参数可省略，省略后识别后对应的操作采用资源组中通过 `QUERY LIMIT` 配置的 `ACTION`，且不会随 `QUERY LIMIT` 配置变化而变化。如果资源组中未配置 `ACTION`，会报错。
- `QueryWatchTextOption` 有三种选项：`SQL DIGEST`、`PLAN DIGEST` 和 `SQL TEXT`。
    - `SQL DIGEST` 与 `SIMILAR` 相同。后续参数接受字符串、用户变量或其他表达式，结果为字符串。字符串长度必须为 64，与 TiDB 中的 Digest 定义一致。
    - `PLAN DIGEST` 与 `PLAN` 相同。后续参数为 Digest 字符串。
    - `SQL TEXT` 将输入的 SQL 作为原始字符串（`EXACT`）匹配，或解析并编译成 `SQL DIGEST`（`SIMILAR`）或 `PLAN DIGEST`（`PLAN`），具体取决于后续参数。
...
+ `mysql.tidb_runaway_queries` 表包含过去 7 天内所有识别的 runaway 查询的历史记录。以其中一行为例：
