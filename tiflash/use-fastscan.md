...
虽然 TiFlash 可以在后台自动启动旧数据的压缩，但旧数据不会在物理上被清理，直到它经过压缩且其数据版本早于 GC 安全点。物理清理完成后，已清理的旧数据将不再在 FastScan 模式下返回。数据压缩的时机由多种因素自动触发，你也可以通过 [`ALTER TABLE ... COMPACT`](/sql-statements/sql-statement-alter-table-compact.md) 语句手动触发数据压缩。
